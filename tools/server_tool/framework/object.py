...
class Copyable(object):
	def __deepcopy__(self, memo):
		...
		for k, v in kvs.iteritems():
			...
			if not any(itertools.ifilter(func, [Csv, DBClient])):
...
class ObjectDBase(ObjectBase):
	...
	def save_async(self, forget=False):
		...
		if len(kvs) == 0:
			...
			return okfu
		# print '!!! save_async', type(self), self.id, kvs.keys()
		return self._dbc.call_async('dbUpdate', self.DBModel, self.pkey, kvs, forget)

	def delete_async(self):
		if not hasattr(self, '_dbc') or self._db is None:
			return okfu
		fu = self._dbc.call_async('dbDelete', self.DBModel, self.pkey)
		fu.add_done_callback(lambda _: self.gc_destroy) # TODO: test
		return fu
