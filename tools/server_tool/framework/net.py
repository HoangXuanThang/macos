...
class NetConn(object):
	...
	def __init__(self, stream, address, taskRecvCB, sockCloseCB = None, ntaskCls = LANetTask, logger_ = logger):
		...
		self.logger.info('new NetConn %s <-> %s', sock.getsockname(), sock.getpeername())
	...
	def read_bytes(self, size, cb):
		...
		if self._last_read_size is None:
			...
			self._last_cb = cb
			return self.stream.read_bytes(size, _on_read)
		else:
	...
	def onClose(self):
		...
		'''
		self.logger.info('NetConn connection has left.', clientip=self.address)
		self.close()
