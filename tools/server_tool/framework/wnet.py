...
class WANetTaskFlag(object):
	...
	def __init__(self, ok_ack = False, err_ack = False, heart_syn = False, zip_flag = False, long_len_flag = False, ack_flag = False, old_flag = None):
	...
	def __str__(self):
		return '%s ok_ack %d err_ack %d heart_syn %d zip_flag %d long_len_flag %d ack_flag %d' % (object.__str__(self), self.ok_ack, self.err_ack, self.heart_syn, self.zip_flag, self.long_len_flag, self.ack_flag)
...
class WANetTask(object):
	...
	normal task: [len(*2 bytes) pad_len(1 bytes) crc32(4 bytes) cmd(2 bytes) syn_id(2 bytes)][data(len)]
	...
	headStruct = struct.Struct('!HBIHH')
	headLongStruct = struct.Struct('!IBIHH')
	synIDCounter = 1
	...
	def __init__(self, conn, cmd = None, data = None, synID = None, pwd = None):
		self.len = 0
		self.cmd = cmd
		self.flag = None
		self.ackID = None
	...
	def write(self):
		flag = WANetTaskFlag(zip_flag = True, ack_flag = True if self.ackID else False)
		pad_len = 0
		data = msgpack.packb(self.data, use_bin_type=True)
		# print 'after msgpack', len(data)
		...
		packData = flag.pack() + packObj.pack(len(zdata), pad_len, crc, self.cmd, self.synID) + zdata
		self.conn.stream.write(packData)
	...
	def readHead(self, data):
		packStruct = self.getPackStruct()
		self.len, self.pad_len, self.crc, self.cmd, self.synID = packStruct.unpack(data)
	...
	def readBody(self, data):
		try:
			...
			# print self.conn, self, 'aes_password', self.pwd
			...
			self.destroy()
			logger.warning('readBody error! cmd: %s, dataLen: %d' % (str(self.cmd),len(data)))
			logger.info('%s', ' '.join(['%d' % ord(x) for x in data]))
			raise
