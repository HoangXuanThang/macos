	...
	btn:addClickEventListener(function()
		...
		if doubleClickDuration then
			...
			if nowTime - lastClickTime > doubleClickDuration then
				...
				self:addTipLabel(string.format("%d Click again within seconds to take effect", doubleClickDuration), "_reloadlua_draw_")
			else
				lastClickTime = 0
				callback()
			end
...
function editor:initUI()
	...
	self:addTestButton("CSVReload", "onReloadCsv", 5)
	self:addTestButton("LUAReload", "onReloadLua")
	self:addTestButton("NodeLocate", "onNodeLocate")
	self:addTestButton("SpineLocate", "onSpineLocate")
	self:addTestButton("BattleLocate", "onBattleLocate")

	self.uiOffset = cc.p(100, -50)
	self.itemIndex = 1
	self:addTestButton("BattleSave", "onBattleSave")
	self:addTestButton("BattleLoad", "onBattleLoad")
	self:addTestButton("CraftBattleLoad", "onCraftBattleLoad")
	self:addTestButton("BattleTest", "onBattleTest")

	self.uiOffset = cc.p(100, -200)
	self.itemIndex = 1
	self:addTestButton("ShowBind", "onShowBind")
	self:addTestButton("RunLua", "onRunLua")
	self:addTestButton("StartProfile", "onProfile")
	self:addTestButton("LuaDebug", "onLuaDebug")
	self:addTestButton("CSpriteStats", "onCSpriteStats")

	self.uiOffset = cc.p(100, -350)
	self.itemIndex = 1
	self:addTestButton("CheatRequest", "onCheatRequest")
...
function editor:onReloadCsv()
	...
	os.execute("cd ..\\..\\..\\..\\..\\tools\\csv2lua\\ && python csv2lua_dev.py cn")
