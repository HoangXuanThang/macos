...
function globals.getMergeServers(servers)
	...
	for _, server in ipairs(servers) do
		...
		local mServer = gDestServer[server]
		if mServer then
			for _, m_server in ipairs(mServer.servers) do
				if not hash[m_server] then
					hash[m_server] = true
					table.insert(mergeServers, m_server)
				end
			end
		else
			local id = tonumber(string.split(server, ".")[3])
			local mergeKey = string.format("game.%s.%s", tag, id)
