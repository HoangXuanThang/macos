...
function Packet:parseNext(data)
	...
	elseif self.readStep == self.ReadStepNormData then
		...
		xpcall(function()
			...
			local head = string.format("flag %d, len %d, padLen %d, crc32 %d, cmd %d, synID %d, url %s",
				string.byte(packdata, 1, #packdata), self.len, self.padLen, self.crc32, self.cmd, self.synID, self.url)
