		...
		if self.lastTime == 0 then
			...
			if ret == nil then
				...
				elseif err ~= 'timeout' then
					printWarn('%s %s can not connect %s:%s, err=%s', tostring(self), self.sockID, self.host, self.port,
						err)
					self:_onClose(err)
					return false
				end
...
function Session:_send(packet)
	...
	printDebug('%s %s sending len=%d last=%d index=%s pindex=%s err=%s', tostring(self), self.sockID, #packet.data,
		self.lastSendIdx, index, lastIndex, err)
