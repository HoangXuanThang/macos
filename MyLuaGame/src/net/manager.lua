...
function NetManager:sendHttpRequest(reqType, reqUrl, reqBody, resType, cb)
	...
	if resType == cc.XMLHTTPREQUEST_RESPONSE_JSON then
...
function NetManager:doPOST(reqUrl, resType, cb, data)
	local reqBody = json.encode(data)
	return self:sendHttpRequest("POST", reqUrl, reqBody, resType, function(xhr)
		if xhr.status == 200 then
			cb(xhr.response)
		else
			dump(xhr)
			if xhr.response and #xhr.response > 0 then
				cb(xhr.response)
			else
				local tipsTxt = gLanguageCsv.unknown
				if string.find(reqUrl, "create") then
					tipsTxt = gLanguageCsv.orderCreate
				elseif string.find(reqUrl, "ack") then
					tipsTxt = gLanguageCsv.orderAck
				end

				if not xhr.response then
					gGameUI:showDialog({
						title = gLanguageCsv.orderNet,
						content = string.format(gLanguageCsv.orderTips, tipsTxt, xhr.status, xhr.statusText),
