...
local maxBuffID = 99999

local function checkObjState(obj, force, state)
	...
	if obj:isFakeDeath() and not state then
		return false
	end
...
local function filterByForceAll(scene, force, f, typ, state)
	for _, obj in (typ and scene.ipairsOnSiteHeros or scene.ipairsHeros)(scene) do
		if checkObjState(obj, force, state) then
			f(obj)
		end
	end
end

local function filterByExcludeBuffGroup(scene, force, formulaIDs, f, typ)
	filterByForceAll(scene, force, function(obj)
		local tmpFunc = f

		for _, id in ipairs(formulaIDs) do
			if id == maxBuffID and obj:isAlreadyDead() or obj:hasBuffGroup(id) then
				tmpFunc = nil
				break
			end
		end
	...
	end, typ and true or false, true)
end
...
function CsvScene:countObjByBuffExGroup(force, buffCfgIDs, buffGroupIDs, formulaIDs)
	local noFilterOver = false
	local count = 0
	filterByExcludeBuffGroup(self.model, force, formulaIDs, function(obj)
		for _, id in pairs(buffCfgIDs) do
			if obj:hasBuff(id, noFilterOver) then
				count = count + 1
		...
		for _, id in pairs(buffGroupIDs) do
			if obj:hasBuffGroup(id, noFilterOver) then
				count = count + 1
...
function CsvScene:countForceNumExGroup(force, formulaIDs)
	local count = 0
	filterByExcludeBuffGroup(self.model, force, formulaIDs, function(obj)
		count = count + 1
	end)
...
-- 统计阵营中相关battleFlag的目标人头数
-- @param force int 阵营
-- @param ... int battleFlag 类1972,1982
-- @return 统计阵营中最多的自然属性,只计算精灵的第一属性
function CsvScene:countObjByMulFlag(force, ...)
	local count = 0
	local flags = { ... }
	filterByForce(self.model, force, function(obj)
...
-- 获取当前额外攻击模式来源buffCfgId
-- @return 默认返回0
function CsvScene:getExtraRoundId()
	return self.model.extraRoundId or 0
end

function CsvScene:getGroupObj(force, seat)
	local obj = self.model:getGroupObj(force, seat)
	if obj then
		return obj:getCsvObject()
	end
...
function CsvScene:isSoloFightType()
	...
	if SoloGateType[self.model.gateType] then
		return true
	end

	return self.model.gateType == game.GATE_TYPE.gym and self.model.play.deployType == game.DEPLOY_TYPE.OneByOneType
end

function CsvScene:isBrawlMode(brawlType, force)
	-- local extraRoundInfo = self.model:getExtraRoundLastInfo(battle.ExtraAttackMode.brawl)
	-- if extraRoundInfo and extraRoundInfo.inited and extraRoundInfo.brawlType == brawlType then
	-- 	if force and force > 0 then
	-- 		return extraRoundInfo.buff.caster.force == force
	-- 	end
