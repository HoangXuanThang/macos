...
local function getObjAttr(target)
	local obj = {
		hp = function ()
			return target:hp()
		end,
	...
	for attr, _ in pairs(target.attrs.AttrsTable) do
		obj["B" .. attr] = function ()
			return target:getBaseAttr(attr)
		end

		obj["A" .. attr] = function ()
			return target:getBuffAttr(attr)
		end
...
-- 上面扩展
function process.attr2(typs, comp, count, targets)
	local sign = (comp == "max" and 1 or -1)
    local count = count or 1
    if typs == "selectAttr" then
        typs = selectObj[typs]
	...
	return searchTarget(filteredTargets, function (target)
		local typ = typs
		local attrObj = getObjAttr(target)
		if type(typs) == "table" then
			typ = searchTarget(typs, function (_typ)
				return sign * attrObj[_typ]()
			end, valBigger, 1)[1]
		end
		return sign * attrObj[typ]()
    end, valBigger, count)
end
...
-- 非拥有战斗标记
function process.battleFlagDifferEx(typ, battleFlags, targets)
	local retT = {}
	if typ == "battleFlag" then
		retT = getTargetsByBattleFlag(battleFlags, targets, false)
	end
	return retT
end
