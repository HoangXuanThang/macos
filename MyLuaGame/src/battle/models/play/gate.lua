...
function Gate:onNewRound()
	...
	if self.showSkipFlag and self.data.skipRound and self.totalRound >= self.data.skipRound then
		self.showSkipFlag = false
		gRootViewProxy:notify('showSkipButton')
	end
...
function Gate:onNewBattleTurn()
	...
	log.battle.gate.onNewBattleTurn('---------- gate onNewBattleTurn() ---------- =', self.curBattleRound,
		self.totalRoundBattleTurn)
	-- 旧数据清理
	...
	if self.curHero then
		local notExistLeftHeros = not itertools.include(self.roundLeftHeros,
			function(data) return data.obj.id == self.curHero.id end)
		...
		if self.curHero.multiShapeTb and (notExistLeftHeros or notExistHasAttackedHeross) then
			local unitID = battleEasy.ifElse(self.curHero.orginUnitId % 2 ~= 0, self.curHero.data.role2Data.roleId,
				self.curHero.orginUnitId)
			if not notExistHasAttackedHeross then
				unitID = battleEasy.ifElse(self.curHero.orginUnitId % 2 == 0, self.curHero.data.role2Data.roleId,
					self.curHero.orginUnitId)
			end
...
function Gate:endBattleTurn(target)
	...
	local waitTime = 0
	if self.curHero and self.curHero.curSkill and self.curHero.curSkill.id and csv.skill[self.curHero.curSkill.id] and csv.skill[self.curHero.curSkill.id].damageWaitTime then
		waitTime = csv.skill[self.curHero.curSkill.id].damageWaitTime or 0
	end
	-- 先播放效果 后删除单位
	-- gRootViewProxy:proxy():flushCurDeferList()
	-- 保证战斗回合结束的逻辑在技能后执行
...
function Gate:autoAttack()
	...
	logf.battle.gate.autoAttack(" autoAttack, choose skillId= %d, attackId= %d", skillID or 999999,
		target and target.id or 999999)
...
local AUTO_CHOOSE_STEP = {
	...
	[4] = function(self, skillId, enemyForce, _)
		if skillId then
			local autoSkill = self.curHero.skills[skillId] or self.curHero.passiveSkills[skillId] or
				self.curHero.curSkill
			if autoSkill then
				-- 混合类型 阵营应该是全体
		...
		battleEasy.queueEffect(function()
			...
			if self.endAnimation then
				...
				battleEasy.queueEffect('effect',
					{ effectType = 1, effectRes = self.endAnimation.res, effectArgs = args, faceTo = 1 })
			end
			--延迟 2秒吧
			battleEasy.queueEffect('delay', { lifetime = 100 + (self.endMoreDelayTime or 0) })
		end)
	end)
