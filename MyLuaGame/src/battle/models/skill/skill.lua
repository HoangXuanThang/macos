...
function SkillModel:canSpell()
	...
	-- 冷却回合数
	logf.battle.skill.skillCD(' skillCD: 当前round= %d, 使用时spellRound= %d, cd= %d spellType= %d', round, self.spellRound,
		self.cdRound, judgeNum)
...
function SkillModel:updateSkillDamageProcessId()
	...
	if dmgProcessIdMap.exAttackMode then
		self.skillCalDamageProcessId = dmgProcessIdMap.exAttackMode[self.owner.exAttackMode] or
			self.cfg.skillCalDamageProcessId
	end
end
...
function SkillModel:isJumpBigSkill()
	return self.skillType2 == battle.MainSkillType.BigSkill and self.canJump and
		userDefault.getForeverLocalKey("mainSkillPass", false)
end
...
function SkillModel:processPlay(skillBB)
	local processArgs, noMissTargetsArray, allDamagedOrder = skillBB.processArgs, skillBB.noMissTargetsArray,
		skillBB.allDamagedOrder
	...
	for i, processCfg in self:ipairsProcess() do
		...
		processArgs[i].buffTb = self:processAddBuff(processCfg, processTargets, extraTarget,
			battle.SkillAddBuffType.InPlay)
	end
...
function SkillModel:processTarget(skillBB)
	...
	for i, processCfg in self:ipairsProcess() do
		...
		self.scene:excuteGroupObjFunc(1, battle.SpecialObjectId.teamShiled, "syncView", self, args,
			processCfg.isSegProcess)
		self.scene:excuteGroupObjFunc(2, battle.SpecialObjectId.teamShiled, "syncView", self, args,
			processCfg.isSegProcess)
...
function SkillModel:processAfter(skillBB)
	...
	local mainNumShowType = self:isMainNumShowType() == battle.SkillFormulaType.damage and battle.SkillSegType.damage or
...
function SkillModel:processAfterObjTrigger(skillBB)
	...
	local target, allDamagedOrder, noMissTargetsArray, noDeadObjectTb = skillBB.target, skillBB.allDamagedOrder,
		skillBB.noMissTargetsArray, skillBB.noDeadObjectTb
	...
	for _, obj in maptools.order_pairs(self.allTargets, 'id') do
		...
		if (clickTarget[1] and clickTarget[1].id == obj.id) or obj.id ~= self.owner.id then
			self.owner:onPassive(PassiveSkillTypes.recoverHp, obj,
				{ skillType = self.skillType, hpFormula = skillCfg.hpFormula })
		end
...
function SkillModel:processAfterRefresh(skillBB)
	...
	if mp1PointData and mp1PointData.mode == 1 and self.skillType2 == battle.MainSkillType.BigSkill then
		...
		-- 换算point
		local costMp1Point = cc.clampf(math.floor(mpOverflow / mp1PointData.rate), 0,
			mp1PointData.cost or math.floor(mp1PointData.limit / mp1PointData.rate))
...
function SkillModel:_spellTo(target)
	log.battle.skill.spellTo('curSkillId=', self.id, 'owner.id=', self.owner.seat, 'target.id=', target and target.seat,
...
function SkillModel:onSpellView(skillBB)
	...
	if self.skillType2 == battle.MainSkillType.BigSkill then -- skillType2 大招是 2
		...
		if (self.type == 1) then
			...
			battleEasy.queueEffect(function()
				gRootViewProxy:notify('skillPreAni', hideHero)
			end)
		end
	end
	...
	battleEasy.queueNotifyFor(view, 'skillBefore', self.disposeDatasOnSkillStart, self.skillType, false,
		self:getSkillSceneTag())
	...
	if skillCfg.sound and not self.canjumpBigSkill then
		battleEasy.queueEffect('sound',
			{ delay = skillCfg.sound.delay, sound = { res = skillCfg.sound.res, loop = skillCfg.sound.loop } })
	end
...
function SkillModel:getTargetsHint(hitFormula)
	...
	if self:isSameType(battle.SkillFormulaType.fix) and cfg.hintChoose > 100 then
		...
		local healthTargets = self:getTargets(cfg.hintTargetType == 1 and 0 or 1, healthChoose, sneerAtOwnerObj, args,
			targetCfg)
...
function SkillModel:calcFormulaFinal(target, args)
	...
	if args.process.segType == battle.SkillSegType.resumeHp then
		if not final.value then
			...
			log.battle.skill.formula(" 技能表公式配置:hpFormula owner.id=%s, cfg.hpFormula=%s, hpFormula=%s, target.id=%s",
				self.owner.seat, skillCfg.hpFormula, formHp, target and target.id)
			local skillNatureType = self:getSkillNatureType() or 1
			local objNatureName = game.NATURE_TABLE[skillNatureType]
			formHp = formHp *
				(1 + self.owner:cure() + self.owner[objNatureName .. 'Cure'](self.owner) + self.owner:healAdd())
	...
	if args.process.segType == battle.SkillSegType.damage then
		...
		if not final.value then
			...
			local randFix = self.scene.closeRandFix and 1 or
				ymrand.random(9000, ConstSaltNumbers.wan) / ConstSaltNumbers.wan
			formDamage = formDamage * randFix
			logf.battle.skill.damageFormula(
				' 技能表公式配置:damageFormula  ownerId= %s, formula damge= %s, cfg.damageFormula= %s', self.owner.seat,
				formDamage,
				skillCfg.damageFormula)
			-- 伤害增幅数据
			local exArgs = {
				skillId = skillCfg.id,
			...
			local damage, damageArgs = target:calcInternalDamage(self.owner, formDamage, self.skillCalDamageProcessId,
				exArgs)
			final.value = damage
...
function SkillModel:onSputterTarget(target, args, final, damageArgs, skillDamageBB)
	...
	if data then
		...
		if damageArgs.segId == 1 then
			...
			args.sputObjs[target.id] = self:getSpurtTargets(target, args.targets, table.length(args.targets) > 1,
				args.oriTargets)
		...
		for _, obj in ipairs(args.sputObjs[target.id]) do
			...
			local _damage, _damageArgs = obj:beAttack(self.owner, spurtDamage, skillDamageBB.damageProcessAfterCal,
				damageArgs)
			local _final = self:getTargetsFinalResult(obj.id, battle.SkillSegType.damage)
			...
			if not self.canjumpBigSkill then
				battleEasy.deferNotifyCantJump(nil, 'showNumber',
					{
						delta = math.floor(_damage:get(battle.ValueType.normal)),
						skillId = self.id,
						typ = battle
							.SkillSegType.damage
					})
...
function SkillModel:onPenetrateTarget(target, args, final, damageArgs, skillDamageBB)
	...
	if data then
		...
		if args.penetrateObj then
			...
			damageArgs.beHitNotWakeUp = true
			local _damage, _damageArgs = args.penetrateObj:beAttack(self.owner, penetrateDamage,
				skillDamageBB.damageProcessAfterCal, damageArgs)
			local _final = self:getTargetsFinalResult(args.penetrateObj.id, battle.SkillSegType.damage)
			_final.real:add(_damage)
			_final.args = _damageArgs
			--穿透目标死亡
...
function SkillModel:onHealToDamageTarget(target, processCfg, final, effectCfg, segId)
	if target:checkOverlaySpecBuffExit(battle.OverlaySpecBuff.healTodamage) then
		...
		if segId == 1 then
			local toDamageData = target:doOverlaySpecBuffFunc(battle.OverlaySpecBuff.healTodamage, "getDamage",
				final.value)
			local allDamage = 0
