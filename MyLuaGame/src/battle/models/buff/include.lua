...
local function updateLifeRoundArg(cfgId, holder, args)
    ...
    -- Đặt mặc định để chặn sự cố: kéo từ args > cfg > 0
    local baseRound = args.lifeRound
    if baseRound == nil then
        baseRound = (buffCfg and buffCfg.lifeRound) or 0
    end
    args.lifeRound = tonumber(baseRound) or 0

    -- Sử dụng pcall để chặn các lỗi cụ thể ảnh hưởng đến các hệ thống khác.
    pcall(function()
        for _, data in holder:ipairsOverlaySpecBuff(battle.OverlaySpecBuff.changeBuffLifeRound) do
            local add = tonumber(data.getExtraRound(buffCfg.group, cfgId)) or 0
            extraLifeRound = extraLifeRound + add
        end
    end)
