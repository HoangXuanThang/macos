...
function ObjectModel:getLastSkillRecord(index)
	local record = self.skillReplaceReocrd[index]
    if not record then
        print("⚠️ No record table at index", index)
        return nil
    end

    local lastId = table.length(record)
    if lastId == 0 then
        print("⚠️ Empty record list at index", index)
        return nil
    end

    local lastRecord = record[lastId]
    if not lastRecord then
        print("⚠️ No last record at index", index, "lastId=", lastId)
        return nil
    end
	return lastRecord.skillId
end
