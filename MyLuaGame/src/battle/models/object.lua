...
function ObjectModel:setBaseData(base)
	local demonCorrect = battleEasy.ifElse(self.force == 1, self.scene.sceneConf.demonCorrectSelf,
		self.scene.sceneConf.demonCorrect)
	local csv_fix = nil
...
function ObjectModel:onInitSkills(skillLevels, additionalPassive)
	...
	if self.unitCfg.combinationSkillId then
		self.passiveSkills[self.unitCfg.combinationSkillId] = newSkillModel(self.scene, self,
			self.unitCfg.combinationSkillId, skillLevel)
	end
...
function ObjectModel:onNewBattleTurn()
	...
	if isSelfTurn then
		...
		self.scene.extraRoundId = self:beInExtraAttack() and self.exAttackCfgId
	end
...
function ObjectModel:onNewExtraBattleTurn()
	...
	if curExtraData then
		...
		self:setExtraAttackMode(curExtraData.mode, curExtraData.cfgId)
...
function ObjectModel:addExtraBattleData(extraTargetId, extraSkillId, mode, extraArgs, cfgId)
...
function ObjectModel:onExtraAttackEnd()
	...
	self.exAttackCfgId = nil
	self.exAttackArgs = nil
end

function ObjectModel:replaceExtraBattleTurnSkill()
	if self.exAttackArgs and self.exAttackArgs.extraSkill then
		local newIdList = {}
		local oldIdList = {}
		local newSkillTypeToId = {}

		for _, skillId in ipairs(battleCsv.doFormula(self.exAttackArgs.extraSkill, self.protectedEnv)) do
		...
		for _, skillId in pairs(self.skillsOrder) do
			local oldSkill = self.skills[skillId]
			if newSkillTypeToId[oldSkill.skillType2] then
				table.insert(newIdList, newSkillTypeToId[oldSkill.skillType2])
				table.insert(oldIdList, skillId)
			end
		end

		self:replaceSkill(oldIdList, newIdList, self.exAttackCfgId)
	end
...
function ObjectModel:resumeExtraBattleTurnSkill()
	if self.exAttackArgs and self.exAttackArgs.extraSkill then
		self:resumeSkill(self.exAttackCfgId)
	end
end

-- 按mode清理额外回合数据
function ObjectModel:clearExtraBattleDataByMode(mode)
	local function clearSkillRound()
...
function ObjectModel:updateBattleRound(skillTimePos)
	self.battleRound[skillTimePos] = self.battleRound[skillTimePos] + 1
	self.battleRoundAllWave[skillTimePos] = self.battleRoundAllWave[skillTimePos] + 1

	for _, obj in self.followExObjs:order_pairs() do
		obj.battleRound[skillTimePos] = obj.battleRound[skillTimePos] + 1
		obj.battleRoundAllWave[skillTimePos] = obj.battleRoundAllWave[skillTimePos] + 1
	end
end
...
function ObjectModel:beAttack(attacker, damage, damageProcessId, damageArgs)
	...
	logf.battle.object.causeDamage(" seat:%d 伤害类型: %d, damage:%f , 总伤害:%f", attacker and attacker.seat or -99,
		damageArgs.from, damage, totalDamage)
	...
	logf.battle.object.beAttack(" %d 攻击-> %d, 当前hp:%f - damage:%f -> 最终hp:%f", attacker and attacker.seat or -99,
		self.seat, self:hp(), damage, hp)
...
function ObjectModel:isHit(target, skillCfg)
	...
	if target.force == self.force then
		delta = self:hit()
	end
