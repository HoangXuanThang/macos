		...
		if processArgs then
			...
			for type, oneEventArgs in pairs(events) do
				for i, obj in ipairs(processArgs.viewTargets) do
					...
					if(effectCfg.effectType == 2)then
						args.exhit = effectCfg.effectRes
					end
...
local getEffectPositionByPos = {
	...
	[7] = function(sprite, offsetPos)
		local cx = -230
		if sprite.model.force == 2 then
			cx = -cx
		end
		return cc.pAdd(cc.p(battle.StandingPos[13].x + cx, battle.StandingPos[2].y + 840), offsetPos)
	end,
}
-- 添加buff特效
