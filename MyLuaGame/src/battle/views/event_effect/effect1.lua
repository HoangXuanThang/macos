...
function SegShow:onPlay()
	...
	if not self.args.processArgs or not self.args.processArgs.values[self.model.id] then
		local processId = (self.args.processArgs and self.args.processArgs.process) and self.args.processArgs.process.id or
			-1
		local unitId = self.model and self.model.unitID or -1
		print("====== SegShow Debug ======")
		print("unitId:", self.model and self.model.unitID)
		print("spriteId:", self.model and self.model.id)
		print("effect cfg id:", self.args and self.args.cfgId)
		print("effect type:", self.args and self.args.effectType)
		print("processArgs:", dump(self.args.processArgs))
		print("raw args:", dump(self.args))
		print("===========================")
	...
	if self.type == battle.SkillSegType.damage then
		self.view:beHit(0, 0, self.args.exhit)
	end
end
...
function SegShow:onUpdate(delta)
	...
	if self.type == battle.SkillSegType.damage then
		self.view:beHit(delta, nil, self.args.exhit)
	end
	if self.tick >= self.waitTick and battleView then
		...
		if not assertInWindows(valueArg, 'effect_event seg is missed!!') then
			...
			if self.numShow then
				if (v > 0) then
					...
					gRootViewProxy:notify('showNumber',
						{ delta = v, skillId = self.args.processArgs.skillId, typ = self.type })
				end
			end
			...
			if self.type == battle.SkillSegType.damage then
				--这个时间暂定,后面根据美术动画效果修改
				self.view:beHit(0, 600, self.args.exhit)
			end
		end
		...
		if self.idx > endSeg or not self.valueArgs[self.idx] then
			if self.type == battle.SkillSegType.damage then --这里的延迟可能会因为被杀死导致不存在了
				...
				battleEasy.effect(self.view.model, function()
					...
					if string.find(self.view.actionState, "hit") ~= nil then
