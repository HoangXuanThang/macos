...
function BattleSprite:onPlayAction(action, time, noQueue)
	...
	if battle.LoopActionMap[action] then
		table.insert(self.battleView.effectJumpCache,
			usedView:onAddEventEffect('effect', { action = action, lifetime = time }, noQueue))
		return
	end
	-- onComplete just for SpriteEffect.onUpdate
	table.insert(self.battleView.effectJumpCache,
		usedView:onAddEventEffect('effect', {
			action = action,
			lifetime = time,
			onComplete = function()
			end
...
function BattleSprite:onShowBuffImmuneEffect(group)
	...
	if groupRelation and groupRelation.immuneEffect then
		gRootViewProxy:notify('showBuffText', self,
			string.format(battle.ShowHeadNumberRes.txtTypeImmune, groupRelation.immuneEffect))
	end
	return
end
...
function BattleSprite:beHit(delta, init, exhit)
	if init then
		self.beHitTime = init
		if self.beHitTime > 0 then
			if (exhit) then
				self:setActionState(exhit)
			else
...
function BattleSprite:onShowHeldItemEffect(itemId)
	...
	if not itemCfg then
		if cc.PLATFORM_OS_WINDOWS == cc.Application:getInstance():getTargetPlatform() then
			print("held_item.items[%d] is nil", itemId)
		end
		return
	end
...
function BattleSprite:getMoveToTargetPos(skillCfg, targets)
	...
	if table.length(targets) > 0 then
		...
		elseif posChoose == 8 then
			...
			--9-溅射目标
		elseif posChoose == 9 then
			local targetNum = #targetViews
			if (targetNum == 4) then
				posIdx = targetViews[2].seat
			elseif (targetNum == 3) then
				if (targetViews[1].seat % 3 == 1) then
					posIdx = targetViews[1].seat
				else
					posIdx = targetViews[2].seat
				end
