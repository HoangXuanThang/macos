...
BattleEndPvpWinView.RESOURCE_BINDING = {
	...
	["imgBestBg.bestName"] = {
		...
		binds = {
			event = "effect",
			data = { outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW }, italic = true }
		}
	},
	...
	["sliderRankBg.head"] = {
		binds = {
			event = "effect",
			data = { outline = { color = ui.COLORS.NORMAL.BLACK }, italic = true }
		}
	},
	["sliderRankBg.rankUp"] = {
		binds = {
	...
	["sliderRankBg.teamScoreBg.text"] = {
		binds = {
			event = "effect",
			data = { outline = { color = ui.COLORS.NORMAL.RED, size = 4 } }
		}
	},
...
function BattleEndPvpWinView:onCreate(sceneID, data, results)
	...
	if results.flag == "crossArena" then
		...
		adapt.oneLineCenterPos(cc.p(400, 80), { self.rank, self.imgUp, self.rankUp }, cc.p(40, 0))
	...
	elseif results.flag == "gymLeader" then
		...
		self.newRecord:loadTexture("city/pvp/reward/panle_gx.png")
			...
			:visible(true)
			:xy(posx, posy)
		local richStr = results.gymMember and gLanguageCsv.gymMemberBattleWin or
			string.format(gLanguageCsv.gymLeaderBattleWin, results.gymName)
	...
	elseif results.flag == "crossMine" then
		local posx, posy = self.imgRankPanel:x(), self.imgRankPanel:y()
		self.newRecord:y(self.newRecord:y() + 40)
		self.imgRankPanel:xy(posx, posy + 50)
		local str = ""
		if serverDataView.speed then
			str = string.format(gLanguageCsv.crossMinePVPSpeed, serverDataView.speed)
		end
		-- if serverDataView.robNum then
		-- 	str = str .. "  " .. string.format(gLanguageCsv.crossMinePVPRob, serverDataView.robNum)
		-- end
	...
	self.sharePanel:y(self.sharePanel:y() - 80)
	self.playBackPanel:y(self.playBackPanel:y() - 80)
end
