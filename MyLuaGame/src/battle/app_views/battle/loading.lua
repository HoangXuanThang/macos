...
function BattleLoadingView:onCreate(data, sceneID, modes, entrance)
	...
	self.bg:texture(string.format("loading/bg_%d.jpg", gCommonConfigArrayCsv.loadingBgTotal[bgIdx]))
	...
	local effect = widget.addAnimationByKey(self:getResourceNode(), "loading/loading_dog.skel", "effect", "effect_loop",
			5)
...
function BattleLoadingView:onLoading()
	...
	-- 加载多个 Plist
	local plists = {
		"battle/txt/txts0.plist",
		"battle/txt/txts1.plist",
		-- 添加更多 Plist 文件路径
	}
	for i, plist in ipairs(plists) do
