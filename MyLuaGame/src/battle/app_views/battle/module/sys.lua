...
function SysSetting:ctor(parent)
	...
	self.buttonCapture = CRenderSprite.newWithNodes(cc.TEXTURE2_D_PIXEL_FORMAT_RGB_A4444, self.pauseButton,
		self.autoButton, self.speedButton, self.passButton)
...
-- 只显示跳过
function SysSetting:onShowSkipButton()
	if self.skipButton then
...
function SysSetting:setSkipButton(opeArgs)
	...
	if self.parent:isMultiOneByOneScenes() then
		self.originPassStr = self.skipButton:get("text"):text()
		self.skipButton:get("text"):text(gLanguageCsv.crossMinePVPSkip)
	end
	text.addEffect(self.skipButton:get("text"), { outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4 } })
...
function SysSetting:onNewBattleRound(args)
	if not (self.parent:isMultiOneByOneScenes() and args.wave and args.totalWave) then
		return
	end
