...
function SpeedRank:ctor(parent)
	...
	if not self.parent:hasGuide() then
		...
		self.captureSprite:setCaptureOffest(cc.p(90, 0))
	end
...
function SpeedRank:initSpecRoundObject()
	...
	local item = self.iconItem:clone()
	local icon = item:get("icon")
	local tipSprite = newCSprite("battle/logo_sxd.png")
	...
	local text = ccui.Text:create("1", "font/youmi.ttf", 48)
	item:addChild(text, 99, "round")
	text:setPosition(cc.p(15 - item:width(), item:height() - 25))
	-- gray 需要缩放
