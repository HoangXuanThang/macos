...
function DebugArea:newBtn(x, y, s)
	...
	btnContent:setPosition(100, 30)
	local btn = ccui.Button:create("common/box/mask_fangd.png")
	btn:add(btnContent)
...
function DebugArea:addFrameControlBtn()
	local btn = self:newBtn(80*2, display.cy + 160*2 - 80, "Next Frame")
	btn:onClick(function()
		display.director:resume()
		print('resume')
...
function DebugArea:addEffectControlBtn()
	local btn = self:newBtn(80*2, display.cy + 160*2 - 2*80, "Next Effect")
	btn:onClick(function()
		display.director:resume()
		print('resume effect')
...
function DebugArea:addEnterCity()
	local btn = self:newBtn(80*2, display.cy + 160*2 - 3*80, "Return city")
	btn:onClick(function ()
...
function DebugArea:addDebugEnabled()
	local flag = false
	local btn = self:newBtn(80*2, display.cy + 160*2 - 8*80, "debug switch")
	btn:onClick(function ()
		flag = not flag
...
function DebugArea:addFullManual(  )
	local flag = false
	local btn = self:newBtn(80*2, display.cy + 160*2 - 9*80, "Global manual off")
	local isAuto
	btn:onClick(function ()
		...
		if scene.isRecord or gate.data.gateType == game.GATE_TYPE.test or gate.data.gateType == game.GATE_TYPE.friendFight then
			if not flag then
				...
				if not gate:isMyTurn() then
					gGameUI:showTip("The attack switches to its own side and then closes")
					return
				end
				scene:setAutoFight(isAuto)
			end
			flag = not flag
			btn.content:setString(flag and "Global manual on" or "Global manual off")
			scene:setFullManual(flag)
		else
			gGameUI:showTip("This scenario does not support global manual")
		end
	end)
end
...
function DebugArea:addAutoTestDebug()
	local btn = self:newBtn(80*2, display.cy + 160*2 - 7*80, "Simulate 100 battles")
	btn:onClick(function ()
		...
		performWithDelay(gGameUI.scene,function()
			...
			-- hacked some data and func
				...
				print(string.format("[INFO] current: %d (%d vs %d)",testTimes,winTimes,loseTimes))
				print(string.format("[INFO] RunTime: (%d/%d), Seed: %d",i,testTimes,data.randSeed))
			end
			print(string.format("[INFO] Total number of combat tests: %d Which wins %d times lose %d times",testTimes,winTimes,loseTimes))
		end,5)
	end)
end
...
function DebugArea:addSpeedControlBtn()
	local btn1 = self:newBtn(950, 150, "Speed")
	btn1:onClick(function()
		...
		gGameUI:showTip("Accelerate, now speed"..new)
		print('speed up', new)
	end)

	local btn2 = self:newBtn(950, 150 - 80, "Slow")
	btn2:onClick(function()
		...
		display.director:getScheduler():setTimeScale(new)
		if new >= 1 then
			gGameUI:showTip("Slow down, now speed"..new)
		else
			gGameUI:cleanTip()
		end
...
function DebugArea:addFrontBgBtn()
	local btn = self:newBtn(1150, 150, "Previous")
	btn:onClick(function ()
		self:chooseBgImage("front")
	end)


	local btn1 = self:newBtn(1150, 150 + 80, "Next")
	btn1:onClick(function ()
		self:chooseBgImage("next")
	end)
end


function DebugArea:addBattleRecordBtn()
	local btn = self:newBtn(1150, 150 - 80, "Combat data")
	local lastUnitIds = {}
	local ignoreUnitIds = {
		[3245] = true, --酋雷姆的冰块
	}
...
function DebugArea:addBattleSpriteDebug()
	local flag = false
	local btn = self:newBtn(1350, 150, "Point display")
	btn:onClick(function ()
		btn.content:setString(flag and "Point display" or "Point hidden")
		flag = not flag
...
function DebugArea:addEffectDebug()
	local flag = false
	local btn = self:newBtn(1350, 150 - 80, "Effect display")
	btn:onClick(function ()
		btn.content:setString(flag and "Effect display" or "Effect hidden")
		flag = not flag
		local objs = gRootViewProxy:call("getSceneObjs")
		for key, sprite in pairs(objs) do
			if sprite.seat < 13 then
...
function DebugArea:addTintColorDebug()
	local flag = false
	local btn = self:newBtn(950, 150 + 80, "TintColor on")
	btn:onClick(function ()
		btn.content:setString(flag and "TintColor on" or "TintColor off")
		flag = not flag
...
function DebugArea:addCRenderTargetDebug()
	local flag = true
	local btn = self:newBtn(1150, 150 + 80*2, "RT off")
	btn:onClick(function ()
		btn.content:setString(flag and "RT on" or "RT off")
		flag = not flag
...
function DebugArea:addA4Debug()
	local flag = false
	local btn = self:newBtn(1150, 150 + 80, "A4 on")
	btn:onClick(function ()
		btn.content:setString(flag and "A4 on" or "A4 off")
		flag = not flag
...
function DebugArea:addThreadDebug()
	local flag = false
	local btn = self:newBtn(1350, 150 + 80*2, "Thread on")
	btn:onClick(function ()
		btn.content:setString(flag and "Thread on" or "Thread off")
		flag = not flag
...
function DebugArea:addSpineDebug()
	local btn = self:newBtn(1350, 150 + 80, "Spine hidden")
	btn:onClick(function ()
		local cnt = 0
		for sp, sprite in pairs(SpineSpritesMap) do
			if tolua.isnull(sp) then
...
function DebugArea:addSpineTimeScaleDebug()
	local step = 10
	local fps = 60
	local btn = self:newBtn(950, 150 + 80 * 2, "Spine fps"..tostring(fps))
	btn:onClick(function ()
		...
		btn.content:setString("Spine fps"..tostring(fps))
	end)
end
