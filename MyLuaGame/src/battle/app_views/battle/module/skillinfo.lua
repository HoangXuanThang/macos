...
function SkillInfo:playMainSkillEffect(widget, state, model)
	...
	if not widget.kuang then
		...
		widget.kuang:setPosition(cc.p(wsize.width / 2, -12))
	end
	-- state = false
...
function SkillInfo:skillInit(args, needMove)
	...
	-- 技能名 伤害类型 技能属性类型
	widget:get("skillName"):setVisible(false):setString(skillCfg.skillName)
	widget:get("skill"):loadTexture(skillCfg.iconRes)
	widget:get("damageType"):loadTexture('battle/skillInfo/type_' .. skillCfg.skillDamageType .. '.png')
	widget:get("skillAttribute"):loadTexture(ui.FIGHT_SKILL_ICON[args.natureType])

	widget:setTouchEnabled(true)
	local zSkillCfg, isZawake = self:isZawakeSkill(skillCfg, model)
	widget:get("skill_m"):setVisible(zSkillCfg ~= nil)
	-- 大招面板动画: 有个水的动画效果
	if orderId == 1 then
...
function SkillInfo:skillButtonInit(args)
	...
	local function showSkillInfo(isShow)
		...
		targetDesc:setString(skillCfg["targetTypeDesc_"..LOCAL_LANGUAGE] or skillCfg.targetTypeDesc)

		-- 添加富文本描述信息
		local descSize = skillDescribe:getContentSize()
		local widthAdapt = math.max(0,
			skillName:getContentSize().width + targetDesc:getContentSize().width - descSize.width)

		-- 多语言适配 cn, tw, kr, en
		local offsetValue = 0
		if matchLanguage({ "en","th","ma","vn","ph" }) then offsetValue = 150 end
		self.descSizeWidth = descSize.width + widthAdapt + offsetValue

		local describe = skillCfg["describe_"..LOCAL_LANGUAGE] or skillCfg.describe
		...
		if zSkillCfg then
			...
			if skillCfg.zawakeEffect[2] == 1 then
				...
				describe = skillCfg["zawakeEffectDesc_"..LOCAL_LANGUAGE] or skillCfg.zawakeEffectDesc
			end
		end
		...
		local skillContent = rich.createWithWidth(
			string.format("#C0x5b545b#%s",
				eval.doMixedFormula(describe, { skillLevel = skillLv or 1, math = math }, nil) or "no desc") .. str, 40,
			nil,
			self.descSizeWidth)
		...
		targetDesc:setPosition(cc.p(self.infoPanelSize.width / 2 + widthAdapt + offsetValue / 2,
			targetDesc:getPositionY()))
	...
	widget:onClick(function()
		...
		if not selectedSkill then
			errorInWindows("selectedSkill is nil model unitID(%s), orginUnitId(%s), skillID(%s), skillsOrder(%s)",
				model.unitID, model.orginUnitId, skillID, dumps(model.skillsOrder or {}))
			return
		end
