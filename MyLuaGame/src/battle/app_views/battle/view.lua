...
battleUIWidget.RESOURCE_BINDING = {
	...
	["midPanel.widgetPanel.wavePanel.wave"] = {
		binds = {
			event = "effect",
			data = { outline = { color = cc.c4b(218, 114, 51, 255) } }
		}
	},
...
function BattleView:onUpdate(delta)
	...
	if self._scene then
		if not self:visible() and not self._scene.isBattleAllEnd then
			self:show()
		end
	end
...
function BattleView:_onShowArenaEndView(results)
	...
	if self.modes.noShowEndRewards or self.modes.isRecord then
		...
		if isWin and not self.modes.isRecord then
			...
			gGameUI:stackUI("battle.battle_end.pvp_fail", nil, gateEndStyles, self.sceneID, self.data, results):z(999)
				:initModes(self.modes)
		end
	else
		-- 回放, 有领取奖励
		self.modes.noShowEndRewards = true
		gGameUI:stackUI("battle.battle_end.pvp_reward",
			{ showEndView = self:createHandler("_onShowArenaEndView", results, true) }, gateEndStyles, results):z(999)
	end
end
...
function BattleView:_onShowCrossArenaEndView(results)
	...
	-- local version = gGameModel.cross_arena:read("version")
		...
		if not self.modes.nextShowStageUp then
			if curdata.stageName == predata.stageName then
				...
				gGameUI:stackUI("battle.battle_end.pvp_win",
					{ showEndView = self:createHandler("_onShowCrossArenaEndView", results, true) }, gateEndStyles,
					self.sceneID, self.data, results):z(999)
...
local gateEndFuncs = {
	...
	[game.GATE_TYPE.gift] = function(self, results)
		if results.result == "win" then
			gGameUI:stackUI("battle.battle_end.simple_activity_win", nil, gateEndStyles, self.sceneID, self.data, results)
				:z(999)
		else
			self:jumpToPveFailView(results, 3)
		end
	...
	[game.GATE_TYPE.crossOnlineFight] = function(self, results)
		if results.showReward then
			gGameUI:stackUI("battle.battle_end.reward", nil, gateEndStyles,
				self:createHandler("_onShowRewardEndView", results), results):z(999)
			return
		end
	...
	[game.GATE_TYPE.braveChallenge] = function(self, results)
		if results.result == "win" then
			gGameUI:stackUI("battle.battle_end.brave_challenge_win", nil, gateEndStyles, self.sceneID, self.data, results)
				:z(999)
		else
			self:jumpToPveFailView(results, 3)
		end
	...
	[game.GATE_TYPE.summerChallenge] = function(self, results)
		if results.result == "win" then
			gGameUI:stackUI("battle.battle_end.activity_challenge_win", nil, gateEndStyles, self.sceneID, self.data,
				results):z(999)
		else
			gGameUI:stackUI("battle.battle_end.pvp_fail", nil, gateEndStyles, self.sceneID, self.data, results):z(999)
				:initModes(self.modes)
	...
	[game.GATE_TYPE.hunting] = function(self, results)
		if results.result == "win" then
			gGameUI:stackUI("battle.battle_end.hunting_win", nil, gateEndStyles, self, self.sceneID, self.data, results)
				:z(999)
		else
			self:jumpToPveFailView(results, 3)
		end
