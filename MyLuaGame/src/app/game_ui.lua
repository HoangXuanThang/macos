...
function GameUI:switchUI(name, ...)
	...
	if stash then
		...
		for i, info in ipairs(self.uiStack) do
			...
			print(info.view:getParent(), info.view:isVisible(), info.view:z(), info.view:getResourceNode(),
				info.view:getResourceNode():isVisible())
...
function GameUI:checkStyles_(name, view, styles)
	...
	printDebug('check full style %s %s %s %s %s %s', view, name, tj.type(view), dumps(size), dumps(styles),
		isDialog(view))
...
function GameUI:showFishDetail(target, params)
	...
	local view = tip.create(name, nil,
		{ relativeNode = target, canvasDir = canvasDir, childsName = childsName, dir = "right" }, params)
	view:onNodeEvent("exit", functools.partial(self.unModal, self, view))
	self:doModal(view)
	self.itemDetailView = view
end
...
local ConnectingRes = {
	"naruto_effect/loadingdog1.skel",
	"naruto_effect/loadingdog2.skel",
}
function GameUI:showConnecting()
	performWithDelay(self.connectingLayer, function()
		...
		local sprite = widget.addAnimationByKey(self.connectingLayer, ConnectingRes[math.random(#ConnectingRes)], "spine",
				"effect_loop")
...
function GameUI:playVideo(path, cb)
	...
	layer:addTouchEventListener(function(sender, eventType)
		if eventType == ccui.TouchEventType.ended or eventType == ccui.TouchEventType.canceled then
			if times == 0 then
				performWithDelay(self.scene, function()
					printInfo("video skip click performWithDelay %d ", times)
					times = 0
				end, 1)
			end
			times = times + 1
