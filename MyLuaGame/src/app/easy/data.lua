...
function dataEasy.getCfgByKey(key)
	...
	if not cfg  then
		printWarn("id(%d) was not in csv", id)
	end
	return cfg
end
...
function dataEasy.getIconResByKey(key)
	return ui.COMMON_ICON[key] or (dataEasy.getCfgByKey(key) and dataEasy.getCfgByKey(key).icon)
end
...
function dataEasy.getAttrTypeValue(key, val)
	local attrKey = game.ATTRDEF_TABLE[key]
	attrKey = 'attr' .. attrKey:gsub("^%l", string.upper)
	local attrType = gLanguageCsv[attrKey]

	return attrType, dataEasy.getAttrValueString(key, val)
end
...
-- @desc 解析关卡
function dataEasy.getGateData(gateId)
	if not gateId or gateId == 0 then
		return nil
	end
	local type = math.floor(gateId / 10000)
	local val = gateId % 10000
	return type, math.floor(val / 100), val % 100
end
...
function dataEasy.getNumeral(num)
	local romanNumeral = {
		"一",
		"二",
		"三",
...
function dataEasy.getStarData(star)
    ...
    local stage = math.floor((star - 1) / perStage)  -- bậc hiện tại (0= vàng, 1= tím, 2= đỏ, 3= mới)
    ...
    for i = 1, perStage do
        local icon = "common/icon/icon_star_d.png" -- mặc định tắt
...
function dataEasy.getActivityIdInYYOPEN(huodongType, independent)
	for _, yyId in ipairs(gGameModel.role:read("yy_open")) do
		local cfg = csv.yunying.yyhuodong[yyId]
		if cfg.type == huodongType then
			if independent then
				if independent == cfg.independent then
					return yyId
				end
			else
				return yyId
...
-- @desc 获取跨服标记
function dataEasy.getServiceTag(serviceKey)
	local keys = string.split(serviceKey, ".")
	if #keys == 1 then
		return keys[1]
	end
	return keys[2]
								...
								if state ~= game.SYNC_SCENE_STATE.start then
									...
									if state == game.SYNC_SCENE_STATE.banpick then
										if okCb then
											...
											gGameUI:stackUI("city.pvp.online_fight.ban_embattle", nil, { full = true },
												{ startFighting = dataEasy.onlineFightStartFighting })
										end
...
function dataEasy.getCitySceneIdx()
	...
	local idx = userDefault.getForeverLocalKey("citySceneIdx", 1, { rawKey = true })
	if csv.cityscene[idx] then
		return idx
	end
	return 1
end
...
--@desc 检查伴侣是否拥有
function dataEasy.checkFereHad(key)
	local fereStates = {}
	-- 伴侣状态
	local feres = gGameModel.role:getIdler("house_lovers"):read()
	for k, v in csvPairs(csv.banlvxiaowu) do
		local lockState = true
		if not feres[k] then
			lockState = dataEasy.isGateFinished(v.gateID)
		end
		fereStates[k] = lockState
	end

	local itemCfg = dataEasy.getCfgByKey(key)
	if itemCfg and itemCfg.type == game.ITEM_TYPE_ENUM_TABLE.fere then
		local fereId = itemCfg.specialArgsMap.fere
		return fereStates[fereId]
	end
	return false
end
...
-- 获取首充奖励信息
function dataEasy.getFirstRechargeData(huodongID)
	local dayCardData = {}
	for k, v in csvPairs(csv.yunying.weeklycard) do
		if v.huodongID == huodongID then
			dayCardData[v.day] = { award = v.award, id = k }
		end
	end
	return dayCardData
end
