...
-- @desc:直购礼包（外部）专场
-- --@t:{direcBuy} 每日显示，点击进入界面后取消
function redHintHelper.activitySessionGift(t)
	for _, id in ipairs(t.yyOpen) do
		local cfg = csv.yunying.yyhuodong[id]
		if cfg.type == YY_TYPE.directBuyGift and cfg.independent == 4 then
			t.id = id
			t.huodongID = cfg.huodongID
			if redHintHelper.activityDirectBuyGiftExternal(t) then
				return true
...
function redHintHelper.nvalue(t)
	...
	return cache.queryRedHint({ "nvalue", dbid }, function()
		local ret, cardId, nvalueLocked, realNValue = checkAndGetCardFields(t, isCardInBattleCards, "card_id",
			"nvalue_locked", "nvalue")
		if not ret then
			return false
		end
...
function redHintHelper.skill(t)
	...
	return cache.queryRedHint({ "skill", dbid }, function()
		local ret, cardLv, skills, cardId, skinId, star, advance = checkAndGetCardFields(t, isCardInBattleCards, "level",
			"skills", "card_id", "skin_id", "star", "star", "advance")
		if not ret then
			return false
		end
...
function redHintHelper.effortValue(t)
	...
	return cache.queryRedHint({ "effortValue", dbid }, function()
		local ret, cardId, effortValue, effort_advance, level = checkAndGetCardFields(t, isCardInBattleCards, "card_id",
			"effort_values", "effort_advance", "level")
		if not ret then
			return false
		end
		...
		for i, v in orderCsvPairs(csv.card_effort) do
			if v.advance == 1 then
				...
				local maxVal, total
				maxVal, total, isNotMaxAdvance, isCanLevelUp = dataEasy.getCardEffortMax(i, cardId, attrType,
					effort_advance, level)
...
function redHintHelper.cardDevelop(t)
	...
	return cache.queryRedHint({ "cardDevelop", dbid }, function()
		local ret, cardId, level, advance, star = checkAndGetCardFields(t, isCardInBattleCards, "card_id", "level",
			"advance", "star")
		if not ret then
			return false
		end
...
function redHintHelper.levelRightBtnGift(t)
	return isMapStarAward(t.chapterId, t.mapStar, "right") or
		isGateStarAward(t.chapterId, t.gateStar, "right", t.mapType)
end
...
-- @desc:新首冲红点
-- @t:{firstRecharge} firstRecharge:今天是否点击
function redHintHelper.activityWeeklyCardFR(t)
	local activityId = t.activityId
	if not activityId then
		return false
	end
	...
	for _, id in pairs(t.yyOpen) do
		...
		if (cfg.independent == 1) and cfg.type == YY_TYPE.weeklyCardFR then
			local huodong = t.yyHuodongs[id] or {}
			if huodong.buy then
				for id, v in csvPairs(csv.yunying.weeklycard) do
					if v.huodongID == cfg.huodongID then
						...
						-- 没领取完
						if huodong.stamps and huodong.stamps[id] == 1 then
							return true
						end
					end
				end
	...
	return (not t.weeklyCardFRClick)
end

-- @desc:永久特权卡
-- @t:{supremeCard} supremeCard:今天是否点击
function redHintHelper.supremeCard(t)
	local activityId = t.activityId
	if not activityId then
		return false
	end

	return not t.supremeCardClick
end
...
function redHintHelper.starSwapAid(t)
	return false
end

function redHintHelper.starSwapExchange(t)
	if t.starExchangeDailyClick or not dataEasy.isUnlock(gUnlockCsv.cardStarSwap) then
		return false
	end

	local sPropCount = dataEasy.getNumByKey(StarTools.SPROPKEY)
	local sPlusPropCount = dataEasy.getNumByKey(StarTools.SPLUSPROPKEY)
	local allCount = sPropCount + sPlusPropCount

	if t.starExchange[3] then
		allCount = allCount + t.starExchange[3]
	else
		allCount = allCount + gCommonConfigCsv.cardStarSwapRaritySDefaultTimes
	end

	if allCount > 0 or StarTools.getReceiveCount() > 0 then
		return true
	end
...
function redHintHelper.starSwapReceiveCount(t)
	if not dataEasy.isUnlock(gUnlockCsv.cardStarSwap) then
		return false
	end
	return StarTools.getReceiveCount() > 0
end
...
function redHintHelper.unionDailyGift(t)
	if not t.unionId or not t.unionLevel then return false end
...
function redHintHelper.arenaAward(t)
	...
	for i, v in pairs(t.rankAward) do
		if v == 1 and csv.pwrank_award[i] then
...
function redHintHelper.onHonourableVip(t)
	...
	local roleVip = gGameModel.role:read("vip_level") or 0
...
-- 抽卡，钻石抽卡，金币抽卡，饰品抽卡，探险寻宝，符石抽取, 秘籍抽取
...
function redHintHelper.dailyAssistantAdventure(t)
	...
	if dailyAssistantTools.getEndlessTowerRedHintState() then
		return true
	end
	return false
end
...
-- function redHintHelper.crossSupremacyAward(t)
-- 	local round = t.crossSupremacyRound

-- 	if round ~= "closed" and t.crossSupremacyRemind then
-- 		return true
-- 	end
...
local function getCardStar(cardID)
	local star = 0

	if cardID > 0 then
		local curCardCsv = csv.cards[cardID]

		local cards = gGameModel.role:getIdler("cards") -- 卡牌
		for i, v in ipairs(cards:read()) do
			local card = gGameModel.cards:find(v)
			if card then
...
-- 限定神兽升星
function redHintHelper.limitUpgradeSprite(t)
	local activityId = t.activityId
	local yyCfg = csv.yunying.yyhuodong[activityId]
	local yyData = t.yyHuodongs[activityId] or {}

	local curLockID = yyCfg.paramMap.card
	local cardStar = getCardStar(curLockID)

	for key, data in orderCsvPairs(csv.yunying.card_award) do
		if yyCfg.huodongID == data.huodongID and curLockID == data.cardid then
			if not yyData.info or (not yyData.info[tostring(key)]) then
				if cardStar >= data.star then
					return true
				end
			end
		end
...
-- 本命神兽升星
function redHintHelper.customizeUpgradeSprite(t)
	local activityId = t.activityId
	local yyCfg = csv.yunying.yyhuodong[activityId]
	local yyData = t.yyHuodongs[activityId] or {}

	local info = yyData.info

	local curLockID = info and info.card_id or 0
	if curLockID == 0 then
		local defCardID = string.split(yyCfg.paramMap.card, ",")[1]
		curLockID = tonumber(defCardID)
	end
	...
	for key, data in orderCsvPairs(csv.yunying.card_award) do
		if yyCfg.huodongID == data.huodongID and curLockID == data.cardid then
			if not info or (not info[tostring(key)]) then
				if cardStar >= data.star then
					return true
				end
			end
...
function redHintHelper.activityMiixBuyGift(t)
	return not t.miixGiftClick
end

-- 主题皮肤签到
function redHintHelper.activityThemeSkinSignIn(t)
	for _, id in ipairs(t.yyOpen) do
		local yyCfg = csv.yunying.yyhuodong[id]
		if yyCfg.type == YY_TYPE.LoginGift and yyCfg.independent == 7 then
			if t.yyHuodongs[id] then
				local stamps = t.yyHuodongs[id].stamps or {}
				local huodongID = yyCfg.huodongID
...
function redHintHelper.activityThemeSkinGift(t)
	return not t.themeSkinGiftClick
end

function redHintHelper.activityThemeSkinGive(t)
	return not t.themeSkinGiveClick
end

function redHintHelper.activityThemeSkinExchange(t)
	return not t.themeSkinExchangeClick
end

-- 活动宝箱次数奖励
function redHintHelper.activityDrawBoxAward(t)
	if not t.drawBoxType then
		return false
	end
	local activityId = t.activityId
	-- local yyCfg = csv.yunying.yyhuodong[activityId]
	...
	local draw_sum_box = t.draw_sum_box[t.drawBoxType] or {}

	for k, v in orderCsvPairs(csv.draw_count) do
		if v.huodongID == activityId then
			local boxState = draw_sum_box[k] or 1
			if v.count <= limit_counter_all and boxState == 1 then
				return true
			end
		end
	end
...
-- 圣诞活动
function redHintHelper.activityChristmasWishGift(t)
	return not t.christmasClick
end

function redHintHelper.activityChristmasWish(t)
	local currNum = t.items[game.ITEM_TICKET.christmasItem] or 0
	return currNum > 0
end

function redHintHelper.activityZhutiYingxiongShop(t)
	return not t.zhutiYingxiongShopClick
end

function redHintHelper.playPassportZhutiYingxiong(t)
	for _, id in ipairs(t.yyOpen) do
		local yyCfg = csv.yunying.yyhuodong[id]
		if yyCfg.type == YY_TYPE.playPassport and yyCfg.paramMap.type == game.PASSPORT_IDX.T_ZHUTI_YINGXIONG then
			local yyData = t.yyHuodongs[id]
			if yyData then
				for key, val in pairs(yyData.stamps) do
					if val == 1 then
...
function redHintHelper.activityZhutiYingxiongSignIn(t)
	for _, id in ipairs(t.yyOpen) do
		local yyCfg = csv.yunying.yyhuodong[id]
		if yyCfg.type == YY_TYPE.LoginGift and yyCfg.independent == game.SIGNIN_TYPE.T_ZHUTI_YINGXIONG then
			if t.yyHuodongs[id] then
				local stamps = t.yyHuodongs[id].stamps or {}
				local huodongID = yyCfg.huodongID
				for k, v in csvPairs(csv.yunying.loginweal) do
...
-- 签到
function redHintHelper.activitySignIn(t)
	for _, id in ipairs(t.yyOpen) do
		local yyCfg = csv.yunying.yyhuodong[id]
		if yyCfg.type == YY_TYPE.LoginGift and yyCfg.independent == t.independent then
			if t.yyHuodongs[id] then
				local stamps = t.yyHuodongs[id].stamps or {}
				local huodongID = yyCfg.huodongID
				for k, v in csvPairs(csv.yunying.loginweal) do
...
-- 签到
function redHintHelper.activityDarwDayGift(t)
	for _, id in ipairs(t.yyOpen) do
		local yyCfg = csv.yunying.yyhuodong[id]
		if yyCfg.type == YY_TYPE.LoginGift and yyCfg.independent == 3 and (not yyCfg.paramMap.type) then
			if t.yyHuodongs[id] then
				local stamps = t.yyHuodongs[id].stamps or {}
				local huodongID = yyCfg.huodongID
				for k, v in csvPairs(csv.yunying.loginweal) do
...
-- 通用 每日红点提示 game.DAILY_REDHINT 增加类型 listenData sign 区分
function redHintHelper.redHintDaily(t)
	-- local activityId = t.activityId
	if t.sign then
		if type(t.sign) == "table" then
			for sign in pairs(t.sign) do
				if not t.redHintDailyClick[sign] then
					return true
				end
			end
		else
			if not t.redHintDailyClick[t.sign] then
				return true
			end
		end
	end
...
function redHintHelper.activityJiuWeiSignIn(t)
	for _, id in ipairs(t.yyOpen) do
		local yyCfg = csv.yunying.yyhuodong[id]
		if yyCfg.type == YY_TYPE.LoginGift and yyCfg.independent == 8 then
			if t.yyHuodongs[id] then
				local stamps = t.yyHuodongs[id].stamps or {}
				local huodongID = yyCfg.huodongID
				for k, v in csvPairs(csv.yunying.loginweal) do
...
function redHintHelper.activityJiuWeiGift(t)
	return not t.jiuWeiGiftClick
end

function redHintHelper.activityJiuWeiExchange(t)
	return not t.jiuWeiExchangeClick
end

function redHintHelper.playPassportJiuWei(t)
	for _, id in ipairs(t.yyOpen) do
		local yyCfg = csv.yunying.yyhuodong[id]
		if yyCfg.type == YY_TYPE.playPassport and yyCfg.paramMap.type == 7 then
			local yyData = t.yyHuodongs[id]
			if yyData then
				for key, val in pairs(yyData.stamps) do
					if val == 1 then
...
function redHintHelper.monopolyWalkTask(t)
	local activityId = t.activityId
...
function redHintHelper.monopolyWalkBoxAward(t)
	local activityId = t.activityId
...
-- 登天之阶
function redHintHelper.activityThroneSignIn(t)
	for _, id in ipairs(t.yyOpen) do
		local yyCfg = csv.yunying.yyhuodong[id]
		if yyCfg.type == YY_TYPE.LoginGift and yyCfg.independent == game.SIGNIN_TYPE.T_THRONE_DRAW then
			if t.yyHuodongs[id] then
				local stamps = t.yyHuodongs[id].stamps or {}
				local huodongID = yyCfg.huodongID
				for k, v in csvPairs(csv.yunying.loginweal) do
...
function redHintHelper.playPassportThrone(t)
	for _, id in ipairs(t.yyOpen) do
		local yyCfg = csv.yunying.yyhuodong[id]
		if yyCfg.type == YY_TYPE.playPassport and yyCfg.independent == game.PASSPORT_IDX.T_THRONE_DRAW then
			local yyData = t.yyHuodongs[id]
			if yyData then
				for key, val in pairs(yyData.stamps) do
					if val == 1 then
...
function redHintHelper.activityThrone(t)
	if redHintHelper.activityThroneSignIn(t) then
		return true
	end

	if redHintHelper.playPassportThrone(t) then
		return true
	end

	t.sign = game.DAILY_REDHINT.throneShop
	if redHintHelper.redHintDaily(t) then
		return true
	end
	t.sign = game.DAILY_REDHINT.throneExchange
	if redHintHelper.redHintDaily(t) then
		return true
	end
...
function redHintHelper.activityDrawLimitExplor(t)
	local yyhuodong = t.yyHuodongs[t.activityId]
	local freeDrawTimes = 0
	if yyhuodong and yyhuodong.info then
		local info = yyhuodong.info
	...
	t.activityId = dataEasy.getActivityIdInYYOPEN(YY_TYPE.drawLimitTask, game.GENERAL_TASK_IND.DRAW_EXPLOR)
	if freeDrawTimes > 0 or redHintHelper.activityGeneralTask(t) then
		return true
	end

	t.sign = game.DAILY_REDHINT.drawExplorGift
	if redHintHelper.redHintDaily(t) then
		return true
	end
	t.sign = game.DAILY_REDHINT.drawExplorEx
	if redHintHelper.redHintDaily(t) then
		return true
	end
...
function redHintHelper.activityDrawLimitHelditem(t)
	local yyhuodong = t.yyHuodongs[t.activityId]
	local freeDrawTimes = 0
	if yyhuodong and yyhuodong.info then
		local info = yyhuodong.info
	...
	t.activityId = dataEasy.getActivityIdInYYOPEN(YY_TYPE.drawLimitTask, game.GENERAL_TASK_IND.DRAW_HELDITEM)
	if freeDrawTimes > 0 or redHintHelper.activityGeneralTask(t) then
		return true
	end

	t.sign = game.DAILY_REDHINT.drawHeldItemGift
	if redHintHelper.redHintDaily(t) then
		return true
	end
	t.sign = game.DAILY_REDHINT.drawHeldItemEx
	if redHintHelper.redHintDaily(t) then
		return true
	end
