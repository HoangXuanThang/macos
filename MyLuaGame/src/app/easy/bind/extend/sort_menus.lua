...
function sortMenus:initExtend()
	...
	text.addEffect(btnTitle, {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 3}, color = color})
	btn:get("img"):x(btnWidth - 45 * (self.btnType == 2 and 0.9 or 1))
	...
	idlereasy.any({self.data, self.lock}, function(obj, data, lock)
		...
		if not self.expandUp then
			...
			node.list:size(width+6, height * maxCount + marg)
				:anchorPoint(0, 1)
...
function sortMenus:onItem_(list, node, k, v)
	...
	title:text(v.name)
	local color = v.selected and ui.COLORS.NORMAL.WHITE or ui.NEWCOLORS.WHITE1
