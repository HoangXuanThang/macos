...
function drawAttr:refresh(nvalue)
	...
	for i, v in ipairs(self.itemNodes) do
		...
		v:xy(itemPos[i])
		text.addEffect(v:get("num"), { color = self.texColor })
		text.addEffect(v:get("txt"), { color = self.texColor })
	end
	...
	for i = 1, #pointTab do
		...
		drawNode:drawPolygon(ps, 3, cc.c4f(0x71 / 255, 0xff / 255, 0xb7 / 255, 0.6), 0.5,
			cc.c4f(0x71 / 255, 0xff / 255, 0xb7 / 255, 0.6))
		idx = idx + 1
	end
