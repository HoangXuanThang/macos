...
function iconByKey:initExtend()
	...
	helper.callOrWhen(self.data, function(data)
		...
		if data.key == "card" then
			...
			local scale = 2
			if (id ~= nil and id >= 8500 and id < 8700) then
				scale = 1.3
			end
			panel:get("icon"):texture(path):scale(scale):z(5)
...
function iconByKey:setNum(num, targetNum, quality, noColor)
	...
	if not targetNum then
		...
		text.addEffect(label1, { outline = { color = ui.NEWCOLORS.OUTLINE.BLACK, size = outlineSize } })
		text.addEffect(label2, { outline = { color = ui.NEWCOLORS.OUTLINE.BLACK, size = outlineSize } })
		...
		if not noColor then
			label2Color = (num >= targetNum) and ui.NEWCOLORS.GREEN2 or ui.NEWCOLORS.REDLIGHT
		end
		label1:setFontSize(fontSize)
		label2:setFontSize(fontSize)
		text.addEffect(label1, { outline = { color = ui.NEWCOLORS.OUTLINE.BLACK, size = outlineSize } })
		text.addEffect(label2,
			{ color = label2Color, outline = { color = ui.NEWCOLORS.OUTLINE.BLACK, size = outlineSize } })
...
function iconByKey:setItemState(data)
	...
	helper.callOrWhen(self.specialKey, function(specialKey)
		...
		if specialKey.lv then
			...
			text.addEffect(lv, { outline = { color = ui.NEWCOLORS.OUTLINE.BLACK } })
		end
		-- panel:removeChildByName('gemLevelBg')
		panel:removeChildByName('gemLevel')
		if specialKey.leftTopLv then
			...
			text.addEffect(level, { color = ui.COLORS.NORMAL.WHITE, outline = { color = ui.NEWCOLORS.OUTLINE.BLACK } })
		...
		if specialKey.maxStar and dataEasy.isFragment(data.key) then
			...
			if cardCsv and dataEasy.getCardMaxStar(cardCsv.cardMarkID) == 12 then
				local label = cc.Label:createWithTTF(gLanguageCsv.maxStar, "font/youmi.ttf", 40)
