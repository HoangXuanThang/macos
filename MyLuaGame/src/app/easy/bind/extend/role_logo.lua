...
function roleLogo:showLogo(logoImg)
	...
	local mask = cc.Sprite:create("common/box/panel_portrait_d.png")
		:alignCenter(size)
...
function roleLogo:showFrame()
	...
	helper.callOrWhen(self.frameId, function(frameId)
		...
		if frameId == false then
			...
			local isSpine = string.find(frameRes, ".json")
			if isSpine then
				frameImg:hide()
				local frameSpine = widget.addAnimationByKey(self.panel, frameRes, "frameSpine", "default", 4, true)
					:alignCenter(size):scale(1.4)
...
function roleLogo:initExtend()
	...
	local bottomImg = ccui.ImageView:create("common/box/panel_portrait_d.png")
