...
function cardIcon:initExtend()
	...
	text.addEffect(frameNumNode, { outline = { color = ui.NEWCOLORS.OUTLINE.BLACK, size = 3 } })
	...
	helper.callOrWhen(self.unitId or self.cardId, function(id)
		...
		if id == -1 then
			...
			if dev.SHOW_MAX_STAR then
				...
				if cards then
					local maxStar = 0
					local markId = csv.unit[unitId] and csv.cards[csv.unit[unitId].cardID] and
						csv.cards[csv.unit[unitId].cardID].cardMarkID
...
function cardIcon:setStar(panel) 
	...
	helper.callOrWhen(self.star, function(star) 
		...
		if star > 0 then 
			local perStage = 6 
			local icons = { 
				"city/card/equip/icon_star.png",   -- vàng 
				"common/icon/icon_star_z1.png",    -- tím 
				"common/icon/icon_star_r1.png",    -- đỏ 
				"common/icon/icon_star_s1.png",    -- mới 
			} 
 
			local starNum = star > 6 and 6 or star  -- luôn hiển thị 6 icon nhỏ 
			local firstStar = star - starNum + 1 
			if firstStar < 1 then firstStar = 1 end 
 
			-- Tạo mảng để lưu thông tin các sao cần hiển thị
			local starsToShow = {}
			
			for i = 1, starNum do 
				local i_real = firstStar + i - 1 
				local stage = math.floor((i_real - 1) / perStage) + 1 
				local posInStage = (i_real - 1) % perStage + 1 
 
				-- tính màu: sao của bậc hiện tại 
				local icon = icons[stage] or icons[#icons] 
 
				-- nếu sao hiển thị là sao của bậc trước, dùng màu bậc trước 
				if stage > 1 and i_real <= (stage - 1) * perStage then 
					icon = icons[stage - 1] or icons[1] 
				end 
 
				starsToShow[i] = {icon = icon, stage = stage}
			end
			
			-- Sắp xếp lại: sao bậc cao nhất ở đầu (bên trái)
			table.sort(starsToShow, function(a, b)
				return a.stage > b.stage  -- bậc cao hơn đứng trước
			end)
			
			-- Hiển thị các sao từ trái sang phải với khoảng cách đều và căn giữa
			local interval =(self.params.starInterval or 18) + 8 -- khoảng cách giữa các sao
			local totalWidth = interval * (starNum - 1)  -- tổng chiều rộng của dãy sao
			local startX = (size.width - totalWidth) / 2  -- căn giữa
			
			for i = 1, starNum do
				local xPos = startX + interval * (i - 1)  -- từ trái sang phải
				
				ccui.ImageView:create(starsToShow[i].icon) 
					:xy(xPos, 22) 
					:addTo(panel:get("star"), 4, "star") 
					:scale(self.params.starScale or 0.8) 
			end 
		end 
	end) 
end
