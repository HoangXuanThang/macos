...
function uiEasy.setIconName(key, num, params)
	...
	effect = { color = ui.COLORS.QUALITY[quality] }
	if params.node then
		...
		if params.width then
			if params.node:width() > params.width then
				...
				adapt.setTextAdaptWithSize(params.node,
					{ size = cc.size(params.width, nodeHeight * 3), vertical = "top" })
			end
		end
	end
	return name, effect
...
function uiEasy.getIconDesc(key, num)
	...
	if dataEasy.isFragment(key) then
		desc = string.format(cfg["desc_"..LOCAL_LANGUAGE] or cfg.desc, cfg.combCount)
	else
		desc =  cfg["desc_"..LOCAL_LANGUAGE] or cfg.desc
	end
	return desc
end
...
function uiEasy.showDialog(name, params, styles)
	...
	elseif name == "rmb" then
		gGameUI:showDialog(
			{ title = gLanguageCsv.rmbNotEnough, content = content or gLanguageCsv.noDiamondGoBuy, cb = tryOpenRecharge, btnType = 2, clearFast = true },
			styles)

		-- 购买已上限，提升vip
	elseif name == "vip" then
		local defaultContent = { string.format(gLanguageCsv.commonTodayMax, params.titleName), string.format(
			gLanguageCsv.commonVipIncrease, params.titleName) }
		gGameUI:showDialog(
			{
				...
				btnType = 2,
				btnStr =
					gLanguageCsv.showVip,
				clearFast = true
			}, styles)
	else
		local cfg = dataEasy.getCfgByKey(name)
		if cfg then
			local name = cfg['name_'..LOCAL_LANGUAGE] or cfg.name
			gGameUI:showTip(content or string.format(gLanguageCsv.coinNotEnough, name))
...
function uiEasy.checkText(text, params, needSpecialChar)
	params = params or {}
	local noBlackList = true                       -- params.noBlackList or false
...
function uiEasy.setTitleEffect(parent, effectName, params)
	if params and params.mega then
		...
		local effectBg = widget.addAnimationByKey(parent, "level/jiesuanshengli.skel", "effectBg", "jiesuan_shenglitu",
			10)
		effect:setSpriteEventHandler(function(event, eventArgs)
...
function uiEasy.setPrivilegeRichText(privilegeType, parent, txt, pos, isBracket, color)
	local privilegeNum = dataEasy.getPrivilegeVal(privilegeType)

	local colorstr = color and color or "#C0xFFFFFF#"
	...
	if privilegeNum and privilegeNum ~= 0 then
		...
		if isBracket then
			str = colorstr ..
				"(" .. string.format(gLanguageCsv.currentPrivilege, txt, tostring(privilegeNum)) .. colorstr .. ")"
		else
			str = colorstr .. string.format(gLanguageCsv.currentPrivilege, txt, tostring(privilegeNum))
		end
...
function uiEasy.setCardNum(panel, num, targetNum, quality, noColor)
	...
	if not targetNum then
		...
		if not noColor then
			local color = (num >= targetNum) and ui.NEWCOLORS.GREEN2 or ui.NEWCOLORS.REDLIGHT
...
function uiEasy.setTextShader(title, state, effect, gayEffect)
	if state == 1 then
		if title then
			effect = effect or { color = ui.NEWCOLORS.NORMAL.DULL_ORANGE }
			text.addEffect(title, effect)
		end
	else
		if title then
			gayEffect = gayEffect or { color = ui.NEWCOLORS.DISABLED.WHITE }
			text.deleteAllEffect(title)
			text.addEffect(title, gayEffect)
		end
	end
end
...
function uiEasy.isOpenMystertShop()
	...
	local mysteryShopEndTime = gGameModel.mystery_shop:read("last_active_time")
	...
	local leftTime = mysteryShopEndTime + live - 1 - curTime
	return leftTime > 0 and roleLv >= minLv and mysteryTimes < maxTime, leftTime
end

local mysteryShopLastEndTime = 0
function uiEasy.showMysteryShop()
	local mysteryShopEndTime = gGameModel.mystery_shop:read("last_active_time")
	if uiEasy.isOpenMystertShop() and mysteryShopEndTime ~= mysteryShopLastEndTime then
		mysteryShopLastEndTime = mysteryShopEndTime
...
function uiEasy.setSkillInfoToItems(items, skillCfgOrId)
	...
	if skillCfg.iconRes then
		iconPath = skillCfg.iconRes
	end
	...
	if items.name then
		items.name:text(skillCfg["skillName_"..LOCAL_LANGUAGE] or skillCfg.skillName)
	end
	if items.type1 then
	...
	if items.target then
		items.target:text(skillCfg["targetTypeDesc_"..LOCAL_LANGUAGE] or skillCfg.targetTypeDesc)
		if items.name then
			local len = items.target:width() + items.name:width()
			local max = items.target:x() - items.name:x()
			if len > max then
				adapt.setTextAdaptWithSize(items.name,
					{ size = cc.size(max - items.target:width(), items.name:height() * 2), vertical = "center" })
			end
		end
	end
end
...
function uiEasy.getStarSkillDesc(params, typ)
	...
	if skillCsv.starEffect and csvSize(skillCsv.starEffect) > 0 then
		local color = myStar >= skillCsv.starEffect[1] and "#C0x60c456#" or "#C0x979389#"
		...
		local desc = skillCsv["starEffectDesc_"..LOCAL_LANGUAGE] or skillCsv.starEffectDesc
...
function uiEasy.getStarPanel(star, params)
	...
	-- Tính bậc hiện tại (1 = vàng, 2 = tím, 3 = đỏ, 4 = đặc biệt S)
    local tier = math.ceil(star / 6)
    local bright = star % 6
    if bright == 0 and star > 0 then bright = 6 end

    for i = 1, num do
        local res = "common/icon/icon_star_d.png"  -- mặc định mờ

        if i <= bright then
            -- sao sáng theo bậc hiện tại
            if tier == 1 then
                res = "common/icon/icon_star.png"      -- vàng
            elseif tier == 2 then
                res = "common/icon/icon_star_z.png"    -- tím
            elseif tier == 3 then
                res = "common/icon/icon_star_r.png"    -- đỏ
            elseif tier >= 4 then
                res = "common/icon/icon_star_s.png"    -- đặc biệt
            end
        else
            -- sao mờ theo bậc hiện tại
            res = "common/icon/icon_star_d.png"
        end
...
function uiEasy.addTextEffect1(textNote)
	text.addEffect(textNote,
		{ outline = { color = cc.c4b(255, 84, 0, 255), size = 3 }, glow = { color = cc.c4b(255, 71, 0, 255) } })
end
...
function uiEasy.getVipStr(level, short)
	...
	local isSupreme = game.VIP_SUPREME <= lv
	local name = isSupreme and "textSupreme" or "textVIP"
	local title = gLanguageCsv[name]
...
-- {id = i, num = myItemNum, targetNum = needItemNum}
function uiEasy.goGuideGift(itemsList)
	local roleLv = gGameModel.role:read("level")
	if roleLv < 20 then
		return
	end
	...
	-- 指引礼包分类
	local allGuideGiftList = {}
	for id, data in orderCsvPairs(csv.yunying.guidegift) do
		allGuideGiftList[data.itemID] = allGuideGiftList[data.itemID] or {}
		table.insert(allGuideGiftList[data.itemID], id)
	end

	local showGuideGifts = gGameModel.role:getIdler("guide_gifts"):read()
	local nowTime = time.getTime()

	-- 查找指引购买礼包
	for k, v in pairs(itemsList) do
		local foundGiftDatas = allGuideGiftList[v.id]
		local currNum = v.num or dataEasy.getNumByKey(v.id)
		if foundGiftDatas and currNum < v.targetNum then
			foundItemID = v.id
			for _, giftID in pairs(foundGiftDatas) do
				local giftData = showGuideGifts[giftID]
				if giftData then
					if nowTime < giftData.expired_date then
						-- 未过期
						if not giftData.flag or giftData.flag == 0 then
							-- 未购买的
							return
						end
					end
	...
	if foundItemID then
		gGameApp:requestServer("/game/role/guide/gift/active", function(tb)
			if state == "false" then
				if not gGameUI:goBackInStackUI("city.activity.guide_gift") then
					gGameUI:stackUI("city.activity.guide_gift", nil, nil, {
