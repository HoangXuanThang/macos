			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					if v.select then
						normal:hide()
						panel = selected:show()
						text.addEffect(panel:get("txt"), { outline = { color = ui.COLORS.OUTLINE.WHITE } })
					else
						selected:hide()
						panel = normal:show()
					end
...
function LoginPlacardView:onCreate(notice)
	...
	self.leftPanel:onClick(function()
		...
		node:add(blackLayer, -99):add(box):add(btn):xy(display.sizeInView.width / 2, display.sizeInView.height / 2)
			:addTo(gGameUI.scene, 999)
	end)
...
function LoginPlacardView:onCheckBox()
	...
	self.checkStatus:texture(self.checkStatusVisible and "common/icon/radio_selected.png" or
	...
	for i = 1, count do
		local value = {
			...
			effects = {
				...
				['outline'] = self.selected == i and { color = ui.COLORS.OUTLINE.ORANGE } or { color = ui.COLORS.OUTLINE
					.DEFAULT },
			}
		}
...
function LoginPlacardView:setStyle1List(contents)
	...
	for _, content in ipairs(contents) do
		...
		cloneItem:show()
		-- 内容
		local richtext = rich.createWithWidth("#C0x57472E#" .. content.content, 40, deltaSize, richtextWidth)
	...
	clickFunc = function(clickedItem)
		...
		if not clickedItem.isDropDown then -- 给item增加下拉的内容显示
			...
			local function addItem(v)
				...
				item:show()
				local richtext = rich.createWithWidth("#C0x57472E#" .. v.content, 40, deltaSize, kuangWidth - 40) -- richText 内容(目前暂时只设置了文本的)
				local textSize = richtext:size()
				richtext:anchorPoint(0, 1)
				kuangHeight = kuangHeight + textSize.height --+30*2+30*2 + 40
				richtext:xy(10, kuangHeight - 120)
		...
		newItem:xy(newItem:size().width / 2 - 60, newHeight - 44)
		newItem:show()
		-- 先删除, 再添加(这个顺序从0开始的)
