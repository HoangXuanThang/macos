		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					local childs = node:multiget("icon", "note", "num", "bar","barBg")
					...
					-- Thu nhỏ thanh tiến trình (foreground)
					childs.bar:setContentSize(cc.size(400, childs.bar:getContentSize().height))
					childs.bar:setPositionX(childs.bar:getPositionX() + 80)

					-- Thu nhỏ nền (background)
					childs.barBg:setContentSize(cc.size(400, childs.barBg:getContentSize().height))
					childs.barBg:setPositionX(childs.barBg:getPositionX() + 80)
				end,
				asyncPreload = 6,
			},
		},
...
function CardDetailView:onCreate(params)
	...
	local simDes = skillCsv["simDesc_"..LOCAL_LANGUAGE] or skillCsv.simDesc

	beauty.textScroll({
		list = self.skillDescribeList,
		strs = "#C0xFFFFFF#" .. simDes,
		isRich = true,
		fontSize = 40,
	})
	...
	local skillName = skillCsv["skillName_"..LOCAL_LANGUAGE] or skillCsv.skillName
