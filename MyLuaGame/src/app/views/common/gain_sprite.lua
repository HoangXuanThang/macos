...
function GainSpriteView:onCreate(data, hadSprite, showBtn, cb)
	...
	local list = beauty.textScroll({
		...
		strs = skillCfg["simDesc_"..LOCAL_LANGUAGE] or skillCfg.simDesc,
	})
	list:xy(self.skillContent:box())
		:z(self.skillContent:z())
		:addTo(self.skillContent:parent())

	self.skillName:text(skillCfg["skillName_"..LOCAL_LANGUAGE] or skillCfg.skillName)
	...
	if unitCfg.showOpen then
		self.photoSprite = widget.addAnimation(self.cardImg, unitCfg.showOpen, "default", 5, true)
		if string.find(unitCfg.showOpen, ".png") then
			self.photoSprite:xy(size.width / 2, unitCfg.showOpenOffsetY):scale(1.1)
		else
			self.photoSprite:xy(size.width / 2, unitCfg.showOpenOffsetY)
		end
	end

	uiEasy.setIconName("card", cardData.card_id,
		{ node = self.txtName, name = cardData.name, advance = cardData.advance, space = true })
