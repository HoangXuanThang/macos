...
BuyInfoView.RESOURCE_BINDING = {
	...
	["content.num"] = {
		...
		binds = {
			event = "effect",
			data = { color = cc.c4b(0xC9, 0x7D, 10, 255) }
		},
	},
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					if list.flag ~= BUY_INFO.sell then
						text.addEffect(childs.price,
							{
								color = dataEasy.getNumByKey(v.key) >= price and ui.NEWCOLORS.BROWN1 or
									ui.COLORS.NORMAL.ALERT_ORANGE
							})
					end
...
function BuyInfoView:onCreate(costMap, itemData, params, cb)
	...
	if cfg.type == game.ITEM_TYPE_ENUM_TABLE.staminaRecover then
		...
		local remainder
		self.canUseMaxNum, remainder = math.modf((game.STAMINA_LIMIT - gGameModel.role:read("stamina")) /
			self.itemAddStamina)
	...
	beauty.singleTextAutoScroll({
		size = cc.size(500, 50),
		strs = { str = name, fontPath = "font/youmi.ttf" },
	...
	idlereasy.any(idlerTable, function(_, num)
		if (self.flag == BUY_INFO.buy or self.flag == BUY_INFO.exchange) and id ~= "card" then -- 卡牌不计算上限
			...
			for k, v in csvMapPairs(costMap) do
				...
				itemBuyMax = itemBuyMax and itertools.min({ math.floor(money / (v * self.discount)), itemBuyMax }) or
					math.floor(money / (v * self.discount))
			end
	...
	if self.canUseMaxNum and self.flag == BUY_INFO.use then
		...
		if num >= self.canUseMaxNum then
			...
			if (self.itemAddStamina * num + gGameModel.role:read("stamina")) == game.STAMINA_LIMIT then
				...
				text.addEffect(self.maxTip, { color = ui.NEWCOLORS.BROWN2 })
			else
...
function BuyInfoView:onAfterBuildPriceList()
	...
	adapt.oneLineCenterPos(cc.p(self.pricePanel:width() / 2, self.priceDesc:y()), { self.priceDesc, self.priceList },
		cc.p(0, -self.priceList:height() / 2))
end
...
function BuyInfoView:onAfterBuildSelectList()
	...
	adapt.oneLineCenterPos(cc.p(self.selectPanel:width() / 2, self.selectDesc:y()), { self.selectDesc, self.selectList },
		cc.p(0, -self.selectList:height() / 2))
end
