...
function BuyNumberView:onCreate(params, cb)
	...
	self.conversionNumMax = params.itemType ~= 1 and gVipCsv[vipLevel].megaItemMaxTimes or
		gVipCsv[vipLevel].megaCommonItemMaxTimes
	...
	self.conversionChanceMax = self.params.itemType ~= 1 and gCommonConfigCsv.megaBuyChanceLimit or
		gCommonConfigCsv.megaCommonBuyChanceLimit
	self.leftTimes2:text("/" .. self.conversionChanceMax) --总次数
	self.conversionBuyAddTimes = self.params.itemType ~= 1 and gCommonConfigCsv.megaBuyAddTimes or
		gCommonConfigCsv.megaCommonBuyAddTimes
...
function BuyNumberView:conversionUpdata()
	...
	text.addEffect(self.priceText,
		{ color = roleRmb >= rmb and ui.COLORS.QUALITY_OUTLINE[1] or ui.COLORS.NORMAL.ALERT_ORANGE })
	self.leftTimes1:text(self.conversionChanceMax - buyTimes) --用的次时
	self.leftTimes2:x(self.leftTimes1:x() + self.leftTimes1:width())
	text.addEffect(self.leftTimes1,
		{ color = self.conversionChanceMax >= buyTimes and ui.COLORS.QUALITY_OUTLINE[1] or ui.COLORS.NORMAL.ALERT_ORANGE })
