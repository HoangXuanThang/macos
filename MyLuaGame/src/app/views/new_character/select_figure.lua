...
function SelectFigureView:onCreate(cb)
	...
	self.leftSpine = widget.addAnimation(self.leftFigure, self.leftFigureCfg.resSpine, "standby_loop")
		:scale(2)
		:xy(self.leftFigure:size().width / 2, -100)
	self.leftSpine:play("standby_loop")
	--self.rightSpine = widget.addAnimation(self.rightFigure, self.rightFigureCfg.resSpine, "standby_loop")
	--	:scale(2.3)
...
function SelectFigureView:onRandom()
	self.input:text(beauty.singleTextLimitWord(randomName(), { fontSize = 40 },
		{ width = 300, replaceStr = "", onlyText = true }))
end
