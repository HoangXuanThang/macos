...
GrowGuideView.RESOURCE_BINDING = {
	...
	["title.textTitle"] = {
		binds = {
			event = "effect",
			data = {outline = {color =  cc.c4b(188, 124, 10, 255), size = 4}}
		},
	},
	["rightTop"] = "rightTop",
	["rightBall"] = "rightBall",
		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					local bgPath = "city/grow_guide/btn_moren2.png"
					if v.isSel then
						bgPath = "city/grow_guide/btn_moren1.png"
...
function GrowGuideView:initRightPanel(val)
	...
	if(data.state == STATE.get)then
		text.addEffect(self.btnGet:get("Label_83"), {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4}})
	else
		text.deleteAllEffect(self.btnGet:get("Label_83"))
	end
