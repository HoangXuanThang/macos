...
function TestChooseCardView:onCreate(oldData)
	...
	self.testBattle = userDefault.getForeverLocalKey("test_Info", { role = {}, oldData = {} }, { rawKey = true })
	...
	for k, v in pairs(self.testBattle.role) do
		if (self.testBattle.role[k] == "1") then
			self:addRes(k)
		end
	end
	if (oldData ~= nil) then
		self.testBattle.oldData = oldData;
		userDefault.setForeverLocalKey("test_Info", self.testBattle, { rawKey = true })
	end

	self:loadOldData(self.testBattle.oldData, testRoleOut)
...
function TestChooseCardView:onBtnShowHistory()
	...
	local newRecordText = function(ishead, showLabel)
		...
		if type(showLabel) == "string" then
			if ishead then
				assignColor = stringToColor(showLabel)
			else
				assignColor = cc.c4b(0, 190, 255, 255)
			end
		end
		return ccui.Text:create(showLabel, "font/youmi.ttf", 35)
		...
		if len > 1 then
			...
			for k, battleInfo in ipairs(allBattleInfo) do
				...
				if len == k and avgValue1 and avgValue2 and avgValue3 then
					...
					finalString = avgValue1 .. "(" .. math.ceil(avgValue2 / len) .. "/" ..
						math.ceil(avgValue3 / len) .. ")"
				end
			end
			return finalString, avgValue1
		end
		return
			(table.get(battleInfo[i], args[1]) or 0) ..
			"(" .. (table.get(battleInfo[i], args[2]) or 0) .. "/" .. (table.get(battleInfo[i], args[3]) or 0) .. ")",
			table.get(battleInfo[i], args[1])
	end
	...
	newColumnText("Dmg/allocate", 0, getValueInValueTypeTableArray, "totalDamage", battle.DamageFromExtra.allocate,
		valueType)
	...
	newColumnText("Rhp/suckblood", 0, getValueInValueTypeTableArray, "totalResumeHp", battle.ResumeHpFrom.suckblood,
		valueType)
...
function TestChooseCardView:onBtnFightTest()
	...
	local function showBuffEffectTakeEffect()
		...
		if hasData then
			EasyView.addTextListView(editView, cc.p(self.stage:width() / 2 - 800, self.stage:height() / 2 - 500),
				cc.size(1500, 400))
	...
	editView.btnSendToServer:addTouchEventListener(function(sender, eventType)
		if eventType == ccui.TouchEventType.ended then
			...
			local filename = string.format("%s_%s_%s", self.gameEnvData.sceneID, self.gameEnvData.gateType,
				os.date("%y%m%d-%H%M%S"))
...
function TestChooseCardView:onBtnAddPrefab()
	...
	for i, v in ipairs(gHandbookArrayCsv) do
		...
		btn:get("icon"):texture(unitInfo.iconSimple):scale(0.4)
		...
		if self.resources[cardInfo.unitID] then
			...
			btn:addClickEventListener(function(_btn)
				...
				self.testBattle.role[_btn.id] = "1"
				userDefault.setForeverLocalKey("test_Info", self.testBattle, { rawKey = true })
			end)
		end
	end
...
function TestChooseCardView:onBtnLoadRecord(btn, path, view)
	...
	for _, v in ipairs(datas) do
		...
		text:addTouchEventListener(function(sender, eventType)
			text:scale((eventType == ccui.TouchEventType.ended or eventType == ccui.TouchEventType.canceled) and 1 or
				1.05)
			if eventType == ccui.TouchEventType.ended then
				local data = readAndUnpack(name)
			...
			while true do
				...
				if socket.gettime() - oldTime >= 0.2 then
					MultiForce.requestFightSolo(self, fightRoleOut[i], i, function(ret, t, roleOut, idx)
						...
						print("\n")
						print("总场次:" .. tostring(leftWin + rightWin) ..
							" 左胜:" .. tostring(leftWin) .. " 右胜:" .. tostring(rightWin))
					end)
					i = i + 1
...
function TestChooseCardView:delRes(_prefab)
	...
	self.testBattle.role[_prefab.roleId] = nil
	userDefault.setForeverLocalKey("test_Info", self.testBattle, { rawKey = true })
	_prefab:clean()
end
		...
		if cc.rectContainsPoint(self.gameObjectsRect, movePos) then
			...
			for k, _gameObject in ipairs(self.gate.scene) do
				...
				if cc.rectContainsPoint(_gameObject:getRect(), movePos) then
					...
					self.drawNode:clear()
					self.drawNode:drawRect(cc.p(rect.x, rect.y), cc.p(rect.x + rect.width, rect.y + rect.height),
						cc.c4f(1, 0, 0, 1))
					return
				end
			end
...
function TestChooseCardView:createPassiveSkillList(attrNode, _view)
	...
	local function createAllAttr()
		...
		for skillId, skillLevel in pairs(v) do
			...
			local txt = ccui.Text:create(string.format("%s=%s;", skillId, skillLevel), "font/youmi.ttf", 35):anchorPoint(
				cc.p(0, 0.5)):addTo(panel):x(0):y(0)
			local delBtn = self.prefab:get("del"):clone():addTo(panel):anchorPoint(cc.p(0.5, 0.5)):x(400):y(0)
				:setVisible(true):scale(0.5)
...
function TestChooseCardView:runGameLogicByRoleOut(roleOut, time, runOnceCall)
	...
	-- 记录胜负情况
	local result = "draw"
	if winTimes > loseTimes then
		result = "win"
	elseif winTimes < loseTimes then
		result = "fail"
	end
	...
	table.insert(self.battleResultInfo,
		string.format("[INFO] 总战斗测试次数: %d (%d vs %d)    胜率: %d%s", time, winTimes, loseTimes,
			math.floor(100 * winTimes / time), "%"))
end
	...
	while true do
		...
		if socket.gettime() - oldTime >= 0.2 then
			MultiForce.requestFight(self, fightRoleOut[i], i, function(ret, t, roleOut, idx)
				...
				print("\n")
				print("总场次:" .. tostring(leftWin + rightWin) .. " 左胜:" .. tostring(leftWin) .. " 右胜:" ..
					tostring(rightWin))
			end)
			i = i + 1
			oldTime = socket.gettime()
