			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					if v.select == true then
						itertools.invoke({childs.name, childs.level ,childs.lv}, "setTextColor", ui.NEWCOLORS.ORANGE)
						itertools.invoke({childs.name, childs.level ,childs.lv}, "enableOutline", ui.NEWCOLORS.WHITE,3)
					else
						itertools.invoke({childs.name, childs.level ,childs.lv}, "setTextColor", ui.COLORS.NORMAL.WHITE)
						itertools.invoke({childs.name, childs.level ,childs.lv}, "disableEffect")
					end
				...
				onBeforeBuild = function(list)
					...
					list:setScrollBarColor(cc.c3b(249, 225, 130))
					list:setScrollBarOpacity(255)
					list:setScrollBarWidth(13)
					list:setScrollBarAutoHideEnabled(false)
					list:setScrollBarPositionFromCorner(cc.p(size.width-4, (listSize.height - size.height) / 2 + 10))
				end,
			},
			handlers = {
