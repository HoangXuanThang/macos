				...
				onItem = function(list, node, k, v)
					idlereasy.when(list.showTab, function(_, showTab)
						...
						if showTab == k then
							...
							adapt.oneLineCenterPos(cc.p(size.width / 2, size.height / 2 - 10),
								{ node:get("selected.logo"), node:get("selected.txt") }, cc.p(10, 0))
						else
							node:get("normal"):show()
							node:get("selected"):hide()
							node:get("normal.txt"):text(v)
							adapt.oneLineCenterPos(cc.p(size.width / 2, size.height / 2 - 10),
								{ node:get("normal.logo"), node:get("normal.txt") }, cc.p(10, 0))
							bind.touch(list, node:get("normal"),
								{ methods = { ended = functools.partial(list.clickCell, k) } })
						end
					end)
				end,
			},
...
function ChatEmojiView:onCreate(sendData, channel, freshList)
	...
	--else
	if channel == "cross" then
		self.unlockKey = gUnlockCsv.crossChat
	else
...
function ChatEmojiView:onSelPicture(list, k, v)
	if gGameModel.role:getIdler("vip_level"):read() <= 0 and not dataEasy.isUnlock(self.unlockKey) then
