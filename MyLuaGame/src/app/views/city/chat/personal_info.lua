...
function ChatPersonalInfoView:onCreate(pos, personData, params)
	...
	local isCross = personData.channel == "cross"
	...
	if not isCross then
		...
		local length = (380 - self.bg:height()) / 2
		self.bg:height(380)
		...
		self.btnFight:y(self.btnFight:y() + length - 20)
	...
	if params.isKickNum and params.isKickNum ~= 0 and dataEasy.isUnlock(gUnlockCsv.cloneBattleKick) then
		...
		idlereasy.when(self.places, function(_, places)
			...
			if self.canKick then
				...
				text.addEffect(self.btnKick:get("txt"),
...
function ChatPersonalInfoView:onShowInfo(node, event)
	...
	end, self.personData.role.id, self.personData.role.server)
end
...
function ChatPersonalInfoView:onBtnFight()
	gGameApp:requestServer("/game/role_info", function(tb)
		local roleInfo = tb.view
		if roleInfo and roleInfo.pvp_record_db_id then
			gGameUI:stackUI("city.card.embattle.base", nil, { full = true }, {
				fightCb = self:createHandler("startFighting", roleInfo.pvp_record_db_id, roleInfo.server, self)
			})
		else
			gGameUI:showTip("该玩家尚未开启竞技场，无法挑战")
		end
	end, self.personData.role.id, self.personData.role.server)
end

--battleCards 当前阵容
function ChatPersonalInfoView:startFighting(recordId, serverKey, view)
	-- -- 2.正常的开始战斗 跳过布阵
	battleEntrance.battleRequest("/game/society/friend/fight", gGameModel.role:getIdler("id"), recordId, serverKey)
