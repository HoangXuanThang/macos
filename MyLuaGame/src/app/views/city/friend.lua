			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					if v.select then
						...
						text.addEffect(panel:get("txt"), {outline = {color = ui.NEWCOLORS.OUTLINE.WHITE, size = 3}})
					else
						selected:hide()
						panel = normal:show()
						panel:get("txt"):getVirtualRenderer():setLineSpacing(-10)
...
function FriendView:onCreate(showType, data)
	...
	self.nameInput:setPlaceHolderColor(ui.NEWCOLORS.GRAY3)
	self.nameInput:setTextColor(ui.NEWCOLORS.BROWN1)
