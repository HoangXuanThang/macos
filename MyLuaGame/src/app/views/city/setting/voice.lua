		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					children.musicName:text(v.cfg["name_"..LOCAL_LANGUAGE] or v.cfg.name)
				...
				onBeforeBuild = function(list)
					if list.sliderBg:visible() then
						...
						list:setScrollBarPositionFromCorner(
							cc.p(
								list:x() + list:size().width - list.sliderBg:x(),
								(list:size().height - list.sliderBg:size().height) / 2 + 5
							)
						)
						list:setScrollBarWidth(list.sliderBg:size().width)
						list:refreshView()
					else
