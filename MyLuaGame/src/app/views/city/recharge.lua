...
local function createPrivilegeLabel(desc, fontSize, width)
	local richText = rich.createWithWidth("#C0x57472E#" .. desc, fontSize, nil, width)
		:align(cc.p(0, 0), 0, 0)
...
local function isVipDistinguishedOpen(vipSum)
	...
	return matchLanguage({ "en" }) and vipSum >= gCommonConfigCsv.rechargeVip and
		dataEasy.isUnlock(gUnlockCsv.vipDistinguished)
		or (not matchLanguage({ "en" }) and (csvSize(gVipCsv[roleVip].monthGift) >= 1))
end
		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, csvId, v)
					...
					node:get("price"):text(string.format(gLanguageCsv.symbolMoney, cfg["rmbDisplay_"..LOCAL_LANGUAGE] or cfg.rmbDisplay))
					--text.addEffect(node:get("price"), {color = ui.COLORS.NORMAL.WHITE, glow = {color = ui.COLORS.GLOW.WHITE}})
					text.addEffect(node:get("price"), {color = ui.COLORS.NORMAL.WHITE,outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4}})
				...
				onItem = function(list, node, k, v)
					idlereasy.if_(v.show, function()
						...
						for i = 1, math.huge do
							local desc = v.vipDescCfg["desc" .. i .. "_"..LOCAL_LANGUAGE] or v.vipDescCfg["desc" .. i]
						...
						bind.touch(list, node:get("btn"),
							{ methods = { ended = functools.partial(list.clickCell, k, v) } })

						idlereasy.any({ v.hasBuy, v.vipLevelEnough, v.roleLevelEnough },
							function(_, hasBuy, vipLevelEnough, roleLevelEnough)
...
function RechargeView:onCreate(params)
	...
	idlereasy.any({ self.vipLevel, self.vipSum }, function(_, vipLevel, vipSum)
		...
		if curVip == game.VIP_LIMIT then
			...
			local childs = self.topNormalPanel:multiget("label1", "vipIcon1", "label2", "diamondIcon", "num", "label3",
				"vipIcon2")
			...
			if curVip < 1 then
				...
				adapt.oneLinePos(childs.label2, { childs.diamondIcon, childs.num, childs.label3, childs.vipIcon2 },
					cc.p(10, 0))
			else
				itertools.invoke({ childs.label1, childs.vipIcon1 }, "show")
				childs.vipIcon1:texture(ui.VIP_ICON[curVip])
				adapt.oneLinePos(childs.label1,
					{ childs.vipIcon1, childs.label2, childs.diamondIcon, childs.num, childs.label3, childs.vipIcon2 },
					cc.p(10, 0))
			end
		end
