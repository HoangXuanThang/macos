...
local function commonEffortValueShow(list, node, k, v)
	...
	if v.currVal then
		...
		if v.rightVal then
			childs.arrow:texture(v.rightVal > v.currVal and "common/icon/logo_arrow_green.png" or
...
PropertySwapView.RESOURCE_BINDING = {
	...
	["btnExchange.txt"] = {
		...
		binds = {
			event = "effect",
			data = { outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 2 } }
		},
	},
...
function PropertySwapView:onCreate(selTabKey, selectDbId)
	...
	selTabKey = string.upper(selTabKey or "NVALUE")
	...
	self.showTab:addListener(function(val, oldval)
		...
		adapt.oneLineCenterPos(cc.p(self.costPanel:width() / 2, self.costPanel:height() / 2),
			{ self.costTxt, self.cost, self.costIcon }, cc.p(10, 0))
		...
		uiEasy.setIconName("card", cardData.card_id,
			{ node = self.leftName, name = name, advance = advance, space = true })
	end)

	idlereasy.any(
		{ self.leftCard:getIdler("effort_values"), self.leftCard:getIdler("star"), self.leftCard:getIdler("level"), self
			.rightEffortAdvance }, function(_, val, star, level, rightEffortAdvance)
			local t = {}
			local total = 0
			...
			self.effortAdvanceArrow:texture(leftEffortAdvance < rightEffortAdvance and "common/icon/logo_arrow_green.png" or
				"common/icon/logo_arrow_red.png")
	...
	idlereasy.any({ self.showTab, self.sprOrItemIds }, function(_, tab, ids)
		...
		if hasSpr then
			...
			if self.showTab:read() ~= SWAP_TYPE.FEEL then
				...
				cardData = rightCard:read("card_id", "skin_id", "level", "star", "advance", "locked", "name", "nvalue",
					"character", "effort_values", "effort_advance")
			...
			uiEasy.setIconName("card", cardData.card_id,
				{ node = nameTxt, name = cardData.name, advance = cardData.advance, space = true })
			local cardCsv = csv.cards[cardData.card_id]
			local unitCsv = csv.unit[cardCsv.unitID]
			if unitId == 0 then
...
function PropertySwapView:onSelect()
	gGameUI:stackUI("city.card.property_swap.choose", nil, nil, self.showTab:read(), self.selectDbId,
		self:createHandler("setParams"))
end
...
function PropertySwapView:onSelectItem()
	gGameUI:stackUI("city.card.property_swap.choose_item", nil, nil, self.showTab:read(), self.selectDbId,
		self:createHandler("setParams"))
end
...
function PropertySwapView:onExchange()
	...
	if tab ~= SWAP_TYPE.FEEL then
		...
		if tab == SWAP_TYPE.EFFORTVALUE and self:effortsValue() then
			strs = string.format(gLanguageCsv.spriteUpperLimit, self.leftData:atproxy(tab).desc,
				self.leftData:atproxy(tab).desc)
		end
	else
	...
	local params = {
		...
		cb = function()
			...
			if sprId ~= 0 then
				if tab ~= SWAP_TYPE.FEEL then
					req = gGameApp:requestServerCustom("/game/card/property/swap"):params(self.selectDbId, sprId, tab + 1)
				else
