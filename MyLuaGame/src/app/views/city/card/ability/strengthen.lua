...
local function getCostDatas(abilities, abilityId, items)
	...
	if level < abilityCsv.strengthMax then
		...
		for k,v in csvMapPairs(costItemMap) do
			if k ~= "gold" then
				table.insert(costDatas, {key = k, num = items[k] or 0, targetNum = v, id = k})
			end
		end
	end
...
local function getAddStr(nums, level, maxLevel, attrType, oneKeyLevel)
	...
	return "#C0x01A001#+" .. str .. "#C0x5B545B#"
end
--效果描述
local function setDesc(list, abilityCsv, level, oneKeyLevel)
	...
	local csvDesc = abilityCsv["desc_"..LOCAL_LANGUAGE] or abilityCsv.desc
	if abilityCsv.effectType == 2 then
		
		desc = string.format(csvDesc, getSkillStr(csv.skill[abilityCsv.skillID]["describe_"..LOCAL_LANGUAGE] or csv.skill[abilityCsv.skillID].describe, level, abilityCsv.strengthMax))
	end
	if abilityCsv.effectType == 1 then
		...
		desc = string.format(csvDesc, unpack(params))
	end
	beauty.textScroll({
		list = list,
		strs = "#C0x5B545B#" .. desc,
	...
	idlereasy.any({self.abilities, self.items, self.refreshData}, function(_, abilities, items)
		...
		if abilityCsv then
			...
			if(#conditionStr == 0)then
				text.addEffect(self.btnStrengthen:get("textNote"), {color = color1,outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4}})
				text.addEffect(self.btnStrengthenOneKey:get("textNote"), {color = color1,outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4}})
			else
				text.deleteAllEffect(self.btnStrengthen:get("textNote"))
				text.deleteAllEffect(self.btnStrengthenOneKey:get("textNote"))
			end
			--消耗金币和材料的数据
			local costDatas, needCoin = getCostDatas(abilities, self.abilityId, items)
			self.eventDatas:update(costDatas)

			if (not isMax) and self.isShowGuideGift and #conditionStr == 0 then
				uiEasy.goGuideGift(costDatas)
