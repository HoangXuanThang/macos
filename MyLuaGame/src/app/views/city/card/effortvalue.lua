...
local function setEffortAdvance(effortAdvance, parent)
	...
	text.addEffect(childs.txtAdvance, {outline={color=ui.NEWCOLORS.OUTLINE.BLACK}})
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					if currVal then
						...
						text.addEffect(childs.num, {outline={color=ui.NEWCOLORS.BLACK}})
					...
					if v.changeNum then
						...
						widget.addAnimationByKey(childs.bar, "effect/nuizhifankui.skel", 'nuizhifankui', "effect", 10)
							...
							:xy(childs.barBg:width()/2-2, childs.barBg:height()/2-4)
							:play("effect")
					else
...
function CardEffortValueView:onCreate(dbHandler, validAreas)
	...
	idlereasy.when(self.listIndexs, function (_, val)
		...
		if(isCanSend)then
			text.addEffect(self.rightBtn:get("txt"), {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW}})
		else
			text.deleteAllEffect(self.rightBtn:get("txt"))
		end
	...
	idlereasy.any({self.items, self.currCost, self.curState}, function (_, items, cost, state)
		...
		table.insert(self.guideItmsData, {id = id, num = currNum, targetNum = idNum})
		...
		local func = function(panel, num)
			...
			text.addEffect(childs.num,{color = currNum >= showNum and ui.COLORS.NORMAL.WHITE or ui.COLORS.NORMAL.ALERT_ORANGE})
		...
		if(state ~= STATE_TYPE.SAVE)then
			text.addEffect(self.leftBtn:get("txt"), {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW}})
		else
			text.deleteAllEffect(self.leftBtn:get("txt"))
		end
	end)
...
function CardEffortValueView:initButtonStatus()
	...
	if isUnlock and state == STATE_TYPE.ONE_AND_TEN then
		...
		adapt.oneLinePos(self.leftPanel, {self.rightPanel,self.btnCustom}, {cc.p(-120,0),cc.p(-60,0)})
	else
