...
local function setStarIcon(parent, star)
    ...
    -- Chỉ lấy tối đa 6 sao để hiển thị
    local starNum = math.min(#starDatas, 6)

    for i = 1, starNum do
        local icon = starDatas[i].icon
		...
		binds = {
			...
			props = {
				...
				onCell = function(list, node, k, v)
					...
					local desc = string.format("#C0x5b545b#%s", eval.doMixedFormula(skillCfg["describe_"..LOCAL_LANGUAGE] or skillCfg.describe,{skillLevel = v.skillLevel or 1,math = math},nil) or "no desc")
...
function CardStarView:onCreate(dbHandler)
	...
	idlereasy.when(self.canResetEp, function(_, canResetEp)
		...
		if(canResetEp)then
			text.addEffect(self.btnResetEp:get("textNote"), {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4}})
		else
			text.deleteAllEffect(self.btnResetEp:get("textNote"))
		end
	end)
	...
	idlereasy.when(self.gold, function(_, gold)
		...
		local color = (gold >= csvStar.gold) and ui.NEWCOLORS.WHITE or ui.NEWCOLORS.REDLIGHT
...
function CardStarView:refreshView()
	...
	if csvStar then
		...
		local color = (dataEasy.getNumByKey("gold") >= csvStar.gold) and  ui.NEWCOLORS.WHITE or ui.NEWCOLORS.REDLIGHT
		text.addEffect(self.needGoldTxt, {color = color})
...
function CardStarView:setEffectPanel(star, effectCfg)
	...
	local c1 = "#C0xFFFFFF##F44#"
	local c2 = "#C0x25EA11##F44#"
	table.sort(keys)
	local datas = {}
	...
	for k1, i in ipairs(keys) do
		...
		local str = ""
		local args = {}
		local color = i > star and "#C0x989898#" or "#C0xFFFFFF#"
		local color1 = i <= star and c1 or "#C0x989898#"
		local color2 = i <= star and c2 or "#C0x989898#"
		...
		local effectDesc = v["effectDesc_"..LOCAL_LANGUAGE] or v.effectDesc or ""
		str = table.concat(args, " ")
		str = str..color1..(effectDesc or "")
		datas[k1] = {
			str = str,
			value = i,
			star = star,
...
function CardStarView:onCombClick()
	...
	local strs = {
		string.format("#C0x5b545b#"..gLanguageCsv.wantConsumeFragsCombCard, fragCsv.combCount, "#C0x01A001#"..fragCsv.name.."#C0x5b545b#", "#C0x01A001#"..cardCsv.name)
	}
...
function CardStarView:onChangeClick()
	if self.star:read() == 24 and not dataEasy.isUnlock(gUnlockCsv.extremityProperty) then
