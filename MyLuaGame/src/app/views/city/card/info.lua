			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					if v.isSelected then
						text.addEffect(txt,
							{
								outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 2 },
								color = ui.COLORS.NORMAL
									.WHITE
							})
					else
						text.addEffect(txt,
							{ outline = { color = ui.NEWCOLORS.OUTLINE.BLUE, size = 2 }, color = ui.COLORS.NORMAL.WHITE })
					end
				end,
			},
			handlers = {
	...
	["rightPanel.skillPanel.list"] = {
		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					local childs = node:multiget("level", "name", "type", "icon", "bg", "btn", "levelNote", "bg1")
					...
					if dataEasy.isZawakeSkill(v.id, list.zawakeSkills) then
						childs.bg:texture("common/box/box_panel_m.png")
						childs.bg1:texture("common/box/box_tongy_m.png")
						ccui.ImageView:create("city/drawcard/draw/txt_up.png")
							:scale(0.6)
							:align(cc.p(1, 1), 120, 20)
							:addTo(childs.icon, 1, "zawakeUp")
						local zawakeEffectID = csv.skill[v.id].zawakeEffect[1]
						local zawakeText = csv.skill[zawakeEffectID]["skillName_"..LOCAL_LANGUAGE] or csv.skill[zawakeEffectID].skillName
						childs.name:text(zawakeText .. childs.name:text())
					else
						childs.bg:texture("common/box/box_panel.png")
						childs.bg1:texture("common/box/box_tongy.png")
					end
...
function CardInfoView:onCreate(cardData)
	...
	if held_item then
		...
		local lb2 = cc.Label:createWithTTF(held_item.level, ui.FONT_PATH, 30)
			:xy(154, 60)
		...
		adapt.oneLineCenterPos(cc.p(65, 13), { lb1, lb2 }, cc.p(5, 0))
	...
	local childs = self.leftPanel:multiget("rarity", "name", "attr1", "attr2", "starList", "photo", "masterName",
		"masterNameNote")
	childs.rarity:texture(ui.RARITY_ICON[unit.rarity])
	childs.masterName:text(cardData.role_name or self.myName:read())
	adapt.oneLineCenterPos(cc.p(self.photo:x(), childs.masterName:y()), { childs.masterNameNote, childs.masterName },
		cc.p(25, 0))
	childs.name:text(uiEasy.setIconName("card", cardData.card_id,
		{ node = childs.name, name = cardData.name, advance = cardData.advance, space = true }))
	...
	-- Lấy dữ liệu sao
	local starData = dataEasy.getStarData(cardData.star)
	self.starData = idlertable.new(starData)
	
	local size = childs.photo:size()
	childs.photo:removeAllChildren()
	if unit.showOpen then
		self.photoSprite = widget.addAnimation(childs.photo, unit.showOpen, "default", 5, true)
		if string.find(unit.showOpen, ".png") then
			self.photoSprite:y(40)
		else
			self.photoSprite:y(40):scale(0.8)
		end
	end
	...
	adapt.oneLineCenterPos(cc.p(self.photo:x(), self.power:y()), { self.powerNote1, self.power }, cc.p(25, 0))
	self.tabDatas = {
	...
	for i, v in ipairs(game.ATTRDEF_SIMPLE_TABLE) do
		assertInWindows(cardData.attrs[v], "cardId(%s) attrs(%s) %s is nil", self.cardData.card_id, dumps(cardData.attrs),
			v)
		local data = {
