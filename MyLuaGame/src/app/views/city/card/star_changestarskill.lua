			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					if v.isSelected then
						...
						text.addEffect(txt,
							{ outline = { color = ui.NEWCOLORS.OUTLINE.BLUE, size = 4 }, color = ui.COLORS.NORMAL.WHITE })
					end
				end,
			},
			handlers = {
			...
			props = {
				...
				onCell = function(list, node, k, v)
					...
					uiEasy.setIconName("card", v.id,
						{ node = node:get("textName"), name = v.name, advance = v.advance, space = true })
...
function CardStarChangeSkillView:onCreate(dbHandler)
	...
	idlereasy.any({ self.costNum, self.tabIdx, self.getNum }, function(_, costNum, tabIdx, getNum)
		...
		local cardCsv = csv.cards[csv.cards[cardId].cardMarkID]
		...
		if tabIdx == 1 then
			...
			uiEasy.setIconName("card", cardId,
				{ node = cardPanel:get("textName1"), name = cardCsv.name, advance = cardCsv.advance, space = true })
	...
	idlereasy.any({ self.selectedFragId, self.selectEpNum, self.tabIdx, self.frags },
	...
	self.tabIdx:addListener(function(val, oldval, idler)
		...
		if val == 1 then
			...
			str = string.format(gLanguageCsv.starSkillCardChangeTips, textColor .. gLanguageCsv
				["rarityCard" .. (rarity - 1)], cardExchangeRate)
		else
...
function CardStarChangeSkillView:onCombClick()
	...
	local strs = {
		string.format("#C0x5b545b#" .. gLanguageCsv.wantConsumeFragsCombCard, fragCsv.combCount,
			"#C0x01A001#" .. fragCsv.name .. "#C0x5b545b#", "#C0x01A001#" .. cardCsv.name)
	}
	gGameUI:showDialog({
		content = strs,
		cb = function()
			gGameApp:requestServer("/game/role/frag/comb", function(tb)
				gGameUI:stackUI("common.gain_sprite", nil, { full = true }, tb.view, nil, false,
					self:createHandler("resetSelectNum"))
			end, cardCsv.fragID)
		end,
		btnType = 2,
		isRich = true,
