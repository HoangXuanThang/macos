...
CardShareTipView.RESOURCE_BINDING = {
	...
	["checkBox2"] = {
		...
		binds = {
			...
			methods = {
				ended = bindHelper.defer(function(view)
					view:onSelTargetPos(FROM.cross)
				end)
			}
		},
	},
...
function CardShareTipView:onCreate(dbId)
	...
	idlereasy.when(self.from, function(_, from)
		...
		self.checkBox2:get("checkBox"):setSelectedState(from == FROM.cross)
	end)
	...
	local str = "#C0x57472E#" .. string.format(gLanguageCsv.shareCardTo, color .. cfg.name .. numStr .. "#C0x57472E#")
