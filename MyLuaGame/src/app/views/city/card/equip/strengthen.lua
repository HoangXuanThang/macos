...
function CardEquipStrengthenView:update(data, level, gold)
	...
	if data.level < currLevelLimit then
		...
		text.addEffect(childs.cost, {color = gold >= cost and ui.COLORS.NORMAL.WHITE or ui.COLORS.NORMAL.ALERT_ORANGE})
		...
		text.addEffect(childs.leftQuality,{color = currQuality == 1 and ui.COLORS.NORMAL.WHITE or ui.COLORS.QUALITY[currQuality]})
		text.addEffect(childs.rightQuality,{color = nextQuality == 1 and ui.COLORS.NORMAL.WHITE or ui.COLORS.QUALITY[nextQuality]})
		...
		for k,v in csvMapPairs(gEquipAdvanceCsv[data.equip_id][data.advance].costItemMap) do
			...
			table.insert(self.guideItmsData, {id = k, num = hasNum, targetNum = v})
		end
