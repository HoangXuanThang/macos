...
CardEquipSignetView.RESOURCE_BINDING = {
    ...
    ["panel.bottomPanel.btnUpgrade.textNote"] = {
	 ...
	 ["panel.bottomPanel.onekeyUpgrade.textNote"] = {
		binds = {
			event = "effect",
			--data = {glow={color=ui.COLORS.GLOW.WHITE}},
			data = {color = ui.COLORS.NORMAL.WHITE, outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4}},
...
function CardEquipSignetView:onCreate(dbHandler)
	...
	idlereasy.when(self.isShowGuideGift, function (_, isShowGuideGift)
		if isShowGuideGift and not self.isEnoughItem then
...
function CardEquipSignetView:refreshData()
	...
	text.addEffect(self.cost, {color = ui.COLORS.NORMAL.WHITE})
		...
		for key,value in csvMapPairs(costItemMap) do
			if key ~= "gold" then
				...
				if value > hasNum and self.isEnoughItem then
					...
					table.insert(self.guideItmsData, {id = key, num = hasNum, targetNum = value})
				end
				table.insert(newT, {
					key = key,
					targetNum = value,
				...
				text.addEffect(self.cost, {color = self.isEnoughGold and ui.COLORS.NORMAL.WHITE or ui.COLORS.NORMAL.ALERT_ORANGE})
