...
function CardEquipStarView:onCreate(dbHandler)
	...
	idlereasy.any({self.equips, self.equipIndex, self.gold, self.startPotentialFastUnlock}, function (_, equips, index, gold, startPotentialFastUnlock)
		if self.currShowEquipIdx ~= index then
			self.currShowEquipIdx = index
		...
		if self.ability >= 0 and self.starNum == cfg.starMax then
			for key,v in csvMapPairs(csv.base_attribute.equip_ability[self.ability]["costItemMap"..cfg.abilitySeqID]) do
				if key ~= "gold" then
					...
					table.insert(self.guideItmsData, {id = key, num = hasNum, targetNum = v})
					...
					text.addEffect(self.cost, {color = gold >= v and ui.COLORS.NORMAL.WHITE or ui.COLORS.NORMAL.ALERT_ORANGE})
		...
		if(data.ability ~= cfg.abilityMax)then
			text.addEffect(self.btnUpgradePotential:get("textNote"), {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4}})
		else
			text.deleteAllEffect(self.btnUpgradePotential:get("textNote"))
		end
		if data.ability == cfg.abilityMax then
			...
			self.btnUpgradePotentialFast:get("textNote"):setTextColor(ui.COLORS.NORMAL.WHITE)
			text.addEffect(self.btnUpgradePotentialFast:get("textNote"), {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4}})
	...
	idlereasy.when(self.isShowGuideGift, function(_, isShowGuideGift)
		if isShowGuideGift and (not self.isEnoughItems) then
...
function CardEquipStarView:initModel()
	self.isShowGuideGift = idlereasy.new(false)
