...
function CardChangeNameView:onCreate(params)
	...
	self.nameField:setPlaceHolderColor(ui.NEWCOLORS.GRAY3)
	self.nameField:setTextColor(ui.NEWCOLORS.BROWN1)
	...
	if self.params.typ == "ready" or not params.cost then
		...
		idlereasy.when(gGameModel.role:getIdler('rmb'), function(_, rmb)
			self.diamondNum:setTextColor((params.cost > 0 and params.cost > rmb) and ui.COLORS.NORMAL.RED or
			ui.NEWCOLORS.BROWN1)
		end)
...
function CardChangeNameView:nameAdapt(txt)
	...
	local str = beauty.singleTextLimitWord(txt, { fontSize = 40 },
		{ width = self.textWidth, replaceStr = "", onlyText = true })
