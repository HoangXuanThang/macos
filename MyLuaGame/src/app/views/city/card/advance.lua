...
function CardAdvanceView:onCreate(dbHandler, playAction)
	...
	idlereasy.any({self.cardId, self.skinId, self.advance, self.cardLv, self.items}, function(_, cardId,skinId, advance, cardLv, items)
		...
		local goldColor = (dataEasy.getNumByKey("gold") >= csvAdvance.gold) and ui.NEWCOLORS.WHITE or cc.c4b(249,87,114,255)
		...
		local color = (cardLv >= needLevel) and ui.NEWCOLORS.GREENLIGHT or ui.NEWCOLORS.REDLIGHT
		text.addEffect(self.needLevelTxt, {color = color,outline = {color = ui.NEWCOLORS.OUTLINE.BLACK, size = 4}})
	...
	idlereasy.when(self.gold, function(_, gold)
		...
		local goldColor = (gold >= csvAdvance.gold) and ui.NEWCOLORS.WHITE or cc.c4b(249,87,114,255)
		text.addEffect(self.needGoldTxt, {color = goldColor})
	end)
...
function CardAdvanceView:setUI(cardId,advance,items)
	...
	for k,v in pairs(tmpMap) do
		...
		if myItemNum < v.num then
			...
			table.insert(self.guideItmsData, {id = v.id, num = myItemNum, targetNum = v.num})
		end
		self.itemId[i] = v.id
		i = i + 1
	end
