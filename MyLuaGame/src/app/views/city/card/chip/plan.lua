...
local resourceBinding = {
	...
	["leftPlan.btnSuitFilter.txt"] = {
		binds = {
			event = "effect",
			data = { color = ui.COLORS.NORMAL.WHITE, outline = { color = ui.NEWCOLORS.OUTLINE.BLUE, size = 3 } },
		},
	},
	["leftPlan.btnOrder.txt"] = {
		binds = {
			event = "effect",
			data = { color = ui.COLORS.NORMAL.WHITE, outline = { color = ui.NEWCOLORS.OUTLINE.BLUE, size = 3 } },
		},
		...
		binds = {
			...
			props = {
				...
				onCell = function(list, node, k, v)
					local childs = node:multiget("bg", "icon", "name", "curName", "newName", "inEquip", "chipPanel",
						"equipPanel", "inEquipPanel")
					...
					if v.cardDBID ~= nil then
						...
						local richText = rich.createWithWidth("#C0x5B545B#" .. gLanguageCsv.chipInEquip .. name, 26, nil,
							300)
					...
					if v.addNew then
						...
						bind.touch(list, childs.equipPanel,
							{ methods = { ended = functools.partial(list.equipClick, node, k, v) } })
					end
				end
			},
			handlers = {
	...
	["right.btnEditPanel.btnDelete.txt"] = {
		binds = {
			event = "effect",
			--data = {color=ui.COLORS.NORMAL.WHITE, glow = {color = ui.COLORS.GLOW.WHITE}},
			data = { color = ui.COLORS.NORMAL.WHITE, outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4 } },
	...
	["right.btnEquipPanel.btnDelete.txt"] = {
		binds = {
			event = "effect",
			--data = {color=ui.COLORS.NORMAL.WHITE, glow = {color = ui.COLORS.GLOW.WHITE}},
	...
	["right.btnEquipPanel.btnTop.txt"] = {
		binds = {
			event = "effect",
			--data = {color=ui.COLORS.NORMAL.WHITE, glow = {color = ui.COLORS.GLOW.WHITE}},
			data = { color = ui.COLORS.NORMAL.WHITE, outline = { color = ui.NEWCOLORS.OUTLINE.BLUE, size = 4 } },
			...
			props = {
				...
				onCell = function(list, node, k, v)
					...
					if v.selected then
						...
						text.addEffect(node:get("name"), { color = ui.NEWCOLORS.ORANGE })
					else
						node:get("icon"):texture("city/card/chip/btn_w.png")
						text.addEffect(node:get("name"), { color = ui.NEWCOLORS.BROWN2 })
					end
					bind.touch(list, node,
						{ methods = { ended = functools.partial(list.itemClick, node, list:getIdx(k), v) } })
				end
			},
			handlers = {
...
function ChipPlanView:onCreate(params)
	...
	-- 右侧按钮状态
	idlereasy.any({ self.selectPlanId, self.cardChips, self.chipPlans, self.isRefreshBagPanel },
		function(_, selectPlanId, cardChips, plans)
			self:setPlanName()
			itertools.invoke(
			{ self.planBtnNewPanel, self.planBtnSavePanel, self.planBtnEditPanel, self.planBtnEquipPanel }, "hide")
	...
	bind.click(self, self.planSuitFilterPanel, {
		method = function()
			self.planSuitFilterPanel:hide()
	...
	bind.click(self, self.planOrderPanel, {
		method = function()
			self.planOrderPanel:hide()
	...
	if self.page == 1 then
		...
		idlereasy.any(
		{ self.isRefreshPlanPanel, self.selectPlanSuitId, self.selectPlanOrder, self.chipPlans, self.cardChips },
			function()
	...
	idlereasy.when(self.equipShow, function(_, show)
		if show then
			self.btnEquipShow:get("img"):texture("common/btn/btn_nomal_4white.png")
			...
			text.addEffect(self.btnEquipShow:get("txt"),
				{ color = ui.COLORS.NORMAL.WHITE, outline = { color = ui.NEWCOLORS.OUTLINE.BLUE, size = 3 } })
		else
			self.btnEquipShow:get("img"):texture("common/btn/btn_nomal_4red.png")
			...
			text.addEffect(self.btnEquipShow:get("txt"),
				{ color = ui.COLORS.NORMAL.WHITE, outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 3 } })
		end
...
function ChipPlanView:setPlanName(name)
	...
	if selectPlanId and plans[selectPlanId] then
		...
		adapt.oneLineCenterPos(cc.p(self.planNamePanel:width() / 2, self.planNamePanel:height() / 2),
			{ self.planNamePanel:get("name"), self.planNamePanel:get("icon") }, cc.p(10, 0))
	else
...
function ChipPlanView:onRightPlanNewClick()
	...
	self.isRefreshPlanPanel:notify()
	self.planMaskView = gGameUI:createView("city.card.chip.plan_mask", self):init({ onClose = self:createHandler(
	"onPlanMaskClose") })
end
...
function ChipPlanView:onBaseAttrDetailClick()
	...
	if self.page == 1 and not self.showCard:read() then
		gGameUI:stackUI('city.card.chip.total_detail', nil, nil,
			{ typ = 2, curPlan = plan, cardPlan = self.cardChips:read() })
	else
