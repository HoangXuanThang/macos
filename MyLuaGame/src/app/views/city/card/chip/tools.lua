		...
		if key and key ~= 0 then
			...
			if isArray then
				if not ChipTools.ignoreAttr(key) then
					if arrayKey then
						errorInWindows("配置异常，csv.chip.chips[%d] 副属性不能显示 %s(%s), %s(%s)", params.chipId,
							game.ATTRDEF_TABLE[arrayKey], arrayKey, game.ATTRDEF_TABLE[key], key)
					else
						if count then
							arrayKey = key
						end
...
function ChipTools.getAttr(dbId, level, isArray, emptyShow)
	...
	for _, data in ipairs(now) do
		ChipTools.setAttrAddition(secondAttrs, csv.chip.libs[data[1]], data[3], isArray,
			{ chipId = chip.chip_id, now = data })
	end
	...
	if isArray and emptyShow then
		...
		for i = 1, secondAttrLeftCount do
			table.insert(secondAttrs, {
				name = gLanguageCsv.randomAttributes,
				val = string.format(gLanguageCsv.nLvUnlock,
					cfg.acquiredLevels[cfg.acquiredLimit - secondAttrLeftCount + i])
			})
		end
	end
...
--  @desc 检测当前秘籍ID的套装属性激活状态  （激活时返回激活品质，没激活返回所选chipDBID品质）
-- 	返回当前秘籍的所属套装属性 （返回所选chipDBID品质）
...
function ChipTools.getSuitAttrStr(suitId, data)
	...
	local str = isActive and "#C0xFFFFFFFF#" or "#C0xFF9A9795#"
	local formatStr = suitNum < 4 and gLanguageCsv.chipNSuitAttr or gLanguageCsv.chipNSuitAttrEffect
	str = string.format("#L10##F40#%s%s #L0#", str,
		string.format(formatStr, suitNum, gLanguageCsv[ui.QUALITY_COLOR_TEXT[quality]]))
	if suitCfg.skillID ~= 0 then
		...
		for i = 1, math.huge do
			...
			if key and key ~= 0 then
				...
				if isActive then
					str = string.format("%s %s #C0xFF5C9970#+%s#C0xFFFFFFFF#", str, getLanguageAttr(key), val)
				else
...
function ChipTools.ignoreAttr(key)
	...
	{ game.ATTRDEF_ENUM_TABLE.specialDamage, game.ATTRDEF_ENUM_TABLE.specialDefence, game.ATTRDEF_ENUM_TABLE
		.specialDefenceIgnore }, key)
end
