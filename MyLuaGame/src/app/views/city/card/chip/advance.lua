...
function ChipAdvanceView:initAdvanceRuleList()
	...
	for i = 124401, 124405 do
		...
		table.insert(attrDatas, "#C0xFFFFFF#" .. content)
...
function ChipAdvanceView:initLeftPanel()
	...
	idlereasy.when(self.chipLevel, function(_, level)
		...
		if (not self.sign) then
			text.deleteAllEffect(self.btnAdvance:get("title"))
		else
			text.addEffect(self.btnAdvance:get("title"), { outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4 } })
		end
	end)
	...
	idlereasy.any({ self.clientChipAddExp, self.gold }, function(_, addLevelExp, gold)
		...
		if (addLevelExp == 0) then
			text.deleteAllEffect(self.btnStrengthen:get("title"))
		else
			text.addEffect(self.btnStrengthen:get("title"), { outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4 } })
		end
		...
		text.addEffect(childs.cost, { color = isEnough and ui.COLORS.NORMAL.WHITE or ui.COLORS.NORMAL.RED })
	...
	idlereasy.any({ self.clientChipLevel, self.chipLevel, self.chipNow },
		function(_, clientChipLevel, chipLevel, chipNow)
			local idx = 0
...
function ChipAdvanceView:setLeftAttrItem(attrType, idx, params)
	...
	local color = params.key and ui.COLORS.NORMAL.WHITE or ui.COLORS.NORMAL.GRAY
...
function ChipAdvanceView:getStrengthData()
	...
	for _, v in ipairs(gChipExpCsv) do
		...
		if num > 0 then
			...
			table.insert(data,
				{ type = "item", key = v.id, num = num, idx = idx, exp = exp, selectedNum = self.costCsvIDs[v.id] })
		end
	end
	for _, v in ipairs(roleChips) do
		...
		if v ~= self.dbId and not chipDatas.card_db_id then
			...
			table.insert(data,
				{ dbId = v, key = chipId, level = chipDatas.level, locked = chipDatas.locked, cfg = cfg, idx = idx, exp =
				exp, selected = self.costChips[v] })
		end
	end
...
function ChipAdvanceView:onStrengthenClick()
	...
	local function cb()
		...
		gGameApp:requestServer("/game/card/chip/strength", function()
			...
			self:showGuideGiftData()
		end, self.dbId, costChips, costCsvIDs)
	end

	if haveHigh then
		gGameUI:showDialog({ title = gLanguageCsv.spaceTips, content = gLanguageCsv.chipStrengthenTip, isRich = true, btnType = 2, cb =
		cb })
	else
		cb()
	end
...
function ChipAdvanceView:showGuideGiftData()
	local items = {124, 123}
	for k,itemID in pairs(items) do
		if dataEasy.getNumByKey(itemID) <= 0 then
			uiEasy.goGuideGift({
				{id = itemID, num = 0, targetNum = 1}
			})
			break;
		end
	end
		...
		performWithDelay(self, function()
			if refreshTimes > 0 then
				...
				if self.isFirstAdvance then
					...
					list:pushBackCustomItem(lineItem)
					bind.touch(self, lineItem:get("detail"), {
						methods = {
							ended = function()
				...
				for k, v in csvMapPairs(cost) do
					...
					if idx <= 2 then
						...
						text.addEffect(childs["cost" .. idx],
							{ color = isEnough and ui.COLORS.NORMAL.WHITE or ui.COLORS.NORMAL.RED })
				...
				adapt.oneLineCenterPos(cc.p(self.costPanel:width() / 2, self.costPanel:height() / 2),
					{ childs.note, childs.cost1, childs.icon1, childs.cost2, childs.icon2 }, space)
				if advanceMax == 0 or not dataEasy.isUnlock("chipAdvanceReset") or not self.sign then
					...
					adapt.oneLineCenterPos(cc.p(self.resetCostPanel:width() / 2, self.resetCostPanel:height() / 2),
						{ childs.note, childs.cost1, childs.icon1 }, { cc.p(15, 0), cc.p(0, 0) })
				end
...
function ChipAdvanceView:setAdvanceAttrItem(attrType, idx, params)
	...
	if params.now then
		...
		bind.click(self, item:get("checkBox"), {
			method = function()
...
function ChipAdvanceView:initSubAttrDetailPanel()
	...
	bind.click(self, self.subAttrDetailPanel, {
		method = function()
			self.subAttrDetailPanel:hide()
		end
	})
	...
	for _, v in orderCsvPairs(csv.chip.libs) do
		if v.randomLibID == self.chipCfg.acquiredLib then
			...
			table.insert(strs,
				{ str = "#C0x5b545b#" .. v.attrsDesc .. "(" .. "#C0x5C9970#" .. minCount .. "~" ..
				maxCount .. "#C0x5b545b#)" })
		end
	end
...
function ChipAdvanceView:getRuleContext(view)
	...
	table.insert(infoList,
		{ key = csv.note[124706].fmt, main = csv.note[124707].fmt, sec = csv.note[124708].fmt, sign = true })
