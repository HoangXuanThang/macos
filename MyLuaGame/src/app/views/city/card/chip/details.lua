...
function ChipDetailsView:onCreate(params)
	...
	self.panel:get("bg"):get("Image_15"):hide()
	-- 指定卡牌，若选择的秘籍在卡牌上则为卸下
	...
	if self.changeType then
		...
		self.panel:get("bg"):get("Image_15"):show()
	...
	if self.subDBID then
		...
		subPanel:get("bg"):get("Image_15"):hide()
...
function ChipDetailsView:setPanel(panel, dbId)
	local childs = panel:multiget("bg", "bg2", "lock", "icon", "name", "level", "noEquip", "equip", "cardName",
		"gainExpText", "gainExp", "list", "attrItem", "lineItem", "suitItem", "btnChange", "btnStrength")
	...
	if self.showExp then
		...
		if chipData.card_db_id then
			...
			uiEasy.setIconName("card", card:read("card_id"),
				{ node = childs.cardName, advance = card:read("advance"), space = true })
	...
	for k, data in ipairs(secondAttrs) do
		...
		if not key then
			...
			text.addEffect(item:get("key"), { color = ui.NEWCOLORS.GRAY })
			text.addEffect(item:get("val"), { color = ui.NEWCOLORS.GRAY })
			childs.list:pushBackCustomItem(item)
	...
	if dataEasy.isUnlock(gUnlockCsv.chipPlan) then
		...
		if #names > 0 then
			...
			local list, height = beauty.textScroll({
				...
				strs = "#C0xFFFCED##L00100000##LOC0xF13B54#" ..
				gLanguageCsv.chipUsedInPlans .. " #C0xFFFF66##L00100000##LOC0xF13B54#" .. table.concat(names, ", "),
				isRich = true,
			})
