...
ChipDrawView.RESOURCE_BINDING = {
	...
	["panelMidRight.downPanel.item1.number"] = {
		varname = "numberAll",
		binds = {
			event = "effect",
	...
	["panelMidRight.downPanel.item2.number"] = {
		varname = "number2",
		binds = {
			event = "effect",
			data = {outline = {color = ui.NEWCOLORS.OUTLINE.BLACK, size = 3}}
	...
	["panelMidRight.downPanel.item3.number"] = {
		varname = "number3",
		binds = {
			event = "effect",
			data = {outline = {color = ui.NEWCOLORS.OUTLINE.BLACK, size = 3}}
	...
	["panelMidRight.downPanel.item4.number"] = {
		varname = "number4",
		binds = {
			event = "effect",
			data = {outline = {color = ui.NEWCOLORS.OUTLINE.BLACK, size = 3}}
...
function ChipDrawView:onCreate()
	...
	idlereasy.any({self.itemDrawCount, self.vip_level}, function(_, count, vip_level)
		...
		text.addEffect(self.itemRecord, {color = count < max and ui.NEWCOLORS.GREEN2 or ui.NEWCOLORS.REDLIGHT})
	end)
	...
	idlereasy.any({self.rmbDrawCount, self.vip_level}, function(_, count, vip_level)
		...
		text.addEffect(self.diamondRecord, {color = count < max and ui.NEWCOLORS.GREEN2 or ui.NEWCOLORS.REDLIGHT})
	end)
...
function ChipDrawView:draw(costType, drawTimes)
	self:drawRequest(costType, drawTimes, function(data)
		local effect = self.bg:getChildByName("effectBg")
		if effect then
		effect:play("effect")
