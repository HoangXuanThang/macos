		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					if not v.pos then
						text.addEffect(node:get("txt"),
							{ color = v.select and ui.COLORS.NORMAL.WHITE or ui.COLORS.NORMAL.RED, outline = { color = v.select and ui.NEWCOLORS.OUTLINE.BLACK or ui.NEWCOLORS.OUTLINE.WHITE1, size = 3 } })
						node:get("txt"):show()
						node:get("icon"):hide()
					else
						node:get("txt"):hide()
						node:get('icon'):show():texture(v.select and 'city/card/chip/icon_fx_2.png' or
						'city/card/chip/icon_fx_1.png')
							:rotate(60 * (v.pos - 1))
					end
			...
			props = {
				...
				onCell = function(list, node, k, v)
					...
					node:get("count"):text(v.count)
						:setFontSize(36)
					text.addEffect(node:get("count"),
						{ color = ui.COLORS.NORMAL.WHITE, outline = { color = ui.COLORS.OUTLINE.DEFAULT, size = 3 } })
...
function ChipBagView:onCreate(params)
	...
	local _, count = adapt.centerWithScreen("left", "right", {
		...
		{ { self.btnSuitFilter, self.suitFilterPanel:get("panel"), self.btnAttrFilter, self.attrFilterPanel:get("panel") },
			"pos", function(left, right)
			return right - left
		end },
	})
	...
	bind.click(self, self.suitAttrDetailPanel, {
		method = function()
			self.suitAttrDetailPanel:hide()
		end
	})
	...
	bind.click(self, self.suitFilterPanel, {
		method = function()
			self.suitFilterPanel:hide()
	...
	idlereasy.when(self.selectSuitId, function(_, selectSuitId)
		...
		self.suitFilterPanel:hide()
		self.btnSuitFilter:get("arrow"):setFlippedY(false)
		self.btnSuitFilter:get("txt"):text(selectSuitId and gChipSuitCsv[selectSuitId][2][2].suitName or
		gLanguageCsv.chipBagSuitFilter)
	end)

	-- 属性筛选， 多选
	self.attrFilterPanel:hide()
	self.selectAttrIds = idlereasy.new({}) -- {}:默认，{1=true,7=true}筛选包含选择属性的秘籍
	bind.click(self, self.attrFilterPanel, {
		method = function()
			self.attrFilterPanel:hide()
	...
	idlereasy.any(
	{ self.roleChips, self.cardChips, self.selectLeftPos, self.selectSuitId, self.selectAttrIds, self.isRefreshBagPanel },
		function()
...
function ChipBagView:onShowSelectSpriteView()
	gGameUI:stackUI('city.card.chip.select_sprite', nil, nil, self.selectCardDBID:read(),
		self:createHandler('setCardDBID'))
end
...
function ChipBagView:onOnekeyDownClick()
	...
	if state == "first" or state == "true" then
		gGameUI:showDialog({ content = "#C0x57472E#" .. gLanguageCsv.chipOnekeyDownTip, cb = cb, isRich = true, btnType = 2, selectKey =
		key, selectType = 2, selectTip = gLanguageCsv.todayNoTip })
	else
		cb()
	end
end
...
function ChipBagView:onPlanClick()
	...
	gGameUI:stackUI('city.card.chip.plan', nil, nil,
		{ curCardDBID = self.selectCardDBID:read(), cb = self:createHandler("planCb") })
end
...
function ChipBagView:getRuleContext(view)
	...
	table.insert(infoList,
		{ key = csv.note[124606].fmt, main = csv.note[124607].fmt, sec = csv.note[124608].fmt, sign = true })

	local count = 124609
	for index = 1, 6 do
		table.insert(infoList,
			{ key = index, main = csv.note[count + 2 * index - 2].fmt, sec = csv.note[count + 2 * index - 1].fmt, sign = false })
	end
	for index = 1, 7 do
