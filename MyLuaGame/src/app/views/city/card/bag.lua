...
local function setItem(item, childs, v)
	...
	local icon = childs.iconBg:getChildByName("icon")
	if v.rarity and v.rarity >= 5 then
		icon:visible(false)
		widget.addAnimation(childs.iconBg, v.cardShowRes, "default", 5, true):alignCenter(childs.iconBg:size())
		widget.addAnimation(childs.iconBg, v.cardShowRes, "default", 5, true):scale(v.scale)
	else
		icon:visible(true)
		icon:texture(v.icon):scale(0.9)
	end

	childs.qulityNumber:hide()
	uiEasy.setIconName("card", cardId,
		{ node = childs.levelNamePanel:get("name"), name = v.name, advance = 1, space = true })
	...
	adapt.oneLineCenterPos(cc.p(200, -3),
		{ childs.fightPointPanel:get("fightPointTxt"), childs.fightPointPanel:get("fightPoint") }, cc.p(15, 0))
	adapt.oneLineCenterPos(cc.p(200, 13), { childs.levelNamePanel:get("level"), childs.levelNamePanel:get("name") },
		cc.p(15, 0))
	...
	if v.isBg then
		widget.addAnimationByKey(childs.maskBg, "effect/duiwuzhong.skel", "effect", "effect_loop", 20)
			:xy(size.width / 2 + 10, size.height / 2 - 140)
	end
...
local function setItemChip(item, childs, v)
	...
	childs.iconBg:getChildByName("icon"):texture(v.icon)
	childs.material:visible(cardCsv and cardCsv.cardType == 2)
...
CardBagView.RESOURCE_BINDING = {
	...
	["panelBtn.btn4"] = {
		...
		binds = {
			{
				event = "touch",
				methods = { ended = bindHelper.self("onEnterStar") }
			},
			-- {
			-- 	event = "visible",
	...
	["panelBtn.btn4.textNote"] = {
		binds = {
			{
				event = "text",
				data = gLanguageCsv.starTitle,
			},
			{
				event = "effect",
	...
	["panelBtn.btn1.textNote"] = {
		...
		binds = {
			{
				event = "text",
				data = gLanguageCsv.beibaoyingxiong,
			},
			{
				event = "effect",
	...
	["panelBtn.btn2.textNote"] = {
		...
		binds = {
			{
				event = "text",
				data = gLanguageCsv.beihaosuipian,
			},
			{
				event = "effect",
				data = { glow = { color = ui.COLORS.GLOW.WHITE } },
	...
	["panelBtn.btn3.textNote"] = {
		binds = {
			{
				event = "text",
				data = gLanguageCsv.beihaofengjie,
			},
			{
				event = "effect",
				--data = {glow = {color = ui.COLORS.GLOW.WHITE}},
		...
		binds = {
			...
			props = {
				...
				onCell = function(list, cell, k, v)
					local childs = cell:multiget("bg", "attr1", "attr2", "levelNamePanel", "material", "fightPointPanel",
						"rarity", "maskBg", "qulityNumber", "bottomBg", "imgLock", "iconBg")
				...
				onBeforeBuild = function(list)
					if list.sliderBg:visible() then
						...
						list:setScrollBarPositionFromCorner(cc.p(listX + listSize.width - x,
							(listSize.height - size.height) / 2 + 5))
	...
	["centerPanel.fragList"] = {
		...
		binds = {
			...
			props = {
				...
				itemAction = { isAction = true, alwaysShow = true, actionTime = 0.5, duration = 0.3 },
				onCell = function(list, cell, k, v)
					local childs = cell:multiget("bg", "attr1", "attr2", "name", "rarity", "numBg", "maskBg", "fragBg",
						"bottomBg", "material", "iconBg")
...
function CardBagView:onCreate(bagType)
	...
	idlereasy.when(self.type, function(obj, typ)
		...
		local colorOutline1 = ui.NEWCOLORS.OUTLINE.BLUE
		local colorOutline2 = ui.NEWCOLORS.OUTLINE.BLUE
		if typ == SHOW_TYPE.CARD then
			color1 = ui.COLORS.NORMAL.WHITE
			colorOutline1 = ui.NEWCOLORS.OUTLINE.YELLOW
		end
		if typ == SHOW_TYPE.FRAGMENT then
			color2 = ui.COLORS.NORMAL.WHITE
			colorOutline2 = ui.NEWCOLORS.OUTLINE.YELLOW
		end
		text.addEffect(self.textNoteSprite, { color = color1, outline = { color = colorOutline1, size = 4 } })
		text.addEffect(self.textNoteFragment, { color = color2, outline = { color = colorOutline2, size = 4 } })
	end)
	...
	local sortTriggers = idlereasyArgs.new(self, "sortTabDataIndex", "cardInfos", "fragInfos", "type", "sortKey",
		"sortOrder", "rarity", "attr1", "attr2", "atkType")
	local resortTimes = 0
	...
	dataEasy.getListenUnlock(gUnlockCsv.battleRecommend, function(isUnlock)
		...
		self.btnBattleRecommend:visible(false)
	end)

	self.btnStar:setVisible(false)
...
function CardBagView:refushBagData()
	...
	for i, v in ipairs(cards) do
		...
		if card then
			local cardData = card:read("card_id", "unit_id", "skin_id", "fighting_point", "level", "star", "advance",
				"locked", "name", "created_time", "equips", "effort_values")
			...
			datas[i] = {
				...
				atkType = cardCsv.atkType,
				showOpen = unitCsv.showOpen,
				cardShowPosY = unitCsv.cardShowPosY,
				scaleY = unitCsv.scaleY,
				cardShowRes = unitCsv.cardShowRes,
			}
		end
	end
...
function CardBagView:sortFilterData(typ, data, key, order, condition)
	...
	if next(sortResult) == nil then
		local txt = ((next(data) == nil) and (typ ~= SHOW_TYPE.CARD)) and gLanguageCsv.cardBagNoFrags or
			gLanguageCsv.filteringIsEmpty
		self.showEmpty:show()
...
function CardBagView:onItemClick(list, item, t, v)
	if v.isSprite then
		...
		if v.isBg then
			...
			local cardCsv = csv.cards[fragCsv.combID]
			local strs = {
				string.format("#C0x57472e#" .. gLanguageCsv.wantConsumeFragsCombCard, fragCsv.combCount,
					"#C0x01A001#" .. fragCsv.name .. "#C0x57472e#", "#C0x01A001#" .. cardCsv.name)
			}
			gGameUI:showDialog({
				content = strs,
				cb = function()
					gGameApp:requestServer("/game/role/frag/comb", function(tb)
						gGameUI:stackUI("common.gain_sprite", nil, { full = true }, tb.view, nil, false,
							self:createHandler("setCardCenterDbid", tb.view.db_id))
					end, v.id)
				end,
				btnType = 2,
				isRich = true,
