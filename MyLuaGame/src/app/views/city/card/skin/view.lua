...
-- 	"city/drawcard/draw/panel_card_q.png",
-- }
...
function CardSkinView:onCreate(cardDbid)
	...
	idlereasy.when(self.star, function(_, star)
    	...
    	self.starDatas:set({unpack(starDatas, 1, 6)})	
	end)
...
function CardSkinView:initCell(cell, v, num)
	...
	if v.skinFrameType == 1 then
		childs.ImageBg:texture("city/drawcard/draw/panel_card_c.png")
	else
...
function CardSkinView:setLeftPanelInfo(unitId)
	...
	self.cardSprite = widget.addAnimation(self.heroNode, unit.unitRes, "standby_loop", 5)
		:xy(size.width / 2, 0)
	self.cardSprite:scale(unit.scaleU)
	self.cardSprite:setSkin(unit.skin)

	self.imgIcon:texture(unit.cardShow)
	self.cardActionState:set(3, true)

	widget.addAnimationByKey(self.heroNode, "effect/jinhuajiemian.skel", 'spineDown', "effect_down2_loop", 4):xy(size
		.width / 2, -100):scale(1.5)
	widget.addAnimationByKey(self.heroNode, "effect/jinhuajiemian.skel", 'spineUp', "effect_up_loop", 6):xy(size.width /
		2, -100):scale(1.5)
end
