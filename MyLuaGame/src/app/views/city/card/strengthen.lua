...
local LEFT_BTN_DATAS = {
	{
		key = "zawake",
		icon = "city/card/system/attribute/icon_zawake.png",
		unlockKey = "zawake",
		txt = gLanguageCsv.zawake,--'命格'
		click = "onZawakeClick",
	},
	{
		key = "develop",
		icon = "city/card/system/attribute/icon_juexing.png",
		txt = gLanguageCsv.levelUp,
		click = "onDevelopClick",
	},
	{
		key = "feel",
	...
	{
		key = "skin",
		icon = "city/card/system/attribute/icon_shizhuang.png",
		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					uiEasy.updateUnlockRes(v.unlockKey, node, { justRemove = not v.unlockKey, pos = cc.p(120, 80) })
						:anonyOnly(list, list:getIdx(k))
					if v.key == "feel" then
					...
					text.addEffect(childs.textNote, { outline = { color = ui.NEWCOLORS.OUTLINE.BLACK } })
...
function CardStrengthenView:onCreate(selectTabKey, selectDbId, closeHandler)
	...
	self.heldItemBg = idler.new("common/btn/btn_d.png")
	self.heldItemIcon = idler.new("city/card/helditem/icon_jiahao.png")
	local resortTimes = 0
	idlereasy.any({ self.heldItemListen, self.curState, gGameModel.role:getIdler("items") },
		function(_, isShow, curState)
	...
	idlereasy.view_defer(self, "any", { self.seletSortKey, self.tabOrder },
		function(_, seletSortKey, tabOrder, cardDatas)
	...
	local function changeCardRes(unitCfg)
		if self.photoSprite and not tolua.isnull(self.photoSprite) then
			self.photoSprite:removeFromParent()
			self.photoSprite = nil
		end
		if unitCfg.showOpen then
			self.photoSprite = widget.addAnimation(self.photo, unitCfg.showOpen, "default", 5, true)
			if string.find(unitCfg.showOpen, ".png") then
				self.photoSprite:y(-50)
			else
				self.photoSprite:y(50)
			end
		end
...
function CardStrengthenView:onCurStateChanged(curState, prevState)
	...
	if curState == STATE.DECORATION then
		self.topView = gGameUI.topuiManager:createView("default", self,
				{ onClose = self:createHandler("onCloseDecoration") })
...
function CardStrengthenView:onCardStarChanged(_, star)
	...
	local starDatas = dataEasy.getStarData(star)
...
function CardStrengthenView:updateLeftBtnDatas()
	...
	for k, v in ipairs(LEFT_BTN_DATAS) do
		...
		if show then
			if v.key == "skin" then
				if dataEasy.isShowSkinIcon(self.cardId:read()) then
					table.insert(t, v)
				end
			elseif v.key == "develop" then
				local csvCard = csv.cards[self.cardId:read()]				
				if csvCard and csvCard.develop and csvCard.develop > 0 then
					table.insert(t, v)
				end
			elseif v.key == "zawake" then
				if self.curState:read() ~= STATE.DECORATION and dataEasy.isInServer("zawake") then
...
function CardStrengthenView:onResetHeldItemIcon(dbId)
	...
	if dbId then
		...
		text.addEffect(lb1, { outline = { color = ui.NEWCOLORS.OUTLINE.BLACK } })
		text.addEffect(lb2, { outline = { color = ui.NEWCOLORS.OUTLINE.BLACK } })
		if self.btnHeldItem:get("spine") then
			self.btnHeldItem:get("spine"):show()
			self.btnHeldItem:get("spine"):play("effect_loop")
			self.btnHeldItem:get("spine"):scale(0.7)
		else
...
function CardStrengthenView:onZawakeClick()
	...
	local zawakeID = self:getZawakeID(self.selectDbId:read())
	local zawakeTools = require "app.views.city.zawake.tools"
	local stageCfg = zawakeTools.getStagesCfg(zawakeID, 1)
	local isUnlock, labelDatas = zawakeTools.isUnlockByStage(stageCfg, zawakeID)
	if not isUnlock then
		gGameUI:stackUI("city.zawake.unlock_tips", nil, nil, { labelDatas = labelDatas, stageID = 1 })
		return
	else
		gGameUI:stackUI("city.zawake.view", nil, nil,
			{ zawakeID = zawakeID, cb = self:createHandler("onZawakeSelectDbIdChange") })
	end
end
...
function CardStrengthenView:onFeelClick()
	...
	-- end
	-- local zawakeID = self:getZawakeID(self.selectDbId:read())
	-- gGameUI:stackUI("city.zawake.view_copy", nil, nil,
	-- 	{ zawakeID = zawakeID, cb = self:createHandler("onZawakeSelectDbIdChange") })
end

function CardStrengthenView:onRebornClick()
	if not gGameUI:goBackInStackUI("city.card.rebirth.view") then
		gGameUI:stackUI("city.card.rebirth.view", nil, nil, 1, self:createHandler("resetSelDbId"),
			self:createHandler("cardsChanged"))
	end
end
...
function CardStrengthenView:onDevelopClick()
	gGameUI:stackUI("city.card.develop", nil, nil, self.cardId:read())
end
