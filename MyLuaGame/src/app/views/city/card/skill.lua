		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					local childs = node:multiget("textLvNum", "textCost", "textName", "imgType", "btnAdd", "imgIcon",
						"imgBG", "textFastUpgradeNum")
					childs.textLvNum:text(v.skillLevel)
					local skillGold = getCostGold(v.skillLevel, skillInfo.costID, v.fastUpgradeNum)
					local goldColor = (v.clientGold >= skillGold) and ui.NEWCOLORS.WHITE or ui.NEWCOLORS.REDLIGHT
					...
					cache.setShader(childs.btnAdd, false,
						(v.skillLevel + v.fastUpgradeNum <= cardLv) and "normal" or "hsl_gray")
					...
					elseif skillInfo.activeType == 2 then
						...
						name = ui.QUALITYCOLOR[dataEasy.getQuality(skillInfo.activeCondition)] ..
							gLanguageCsv.symbolSquareBracketLeft .. name .. gLanguageCsv.symbolSquareBracketRight
					end
					...
					if dataEasy.isZawakeSkill(v.skillId, list.zawakeSkills:read()) then
						node:get("imgIcon_m"):visible(false)
						node:get("imgBG"):texture("common/box/box_output_m.png")
						node:get("bg1"):texture("common/box/box_nameinput_m.png")
						node:get("bg2"):texture("common/box/box_nameinput_m.png")
						ccui.ImageView:create("city/drawcard/draw/txt_up.png")
							:scale(0.7)
							:align(cc.p(1, 1), 360, 40)
						...
						childs.textName:text(zawakeText .. childs.textName:text())
					else
						node:get("imgIcon_m"):visible(false)
						node:get("imgBG"):texture("common/box/box_output.png")
						node:get("bg1"):texture("common/box/box_nameinput.png")
						node:get("bg2"):texture("common/box/box_nameinput.png")
					end
				end,
				asyncPreload = 5,
			},
...
function CardSkillView:onCreate(dbHandler)
	...
	local valTime = nil
	idlereasy.any({ self.skillPointLast, self.skillPoint, self.tmpSkillPoint, self.refreshPoint },
...
function CardSkillView:upgradeFloatingWord(node, skillId)
	...
	local upLvDesc = csv.skill[skillId]["upLvDesc_"..LOCAL_LANGUAGE] or csv.skill[skillId].upLvDesc
	local data = string.split(upLvDesc, "|")
