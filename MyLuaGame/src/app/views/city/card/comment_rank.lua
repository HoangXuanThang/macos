...
function CommentRankView:showFightingPanel()
	...
	-- 左侧面板
	local lChilds = self.fightingRank:get("left"):multiget("cardName", "cardIcon", "roleName", "lv", "lv1", "fighting",
		"fighting1")
	-- 名字
	lChilds.cardName:text(unitCfg.name)
	lChilds.roleName:text(self.fightOne.role.name)
	lChilds.lv1:text(self.fightOne.role.level)
	adapt.oneLineCenterPos(cc.p(370, 120), { lChilds.roleName, lChilds.lv, lChilds.lv1 }, cc.p(10, 0))
	...
	if unitCfg.showOpen then
		local photoSprite = widget.addAnimation(parent, unitCfg.showOpen, "default", 5, true)
		if string.find(unitCfg.showOpen, ".png") then
			photoSprite:xy(size.width / 2, size.height / 2):scale(0.7)
		else
			photoSprite:xy(size.width / 2, size.height / 2):scale(0.7)
		end
	end
	if gGameModel.role:read("id") ~= self.fightOne.role.id then
		bind.touch(self, parent, {
			methods = {
				ended = function()
	...
	-- 右侧面板个人信息
	local rChilds = self.fightingRank:get("right.myRank"):multiget("txtRank", "rank", "head", "name", "vip", "lv", "lv1",
		"fighting", "card")
	-- 头像
	...
	self.selectEvolution:addListener(function(val, oldval)
		...
		if evolutionDatas then
			...
			if unitCsv.showOpen then
				local photoSprite = widget.addAnimation(childs.cardIcon, unitCsv.showOpen, "default", 5, true)
			...
			adapt.oneLineCenterPos(cc.p(380, childs.name:y()),
				{ childs.rare, childs.name, childs.iconAttr1, childs.iconAttr2 }, cc.p(15, 0))
