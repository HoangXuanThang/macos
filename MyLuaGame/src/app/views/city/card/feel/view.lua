...
function CardFeelView:onCreate(cardId)
	...
	adapt.setTextAdaptWithSize(self.note,
		{ str = gLanguageCsv.cardFeelViewtext, size = cc.size(220, 200), vertical = "center" })
	...
	idlereasy.any({ self.clientLv, self.items, self.clientCurLvExp }, function(_, clientLv, items, clientCurLvExp)
		...
		for _, v in orderCsvPairs(cardCsv.feelItems) do
			...
			table.insert(self.guideItmsData, {id = v, num = items[v] or 0, targetNum = math.ceil(feelExp / csv.items[v].specialArgsMap.feel_exp)})
		end
	...
	if #addAttrDatas > 0 then
		...
		for i = 1, 2 do
			if addAttrDatas[i] then
				str = string.format("%s#C0x57472E# %s#C0x01a001#+%s", str, addAttrDatas[i].name, addAttrDatas[i].num)
			end
		end
		str = str .. "#C0x57472E# )"
...
function CardFeelView:setAddAttrDatas(cardCsv, clientLv, oldClientLv)
	...
	for k, v in pairs(effectCsv) do
		local noteColor = "#C0x57472E#"
		local numColor = "#C0x56A732#"
		if k > clientLv then
			noteColor = "#C0x979389#"
			numColor = "#C0x979389#"
		end
...
function CardFeelView:setLeftPanel(evolutionDatas)
	...
	if unitCsv.showOpen then
		self.photoSprite = widget.addAnimation(childs.cardIcon, unitCsv.showOpen, "default", 5, true)
		if string.find(unitCsv.showOpen, ".png") then
			self.photoSprite:xy(size.width / 2, size.height / 2):scale(0.8)
		else
			self.photoSprite:xy(size.width / 2, size.height / 2):scale(0.8)
		end
	end
...
function CardFeelView:onLvUpEasyClick()
	...
	if self.canOneUp then
		gGameUI:stackUI("city.card.feel.upgrade", nil, nil,
			{ cardMarkID = self.cardMarkID, cardId = self.cardId, type = 2, selectState = self.selectState, cb = function()
				self.isShowGuideGift:set(true)
			end }) -- 传type = 1代表的是等级一键提升
	else
		gGameUI:showTip(gLanguageCsv.materialsNotEnough)
	end
end
