		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					local bottomChilds = childs.bottom:multiget("btnMore", "time", "txtLike", "txtDislike", "btnLike",
						"btnDislike")
					...
					bottomChilds.time:text(tb.year .. "-" .. tb.month .. "-" .. tb.day .. "  " .. tb.hour .. ":" ..
						tb.min)
					...
					bind.touch(node, topChilds.btnDel,
						{ methods = { ended = functools.partial(list.btnDelClick, node, childs, textList, pos, v) } })

					-- 头像点击
					if gGameModel.role:read("id") ~= v.val.role_db_id then
						bind.touch(node, headChilds.head,
							{ methods = { ended = functools.partial(list.headClick, childs.head, k, v) } })
					end
					...
					-- 点赞取消赞按钮
					bind.touch(node, bottomChilds.btnLike,
						{ methods = { ended = functools.partial(list.likeClick, k, v, v.like) } })
					-- 点踩取消踩按钮
					bind.touch(node, bottomChilds.btnDislike,
						{ methods = { ended = functools.partial(list.dislikeClick, k, v, v.dislike) } })
				end,
			},
			handlers = {
	...
	self.selectEvolution:addListener(function(val, oldval)
		...
		if evolutionDatas then
			...
			if unitCfg.showOpen then
				self.photoSprite = widget.addAnimation(childs.photo, unitCfg.showOpen, "default", 5, true)
				if string.find(unitCfg.showOpen, ".png") then
					self.photoSprite:scale(0.8)
				else
					self.photoSprite:y(50):scale(0.8)
				end
			end
			...
			adapt.oneLineCenterPos(cc.p(290, childs.name:y()), { childs.rare, childs.name, childs.iconAttr1 },
				cc.p(15, 0))
...
function CardCommentView:initGrade()
	...
	for i = 1, 5 do
		...
		bind.click(self, self.star:get("star" .. i), {
			method = function()
				-- 是否激活图鉴
...
function CardCommentView:initComment()
	...
	-- 发表评论
	bind.touch(self, self.btnComment, {
		methods = {
			ended = function()
				...
				if times >= gCommonConfigCsv.cardCommentDailyMarkSendTimes then
					gGameUI:showTip(string.format(gLanguageCsv.onedayCanCommentTimes,
						gCommonConfigCsv.cardCommentDailyMarkSendTimes))
					return
				end

				-- 可评论不同进化链次数
				if itertools.size(self.commentCounter) >= gCommonConfigCsv.cardCommentDailyMarkNum and not self.commentCounter[markId] then
					gGameUI:showTip(string.format(gLanguageCsv.onedayDifferentCanCommentTimes,
						gCommonConfigCsv.cardCommentDailyMarkNum))
					return
				end
