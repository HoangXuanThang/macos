			...
			if params.base ~= 0 then
				...
				if not params.special then
					if not params.color then
						-- build icon sao theo base cần
						local t = {}
						local starData = dataEasy.getStarData(params.base)
						for i, v in ipairs(starData) do
							table.insert(t, string.format("#I%s-50-50#", v.icon))
						end
				...
				table.insert(strs, {
					str = str,
					state = params.state >= params.base and 1 or 0,
					tip = params.tip
				})
			end
		end
