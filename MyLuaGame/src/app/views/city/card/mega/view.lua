		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					node:get("item.select"):visible(v.isSel):scale(1.108)
				...
				onItem = function(list, node, k, v)
					...
					if v.costCards then
						...
						text.addEffect(childs.num1,
							{ color = isEnough and ui.COLORS.NORMAL.FRIEND_GREEN or ui.COLORS.NORMAL.ALERT_ORANGE, outline = { color = ui.COLORS.NORMAL.WHITE } })
...
function MegaView:onTabItemClick(list, k, v)
	...
	if v.unitCfg.showOpen then
		self.cardSprite = widget.addAnimation(self.animaCard, v.unitCfg.showOpen, "default", 1, true):alignCenter(self
			.animaCard:size()):y(150):scale(0.9)
	end
	...
	local conditionStr = cfg.condition[1] == 1 and string.format(gLanguageCsv.roleLevelReach, cfg.condition[2]) or
		string.format(gLanguageCsv.roleCardLevelReach, cfg.condition[2])
...
function MegaView:costGoldUpdate()
	...
	if costGold then
		...
		text.addEffect(self.panel:get("gold"),
			{ color = roleGold >= costGold and ui.COLORS.NORMAL.DEFAULT or ui.COLORS.NORMAL.ALERT_ORANGE })
		adapt.oneLineCenterPos(cc.p(400, 40), { self.panel:get("txt1"), self.panel:get("gold"), self.panel:get("icon") },
			cc.p(8, 0))
	else
		self.panel:hide()
	end
...
function MegaView:cardStarUpdate(star)
    ...
    -- Chỉ hiển thị tối đa 6 sao
    local cardStarData = {}
    for i = 1, math.min(#starDatas, 6) do
        table.insert(cardStarData, starDatas[i])
    end
...
function MegaView:cardItemUpdate()
	...
	txt2:text("/" .. 1)
	text.addEffect(txt1,
		{ color = dbid and ui.COLORS.NORMAL.FRIEND_GREEN or ui.COLORS.NORMAL.ALERT_ORANGE, outline = { color = ui.COLORS.NORMAL.WHITE } })
...
function MegaView:animaFunc(cardId)
	...
	self.animaQian = widget.addAnimationByKey(self.anima, "chaojinhua/chaojinhua.skel", "qian", "chaojinhua_effect_qian",
			4)
		:alignCenter(self.anima:size())
		:scale(2)
	local specialCardPosY = {
		[4023] = -250
	}
	...
	local action = cc.RepeatForever:create(cc.Sequence:create(
		cc.CallFunc:create(function()
			...
			self.cardSprite2:rotate(-rotation)
				:scaleX(scaleX * 1.2)
				:scaleY(scaleY * 1.2)
				:xy(cc.p(bxy.x * sx + posx, bxy.y * sy + posy + (specialCardPosY[unitID2] or 0)))
		end)
	))
