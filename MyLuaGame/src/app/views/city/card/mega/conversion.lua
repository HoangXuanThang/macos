					...
					if itertools.size(v) == 0 then
						...
						if v.type == "card" then
							if not v.selectId then
								childs.item:hide()
								local cfg = v.cfg
								local str = gLanguageCsv.selectCardFragments ..
									string.format(gLanguageCsv.selectRarityCardFragments, rarityData[cfg.needCards1[1]])
								for i = 1, math.huge do
									...
									if needCards[2] ~= -1 then
										...
										str = str ..
											ui.ATTRCOLOR[txt] ..
											string.format(gLanguageCsv.selectTypeCardFragments, gLanguageCsv[txt]) ..
											"#C0x5B545B#"
									end
								end
								childs.name:hide()
								...
								list:xy(childs.name2:x() - childs.name2:width() / 2,
									childs.name2:y() - childs.name2:height() / 2 - 20)
						...
						elseif v.type == "frag" then
							...
							if not v.selectId then
								...
								local cfg = v.cfg
								local str = gLanguageCsv.selectCardFragments ..
									string.format(gLanguageCsv.selectRarityCardFragments,
										rarityData[cfg.needFrags1[1] - 2])
								for i = 1, math.huge do
...
function MegaConversionView:onCreate(data)
	...
	--转换的总次数
	local times = self.cardConvertCfg.type ~= 1 and gCommonConfigCsv.megaConvertTimes or
		gCommonConfigCsv.megaCommonConvertTimes
	...
	idlereasy.any({ self.megaConvertTimes, self.megaConvertBuyTimes }, function(_, megaConvertTimes, megaConvertBuyTimes)
		...
		self.conversionNumMax = self.cardConvertCfg.type ~= 1 and gVipCsv[self.vipLevel].megaItemMaxTimes or
			gVipCsv[self.vipLevel].megaCommonItemMaxTimes
		self.timesPanel:get("num2"):text("/" .. self.conversionNumMax)
		adapt.oneLinePos(self.timesPanel:get("num2"), { self.timesPanel:get("num1"), self.timesPanel:get("txt1") },
			cc.p(0, 0), "right")
	end)
...
function MegaConversionView:costGoldUpdate()
	...
	if gold == 0 then
		...
		text.addEffect(self.costPanel:get("num"),
			{ color = roleGold >= gold and ui.COLORS.QUALITY_OUTLINE[1] or ui.COLORS.NORMAL.ALERT_ORANGE })
		adapt.oneLineCenterPos(cc.p(260, 40),
			{ self.costPanel:get("txt1"), self.costPanel:get("num"), self.costPanel:get("icon") }, cc.p(8, 0))
	end
end
...
function MegaConversionView:stateUpdate()
	...
	adapt.oneLineCenterPos(cc.p(200, 30),
		{ self.debrisItem:get("title.txt1"), self.debrisItem:get("title.num"), self.debrisItem:get("title.txt2") },
		cc.p(5, 0))
...
function MegaConversionView:chooseCard(list, k, v)
	if v.type == "card" then
		gGameUI:stackUI("city.card.mega.choose_card", nil, { dialog = true }, self.selectedData,
			self:createHandler('stateChange'))
	elseif v.type == "frag" then
		gGameUI:stackUI("city.card.mega.fragment_select", nil, { dialog = true }, self.selectedData,
			self:createHandler('stateChange'))
	else
