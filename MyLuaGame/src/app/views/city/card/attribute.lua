...
CardAttributeView.RESOURCE_BINDING = {
	...
	["center.top.textDingWeiDesc"] = "textDingWeiDesc",
		...
		binds = {
			...
			props = {
				...
				onCell = function(list, node, k, v)
					...
					node:get("textNote"):text(fetter["name_"..LOCAL_LANGUAGE] or fetter.name)
					if v.isShow then
						text.addEffect(node:get("textNote"), { color = ui.NEWCOLORS.WHITE })
					else
						text.addEffect(node:get("textNote"), { color = ui.NEWCOLORS.GRAY })
					end
				end,
				asyncPreload = 6,
			},
...
function CardAttributeView:onCreate(dbHandler, playAction)
	...
	self:initModel()
	self:enableSchedule()
	adapt.setTextAdaptWithSize(self.medLvUpTextNote,
		{ size = cc.size(835, 80), vertical = "center", horizontal = "center", margin = -8 })
	...
	idlereasy.any({ self.cardDatas, self.fetters, self.cardId, self.natureChosse },
		function(obj, cardDatas, fetters, cardId, natureChosse)
			local cardCsv = csv.cards[cardId]
			self.textDingWeiDesc:text(cardCsv["location_"..LOCAL_LANGUAGE] or cardCsv.location)
...
function CardAttributeView:onExchangeChooseNature()
	...
	local str = string.format(gLanguageCsv.chooseNatureText, ui.ATTR_ICON[natureTypeTb[chooseId]],
		ui.ATTR_ICON[natureTypeTb[3 - chooseId]])
...
function CardAttributeView:onUpGradePanelClick()
	...
	gGameUI:stackUI("city.card.upgrade", nil, nil, { selectDbId = self.selectDbId:read(), type = 1 },
		self:createHandler("isSuccess")) -- 传type = 1代表的是等级一键提升
end
...
function CardAttributeView:upgradeFloatingWord(str)
	...
	text.addEffect(advanceNum,
		{ color = cc.c4b(0, 255, 0, 255), outline = { color = cc.c4b(44, 44, 44, 255), size = 3 } })
...
function CardAttributeView:showGuideGift()
