	...
	idlereasy.any({self.lockNum, self.items}, function(_, lockNum, items)
		...
		for i,v in csvPairs(nvalueData) do
			...
			if myItemNum < v then
				...
				table.insert(guideItmsData, {id = i, num = myItemNum, targetNum = v})
			end
		end
		...
		if lockNum >= 6 then
			...
			text.addEffect(self.btnResetText, {color = ui.COLORS.NORMAL.WHITE, outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4}})
			if self.isShowGuideGift and self.lackMaterial:read() then
				uiEasy.goGuideGift(guideItmsData)
				self.isShowGuideGift = false
			end
		end
	end)
