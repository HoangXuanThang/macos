...
function HeldItemTools.isExclusive(data)
	...
	if heldItemInfo and itertools.size(heldItemInfo.exclusiveCards) > 0 then
		cardId = heldItemInfo.exclusiveCards[1]
		isExclusive = true
	end

	return isDress, isExclusive, cardId
end

function HeldItemTools.insertColor(str, color, isName, idx, single, defaultColor)
	defaultColor = defaultColor or "#C0x57472E#"
	idx = idx or 1
...
function HeldItemTools.getStrinigByData(i, data, colorstr)
	local cfg = data.cfg
	...
	local str = efcInfo["desc_"..LOCAL_LANGUAGE] or efcInfo.desc
	local params = {}
	local startIdx = 1
	-- 是否专属
	if efcInfo.exclusiveCards[1] then
		...
		str = HeldItemTools.insertColor(str, color, true, startIdx, true, colorstr)
		startIdx = startIdx + 1
	end
	...
	elseif efcInfo.type == 2 then
		...
		local desc = csv.skill[efcInfo.skillID]["describe_"..LOCAL_LANGUAGE] or csv.skill[efcInfo.skillID].describe

		-- 技能描述是一长段话 需要对公式那部分单独处理颜色
		desc = HeldItemTools.insertSkillDescColor(desc, "#C0x60c456#", colorstr)
		table.insert(params, eval.doMixedFormula(desc, { skillLevel = skillLv, math = math }))
	end
	str = HeldItemTools.insertColor(str, insertColor, false, startIdx, false, colorstr)
...
function HeldItemTools.getCardNameColor(cardDbId)
	...
	local str = ui.QUALITYCOLOR[quality]
