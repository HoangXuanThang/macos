...
local function setCostTxt(panel, myCoin, needCoin)
	...
	cost:text(needCoin)
	local coinColor = (myCoin >= needCoin) and ui.NEWCOLORS.WHITE or ui.COLORS.NORMAL.RED
...
local function setEffect(parent, effectName, cb)
	...
	if not effect then
		effect = widget.addAnimationByKey(parent, "koudai_gonghuixunlian/gonghuixunlian.skel", effectName, "fangguang2",
				10)
			:xy(40, 20)
			:scale(0.25)
...
local function createAttrRichText(parent, csvId, advance, isNext)
	...
	for i = 1, 100 do
		...
		local resultStr = HeldItemTools.getStrinigByData(i, data, colorstr)
...
HeldItemAdvanceView.RESOURCE_BINDING = {
		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					local selected = node:get("select"):setZOrder(200)
					local panel
					if v.select then
						normal:hide()
						panel = selected:show()
		...
		binds = {
			...
			props = {
				...
				onCell = function(list, node, k, v)
					...
					itertools.invoke({ childs.txtNote1, childs.txtAdd, childs.icon, childs.txtNote2 },
		...
		binds = {
			...
			props = {
				...
				onCell = function(list, node, k, v)
					...
					node:get("select"):visible(v.select == true):setZOrder(200)
					node:onTouch(functools.partial(list.itemClick, t, node, v))
					bind.touch(list, node:get("subIcon"),
						{ methods = { ended = functools.partial(list.subClick, t, v) } })
				end,
				asyncPreload = 24,
		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					node:get("select"):hide():setZOrder(200)
	...
	["advancePanel.clickTipPanel.txt1"] = {
		binds = {
			event = "effect",
			data = { outline = { color = ui.NEWCOLORS.OUTLINE.BROWN } },
		},
	},
...
function HeldItemAdvanceView:onCreate(dbId)
	...
	self.clientLevel = 0
	--监听选择的物品 和物品数量
	idlereasy.any({ self.selectItemIdx, self.selectItemNum, self.level },
		function(_, selectItemIdx, selectItemNum, level)
			...
			if itemData then
				if oldSelectItemIdx ~= selectItemIdx then
					...
					local notAdd = (selectNum >= itemData.targetNum) or level >= self.maxLevel or
						self.clientLevel >= self.maxLevel
	...
	--监听 突破 强化 模拟强化的数据
	idlereasy.any({ self.level, self.advance, self.sumExp, self.totalExp, self.showTab },
...
function HeldItemAdvanceView:setAdvancePanel(advance)
	...
	if not self.advanceMax then
		...
		childs.textTip:visible(self.levelNotEnough):text(string.format(gLanguageCsv.breakthroughInStrengtheningLv,
			advanceLvLimit))
	...
	itertools.invoke({ childs.nextNote, childs.nextList, childs.costNote, childs.itemList, childs.btnAdvance },
...
function HeldItemAdvanceView:setLeftPanel(level, advance, sumExp, totalExp)
	...
	text.addEffect(childs.itemName, { color = quality == 1 and ui.NEWCOLORS.WHITE or ui.COLORS.QUALITY[quality] })
	childs.itemIcon:texture(csvItems.icon)
	childs.itemIconBg:texture(string.format("city/card/helditem/strengthen/img_helddt%d.png", csvItems.quality))
...
function HeldItemAdvanceView:setStrengthenPanel(itemData)
	...
	if itemData then
		...
		text.addEffect(childs.itemName,
			{ color = itemData.cfg.quality == 1 and ui.NEWCOLORS.WHITE or ui.COLORS.QUALITY[itemData.cfg.quality] })
	end
...
function HeldItemAdvanceView:showGuideGiftData()
	local items = {2103, 2102}
...
function HeldItemAdvanceView:onLeftItemClick(list, k, v)
	if v.csvId <= game.HELD_ITEM_CSVID_LIMIT and v.csvId > game.FRAGMENT_CSVID_LIMIT then
		...
		gGameUI:stackUI("city.card.helditem.advance_select", nil, nil, self.dbId, v.csvId, v.targetNum,
			self:createHandler("setCostItem"))
	end
end
