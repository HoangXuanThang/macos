		...
		binds = {
			...
			props = {
				...
				onCell = function(list, node, k, v)
					node:get("imgSel"):visible(v.isSel):setZOrder(2)
...
function HeldItemBagView:onCreate(cardDbId, handlers)
	...
	idlereasy.any({ self.showLeftSelected, self.showRightSelected, self.refreshFlag },
	...
	self.selIdx:addListener(function(idx, oldval)
		...
		local hasRoleDress, isExc, cardId = HeldItemTools.isExclusive(info)

		self.exc:visible(isExc)
		if isExc then
			local txt = self.exc:get("txt")
			txt:text(csv.cards[cardId].name)
			self.exc:width(txt:width() + 60)
			txt:x(30)
		end
		-- 穿戴的卡牌形象
		if dressDbId then
		...
		if gGameModel.held_items:find(info.dbId[1]) then
			...
			for i = 1, 100 do
				...
				local resultStr = HeldItemTools.getStrinigByData(i, info, colorstr)
				table.insert(strTab, resultStr)
			end
			local targetStr = colorstr .. table.concat(strTab, '\n')
...
function HeldItemBagView:onStrengthenClick()
	gGameUI:stackUI("city.card.helditem.advance", { refreshData = self:createHandler("callBackData") }, { full = true },
