		...
		binds = {
			...
			props = {
				...
				onCell = function(list, node, k, v)
					node:get("imgSel"):visible(v.isSel):setZOrder(100)
...
function BagHandbookView:onCreate(cardDbId)
	...
	idlereasy.any({ self.showLeftSelected, self.showRightSelected, self.refreshFlag, self.quality },
		function(_, left, right, refreshFlag, quality)
			local t = {}
			local count = 0
	...
	self.selIdx:addListener(function(idx, oldval)
		...
		local _, isExc, cardId = HeldItemTools.isExclusive({ csvId = info.csvId })
