...
function GemDrawResultView:onCreate(tb, costType, drawTimes, costNum, ticketKey, againCb, gemUp)
	...
	if gemUp  then
		...
		widget.addAnimationByKey(self.bgPanel, "effect/xianshichouka.skel", "efc", "effect_huode_loop", -1)
				:alignCenter(self.bgPanel:size()):scale(2)
		widget.addAnimationByKey(self.bgPanel, "effect/gongxihuode.skel", "efc1", "effect", 11)
		:scale(1)
		:alignCenter(self.bgPanel:size())
		:y(1120)
		self:showItems(tb)
		self.yinjibg:visible(true)
		--local t = 0
		...
		performWithDelay(self, function()
			...
			--if gemUp then
			--	time2 = 0
			--	skel, skelname = 'effect/gongxihuode.skel', "efc"
			--	actionName = "effect_huode_loop"
			--
			--	widget.addAnimationByKey(self.bgPanel, "effect/xianshichouka.skel", "effect_bg", "effect_huode_loop", 1)
			--	:scale(2)
			--	:alignCenter(self.bgPanel:size())
			--end
			widget.addAnimationByKey(self.bgPanel, skel, skelname, actionName, -1)
				:alignCenter(self.bgPanel:size()):scale(2)
			performWithDelay(self, function()
				local effect = widget.addAnimationByKey(self.bgPanel, "effect/gongxihuode.skel", "efc1", "effect", 11)
					:scale(1)
					:alignCenter(self.bgPanel:size())
					:y(1120)
					self:showItems(tb)
	...
	idlereasy.any({self[costType], ticketIdler}, function()
		...
		if tickets >= self.drawTimes and not gemUp then
			...
			text.addEffect(self.textCost, {color = costEnough and ui.COLORS.NORMAL.WHITE or ui.COLORS.NORMAL.RED})
