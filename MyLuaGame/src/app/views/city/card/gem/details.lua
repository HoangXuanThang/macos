...
function GemDetailsView:showText(node, suitNum, quality, cfg)
	...
	txt1 = data.suitName ..
		string.format("%s(%s)%s", txt1, gLanguageCsv[ui.QUALITY_COLOR_TEXT[addQuality]], gLanguageCsv.symbolColon)
	local txt2
	for i = 1, math.huge do
		if data["attrType" .. i] and data["attrType" .. i] ~= 0 then
			...
			if txt2 then
				txt2 = txt2 ..
					'  ' .. name .. '+' .. dataEasy.getAttrValueString(data["attrType" .. i], data["attrNum" .. i])
			else
