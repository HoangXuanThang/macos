...
function RebirthTools.computeEquipReturnItem(dbId)
	...
	for _, equip in ipairs(equips) do
		...
		--signetAdvance
		calculateItems(csv.base_attribute.equip_signet_advance_cost, signetAdvanceList[cfg.advanceIndex].advanceSeqID,
			equip.signet_advance, items)
	end
...
function RebirthTools.computeDecomposeItems(dbIds)
	...
	for _, data in pairs(dbIds or {}) do
		...
		allItems[cardInfo.fragID] = (allItems[cardInfo.fragID] or 0) +
		(targetNum * starFragNum + baseFragNum) * gCommonConfigCsv.rebirthRetrunProportion2
