...
CardRebirthView.RESOURCE_BINDING = {
		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					local fountSize, selFountSize = 36, 38
					if k == 3 then
						fountSize = 36
						selFountSize = 38
					end
					...
					text.addEffect(clickBtn:get("textNote"),
						{ size = selFountSize, outline = { color = ui.NEWCOLORS.OUTLINE.WHITE, size = 2 } })
	...
	["left.panelHeld.imgBg"] = {
		binds = {
			...
			method = function(id)
				local path = "city/card/helditem/strengthen/img_helddt%d.png"
				local quality = 1
				if id then
	...
	["left.panelCard.addbg"] = "roleAddBtnBG",
	...
	["left.panelCard.btnChange.textNote"] = {
		binds = {
			event = "effect",
	...
	["left.panelDecompose.jumpShop.textNote1"] = {
		binds = {
			event = "effect",
			data = { outline = { color = ui.NEWCOLORS.OUTLINE.BROWN, size = 3 } },
		},
	},
	...
	["left.panelDecompose.btnAuto.textNote"] = {
		binds = {
			event = "effect",
			data = { outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4 } }
		},
	...
	["left.panelHeld.btnChange.textNote"] = {
		binds = {
			event = "effect",
			data = { outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4 } }
		},
	...
	self.rebirthData = {
		{
			...
			getTipInfo = function()
				...
				local tips = (isRebirth and self.dbId:read() ~= nil) and gLanguageCsv.cantRebirth or
					gLanguageCsv.notSelRole
		...
		},
		{
			...
			getTipInfo = function()
				...
				local tips = (isRebirth and heldId ~= nil) and gLanguageCsv.heldItemCanotRebirth or
					gLanguageCsv.notSelHeldItem
...
function CardRebirthView:initSelectTipPanel()
	...
	adapt.oneLineCenterPos(cc.p(decSelectTipPanelSize.width / 2, decSelectTipPanelSize.height / 2),
		{ self.decSelectTipBtn, self.decSelectTipPanel:get("textTip") }, cc.p(0, 0))

	local textTip = self.selectTipPanel:get("textTip")
	adapt.setTextAdaptWithSize(textTip,
		{
			str = gLanguageCsv.selectSpriteDecompositionTip,
			size = cc.size(800, 200),
			vertical = "center",
			horizontal =
			"left",
			margin = -8
...
function CardRebirthView:initRebirthCardUI(dbid)
	...
	uiEasy.setIconName("card", cardData.card_id,
		{ node = childs.textName, name = cardInfo.name, advance = cardData.advance, space = true })

	self.roleNode:removeAllChildren()

	local size = self.roleNode:size()

	if unitInfo.showOpen then
		self.photoSprite = widget.addAnimation(self.roleNode, unitInfo.showOpen, "default", 5, true)
		if string.find(unitInfo.showOpen, ".png") then
			self.photoSprite:xy(size.width / 2, size.height / 2):scale(0.7)
		else
			self.photoSprite:xy(size.width / 2, size.height / 2 + 50):scale(0.8)
		end
	end
	...
	local starDatas = dataEasy.getStarData(cardData.star)
	local displayStars = {}
	for i = 1, math.min(#starDatas, 6) do
		table.insert(displayStars, starDatas[i])
	end
	self.starData:update(displayStars)
...
function CardRebirthView:refreshUI(curPage)
	...
	self.roleAddBtnBG:visible(curPage == 1 and not self.dbId:read())
	...
	if (self.itemDatas:size() > 0) then
		text.addEffect(self.btnSure:get("textNote"),
			{ color = color, outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4 } })
	else
		text.deleteAllEffect(self.btnSure:get("textNote"))
...
function CardRebirthView:onChange()
	if self.pageIdx:read() == 3 then
		gGameUI:stackUI("city.card.rebirth.choose_helditem", nil, nil,
			{ curSel = self.heldItemId:read(), handlers = self:createHandler("onSetHeldItemDbId") })
	else
		gGameUI:stackUI("city.card.rebirth.choose_role", nil, nil,
			{ from = 1, curSel = { self.dbId:read() }, handlers = self:createHandler("onSetDbId") })
	end
end
...
function CardRebirthView:onAddCard()
	...
	gGameUI:stackUI("city.card.rebirth.choose_role", nil, nil,
		{ from = 2, curSel = curSel, handlers = self:createHandler("onSetDecomposeDbId") })
end
...
function CardRebirthView:onShowHeldItem()
	if self.pageIdx:read() == 3 then
		gGameUI:stackUI("city.card.rebirth.choose_helditem", nil, nil,
			{ handlers = self:createHandler("onSetHeldItemDbId") })
	end
end
				...
				:onResponse(function(tb)
					...
					if val.isEffect then
						if effect then
							...
							effect = widget.addAnimationByKey(val.parent, "effect/chongsheng.skel", "effect", "effect",
									100)
								:xy(val.pos)
								:scale(val.scale)
						end
