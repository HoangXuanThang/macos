...
ActivityFirstRecharge.RESOURCE_BINDING = {
	...
	["btnRight"] = {
		...
		binds = {
			event = "touch",
			methods = {ended = bindHelper.self("onIndexAdd")}
		},
	},
	["page1.mask"] = "mask1",
	["page2.mask"] = "mask2",
...
function ActivityFirstRecharge:onCreate()
	...
	self.page4:retain()
	self.page4:removeFromParent()
	self.page5:retain()
	self.page5:removeFromParent()
	...
	self.listPos = {
		...
		{x = self.page3:get("list"):x(), y = self.page3:get("list"):y()},
		{x = self.page4:get("list"):x(), y = self.page4:get("list"):y()},
		{x = self.page5:get("list"):x(), y = self.page5:get("list"):y()}
	}
	self.states = {
		...
		for i,v in ipairs(self.activityIDs) do
			...
			if v.state ~= 2 then
				...
				if not val[v.id] then
					...
					text.addEffect(label, {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4}})
	...
	self.pageview:onScroll(function(event)
		...
		local xPos = {}
		itertools.invoke({self.mask1, self.mask2, self.mask3, self.mask4}, "show")
		if event.name == "AUTOSCROLL_ENDED" then
			for i=1,totalPage do
				local baseX = -width * (i-1)
				if math.abs(x - baseX) <= 5 then
					self.isEnter = true
					itertools.invoke({self.mask1, self.mask2, self.mask3, self.mask4}, "hide")
	...
	self.curPage:addListener(function (val, oldval, _)	
		...
		if totalPage > 1 then
			print("111111111111111111:",oldval,val)
			local oldDot = self["dot"..oldval]
			local newDot = self["dot"..val]
			itertools.invoke({oldDot, oldval == val and oldDot or newDot}, "show")
...
function ActivityFirstRecharge:onIndex()
	...
	self.curPage:set(index)
end

function ActivityFirstRecharge:onIndexAdd()
	local index
	if itertools.size(self.pages) > 1 then
		index = self.pageview:getCurPageIndex() + 1
	else
