...
local ActivityDayFoundGift = class("ActivityDayFoundGift", cc.load("mvc").ViewBase)

ActivityDayFoundGift.RESOURCE_FILENAME = "activity_day_found.json"
ActivityDayFoundGift.RESOURCE_BINDING = {
	["item"] = "item",
	["sublist"] = "sublist",
	["panel.list"] = {
		binds = {
			...
			props = {
				...
				onCell = function(list, node, k, v)
					...
					if k == 3 or k == 6 then
						childs.bg:texture("activity/day_found/hhjj_1_floor3.png")
					end
					-- 天数
...
function ActivityDayFoundGift:onCreate(activityID)
	self.activityId = activityID

	gGameModel.currday_dispatch:getIdlerOrigin("redHintDaily")
	:modify(function(data)
		data[game.DAILY_REDHINT.dayFound1] = true
	end, true)
...
function ActivityDayFoundGift:initModel()
...
function ActivityDayFoundGift:sendGetAward()
	gGameApp:requestServer("/game/yy/award/get/onekey", function(tb)
		gGameUI:showGainDisplay(tb)
	end, self.activityId)
end

function ActivityDayFoundGift:sendBuyAward()
