...
local ActivityThronePassportView = class("ActivityThronePassportView", ViewBase)
...
ActivityThronePassportView.RESOURCE_FILENAME = "activity_throne_passport.json"
ActivityThronePassportView.RESOURCE_BINDING = {
	["bg"] = "bg",
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					if k == 1 then
						...
						childs.num:text(string.format(gLanguageCsv.throneNum, v.expNum))
					end
...
function ActivityThronePassportView:onCreate(activityId)
	...
	self.price:text(string.format(gLanguageCsv.symbolMoney, csv.recharges[rechargeCfg[self.csvId].rechargeID].rmbDisplay))
	...
	idlereasy.any({ self.yyhuodongs, self.clientBuyTimes }, function(_, yyhuodongs)
		...
		rich.createByStr(
			...
			string.format(gLanguageCsv.thronePro, self.currentPassportLv .. "/" .. #awardDatas), 44)
			:xy(0, 40)
			:anchorPoint(0, 0.5)
...
function ActivityThronePassportView:initModel()
	self.yyhuodongs = gGameModel.role:getIdler("yyhuodongs")
end

function ActivityThronePassportView:onBuyClick()
	gGameUI:stackUI("city.activity.zhuti_yingxiong.passport_buy", nil, nil, self.activityId,
		self:createHandler("onBtnBuyCb"))
end

function ActivityThronePassportView:onBtnBuyCb()
	self.clientBuyTimes:notify()
end

function ActivityThronePassportView:onBtnGetClick(_, k, v)
	gGameApp:requestServer("/game/yy/award/get", function(tb)
		gGameUI:showGainDisplay(tb)
	end, self.activityId, v.csvId)
end

function ActivityThronePassportView:onGetAllClick()
...
return ActivityThronePassportView
