...
local ActivityThroneShop = class("ActivityThroneShop", cc.load("mvc").ViewBase)

ActivityThroneShop.RESOURCE_FILENAME = "activity_throne_shop.json"
ActivityThroneShop.RESOURCE_BINDING = {
    ["bg"] = "bg",
    ["item"] = "item",
	["item.times"] = {
		binds = {
				...
				onCell = function(list, node, k, v)
					...
					if v.rmb then
						...
						if type(v.rmb) ~= "string" then
							...
							adapt.oneLineCenterPos(cc.p(200, 60), { rmbIcon, price }, cc.p(0, 0))
						end
					elseif v.price then
...
function ActivityThroneShop:initModel()
	self.yyhuodongs = gGameModel.role:getIdler("yyhuodongs")
	self.level = gGameModel.role:read("level")
end

function ActivityThroneShop:onBuyClick(list, k, v)
