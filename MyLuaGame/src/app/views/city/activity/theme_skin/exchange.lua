...
local ActivityThemeSkinExchange = class("ActivityThemeSkinExchange", cc.load("mvc").ViewBase)

ActivityThemeSkinExchange.RESOURCE_FILENAME = "activity_theme_skin_exchange.json"
ActivityThemeSkinExchange.RESOURCE_BINDING = {
	["bg"] = "bg",
	["item"] = "item",
	["sublist"] = "sublist",
	["list"] = {
		...
		binds = {
			...
			props = {
				...
				onCell = function(list, node, k, v)
					...
					childs.times:text(string.format("จำนวนแลก" .. " %s/%s", cfg.exchangeTimes - v.cnt,
						cfg.exchangeTimes))
...
function ActivityThemeSkinExchange:onCreate(activityId)
	gGameModel.currday_dispatch:getIdlerOrigin("themeSkinExchangeClick"):set(true)
	...
	Activity.setCountdown(self, self.activityId, nil, uiTime, {
		...
		tag = "exchange" .. self.activityId
	})
end

function ActivityThemeSkinExchange:initModel()
	self.yyhuodongs = gGameModel.role:getIdler("yyhuodongs")
end

function ActivityThemeSkinExchange:getBuyInfoCb(csvId, num)
	gGameApp:requestServer("/game/yy/award/get", function(tb)
		gGameUI:showGainDisplay(tb)
	end, self.activityId, csvId, num)
end

function ActivityThemeSkinExchange:onExchangeClick(list, k, v)
...
return ActivityThemeSkinExchange
