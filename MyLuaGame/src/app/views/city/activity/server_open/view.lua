		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, index, v)
					...
					text.addEffect(txt, {outline = {color = v.selected and ui.NEWCOLORS.OUTLINE.YELLOW or ui.NEWCOLORS.OUTLINE.BLUE}})
					node:get("lock"):visible(v.isLock == true)
					--panel:setOpacity(v.isLock == true and 255 or 255)
		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, index, v)
					...
					if v.selected then
						text.addEffect(childs.name, {color = ui.NEWCOLORS.BLACK})
					else
						text.addEffect(childs.name, {color = ui.NEWCOLORS.WHITE})
					end
...
function ServerOpenView:onCreate(activityId)
	...
	for i,v in pairs(activityTexture) do
		if self.clientType == i then
			...
			local btnContent1 = cc.Label:createWithTTF(gLanguageCsv.allTargetAward, "font/youmi.ttf", matchLanguage({"kr"}) and 30 or 26)
