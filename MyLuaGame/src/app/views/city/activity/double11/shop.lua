			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					if v.select then
						normal:hide()
						panel = selected:show()
						text.addEffect(selected:get("name"),
							{ color = ui.COLORS.NORMAL.WHITE, glow = { color = ui.COLORS.GLOW.WHITE } })
					else
						selected:hide()
						panel = normal:show()
						text.addEffect(normal:get("name"),
							{ color = cc.c4b(0xce, 0xce, 0xce, 255), glow = { color = ui.COLORS.GLOW.WHITE } })
					end
		...
		binds = {
			...
			props = {
				...
				onCell = function(list, node, k, v)
					...
					childs.title:text(cfg.name):hide()
					--图标
					if cfg.hot == "" then
					...
					if v.state == STATE_TYPE.canbuy then
						...
						text.addEffect(price, { outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 2 } })
					else
						childs.mask:show()
						childs.hot:visible(false)
...
function ActivityDouble11Shop:onBuyClick(list, k, v)
	...
	if not v.rmb then
		...
		if type(v.rmb) ~= "string" and self.rmb:read() < v.rmb then
