		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					local childs = node:multiget("title", "levelTitle", "boxImage", "listUp", "listDown", "price", "mask",
						"despLabel")
					local dataB = dataEasy.getItemData(v.cfg.item)
					local len = itertools.size(v.cfg.item)
					if len == 1 or len == 2 then
						uiEasy.createItemsToList(list, childs.listUp, dataB, {
						...
						uiEasy.createItemsToList(list, childs.listDown, t, {
							onAfterBuild = function()
					...
					if v.isRMB then
						...
						childs.price:get("rmb"):show():text(string.format(gLanguageCsv.symbolMoney,csv.recharges[v.cfg.rechargeID]["rmbDisplay_"..LOCAL_LANGUAGE] or csv.recharges[v.cfg.rechargeID].rmbDisplay))
					else
					...
					if v.cfg.limit > v.hasBoughtNum then
						...
						bind.touch(list, btn,
							{ clicksafe = false, methods = { ended = functools.partial(list.clickCell, k, v) } })
						childs.mask:hide()
					else
