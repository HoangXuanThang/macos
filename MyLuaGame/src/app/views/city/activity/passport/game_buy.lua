...
ActivityGamePassportBuyView.RESOURCE_BINDING = {
	...
	["text"] = {
		...
		binds = {
			event = "effect",
			data = {outline = {color = ui.NEWCOLORS.OUTLINE.BLUE1,  size = 6}}
		}
	},
	["text1"] = {
		varname = "text2",
...
function ActivityGamePassportBuyView:onCreate(activityId, cb)
	...
	self.btn:get("txt"):text(string.format(gLanguageCsv.symbolMoney,  csv.recharges[rechargeCfg[self.csvId].rechargeID]["rmbDisplay_"..LOCAL_LANGUAGE]  or  csv.recharges[rechargeCfg[self.csvId].rechargeID].rmbDisplay))
