...
local function setBtnState(btn, state)
	...
	if state then
		text.addEffect(btn:get("txtNode"), { outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4 } })
	else
...
ActivityPassportView.RESOURCE_BINDING = {
	...
	["rewardPanel.btnOneKeyGet.txtNode"] = {
		binds = {
			event = "effect",
			data = { outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4 } }
		}
	},
	["rewardPanel.btnBuy.txtNode"] = {
		binds = {
			event = "effect",
			data = { outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4 } }
		}
	},
	["rewardPanel.btnBuyExp.txtNode"] = {
		binds = {
			event = "effect",
			data = { outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4 } }
		}
	},
	["rewardItem.btnGet.txtNode"] = {
		binds = {
			event = "effect",
			data = { outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4 } }
		}
	},
	["taskItem.btnGet.txtNode"] = {
		binds = {
			event = "effect",
			data = { outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4 } }
		}
	},
	["taskItem.btnGo.txtNode"] = {
		binds = {
			event = "effect",
	...
	["taskPanel.list"] = {
		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					local childs = node:multiget("bg", "cornerIcon", "cornerIcon1", "txtCorner", "txtCorner1", "txtTitle",
						"txtContent", "txtExp", "txtProgress", "btnGet", "btnGo", "imgGot")
					if v.buyHigh or v.cfg.taskAttribute == 3 then
						...
						bind.touch(list, childs.btnGet,
							{ methods = { ended = functools.partial(list.clickCell, node, v) } })
		...
		binds = {
			...
			props = {
				...
				onCell = function(list, node, k, v)
					local childs = node:multiget("name", "icon", "num", "btnBuy", "maskPanel", "textLimiteNote",
						"textLimiteNum")
					...
					adapt.oneLineCenterPos(cc.p(180, 50), { childs.btnBuy:get("icon"), childs.btnBuy:get("txt") },
						cc.p(5, 0))
...
function ActivityPassportView:onCreate(activityId)
	...
	-- 顶部UI
	self.topView = gGameUI.topuiManager:createView(self.exchangeShop == 1 and "passport" or "default", self,
	...
	-- 通行证活动经验spine
	self.spineExp = widget.addAnimationByKey(self.taskPanel, "tongxingzheng/tongxingzheng.skel", 'spineExp', "effect2",
			999)
		:xy(self.taskIcon:x(), self.taskIcon:y())
		:scale(2)
	self.spineExp:setTimeScale(0)

	self.rewardPanelName:text(yyCfg.name)
	-- passport数据监听
	...
	idlereasy.any({ self.passport, self.clientBuyTimes }, function(_, passport)
		...
		local rewardExpNum = (passport.exp - nextExpTotal + self.awardCfg[passport.level].cfg.needExp) ..
			"/" .. self.awardCfg[passport.level].cfg.needExp
		...
		if isMaxLv then
			...
			uiEasy.createItemsToList(self, self.taskHighList, nexRewardData.cfg.eliteAward,
				{
					margin = 20,
					scale = 0.8,
		...
		self.rewardExpBar:setPercent((passport.exp - nextExpTotal + self.awardCfg[passport.level].cfg.needExp) /
			self.awardCfg[passport.level].cfg.needExp * 100)
		if not self.btnBuyExp:isVisible() then
		...
		self.taskIcon:texture(self.buyHigh and "activity/passport/icon_better_2.png" or
			"activity/passport/icon_regular_2.png")
		self.rewardIcon:texture(self.buyHigh and "activity/passport/icon_better_2.png" or
			"activity/passport/icon_regular_2.png")
		self.taskIconTitle3:texture(self.buyHigh and "activity/passport/task/" .. "txt_ds.png" or
			"activity/passport/task/" .. "txt_mx.png")
		self.taskImgLock:visible(not self.buyHigh)
		self.taskMask:visible(not self.buyHigh)
		self.taskLv:text(passport.level)
		self.taskExpBar:setPercent((passport.exp - nextExpTotal + self.awardCfg[passport.level].cfg.needExp) /
			self.awardCfg[passport.level].cfg.needExp * 100)
	...
	for i = 1, iconDatasMax do
		-- 10 是两个原点之间的间距
		local posX = self.iconPointPanel:width() / 2 - (iconDatasMax - 1) / 2 * (self.pointItem:width() + 16) +
			(i - 1) * (self.pointItem:width() + 16)
	...
	adapt.oneLineCenterPos(cc.p(450, 1000), { self.taskIconTitle1, self.iconTitlePanel, self.taskIconTitle3 },
		{ cc.p(2, -self.iconTitlePanel:size().height / 2), cc.p(2, self.iconTitlePanel:size().height / 2) })
end
...
function ActivityPassportView:updBuyTime(isMaxLv)
	...
	if self.buyHigh and (not isMaxLv) then
		...
		local buyExpShowDayFlag = yyCfg.clientParam.buyExpShowDay and yyCfg.clientParam.buyExpShowDay > 0 and
			dt <= yyCfg.clientParam.buyExpShowDay * 24 * 3600
...
function ActivityPassportView:updShowView()
	...
	if not self.buyHigh then
		local richText = rich.createWithWidth(gLanguageCsv.passwordActivateNote, 40, nil, 1250)
		...
		self.taskNote:hide()
		local richText = rich.createWithWidth(gLanguageCsv.passwordTaskNote, 40, nil, 1250)
	...
	gGameApp:requestServerCustom("/game/yy/passport/task/get_exp")
		...
		:onResponse(function()
			...
			widget.addAnimationByKey(node, "tongxingzheng/tongxingzheng.skel", 'lingqu', "effect1", 999)
				:xy(node:width() / 2 + 25, node:height() / 2)
				:scale(2.2)
				:play("effect1")
		...
		:onResponse(function()
			...
			for i = 1, self.playDatas do
				widget.addAnimationByKey(noteTable[i], "tongxingzheng/tongxingzheng.skel", 'lingqu', "effect1", 999)
					:xy(noteTable[i]:width() / 2 + 25, noteTable[i]:height() / 2)
					:scale(2.2)
					:play("effect1")
			end
			self.spineExp:setTimeScale(1)
	...
	local function onMove()
		...
		if item then
			...
			if dir == "left" and lx > x + itemWidth then                                                -- lx > x + itemWidth 表示最左侧item已滑出显示区域，需要移动至右侧队尾
				...
				local rightLess = (innerWidth - rightItem:x()) / itemWidth -
					1                                                                                   -- 右侧剩余可移动位置，最大奖励已加载后，左侧item不应再向右侧移动
			...
			elseif dir == "right" and rx < x then                                                    -- rx < x 表示最右侧item已向右滑出显示区域，需要移动至左侧队尾
				...
				local leftLess = leftItem:x() /
					itemWidth                                                                        -- 左侧剩余可移动i位置，最小奖励已加载后，右侧item不应再向左侧移动
...
function ActivityPassportView:refreshRewardItem(item, id)
	...
	local childs = item:multiget("lv", "normalPanel", "highPanel1", "highPanel2", "topMask", "bottomMask", "btnGet",
		"noClick")
	...
	bind.click(self, childs.noClick, {
		method = function()
...
function ActivityPassportView:onItemClick(list, t, v)
	...
	if self.buyHigh then
		...
		gGameUI:stackUI("common.buy_info", nil, nil, v.cfg.costMap, { id = v.itemId, num = v.num },
			{ maxNum = v.itemNum, discount = discount, contentType = "num" }, self:createHandler("buyItemCallBack", v))
		-- else
		...
		if coin == game.ITEM_TICKET.passportCoin then
			-- if passportCoin > price then
			gGameUI:stackUI("common.buy_info", nil, nil, v.cfg.costMap, { id = v.itemId, num = v.num },
				{ maxNum = v.itemNum, discount = discount, contentType = "num" },
				self:createHandler("buyItemCallBack", v))
			-- else
			-- 	gGameUI:showTip(gLanguageCsv.noPasswordCoin)
			-- end
		else
