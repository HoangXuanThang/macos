...
local function setIcon(list, v, totalSlotNum, icon, value)
	...
	elseif value.choose and value.choose > 0 then
		...
		if v.isCanBuy then
			bind.touch(list, icon:get("pic"),
				{ clicksafe = false, methods = { ended = functools.partial(list.clickCell, v, totalSlotNum, value.optionSlotNum, value.showAwards, value.choose) } })
		end
	else
		icon:get("add"):show()
		icon:get("select"):hide()
		icon:get("pic"):hide()
		bind.touch(list, icon:get("add"),
			{ clicksafe = false, methods = { ended = functools.partial(list.clickCell, v, totalSlotNum, value.optionSlotNum, value.showAwards, 0) } })
	end
end
		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					local childs = node:multiget("name", "icon1", "icon2", "icon3", "icon4", "icon5", "textContent",
						"btn")
					...
					childs.textContent:get("limitLabel"):text(string.format(gLanguageCsv.foreverLimit, v.hasBoughtNum,
						v.buyTimes))
					if v.hasAllChoose then
						text.addEffect(childs.textContent:get("btn"):get("title"),
					...
					if v.rmbDisplay == 0 then
						childs.textContent:get("btn"):get("title"):text(gLanguageCsv.freeToReceive)
					else
						childs.textContent:get("btn"):get("title"):text(string.format(gLanguageCsv.symbolMoney,v["rmbDisplay_"..LOCAL_LANGUAGE] or v.rmbDisplay))
					end
...
function ActivityCustomizeGiftDialog:initCsv(serverData)
	...
	for key, data in pairs(yycfg) do
		if data.huodongID == self.huodongID then
			...
			local optionAwards = { data.optionalAwards1, data.optionalAwards2, data.optionalAwards3, data
				.optionalAwards4 }
			...
			table.insert(itemsData, {
				...
				rmbDisplay = csv.recharges[data.rechargeID]["rmbDisplay_"..LOCAL_LANGUAGE]   or csv.recharges[data.rechargeID].rmbDisplay  ,
				rmb = data.rechargeID == 0 and 0 or csv.recharges[data.rechargeID].rmb,
...
function ActivityCustomizeGiftDialog:pushView(list, v, slotNums, selectNum, data, choose)
	gGameUI:stackUI("city.activity.customize_gift_select", nil, nil, self.activityId, v, slotNums, selectNum, data,
		choose)
end
...
function ActivityCustomizeGiftDialog:clickBuy(list, v)
	...
	if v.rmbDisplay == 0 then
		gGameApp:requestServer("/game/yy/award/get", function(tb)
			...
			gGameUI:showGainDisplay(awards, { raw = true })
		end, self.activityId, v.csvId)
		return
	end

	gGameApp:payDirect(self,
		{ rechargeId = v.rechargeID, yyID = self.activityId, csvID = v.csvId, buyTimes = v.hasBoughtNum, name = v.name },
		self.clientBuyTimes)
		:serverCb(function()
			gGameUI:showGainDisplay(awards, { raw = true })
		end)
