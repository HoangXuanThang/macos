...
function ActivityResourceRetrieve:initItem(list, node, k, itemData)
	...
	if itemData.canBuy == false then
		text.deleteAllEffect(btnBuy:get("price"))
		btnBuy:get("price"):setTextColor(ui.NEWCOLORS.REDLIGHT)
	else
		btnBuy:get("price"):setTextColor(ui.NEWCOLORS.WHITE)
		text.addEffect(btnBuy:get("price"), {color = color1,outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4}})
	end
