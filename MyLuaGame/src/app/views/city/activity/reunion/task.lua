...
ReunionTaskView.RESOURCE_BINDING = {
	...
	["topPanel.bg.reunion.title"] = {
		binds = {
			event = "effect",
			data = {
				outline = {color = cc.c4b(233, 251, 229, 255), size = 3},
			},
		},
	},
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					if v.selected then
						...
						text.addEffect(node:get("label"), {color = ui.COLORS.NORMAL.WHITE,outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4}})
					else
						text.addEffect(node:get("label"), {color = ui.COLORS.NORMAL.WHITE,outline = {color = ui.NEWCOLORS.OUTLINE.BLUE, size = 4}})
					end
	...
	["rightPanel.item.panel.btnGet.textNote"] = {
		binds = {
			event = "effect",
			data = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					if v.count < cfg.taskParam then
						text.addEffect(childs.textPro, {color =  ui.NEWCOLORS.RED1})
					else
						text.addEffect(childs.textPro, {color =  ui.NEWCOLORS.GREEN1})
					end
...
function ReunionTaskView:showBindInfo(bindInfo)
	...
	local x, y = childs.playerPanel:width()/2, childs.playerPanel:height()/2
