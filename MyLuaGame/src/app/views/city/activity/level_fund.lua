			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					if v.state == STATE_TYPE.canReceive then
						...
						text.addEffect(receiveLabel, {color = ui.COLORS.NORMAL.WHITE, outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4}})
...
function ActivityLevelFund:onCreate(activityId)
	...
	idlereasy.when(self.yyhuodongs, function(_, yyhuodongs)
		...
		if not yydata.buy then
			...
			text.addEffect(self.buyLabel, {color = ui.COLORS.NORMAL.WHITE, outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4}})
		else
