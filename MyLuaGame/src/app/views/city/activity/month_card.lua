...
function ActivityMonthCardView:onCreate()
	...
	for idx, activityData in ipairs(activityIds) do
		...
		local data = {
			...
			state = idler.new(STATE_TYPE.null),
			day = idler
				.new(0)
		}
		self.data[idx] = data
		...
		listView:setContentSize(list:getContentSize())
		listView:setInnerContainerSize(cc.size(340, listView:getContentSize().height))
		list:addChild(listView)

		-- Push từng item
		uiEasy.createItemsToList(self, listView, items, {
			scale = 0.9,
		})
