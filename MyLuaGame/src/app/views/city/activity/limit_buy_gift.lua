		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					--local color1 = v.select and ui.NEWCOLORS.BLACK or ui.NEWCOLORS.WHITE
					--text.addEffect(btn:get("name"), {color = color1})
					if v.select then
						text.addEffect(btn:get("name"), {color = ui.NEWCOLORS.ORANGE, outline = {color = ui.NEWCOLORS.OUTLINE.WHITE, size = 3}})
					else
						text.deleteAllEffect(btn:get("name"))
						text.addEffect(btn:get("name"), {color = ui.COLORS.NORMAL.WHITE})
					end
	...
	self.showTab:addListener(function(val, oldval)       -- 监听Tab页变换
		...
		for id,recharges in orderCsvPairs(rechargesCfg) do
			if id == self.currentInfo.cfg.rechargeID then
				self.price:text(string.format(gLanguageCsv.symbolMoney, recharges["rmbDisplay_"..LOCAL_LANGUAGE] or recharges.rmbDisplay))
				break
			end
		end
	end)
