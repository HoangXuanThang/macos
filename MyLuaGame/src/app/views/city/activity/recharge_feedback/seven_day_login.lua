...
ActivitySevenDayLoginDialog.RESOURCE_BINDING = {
	...
	["textCountDown"] = {
		...
		binds = {
			{
				event = "effect",
				data = {outline = {color = cc.c4b(255, 255, 225, 255), size = 4}},
			},
		}
	},
	["textCountDown1"] = {
		varname = "textCountDown1",
		binds = {
			{
				event = "effect",
				data = {outline = {color = cc.c4b(255, 255, 225, 255), size = 4}},
...
function ActivitySevenDayLoginDialog:initItem(list, node, k, itemData)
	local childs = node:multiget("textDay", "btnGet","list","imgBg","imgDayBg","textDi","textTian")
	-- 天数
	...
	if self.springFestival then
		...
		text.addEffect(childs.textDi, {outline = {color = ui.NEWCOLORS.OUTLINE.WHITE2, size = 4}})
		text.addEffect(childs.textTian, {outline = {color = ui.NEWCOLORS.OUTLINE.WHITE2, size = 4}})
