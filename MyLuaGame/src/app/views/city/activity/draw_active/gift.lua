...
local ActivityThemeSpGift = class("ActivityThemeSpGift", cc.load("mvc").ViewBase)

ActivityThemeSpGift.RESOURCE_FILENAME = "activity_draw_active_gift.json"
ActivityThemeSpGift.RESOURCE_BINDING = {
	["bg"] = "bg",
	["item"] = "item",
	["sublist"] = "sublist",
	["list"] = {
...
function ActivityThemeSpGift:onCreate(activityId, topType)
	...
	if topType == YY_TYPE.drawLimitHelditem then
		...
		:modify(function(data)
			data[game.DAILY_REDHINT.drawHeldItemGift] = true
		end, true)
		...
		gGameModel.currday_dispatch:getIdlerOrigin("redHintDaily")
		:modify(function(data)
			data[game.DAILY_REDHINT.drawExplorGift] = true
		end, true)
...
function ActivityThemeSpGift:initModel()
	self.yyhuodongs = gGameModel.role:getIdler("yyhuodongs")
	self.level = gGameModel.role:read("level")
end

function ActivityThemeSpGift:onBuyClick(list, k, v)
