                            ...
                            onNode = function(panel)
                                ...
                                if scoreEnough and boxGet then
                                    local panel1 = ccui.Layout:create()
                                        ...
                                        :setBackGroundColorOpacity(128)
                                        :addTo(panel, 10, "iconMask")
                                    local text1 = ccui.Text:create(gLanguageCsv.received, "font/youmi.ttf", 40)
...
function ActivityRmbGoldReward:resetTimeLabel()
    ...
    local returnDays = (cfg.paramMap and cfg.paramMap.returnDays) or 0
    local beginTime = tonumber(cfg.beginTime) or 0
    local endTimeCfg = tonumber(cfg.endTime) or 0

    local beginHour, beginMin = math.floor(beginTime / 100), beginTime % 100
    local endHour, endMin = math.floor(endTimeCfg / 100), endTimeCfg % 100

    -- open date
    local openDate = time.getDate(startTime)
    if beginHour > 0 or beginMin > 0 then
        openDate.hour = beginHour
        openDate.min = beginMin
        openDate.sec = 0
    else
        openDate.sec = 0
    end
    local openTime = string.format("%d.%02d.%02d %02d:%02d:%02d",
        openDate.year, openDate.month, openDate.day,
        openDate.hour, openDate.min, openDate.sec)

    -- end date
    local endTimestamp = math.floor(yyEndtime) - returnDays * 86400
    if endHour > 0 or endMin > 0 then
        local endDate = time.getDate(endTimestamp)
        endDate.hour = endHour
        endDate.min = endMin
        endDate.sec = 0
        endTimestamp = time.getTimestamp(endDate)
    end
    local endDate = time.getDate(endTimestamp)
    local endTimeStr = string.format("%d.%02d.%02d %02d:%02d:%02d",
        endDate.year, endDate.month, endDate.day,
        endDate.hour, endDate.min, endDate.sec)

    -- show open - end range
    self.textTimes:text(string.format("%s-%s", openTime, endTimeStr))

    -- countdown
    local countdown = endTimestamp - time.getTime()
    if countdown <= 0 then
