...
ActivityLimitSpriteView.RESOURCE_BINDING = {
	...
	["leftPanel.drawOnePanel.btn.textNote"] = {
		binds = {
			event = "effect",
			data = { outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW } },
		}
	},
		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, rank, v)
					...
					adapt.oneLinePos(children.logo, { children.lv, children.level, children.name },
						{ cc.p(26, 0), cc.p(5, 0), cc.p(38, 0) })
				end,
			},
		}
	},
		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					if state == 1 then
						local effect = widget.addAnimationByKey(node, "effect/jiedianjiangli.skel", effectName,
								"effect_loop", 3)
							:scale(0.5)
					...
					bind.touch(list, img,
						{ methods = { ended = functools.partial(list.clickCell, v.id, state, v.award, v.point) } })
				end,
			},
			handlers = {
...
function ActivityLimitSpriteView:initModel()
		...
		if hasTip then
			...
			if serverOpendDays > paramMap.QualifyServerOpenDays then
				...
				if cfg and (cfg.fightPointLimit > 0 or cfg.vipLimit > 0) then
					...
					self.richText = rich.createWithWidth(
						"   " ..
						string.format(gLanguageCsv.limitTip, day, string.format("%d:%d", hour, min), cfg.levelLimit,
							mathEasy.getShortNumber(cfg.fightPointLimit, 1)), 31, nil, 850)
	...
	idlereasy.any({ self.diamondFreeCount, self.rmb, self.activityId }, function(_, diamondFreeCount, rmb, activityId)
		...
		text.addEffect(self.costOne, { color = rmb < self.oneCost and ui.COLORS.NORMAL.RED or ui.NEWCOLORS.WHITE }) -- 不足则显示红色
		text.addEffect(self.costTen, { color = rmb < self.tenCost and ui.COLORS.NORMAL.RED or ui.NEWCOLORS.WHITE }) -- 不足则显示红色
	end)
	...
	local function setBtn(btn, isCur, list)
		...
		local color = isCur and ui.COLORS.NORMAL.WHITE or ui.COLORS.NORMAL.WHITE
		local outlinecolor = isCur and ui.NEWCOLORS.OUTLINE.BLUE or ui.NEWCOLORS.OUTLINE.YELLOW
		btn:setTouchEnabled(not isCur)
		btn:texture(path)
		text.addEffect(btn:get("text"), { color = color, outline = { color = outlinecolor, size = 3 } })
		list:visible(isCur)
	end
		...
		for developId, data in pairs(gCardsCsv[markId]) do
			...
			for branch, cfg in pairs(data) do
				...
				if cParam.unitId then
					table.insert(self.markTb,
						{ innateSkillID = csv.unit[cParam.unitId].skillList[3], unitID = cParam.unitId, id = cardId })
				end
			end
		end
	...
	idlereasy.when(self.curDevelopId, function(_, curId)
		...
		if unitCsv.showOpen then
			local size = children.sprite:getContentSize()
			self.photoSprite = widget.addAnimation(children.sprite, unitCsv.showOpen, "default", 5, true)
			if string.find(unitCsv.showOpen, ".png") then

			else
				self.photoSprite:xy(size.width / 2, size.height / 2 - 50):scale(0.45)
			end
		end

		adapt.oneLineCenterPos(cc.p(620, 980), { children.rarity, children.name, children.nature1, children.nature2 },
			cc.p(30, 0))

		local len = 1 --table.length(markTb)
	...
	idlereasy.when(self.sprState, function(_, state)
		...
		local playOver = function()
			...
			self.aniLayer:hide()
			self.spritePanel:getChildByName("sprite"):show()
		...
		if state == 1 then
			self.spritePanel:getChildByName("sprite"):hide()
...
function ActivityLimitSpriteView:playAni(playOver)
	...
	local cardSprite = widget.addAnimation(self.aniLayer, args.res, args.aniName, 100)
		:scale(args.scale * (args.scaleArgs.scale or 1) * 1.5)
		:xy(pos)
...
function ActivityLimitSpriteView:resetTimeLabel()
	local activityId = self.activityId:read()
	-- 没有 text 表示在页面内显示
	-- cn 活动结束时间是 周一5点； kr 活动结束时间点会出现 周日 22:30:00
	-- 抽卡结束时间活动开始的第三天的21:30
	local yyCfg = csv.yunying.yyhuodong[activityId]
	local getHuodongEndTime = time.getNumTimestamp(yyCfg.beginDate, 22, 30) + 6 * 24 * 60 * 60
