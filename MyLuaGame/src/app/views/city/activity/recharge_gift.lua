		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					text.addEffect(node:get("name"), { outline = { color = ui.NEWCOLORS.OUTLINE.BLACK } })
				end,
		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					childs.icon:texture("common/icon/icon_signin_box2" ..
						((v.state == STATE_TYPE.received) and "_open.png" or ".png"))
					local boxCanOpen = v.state == STATE_TYPE.canReceive
					if boxCanOpen then
						local effect = widget.addAnimation(node, "effect/jiedianjiangli.skel", "effect_loop",
							childs.icon:z() - 1)
...
function ActivityRechargeGift:onCreate(activityID)
	...
	local getMoney = function(money)
		for k,v in orderCsvPairs(csv.recharges) do
			if matchLanguage(v.languages) and v.type == 1 then
				if v.rmb == money then return   v["rmbDisplay_"..LOCAL_LANGUAGE] or v.rmbDisplay  end
			end
		end
		return 0
	end
	...
	idlereasy.when(self.yyhuodongs, function(_, yyhuodongs)
		...
		self.rechargeNum:set(getMoney(rechargenum))
	...
	idlereasy.any({ self.currDay, self.boxIndex }, function(_, day, index)
		...
		if proxy.state == STATE_TYPE.canReceive then
			...
			text.addEffect(self.recieveBtn:get("label"), { outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4 } })
		elseif proxy.state == STATE_TYPE.noReach then
			if index == day then
				self.rechargeBtn:show()
				text.addEffect(self.rechargeBtn:get("label"),
				...
				text.addEffect(self.recievedBtn:get("label"),
					{ outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4 } })
			end
		else
			self.recievedBtn:show()
			self.recievedBtn:get("label"):text(gLanguageCsv.received)
			text.addEffect(self.recievedBtn:get("label"), { outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4 } })
		end
	end)
	idlereasy.when(self.rechargeNum, function(_, val)
		...
		local richTxt = rich.createByStr(
				string.format(gLanguageCsv.activityRechargeGift, self.money - val >= 0 and self.money - val or 0), 46)
	...
	self.icon:texture(yyCfg.clientParam.hero)
	ActivityView.setCountdown(self, activityID, self.timeLabel, self.time, {
