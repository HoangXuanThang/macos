...
local checkMeetConditions = {
	...
	[4] = function(params, taskCardDatas1, taskCardDatas2)
		...
		return params[itertools.size(params)] <= count, count
	end,
	...
	[11] = function(params, taskCardDatas1, taskCardDatas2)
		...
		for k, v in ipairs(params) do
			check[k] = false
			while true do
				if checkFunc(taskCardDatas1:atproxy(1), v) then
					check[k] = true
					break
				end
				for i = 1, 3 do
					if checkFunc(taskCardDatas2:atproxy(i), v) then
						check[k] = true
					end
				end
				break
...
function ActiviytDispatchSpriteSelect:getConditionStr(conditionsIndex, params, index)
	...
	elseif conditionsIndex == 2 then --元素属性
		local icon = "#I" .. ui.ATTR_ICON_1[params[1]] .. "-50-50#"
	...
	elseif conditionsIndex == 3 then --markId
		...
		str = color .. string.format(str, name)
...
function ActiviytDispatchSpriteSelect:refreshRight()
		...
		if conditionsIndex ~= 0 then
			...
			if conditionsIndex == 11 then --11特殊处理
				...
				for k, v in ipairs(params) do
					...
					item:get("attrIcon"):texture(ui.ATTR_ICON_1[v])
	...
	text.addEffect(self.btnDispatch:get("title"),
	...
	if conditionsIndex ~= 0 then
		...
		if data.extraTime ~= 0 then
			text3:text(string.format(gLanguageCsv.dispatchAddition,
				tostring(mathEasy.getPreciseDecimal(data.extraTime / 60, 2, false))))
		else
			text3:hide()
		end
...
function ActiviytDispatchSpriteSelect:onCreate(activityId, id)
	...
	local actionPointKey = csv.yunying.yyhuodong[self.activityId].paramMap.item
	local str = string.format(gLanguageCsv.dispatchCost,
		tostring(mathEasy.getPreciseDecimal(taskData.duration / 60, 2, false)), taskData.cost[actionPointKey])
	...
	idlereasy.when(self.yyhuodongs, function(_, yyhuodongs)
		...
		self.textTimes:text(string.format(gLanguageCsv.dispatchTips,
			csv.yunying.dispatch[self.id].times - self.dispatchTimes))
	...
	idlereasy.when(self.dispatchStatus, function(_, status)
		...
		elseif status == 4 then
			...
			adapt.oneLineCenterPos(cc.p(500, 520),
				{ self.dispatchedPanel:get("textCountDown"), self.dispatchedPanel:get("textNote") }, cc.p(0, 0))
		...
		text.addEffect(self.btnStart:get("title"),
			{ outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4 } })
	end)
