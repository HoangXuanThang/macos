		...
		for taskId, v in pairs(dispatch) do
			...
			elseif v.status == 1 then
				...
				if time.getTime() > dispatch[taskId].end_time then
					...
					item:get("dispatchingPanel.imgIcon"):texture(unitCsv.iconSimple):scale(0.35)
					self:schedule(function(dt)
						if time.getTime() > dispatch[taskId].end_time then
							item:get("dispatchingPanel.imgIcon"):texture("common/icon/icon_box4.png"):scale(1.5)
							widget.addAnimationByKey(item, "effect/guanqiabaoxiang.skel", "rewardEffect", "effect_loop",
								2)
								:xy(100, 200)
								:scale(0.5)
							return false
...
function DispatchView:initMap()
	...
	for k, v in pairs(self.gameCfg) do
		...
		if v.data.type == 3 then
			item:get("posIcon"):scale(1.5):y(120)
		end
...
function DispatchView:clickGateItem(id)
	if csv.yunying.dispatch[id].type == 3 then
		...
		gGameUI:stackUI("city.activity.dispatch.sprite_select", nil, { clickClose = true, blackLayer = true },
			self.activityId, id)
	end
