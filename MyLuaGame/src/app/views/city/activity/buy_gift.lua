...
function ActivityBuyGiftView:onBuyClick(list, k, v)
	if not v.rmb then
		gGameApp:payDirect(self, {rechargeId = v.cfg.rechargeID, yyID = self.activityId, csvID = v.csvId, v.cfg['name_'..LOCAL_LANGUAGE] or v.cfg.name, buyTimes = v.buyTimes}, self.clientBuyTimes)
			:serverCb(function()
