		...
		if not selectId then
			...
			if heldItem then
				...
				if self.selectData then
					...
					text.addEffect(childs.num, {color = rmb >= self.selectData.rmb and ui.COLORS.NORMAL.WHITE or ui.COLORS.NORMAL.RED})
...
function ActivityQualityExchangeHelditemView:onBtnClick()
	...
	gGameUI:showDialog({
		...
		btnType = 2,
		isRich = true,
		content = "#C0x57472E#" .. gLanguageCsv.qualityExchangeHelditemExchangeTip,
	})
end
