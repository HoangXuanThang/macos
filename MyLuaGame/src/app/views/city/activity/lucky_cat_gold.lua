...
local ActivityLuckyCatGold = class("ActivityLuckyCatGold", Dialog)

local MAX_NUM = 5
local LEFT_SHOW_NUM = 5
ActivityLuckyCatGold.RESOURCE_FILENAME = "activity_lucky_cat_gold.json"
ActivityLuckyCatGold.RESOURCE_BINDING = {
	["btnClose"] = {
		binds = {
			event = "touch",
...
function ActivityLuckyCatGold:onCreate(activityId, tb)
	...
	idlereasy.when(self.leftDatas, function(_, data)
		...
		local function insertItem(v)
			...
			local richText = rich.createWithWidth(
					string.format(gLanguageCsv.congratulationGetGold, v[1], mathEasy.getShortNumber(v[2], 2)),
					label:getFontSize(), nil, node:size().width - 10)
				:anchorPoint(0, 1)
				:addTo(label)
	...
	ActivityView.setCountdown(self, self.activityId, self.timeLabel, self.time, {
...
function ActivityLuckyCatGold:rollCb()
...
function ActivityLuckyCatGold:originRoll()
...
function ActivityLuckyCatGold:getNumbers(number)
	local numbers = {}
...
function ActivityLuckyCatGold:setNumber(isRoll, isClear, index, numbers, time)
	if isClear then
...
function ActivityLuckyCatGold:addList(list, number, z)
...
function ActivityLuckyCatGold:numberRoll(index, number, time, tb)
...
function ActivityLuckyCatGold:onSure(time)
	...
	if self.totalTimes < #self.originData and self.remainTime == 0 then
		self.curState:set(true)
		local content = { gLanguageCsv.luckGoldMax, string.format(gLanguageCsv.commonVipIncrease,
			gLanguageCsv.luckGoldTip) }
