...
local RED_HINTS = {
	...
	[YY_TYPE.surpriseGift] = { -- 资源找回红点
		specialTag = "activitySurpriseGift",
	},
	[YY_TYPE.LoginGift] = { -- 资源找回红点
		specialTag = "activityDarwDayGift",
	},
}
...
ActivityView.RESOURCE_BINDING = {
			...
			props = {
				...
				onItem = function(list, node, id, v)
					...
					if RED_HINTS[yyType] then
						...
						if yyType == YY_TYPE.LoginGift then
							if cfg.paramMap.type == 1 then
								props = { -- 资源找回红点
									specialTag = "redHintDaily",
									listenData = {
										sign = game.DAILY_REDHINT.dayFound1,
									},
								}
							elseif cfg.paramMap.type == 2 then
								props = { -- 资源找回红点
									specialTag = "redHintDaily",
									listenData = {
										sign = game.DAILY_REDHINT.dayFound2,
									},
								}
							end
						end
		...
		binds = {
			{
				event = "effect",
				data = {
					outline = { color = cc.c4b(233, 251, 229, 255), size = 4 },
					shadow = { color = cc.c4b(0, 0, 0, 255), offset = cc.size(0, -6), size = 6 }
				},
			}
		}
	},
	["rightPanel.topPanel.title"] = {
		...
		binds = {
			{
				event = "effect",
				data = { outline = { color = cc.c4b(0, 0, 0, 255), size = 3 } },
			}
		}
	},
...
function ActivityView:onCreate(independentStyle, yyId)
	...
	self.viewDatas = {
		...
		[YY_TYPE.surpriseGift] = { viewName = "city.activity.surprise_gift", showRightInfo = false },
		[YY_TYPE.LoginGift] = { viewName = "city.activity.draw_day_gift", showRightInfo = false },
	}
	self.subViews = {}
		...
		if cfg then
			...
			if viewData then
				...
				if cfg.type == YY_TYPE.LoginGift then
					if cfg.paramMap.type == 1 then
						viewName = "city.activity.day_found_gift"
					elseif cfg.paramMap.type == 2 then
						viewName = "city.activity.day_found_gift2"
					end
				end

				if not self.subViews[val] then
					self.subViews[val] = gGameUI:createView(viewName, self):init(val,
			...
			if isShowTime then
				ActivityView.setCountdown(self, val, uiTimeLabel, uiTime, {
...
function ActivityView:estimateType(types)
	...
	if types == 14 then
		...
		if (number == 1) then
			text.addEffect(self.iconAll:get("txt"),
				{ color = color1, outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4 } })
		else
			text.deleteAllEffect(self.iconAll:get("txt"))
		end
	else
