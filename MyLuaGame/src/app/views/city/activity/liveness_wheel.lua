...
function ActivityLivenessWheelView:initReward()
    ...
    for i = 1, MAX_REWARD_NUMS do
        ...
        if reward then
            ...
            childs.name:hide()
            local label = beauty.singleTextLimitWord(childs.name:text(), { fontSize = childs.name:getFontSize() },
                    { width = 240 })
                :xy(childs.name:xy())
...
function ActivityLivenessWheelView:initData()
    ...
    adapt.oneLineCenterPos(cc.p(self.taskPanel:width() / 2, self.todayGetTitle:y()),
        { self.todayGetTitle, self.todayGetTimes, self.todayGetTotal })
end

function ActivityLivenessWheelView:initSkipAni()
    idlereasy.when(self.skipAni, function(_, skipAni)
        local res = skipAni and "activity/liveness_wheel/radio_selected.png" or
            "activity/liveness_wheel/radio_normal.png"
...
function ActivityLivenessWheelView:getAniParams(drawType, gainData)
    ...
    local aniTime = ANI_PARAMS.dt * (ANI_PARAMS.stage2Nums + stopIndex - (ANI_PARAMS.stage1Nums + ANI_PARAMS.stage3Nums)) +
        ANI_PARAMS.stage1Time * ANI_PARAMS.stage1Nums + ANI_PARAMS.stage3Time * ANI_PARAMS.stage3Nums +
        ANI_PARAMS.showTime
...
function ActivityLivenessWheelView:initSkel()
    ...
    self.wheelSkel:anchorPoint(cc.p(0.5, 0.5))
        :xy(display.sizeInView.width / 2, display.sizeInView.height / 2 + 15)
        :scale(2)
    -- 选中特效
    self.selectedSkel = widget.addAnimationByKey(self.wheelPanel, "huoyueduobao/huoyueduobao.skel", 'selectedSkel',
        "effect_kuang_loop", 10)
...
function ActivityLivenessWheelView:setSelectedReward(index)
    ...
    self.wheelSelected:addTo(self.wheelPanel:get("reward" .. index), 1):xy(self.selectedItem:width() / 2,
        self.selectedItem:height() / 2)
