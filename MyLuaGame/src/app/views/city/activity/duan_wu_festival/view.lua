...
function DuanWuView:onCreate(activityID)
	...
	local textRed = ui.NEWCOLORS.REDLIGHT
	idlereasy.when(gGameModel.role:getIdler("items"), function(_, items)
		for i = 1, 3 do
			local str1, str2 = string.format(gLanguageCsv.possessIngredient, 0),
				string.format(gLanguageCsv.possessIngredient, 0)
			local color1, color2 = ui.COLORS.NORMAL.WHITE, ui.COLORS.NORMAL.WHITE
			...
			for i = 1, 3 do
				self["item" .. i]:get("item"):runAction(cc.Sequence:create(
					cc.MoveTo:create(0.5,
						cc.p(self["item" .. i]:get("item"):x(), self["item" .. i]:get("item"):y() + 750)),
				...
				self["icon" .. i]:get("icon"):runAction(cc.Sequence:create(
					cc.MoveTo:create(0.5,
						cc.p(self["icon" .. i]:get("icon"):x(), self["icon" .. i]:get("icon"):y() + 750)),
...
function DuanWuView:speedUseClick()
	...
	self.hintTxt:text(gLanguageCsv.clickAddZongzi)
	self.zongziData = {}
	gGameUI:stackUI("city.activity.duan_wu_festival.speed_fabrication", nil, nil, self.activityID,
		self:createHandler("btnClick"))
end
