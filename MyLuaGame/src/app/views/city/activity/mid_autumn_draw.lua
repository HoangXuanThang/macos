...
function MidAutumnDraw:onCreate(activityId)
    ...
    local unitCsv  =  csv.unit[1396]
    self.player = widget.addAnimationByKey(self.heroSpine, unitCsv.showOpen, "hero", "default", 10, true)
        :scale(1)
        :xy(100, unitCsv.showOpenOffsetY + 300)
    --self:runAnimation()
    self:showEndTime()
...
function MidAutumnDraw:initTickets(num)
    local POSITION= {
        CENTER = 900,  -- 中心点， 偶数个时535
        Y = 275,  -- 每个牌子大小
        SIZE = 1125,  -- 每个所有牌子大小
    }
    num = num or 5
    local POS_Y = { 260, 180, 120, 60, 30}
    local marge =  300--POSITION.SIZE/num
