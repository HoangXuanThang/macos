...
local ActivityJiuWeiGift = class("ActivityJiuWeiGift", cc.load("mvc").ViewBase)

ActivityJiuWeiGift.RESOURCE_FILENAME = "activity_jiuwei_gift.json"
ActivityJiuWeiGift.RESOURCE_BINDING = {
	["bg"] = "bg",
	["item"] = "item",
		...
		binds = {
			...
			props = {
				...
				onCell = function(list, node, k, v)
					...
					-- childs.bg:texture(string.format("activity/theme_skin/%s/choupifu_shandianfloor1.png", list.curTheme))
					...
					-- price:setTextColor(ui.NEWCOLORS.THEME_SKIN[list.themeColor[1]])
...
function ActivityJiuWeiGift:onCreate(activityId)
	...
	self.themeColor = yyCfg.clientParam.color or { 1 }
end

function ActivityJiuWeiGift:initModel()
	self.yyhuodongs = gGameModel.role:getIdler("yyhuodongs")
	self.level = gGameModel.role:read("level")
end

function ActivityJiuWeiGift:onBuyClick(list, k, v)
