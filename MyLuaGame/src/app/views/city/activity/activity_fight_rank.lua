...
local OUTLINE_COLORS = {
	[1] = cc.c4b(166, 68, 3, 255),
	[2] = cc.c4b(86, 86, 145, 255),
	[3] = cc.c4b(111, 59, 27, 255),
	[4] = cc.c4b(13, 73, 68, 255),
}
...
ActivityFightRankView.RESOURCE_BINDING = {
		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, id, v)
					...
					if v.selected then
						text.addEffect(node:get("name"), {color = ui.NEWCOLORS.OUTLINE.BLACK})
					else
						text.deleteAllEffect(node:get("name"))
						text.addEffect(node:get("name"), {color = ui.NEWCOLORS.OUTLINE.WHITE})
					end
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					text.addEffect(children.fightPoint, {
						outline = {color =  ui.NEWCOLORS.OUTLINE.WHITE1},
					})
					if k < 4 then
	...
	["centerPanel.mainPanel.rankPanel.item.name"] = {
		binds = {
			event = "effect",
			data = {outline = {color = ui.NEWCOLORS.OUTLINE.WHITE1, size = 3}}
		},
	},
	["centerPanel.mainPanel.rankPanel.item.lv"] = {
		binds = {
			event = "effect",
			data = {outline = {color = ui.NEWCOLORS.OUTLINE.WHITE1, size = 3}}
		},
	},
	["centerPanel.mainPanel.rankPanel.item.level"] = {
		binds = {
			event = "effect",
			data = {outline = {color = ui.NEWCOLORS.OUTLINE.WHITE1, size = 3}}
		},
	},
	["centerPanel.mainPanel.rankPanel.item.fightPoint"] = {
		binds = {
			event = "effect",
			data = {outline = {color = ui.NEWCOLORS.OUTLINE.WHITE1, size = 3}}
		},
	},
	["centerPanel.mainPanel.rankPanel.item.text"] = {
		binds = {
			event = "effect",
			data = {outline = {color = ui.NEWCOLORS.OUTLINE.WHITE1, size = 3}}
		},
	...
	["centerPanel.spritePanel.list"] = {
		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					local children = node:multiget("icon", "name", "type", "note", "targetType","richList","bg","iconbg")
...
function ActivityFightRankView:onCreate(activityId, data)
	...
	text.addEffect(textLabel, {
		outline = {color =  ui.NEWCOLORS.OUTLINE.YELLOW},
	})

	local str = gLanguageCsv.fightRankBottomRichText
	local richtext = rich.createByStr(string.format(str, "#C0xFFFFFF#","#C0x0f9932#","#C0xFFFFFF#","#C0xff794c#","#C0xFFFFFF#"), 40)
