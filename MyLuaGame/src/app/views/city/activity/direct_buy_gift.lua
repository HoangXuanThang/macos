...
ActivityDirectBuyGiftView.RESOURCE_BINDING = {
	...
	["rightPanel.item.selected.name"] = {
		binds = {
			event = "effect",
			data = { outline = { color = ui.NEWCOLORS.OUTLINE.WHITE, size = 4 } }
		}
	},
	["rightPanel.list"] = {
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					panel:get("name"):text(v['desc_'..LOCAL_LANGUAGE] or v.desc)
					if matchLanguage({ "kr" }) then
						adapt.setTextScaleWithWidth(panel:get("name"), nil, 250)
					elseif matchLanguage({ "en" }) then
						adapt.setTextAdaptWithSize(panel:get("name"),
							{ size = cc.size(255, 70), vertical = "center", horizontal = "center", margin = -5, maxLine = 2 })
					end
		...
		binds = {
			...
			props = {
				...
				onCell = function(list, node, k, v)
					...
					childs.title:text(cfg["name_"..LOCAL_LANGUAGE] or cfg.name)
					childs.title:x(node:width() / 2)
					childs.title:y(625)
					adapt.setTextAdaptWithSize(childs.title,
						{ size = cc.size(330, 70), vertical = "top", horizontal = "center", margin = -8, maxLine = 2 })
					...
					if matchLanguage({ "en","th","ph","ma","vn","in" }) then
						price:anchorPoint(0.5, 0.5)
						panel:x(btn:size().width / 2)
						childs.mask:get("img"):size(childs.mask:get("label"):size().width + 45,
							childs.mask:get("img"):size().height)
					end
					local props = {
						class = "red_hint",
						props = {
...
function ActivityDirectBuyGiftView:onCreate()
	...
	for _, id in ipairs(self.yyOpen:read()) do
		...
		if not (cfg.clientParam.theme == "zhuangbei" or cfg.clientParam.theme == "shengxing" or cfg.clientParam.theme == "shenqi" or cfg.clientParam.theme == "zhibao" or cfg.clientParam.theme == "zhoumo" or cfg.clientParam.theme == "kfzx") then
			if cfg.type == game.YYHUODONG_TYPE_ENUM_TABLE.directBuyGift and (cfg.independent == 1 or cfg.independent == 2) and not cfg.clientParam.double11 then
				table.insert(dataTab, { desc = cfg['desc_'..LOCAL_LANGUAGE] or cfg.desc, id = id, sortWeight = cfg.sortWeight })
			end
		end
	end
		...
		gGameApp:payDirect(self,
			{
				...
				yyID = activityId,
				csvID = v.csvId,
				name = v.cfg["name_"..LOCAL_LANGUAGE] or v.cfg.name,
				buyTimes = v
					.buyTimes
			}, self.clientBuyTimes)
			:serverCb(function()
