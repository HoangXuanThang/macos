...
local DRAETYPE_DRAWFUNC = {
	...
	theme_skin = function(cb, times, isFree, yyId, errCb)
		local stingTb = {
			[1] = "limit_card_rmb1",
			[10] = "limit_card_rmb10",
		}
		local str = stingTb[times]
		local url = "/game/lottery/yy/draw"
		requestServer(url, cb, errCb, yyId, str)
	end,
	...
	christmas_item = function(cb, times, isFree, yyId, errCb)
		local stingTb = {
			[1] = "christmas_wish_rmb1",
			[10] = "christmas_wish_rmb10",
		}
		local str = stingTb[times]
		local url = "/game/lottery/yy/draw"
		requestServer(url, cb, errCb, yyId, str)
	end,
	zhuti_yingxiong = function(cb, times, isFree, yyId, errCb)
		local stingTb = {
			[1] = "limit_card_rmb1",
			[10] = "limit_card_rmb10",
		}
	...
	jiu_wei = function(cb, times, isFree, yyId, errCb)
		local stingTb = {
			[1] = "limit_explorer_rmb1",
			[10] = "limit_explorer_rmb10",
		}
		local str = stingTb[times]
		local url = "/game/lottery/yy/draw"
		requestServer(url, cb, errCb, yyId, str)
	end,
	active_helditem = function(cb, times, isFree, yyId, errCb)
		local stingTb = {
			[1] = "active_helditem_rmb1",
			[10] = "active_helditem_rmb10",
			free = "active_helditem_free1",
		}
		local str = stingTb[isFree and "free" or times]
		local url = "/game/lottery/yy/draw"
		requestServer(url, cb, errCb, yyId, str)
	end,
	active_explor = function(cb, times, isFree, yyId, errCb)
		local stingTb = {
			[1] = "active_explorer_rmb1",
			[10] = "active_explorer_rmb10",
			free = "active_explorer_free1",
		}
		local str = stingTb[isFree and "free" or times]
		local url = "/game/lottery/yy/draw"
		requestServer(url, cb, errCb, yyId, str)
...
local DRAWCOST_FUNC = {
	...
	theme_skin = function(self)
		return csv.yunying.yyhuodong[self.yyId].paramMap["SKIN" .. self.times],
			dataEasy.getNumByKey(game.ITEM_TICKET.lotterySkinItem),
			dataEasy.getIconResByKey(game.ITEM_TICKET.lotterySkinItem)
	end,
	...
	christmas_item = function(self)
		return csv.yunying.yyhuodong[self.yyId].paramMap["DRAW" .. self.times],
			dataEasy.getNumByKey(game.ITEM_TICKET.christmasItem),
			dataEasy.getIconResByKey(game.ITEM_TICKET.christmasItem)
	end,
	zhuti_yingxiong = function(self)
		return csv.yunying.yyhuodong[self.yyId].paramMap["draw" .. self.times],
			dataEasy.getNumByKey(game.ITEM_TICKET.lotteryZhuTiYingXiongItem),
			dataEasy.getIconResByKey(game.ITEM_TICKET.lotteryZhuTiYingXiongItem)
	end,
	jiu_wei = function(self)
		return csv.yunying.yyhuodong[self.yyId].paramMap["draw" .. self.times],
			dataEasy.getNumByKey(game.ITEM_TICKET.lotteryJiuWeiItem),
			dataEasy.getIconResByKey(game.ITEM_TICKET.lotteryJiuWeiItem)
	end,
	active_helditem = function(self)
		-- local times = self.times
		-- local rmbCard = dataEasy.getNumByKey(game.ITEM_TICKET.drawLimitHelditemItem)
		-- if rmbCard >= times then
		-- 	return times, rmbCard, dataEasy.getIconResByKey(game.ITEM_TICKET.drawLimitHelditemItem)
		-- end
		-- local paramMap = csv.yunying.yyhuodong[self.yyId].paramMap
		-- local cost = { [1] = paramMap.rmb1, [10] = paramMap.rmb10 }
		-- local val = cost[times]
		-- return val, self.rmb:read()
		return csv.yunying.yyhuodong[self.yyId].paramMap["draw" .. self.times],
			dataEasy.getNumByKey(game.ITEM_TICKET.drawLimitHelditemItem),
			dataEasy.getIconResByKey(game.ITEM_TICKET.drawLimitHelditemItem)
	end,
	active_explor = function(self)
		-- local times = self.times
		-- local rmbCard = dataEasy.getNumByKey(game.ITEM_TICKET.drawLimitExpItem)
		-- if rmbCard >= times then
		-- 	return times, rmbCard, dataEasy.getIconResByKey(game.ITEM_TICKET.drawLimitExpItem)
		-- end
		-- local paramMap = csv.yunying.yyhuodong[self.yyId].paramMap
		-- local cost = { [1] = paramMap.rmb1, [10] = paramMap.rmb10 }
		-- local val = cost[times]
		-- return val, self.rmb:read()
		return csv.yunying.yyhuodong[self.yyId].paramMap["draw" .. self.times],
			dataEasy.getNumByKey(game.ITEM_TICKET.drawLimitExpItem),
			dataEasy.getIconResByKey(game.ITEM_TICKET.drawLimitExpItem)
	end,
}
...
function DrawCardResultView:onCreate(params)
	...
	self:playEffect(false, function()
		...
		local extra = self.params.extra
		if itertools.isempty(items) then
			printWarn("Kết quả rút thẻ trống, hãy kiểm tra dữ liệu cấu hình")
			return
		end
		self.hasMore = hasMoreCard(items)
		self.listview:visible(true)
		self:onInitList(items[1], extra)
	...
	idlereasy.any(
		{ self.diamondCount, self.goldCount, self.allCount, self.halfDiamondCount, self.trainerGoldCount, self
			.equipCount, self.drawEquipCount, self.lastDrawTime, self.items },
		function(_, diamondCount, goldCount, allCount, half, trainerCount, equipCount, drawEquipCount, lastDrawTime)
			...
			elseif self.drawType == "theme_skin" then
				--print("params",dump(params))
				-- local str = string.format("#L10#Get an additional %d pieces of Dragon Soul Crystals #I%s-50-50#", params.extra, "config/item/icon_lhsj.png")
			...
			local childs = self.privilegePanel:multiget("line", "textNote", "imgIcon", "textCost", "textDiscount")
			adapt.oneLineCenterPos(cc.p(168, 35), { childs.textNote, childs.textCost, childs.textDiscount, childs
				.imgIcon }, cc.p(6, 0))
...
function DrawCardResultView:playDownEffect(isAgain)
	...
	if not isAgain then
		...
		elseif self.drawType == "equip" then
			...
			widget.addAnimationByKey(self.effectDown, "effect/gongxihuode.skel", 'efc1', "effect", 11)
				:anchorPoint(cc.p(0.5, 0.5))
				:xy(size.width / 2, size.height / 4 * 3 + 50)
				:addPlay("effect_loop")
		...
		elseif self.drawType == "lucky_egg" then
			local size = self.effectDown:size()
			widget.addAnimationByKey(self.effectDown, "effect/gongxihuode.skel", 'efc1', "effect", 11)
				:anchorPoint(cc.p(0.5, 0.5))
				:xy(size.width / 2, size.height / 4 * 3 + 50)
...
function DrawCardResultView:playEffect(isAgain, cb, tb)
	...
	elseif not isAgain then
		local actionName = self.params.times > 1 and "effect_shilianchou" or "effect_danchou"
		time = 128
		--local efc1 = widget.addAnimationByKey(self.effect, "effect/chouka.skel", "efc", actionName, 1)
		--	:alignCenter(self.effect:size())
		--	:scale(2)
		local size = self.effect:size()
		local sprite = widget.addAnimationByKey(self.effect, "effect/CJhamawu.skel", "effect", "call", 10)
			:scale(2.65)
			:alignCenter(self.effect:size())
			:xy(size.width / 2, 250)
	end
...
function DrawCardResultView:onClickBtn()
	...
	if self.drawType == "theme_skin" and self.isStartItem then
		self.initItemTime = 0
	end
end
...
function DrawCardResultView:refreshCostTextColor()
	...
	local color = isEnough and ui.COLORS.NORMAL.WHITE or ui.COLORS.NORMAL.RED
...
function DrawCardResultView:onDrawAgain(list)
	...
	if not self:isEnough2Draw() then
		if itertools.include({ "christmas_item", "jiu_wei", "theme_skin" }, self.drawType) then
			gGameUI:showTip(gLanguageCsv.inadequateProps)
		elseif self.drawType == "limit" then
			uiEasy.showDialog("limit")
		else
...
function DrawCardResultView:drawCard()
	local callback = function(tb)
		self:playEffect(true, function()
			...
			self.listview:visible(true)
			if itertools.include({ "jiu_wei", "active_helditem", "active_explor" }, self.drawType) then
				self:onInitList(items[1], extra)
			else
...
function DrawCardResultView:onInitList(items, extra)
		...
		if not self.isShowHero and i >= self.initItemTime then
			...
			if info.new ~= nil then
				...
				if not isJumpSpriteView --[[or rarity >= gCommonConfigCsv.showCardRarityMin or (info.new and not self.hasSprite[cardId])]] then
					...
					gGameUI:stackUI("common.gain_sprite", nil, { full = true }, info, self.hasSprite, showBtn,
						self:createHandler("changeState"))
				end
			end
			...
			text.addEffect(label, { color = ui.COLORS.NORMAL.WHITE })
			...
			if len == 1 then
				...
				self.listview:xy(self.originListPos.x + 10 + margin * 2 + itemSize.width * 2,
					self.originListPos.y - innerSize.height * 0.5)
			end
			...
			self:playAction(listItem, cc.p(x, y), function()
				...
				if count >= len then
					...
					if extra then
						local key, num = next(extra)
						self.equipOtherShow:set(string.format(gLanguageCsv.getOtherItemNum, num))
						self.equipImgIcon:texture(dataEasy.getIconResByKey(key))
						adapt.oneLineCenterPos(cc.p(400, 25), { self.equiTextNote, self.equipImgIcon })
						self.equipPanel:show()
					end
				end
