...
DrawCardView.RESOURCE_BINDING = {
		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					if v.isSel then
						imgBtn:x(225) --偏移10个像素 图片有透明处13个像素
					else
						imgBtn:x(225)
					end
					if v.redHint then
						list.state = v.isSel ~= true
						v.redHint.props.onNode = function(node)
							node:xy(400, 150)
	...
	["diamondPanel"] = {
		...
		binds = {
			...
			idler = bindHelper.self("curType"),
			method = function(val)
				return val == TYPE_DEFINE.diamond
			end,
		},
	},
...
function DrawCardView:onCreate(tabName)
	...
	local equipData = {
		key = "equip",
		txt = gLanguageCsv.drawEquip,
		type = TYPE_DEFINE.equip,
		isSel = false,
	...
	local limitData = {
		key = "limit",
		txt = gLanguageCsv.drawLimit,
		type = TYPE_DEFINE.limit,
		isSel = false,
	...
	local diamondUpData = {
		key = "diamondup",
		txt = gLanguageCsv.diamondUpDrawCard,
		type = TYPE_DEFINE.diamond_up,
		isSel = false,
	...
	idlereasy.any({ self.vip, self.dcGoldCount }, function(_, vip, dcGoldCount)
		...
		local color = ui.NEWCOLORS.GREEN2
		if leftNum <= 0 then
	...
	idlereasy.when(self.freeTimes, function(_, freeTimes)
		self.txtFree:text(gLanguageCsv.free .. freeTimes)
	end)
...
-- 初始化页面相关的显示内容
function DrawCardView:initPageItem(_, curType, goldCount, diamondCount, allCount, half, trainerCount, equipCount,
								   drawEquipCount)
	...
	performWithDelay(self, function()
		adapt.oneLineCenterPos(cc.p(88, 38), { self.imgOne, self.costOne }, cc.p(6, 0))
		adapt.oneLineCenterPos(cc.p(160, 42), { self.imgTen, self.costTen }, cc.p(6, 0))
		adapt.oneLinePos(self.imgOne, self.costOne, cc.p(6, 0), "left")
		adapt.oneLinePos(self.imgTen, self.costTen, cc.p(6, 0), "left")
		local childs = self.privilegePanel:multiget("line", "textNote", "imgIcon", "textCost", "textDiscount")
		adapt.oneLineCenterPos(cc.p(125, 30), { childs.imgIcon, childs.textCost, childs.textDiscount, },
			cc.p(6, 0))
...
function DrawCardView:initCardData(data, target, idx, ids)
	...
	widget.addAnimation(target:get("imgIcon"), iconPath, "default", 5, true)
	...
	adapt.oneLinePos(target:get("name.imgIcon"), target:get("name.imgName"), cc.p(6, 0), "left")
	table.insert(ids, data["card" .. idx])
	target:get("info.btnJump"):visible(false)
	target:get("name.imgInfoBg"):setTouchEnabled(true)
	bind.touch(self, target:get("name.imgInfoBg"), {
		methods = {
			ended = function()
...
function DrawCardView:onSwitchClick()
	gGameUI:stackUI("city.drawcard.select_property", nil, { blackLayer = true, clickClose = true },
		self.selfChooseNum:read() or 1)
end
...
function DrawCardView:resetTopUIView(curType)
	...
	self.topView = gGameUI.topuiManager:createView(curType and tb[curType] or "default", self,
...
-- 刷新页面信息显示
function DrawCardView:initPageItemFunc(curType, goldCount, diamondCount, allCount, half, trainerCount, equipCount,
									   drawEquipCount)
end
