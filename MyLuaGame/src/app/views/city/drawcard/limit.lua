...
function t.initPageItemFunc(self, curType, goldCount, diamondCount, allCount, half, trainerCount, equipCount,
	...
	widget.addAnimation(self.limitPanel, "effect/xiandingchouka.skel", "default", 1, true):alignCenter(self.limitPanel
		:size()):scale(2)
	...
	adapt.oneLineCenterPos(cc.p(titlePanel:get("imgBg"):xy()),
		{ titlePanel:get("textNote1"), titlePanel:get("textNote2") })
...
function t:initCountDown(limitPanel)
	...
	bind.extend(self, timePanel:get("textTime"), {
		...
		props = {
			...
			endTime = endTime + 1,
			callFunc = function()
				adapt.oneLineCenterPos(cc.p(timePanel:get("imgBg"):xy()),
					{ timePanel:get("textNote"), timePanel:get("textTime") })
...
function t.drawOneClickFunc(self)
	...
	local function cb()
		gGameApp:requestServer("/game/yy/award/draw", function(tb)
			...
			local sprite = widget.addAnimationByKey(self.effectView, "effect/xianshichouka.skel", "effect",
					"effect_danchou", 10)
				:scale(2)
			...
			local params = {
				...
				closeCb = function()
					self.isShowEffect:set(true)
					self.effectView:removeAllChildren()
					local effectBg = widget.addAnimationByKey(self.effectView, "effect/xianshichouka.skel", "effectBlack",
							effectName, 999)
						:scale(2)
						:alignCenter(self.effectView:size())
