...
local function onInitItem(list, node, k, itemDatas)
	...
	node:get("item1"):visible(false)
	node:get("item2"):visible(false)
	node:get("item3"):visible(false)
	if sign == "title" then
		...
		node:size(cc.size(listW, 160))
		for i, v in ipairs(itemDatas) do
			local item = node:get("item" .. i):visible(true)
			local figureCsv = gRoleFigureCsv[v.id]
			item:get("icon"):texture(figureCsv.logo):scale(1.7)
			item:get("selected"):visible(v.isSel)
			item:get("used"):visible(v.isUse)
			item:get("locked"):visible(v.unlocked ~= FIGURE_TYPE.UNLOCKED)
			item:get("locked.lock"):visible(v.unlocked ~= FIGURE_TYPE.UNLOCKED and v.unlocked ~= FIGURE_TYPE.CAN_UNLOCK)

			v.figureSprite = widget.addAnimationByKey(item, "figure/touxiang.json", "effect", STATE_ACTION[1], 16)
				:alignCenter(item:size())
				:scale(0.55)
				:hide()

			if v.unlocked == FIGURE_TYPE.CAN_UNLOCK then
				widget.addAnimation(item, "figure/touxiang.json", STATE_ACTION[1], 15)
					:alignCenter(item:size())
					:scale(0.55)
			end
			...
			bind.touch(list, item, { methods = { ended = functools.partial(list.clickCell, k, i, v) } })
		end
	end
end
...
function PersonalFigureView:onCreate(selectDbId)
	...
	self.unLockSkillCost  = gCostCsv
		["figure_skill_unlock_cost"]                                                               -- 解锁技能对应技能花费{50,50} 对应二三技能槽
	...
	-- 基本数据变动
	idlereasy.any({ self.figures, self.vipLevel, self.roleLv, self.figure, self.gold, self.rmb },
		function(_, figures, vipLevel, roleLv, figure)
...
function PersonalFigureView:getConditionLevel(typ, num)
	...
	if typ == 119 then
		return self.pokedex:read()[num] ~= nil
	end
	if typ == 44 then
...
function PersonalFigureView:setFigureBaseInfo(figureId)
	...
	for typ, attrData in pairs(attrDatas) do
		for k, v in pairs(attrData) do
			...
			local titleTxt = typ == 1 and gLanguageCsv.allSprite or
				string.format(gLanguageCsv.someSprite, gLanguageCsv[game.NATURE_TABLE[typ - 1]])
		...
		local y = self.attrList:y() + self.attrList:height() -
			math.min(self.attrList:height(), self.attrList:getInnerItemSize().height) - 30
...
function PersonalFigureView:setLockSkillPanel(figureId)
	...
	local skillName = skillTab["skillName_"..LOCAL_LANGUAGE] or skillTab.skillName
	self.lockSkillPanel:get("name"):text(skillName)
	local skillDescribe = skillTab["describe_"..LOCAL_LANGUAGE] or skillTab.describe

	beauty.textScroll({
		list = self.lockSkillPanel:get("descList"),
		strs = "#C0x5B545B#" .. skillDescribe,
		isRich = true,
	})
end
...
function PersonalFigureView:showActiveState(figureId, unlocked)
	...
	text.addEffect(self.condition,
		{ color = unlocked == FIGURE_TYPE.CAN_UNLOCK and ui.NEWCOLORS.GREENLIGHT or ui.NEWCOLORS.RED })
	...
	local color = (dataEasy.getNumByKey(id) >= num) and ui.NEWCOLORS.BROWN1 or ui.COLORS.NORMAL.RED
...
function PersonalFigureView:setBtnState(unlocked)
	...
	if unlocked == FIGURE_TYPE.NOT_UNLOCK then
		...
		text.addEffect(self.btnSave:get("txt"), { outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4 } })
	end
