...
function BagView:resetShowPanel(data)
	...
	local list = beauty.singleTextAutoScroll({
		...
		strs = {
			...
			effect = {color = ui.NEWCOLORS.WHITE,outline = {color = ui.NEWCOLORS.OUTLINE.BLACK, size = 3}},
		},
		fontSize = 60,
	})
	if not name then
	...
	local strs = {{str = "#C0xFCF6D6#" .. uiEasy.getIconDesc(data.id, data.num)}}
	if data.cfg.desc1 and data.cfg.desc1 ~= "" then
		table.insert(strs, {fontSize = 10})
		table.insert(strs, {str = "#C0xFFFFFF#" .. data.cfg.desc1})
	end
	-- 过期信息显示
