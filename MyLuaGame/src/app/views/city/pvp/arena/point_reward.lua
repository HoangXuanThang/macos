		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					if(v.canReceive == 1)then
						text.addEffect(childs.btnGet:get("textNote"), { outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4}})
					end
				end,
			},
			handlers = {
...
function ArenaPointRewardView:onCreate()
	...
	idlereasy.when(self.resultPointAward, function(_, resultPointAward)
		...
		if(canOneKeyReceive)then
			text.addEffect(self.getBtn:get("textNote"), { outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4}})
		end
	end)
