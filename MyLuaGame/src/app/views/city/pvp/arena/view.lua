...
local function onInitItem(list, node, k, v)
	...
	--panel:get("textRank2"):x(300)
	adapt.oneLinePos(panel:get("textRank2"), panel:get("textRankNote"), cc.p(20, 0), "right")
	...
	local passMoveNode = { "textFightNode", "textLvNote", "textLv", "textFightPoint", "textName", "textRank2",
		"textRankNote", "textRank", "imgRank" }
	...
	elseif isLowerRanking and dataEasy.isUnlock(gUnlockCsv.pvpPass) then
		...
		text.addEffect(panel:get("passPanel.btnChallenge.textNote"),
			{ outline = { color = ui.NEWCOLORS.OUTLINE.BLUE, size = 3 } })
		text.addEffect(panel:get("passPanel.btnPass.textNote"),
			{ outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 3 } })
		...
		text.addEffect(panel:get("btnChallenge.textNote"), { outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 3 } })
	end
...
ArenaView.RESOURCE_BINDING = {
	...
	["rightDown.costInfo.textCostNum"] = {
		varname = "textCost",
		binds = {
			event = "effect",
			data = { outline = { color = ui.NEWCOLORS.OUTLINE.BLACK, size = 3 } },
	...
	["rightDown.ticket.textNoteNum"] = {
		binds = {
			event = "effect",
			data = { outline = { color = ui.NEWCOLORS.OUTLINE.BLACK, size = 3 } },
		},
	},
	["rightDown.ticket.textNum"] = {
		binds = {
			event = "effect",
			data = { outline = { color = ui.NEWCOLORS.OUTLINE.BLACK, size = 3 } },
		},
	...
	["item.panel.textLvNote"] = {
		binds = {
			event = "effect",
			data = { outline = { color = ui.NEWCOLORS.OUTLINE.BLACK, size = 3 } }
		},
	},
	["item.panel.textFightNode"] = {
		binds = {
			event = "effect",
			data = { outline = { color = ui.NEWCOLORS.OUTLINE.BLACK, size = 3 } }
		},
	},
	["item.panel.textRankNote"] = {
		binds = {
			event = "effect",
			data = { outline = { color = ui.NEWCOLORS.OUTLINE.BLACK, size = 3 } }
		},
	...
	["item.panel.textRank2"] = {
		binds = {
			event = "effect",
			data = { outline = { color = ui.NEWCOLORS.OUTLINE.BLACK, size = 3 } }
		},
	},
	["item.panel.textFightPoint"] = {
		binds = {
			event = "effect",
			data = { outline = { color = ui.NEWCOLORS.OUTLINE.BLACK, size = 3 } }
		},
...
function ArenaView:onCreate()
	...
	self:initModel()
	local txtBtn = { self.txtBtnRankReward, self.txtBtnScoreReward, self.txtBtnHonourReward, self.txtBtnFightReport, self
		.txtBtnRank, self.txtBtnRule }
	for i = 1, #txtBtn do
	...
	local idlerTab = { self.pvpPwTimes, self.buyPwTimes, self.pvpPwLastTime, self.buyPwCdTimes, self.itemPwTimes, self
		.refreshEnemyTimes, self.items }
	idlereasy.any(idlerTab,
		function(_, pvpPwTimes, buyPwTimes, pvpPwLastTime, buyPwCdTimes, itemPwTimes, refreshEnemyTimes, items)
			-- 总次数是固定的 和策划确认过
...
function ArenaView:onShowHeadIcon(node, event)
	gGameUI:stackUI("city.pvp.arena.head_icon", nil, nil, self:createHandler("onChangeLogoID"),
		self:createHandler("onChangeSpine"), self.selectCardId)
end
...
function ArenaView:onItemBtnClick(list, data)
	...
	local function inEmbattle()
		gGameUI:stackUI("city.card.embattle.arena", nil, { full = true },
			{ from = "arena", team = true, fightCb = self:createHandler("startFighting", data) })
	end
...
function ArenaView:setCost(cost)
	...
	idlereasy.when(gGameModel.role:getIdler('rmb'), function(_, rmb)
		self.textCost:setTextColor(cost > 0 and cost > rmb and ui.COLORS.NORMAL.RED or ui.COLORS.NORMAL.WHITE)
