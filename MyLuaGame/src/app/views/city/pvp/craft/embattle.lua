...
CraftEmbattleView.RESOURCE_BINDING = {
	...
	["startDown.btnSave.textNote"] = {
		binds = {
			event = "effect",
			data = {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4}}
		},
	},
	["startDown.btnCancle.textNote"] = {
		binds = {
			event = "effect",
			data = {outline = {color = ui.NEWCOLORS.OUTLINE.BLUE, size = 4}}
		},
...
function CraftEmbattleView:upHero(dbId, isShowTip, idx)
	...
	local starDatas = dataEasy.getStarData(starNum)
	-- xóa sao cũ
	for i=1,6 do
		local star = item:getChildByName("star" .. i)
		if star then star:removeFromParent() end
	end

	-- hiển thị sao
	local interval = 15
	local starNum = #starDatas
	for i, v in ipairs(starDatas) do
		local pos = cc.p(165 - interval * (starNum + 1 - 2 * i), 85)
		ccui.ImageView:create(v.icon)
			:xy(pos)
			:addTo(item, 10, "star" .. i)
			:scale(0.35)
	end
	self.roleNum:set(string.format("%s/%s", self.showDatas:size(), MAXROLENUM))
	if isShowTip then
