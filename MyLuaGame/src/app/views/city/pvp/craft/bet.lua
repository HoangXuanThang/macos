		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					if(itertools.size(v.craft_bets) == 0 and v.round == "signup")then
						text.addEffect(childs.btnNormal:getChildByName("txtNode"), {outline = {color = ui.NEWCOLORS.OUTLINE.BLUE, size = 3}})
					end
					...
					if(itertools.size(v.craft_bets) == 0 and v.round == "signup")then
						text.addEffect(childs.btnHigh:getChildByName("txtNode"), {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 3}})
					end
