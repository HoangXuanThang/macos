...
local function onInitItem(list, node, k, v)
	...
	local children = node:multiget("imgResultL", "imgResultR", "imgIconL", "imgIconR", "imgHeroL", "imgHeroR",
		...
		"leftIconEight1", "leftIconEight2", "leftIconEight3", "rightIconEight1", "rightIconEight2", "rightIconEight3",
		"imgTimeBg", "textLK", "righttouxiang1", "righttouxiang2", "righttouxiang3",
		"lefttouxiang1", "lefttouxiang2", "lefttouxiang3")
	if v.round == "out" then
	...
	local rightChildren = { "imgResultR", "imgIconR", "imgHeroR", "textLvRight", "textNameRight", "textNoteRight",
		"textFightPointR", "btnShowInfoR", "rightIconEight1", "rightIconEight2", "rightIconEight3", "righttouxiang1",
		"righttouxiang2", "righttouxiang3" }
	...
	for i = 1, 3 do
		...
		children["lefttouxiang" .. i]:visible(false)
		children["righttouxiang" .. i]:visible(false)
	end
	...
	local function createEnemy(path)
		path = path or "config/big_role/show/img_21.png"
	...
	if not isFinal then
		...
		for i, v in ipairs(v.cards) do
			...
			children["leftIconEight" .. i]:texture(path):scale(2.5)
		end
		if not itertools.isempty(v.enemy) then
			...
			for i, v in ipairs(v.enemy.cards) do
				...
				children["rightIconEight" .. i]:texture(path):scale(2.5)
			end
		else
			setNotEnemy()
			for i = 1, 3 do
				children["rightIconEight" .. i]:setColor(cc.c3b(0, 0, 0))
				children["rightIconEight" .. i]:setOpacity(0)
			end
			local sp = createEnemy()
			sp:setColor(cc.c3b(0, 0, 0))
			sp:setOpacity(0)
	...
	bind.click(list, children.btnShowInfoL,
		{ method = functools.partial(list.showMyInfo, k, v, state, string.find(list.round:read(), "_lock")) })
	bind.click(list, children.btnShowInfoR,
		{ method = functools.partial(list.showEnemyInfo, k, v, state, string.find(list.round:read(), "_lock")) })
end
...
CraftMyScheduleView.RESOURCE_BINDING = {
	...
	["item.btnReplay.textNote"] = {
		binds = {
			event = "effect",
			data = { outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 3 } }
		},
	},
	["slider"] = "slider",
	["roleItem"] = "roleItem",
...
function CraftMyScheduleView:onCreate(isRecord)
	...
	local count = -1
	idlereasy.any({ self.history, self.vsinfo, self.curTime, self.round, self.info },
		function(_, history, vsinfo, curTime, round, info)
			local function dothing()
				...
				if not isRecord then
					...
					if not isClose and self.isSignup and not info.isout and not itertools.include(self.rounds, curStatrStr) then
						local str = string.format(
							"craft, info.isout(%s), round(%s), vsinfo.round(%s), history(len:%d)\nhistory:last(%s)\nvsinfo(%s)",
							tostring(info.isout), round, tostring(vsinfo.round), len, dumps(history[len]), dumps(vsinfo))
						-- print(str)
						-- 轮空不显示
					...
					if not self.isSignup or info.isout then
						local str = string.format(gLanguageCsv.craftIsOver, (info.win or 0),
							math.min(info.round or 0, 13) - (info.win or 0))
				...
				self.winAndloseNum:set(string.format(gLanguageCsv.winAndLoseNum, (info.win or 0),
					math.min(info.round or 0, 13) - (info.win or 0)))
