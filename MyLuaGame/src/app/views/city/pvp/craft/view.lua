...
CraftMainView.RESOURCE_BINDING = {
		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					text.addEffect(node:get("textName"), { outline = { color = ui.NEWCOLORS.OUTLINE.BLACK, size = 3 } })
					text.addEffect(node:get("textVal"), { outline = { color = ui.NEWCOLORS.OUTLINE.BLACK, size = 3 } })
	...
	["top1.down.textFightPointNote"] = {
		binds = {
			event = "effect",
			data = { outline = { color = ui.NEWCOLORS.OUTLINE.WHITE2, size = 3 } }
		},
	},
...
function CraftMainView:onCreate()
	...
	idlereasy.any({ self.isSignup, self.round, self.time, self.buffs }, function(_, isSignup, round, stateTime, buffs)
		...
		local strTime = string.format("%s-%s", dataEasy.getTimeStrByKey("craft", "signUpStart"),
			dataEasy.getTimeStrByKey("craft", "signUpEnd"))
...
function CraftMainView:initItem(parent, data)
	parent:get("role"):removeChildByName("card")
	parent:get("role"):removeChildByName("figure")
	...
	--	:xy(size.width / 2 , size.height / 4 + 10)
	--	:scale(unit.scale * 1.4)
	--cardSprite:setSkin(unit.skin)

	if string.len(figureCfg.resSpine or "") > 0 then
		-- widget.addAnimationByKey(parent:get("role"), figureCfg.resSpine, "figure", "standby_loop", 3)
		-- 	:xy(size.width / 2 + 200, size.height / 4 + 10)
		-- 	:scale(1)

		bind.extend(self, parent:get("role"), {
			event = "extend",
			class = "role_figure",
			props = {
				data = data.figure,
				onNode = function(node)
					node:scale(1.2):setAnchorPoint(cc.p(0.5, 0)):y(100)
				end,
			},
		})
	end
		...
		bind.extend(self, parent, {
			...
			props = {
				...
				onNode = function(panel)
					panel:xy(size.width / 2 + 50, size.height + 50)
					panel:scale(2)
					panel:z(3)
				end,
			},
	...
	idlereasy.when(yesterdayTop, function(_, topData)
		...
		if champion and third then
			...
			for i, v in ipairs(self.topData) do
				...
				text.addEffect(self["top" .. i .. "Lv"],
					{ color = color1, outline = { color = ui.NEWCOLORS.OUTLINE.WHITE2, size = 3 } })
				text.addEffect(self["top" .. i .. "Name"],
					{ color = color1, outline = { color = ui.NEWCOLORS.OUTLINE.WHITE2, size = 3 } })
				text.addEffect(self["top" .. i .. "FightPoint"],
					{ color = color1, outline = { color = ui.NEWCOLORS.OUTLINE.WHITE2, size = 3 } })
