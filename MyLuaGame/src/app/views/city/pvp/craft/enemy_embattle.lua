...
function CraftEnemyEmbattleView:onCreate(data, state, isFighting)
	...
	if not data.online then
		...
		text.addEffect(self.roleState, {color = ui.NEWCOLORS.GRAY})
	end
	...
	for i=1,MAXPREROUNDNUM do
		for k,v in pairs(data.card_attrs) do
			if v.id == cards[i] then
				...
				local starDatas = dataEasy.getStarData(v.star)  -- trả về tối đa 6 sao với icon đúng bậc
				local starNum = #starDatas
				local interval = 15

				-- xóa các sao cũ
				for i = 1, 6 do
					local star = node:getChildByName("star" .. i)
					if star then star:removeFromParent() end
				end

				-- hiển thị sao
				for i, data in ipairs(starDatas) do
					local posX = state == 1 and (160 - interval * (starNum + 1 - 2 * i)) or (240 + (i - 1) * 40)
					local posY = state == 1 and 85 or 115
					ccui.ImageView:create(data.icon)
						:xy(posX, posY)
						:addTo(node, 10, "star" .. i)
						:scale(0.35)
				end
