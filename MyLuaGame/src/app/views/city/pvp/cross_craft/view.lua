...
CrossCraftView.RESOURCE_BINDING = {
	...
	["signupPanel.btn.txt"] = {
		binds = {
			event = "effect",
			data = { color = ui.COLORS.NORMAL.WHITE, outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW } },
		},
	},
	...
	["leftBtn2.mainSchedule.name"] = {
		binds = {
			event = "effect",
			data = { outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW } },
		},
	},
	["leftBtn2.mySchedule.name"] = {
		binds = {
			event = "effect",
			data = { outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW } },
		},
	},
	["leftBtn2.myTeam.name"] = {
		binds = {
			event = "effect",
			data = { outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW } },
		},
	},
	["leftBtn1.myTeam.name"] = {
		binds = {
			event = "effect",
			data = { outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW } },
		},
...
function CrossCraftView:onCreate()
	...
	local panels = { self.firstPanel, self.signupPanel, self.overPanel, self.countdownPanel, self.leftBtn1, self
		.leftBtn2, self.signupBtn, self.signedPanel, self.vipShow }
	self.subViewFunc = {
		battleMessages = function(...)
			return gGameUI:createView("city.pvp.cross_craft.battle_messages", self:getResourceNode()):init(...):x(
				display.uiOrigin.x)
		end,
		mySchedule = function(...)
			return gGameUI:createView("city.pvp.cross_craft.myschedule", self:getResourceNode()):init(...):x(display
				.uiOrigin.x)
		end,
		mainSchedule = function(...)
			return gGameUI:createView("city.pvp.cross_craft.main_schedule_final", self:getResourceNode()):init(...):x(
				display.uiOrigin.x)
		end,
	}
...
function CrossCraftView:setTopInfo(i)
	...
	text.addEffect(childs.level, { color = color1, outline = { color = ui.NEWCOLORS.OUTLINE.WHITE2, size = 3 } })
	text.addEffect(childs.server, { color = color1, outline = { color = ui.NEWCOLORS.OUTLINE.WHITE2, size = 3 } })
	text.addEffect(childs.fightPoint, { color = color1, outline = { color = ui.NEWCOLORS.OUTLINE.WHITE2, size = 3 } })
	text.addEffect(childs.fightPointText, { color = color1, outline = { color = ui.NEWCOLORS.OUTLINE.WHITE2, size = 3 } })
	text.addEffect(childs.name, { color = color1, outline = { color = ui.NEWCOLORS.OUTLINE.WHITE2, size = 3 } })
	...
	item:get("role"):removeChildByName("card")
	item:get("role"):removeChildByName("figure")
	...
	--	:xy(size.width / 2 , size.height / 4 + 50)
	--	:scale(unitCfg.scale * 1)
	--cardSprite:setSkin(unitCfg.skin)

	if figureCfg.resSpine ~= "" then
		widget.addAnimationByKey(item:get("role"), figureCfg.resSpine, "figure", "standby_loop", 4)
			:xy(size.width / 2 + 200, size.height / 4 + 60)
			:scale(1)
	end
		...
		bind.extend(self, item, {
			...
			props = {
				...
				onNode = function(node)
					node:xy(size.width / 2 + 50, size.height + 50)
						:scale(2)
						:z(5)
				end
			}
...
function CrossCraftView:resetTopuiView()
	...
	if state == 1 then
		...
		self.topuiView = gGameUI.topuiManager:createView("default", self,
				{ onClose = self:createHandler("onCloseSubView") })
			:init({ title = gLanguageCsv.crossCraft, subTitle = "CROSS - INDIGO PLATEAU CONFERENCE" })
	end
end
