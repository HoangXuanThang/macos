		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					local str = "#C0x0BFA87##F48##L10#" .. name1 ..string.format(gLanguageCsv.brackets, getServerArea(server1, true))
					if v[1] ~= "streak" then
					--普通
						local txt = TEXTS[v[4]]
						local endStr = ""
						if type(txt) == "table" then
							str = str .. "#C0xFFFFFF##F48#" .. txt[math.random(1, #txt)]
							endStr = gLanguageCsv.chenggongjinji
						else
							str = str .. "#C0xFFFFFF##F48#" .. txt
							endStr = gLanguageCsv.yihanluobai
						end
						local name2 = v[3][2]
						local server2 = v[3][1][1]
						str = str .. "#C0x0BFA87##F48##L10#" .. name2 ..string.format(gLanguageCsv.brackets, getServerArea(server2, true)) .. "#C0xFFFFFF##F48#," .. endStr..string.format(gLanguageCsv.craftMessageGotScroe, v[5][1])
					else
					--连胜
						local winNum = math.min(v[3], 8)
						local colorAndSize = ui.QUALITYCOLOR[winNum - 1] .. "#F48#"
						str =  str .. "#C0xFFFFFF##F48#" .. string.format(gLanguageCsv.isStreakWin, colorAndSize, WINNUN2TEXT[v[3]]) .. ","
