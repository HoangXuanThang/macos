		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					if(list.canBet:read())then
						text.addEffect(childs.btnNormal:getChildByName("txtNode"), {outline = {color =  ui.NEWCOLORS.OUTLINE.BLUE}})
					end
					...
					if(list.canBet:read())then
						text.addEffect(childs.btnHigh:getChildByName("txtNode"), {outline = {color =  ui.NEWCOLORS.OUTLINE.YELLOW}})
					end
					--已下注信息
					local betNumData = {
...
function CrossCraftBetView:onCreate(data)
	...
	--widget.addAnimation(imgBg, "kuafushiying/yazhubj.skel", "effect_loop", 99)
