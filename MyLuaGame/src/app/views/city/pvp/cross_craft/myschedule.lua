...
local function onInitItem(list, node, k, v)
	...
	local children = node:get("fight"):multiget("imgResultL", "imgResultR", "imgIconL", "imgIconR", "infoL", "infoR",
		...
		"imgRightIcon1", "imgRightIcon2", "imgRightIcon3", "title", "RightIcon1", "RightIcon2", "RightIcon3")
	local roundStatus = {
		["out"] = function()
	...
	local rightChildren = { "imgResultR", "imgIconR", "infoR", "imgRightIcon1", "imgRightIcon2", "imgRightIcon3",
		"RightIcon1", "RightIcon2", "RightIcon3" }
	...
	if v.paths then
		...
		adapt.oneLineCenterPos(cc.p(250, 60), nodes, cc.p(5, 0))
	end
	...
	if hasEnemy then
		...
		for i = 1, 3 do
			...
			children["imgRightIcon" .. i]:setOpacity(0)
		end
	...
	adapt.oneLineCenterPos(cc.p(700, 50), { children.infoL:get("textNote"), children.infoL:get("textFightPoint") },
		cc.p(10, 0))
	adapt.oneLineCenterPos(cc.p(250, 50), { children.infoR:get("textNote"), children.infoR:get("textFightPoint") },
		cc.p(10, 0))
...
function CrossCraftMainScheduleView:onCreate(isRecord)
	...
	local count = -1
	idlereasy.any({ self.history, self.vsinfo, self.curTime, self.round, self.info, self.infoCards, self.cardAttrs },
		function(_, history, vsinfo, curTime, round, info)
			local function dothing()
				...
				adapt.oneLinePos(self.infoPanel:get("textNote1"),
					{ self.infoPanel:get("textVal1"), self.infoPanel:get("textNote2"), self.infoPanel:get("textVal2") },
					{ cc.p(5, 0), cc.p(50, 0), cc.p(5, 0) })
			end
			if count < 0 then
				count = 0
				dothing()
