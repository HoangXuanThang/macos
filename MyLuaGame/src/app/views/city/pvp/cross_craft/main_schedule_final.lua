		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					node:get("textNote"):text(v.str)
					if(v.isSel)then
						text.addEffect(node:get("textNote"), {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW}})
					else
						text.addEffect(node:get("textNote"), {outline = {color = ui.NEWCOLORS.OUTLINE.BLUE}})
					end
...
function CrossCraftMainScheduleFinalView:onCreate(isRecord)
	...
	local setLabel = function()
		...
		text.addEffect(self.scheduleTime, {color = color})
...
function CrossCraftMainScheduleFinalView:initUI(datas)
	...
	for i,v in ipairs(datas) do
		...
		for j=1,2 do
			...
			text.addEffect(player:get("textLv"), {outline = {color = ui.NEWCOLORS.OUTLINE.BLACK, size = 2}})
			text.addEffect(player:get("textName"), {outline = {color = ui.NEWCOLORS.OUTLINE.BLACK, size = 2}})
		...
		if i == maxNum and canShowResult then
			...
			text.addEffect(winner:get("textLv"), {outline = {color = ui.NEWCOLORS.OUTLINE.BLACK, size = 2}})
			text.addEffect(winner:get("textName"), {outline = {color = ui.NEWCOLORS.OUTLINE.BLACK, size = 2}})
	...
	for i=count + 1,maxNum do
		...
		team:get("win"):hide()
		team:get("fail"):hide()
		text.addEffect(team:get("item1.normal.textNote"), {outline = {color = ui.NEWCOLORS.OUTLINE.BLACK, size = 2}})
	end
...
function CrossCraftMainScheduleFinalView:initTopStageView(data)
	...
	for i=1,2 do
		...
		player:get("role"):removeChildByName("card")
		player:get("role"):removeChildByName("figure")
		...
		text.addEffect(player:get("info1.textName"), {outline = {color = ui.NEWCOLORS.OUTLINE.BLACK, size = 2}})
		text.addEffect(player:get("info1.textFightPoint"), {outline = {color = ui.NEWCOLORS.OUTLINE.BLACK, size = 2}})
		text.addEffect(player:get("info1.textLvAndSer"), {outline = {color = ui.NEWCOLORS.OUTLINE.BLACK, size = 2}})
		text.addEffect(player:get("info1.textNote"), {outline = {color = ui.NEWCOLORS.OUTLINE.BLACK, size = 2}})
		...
		--	:xy(size.width / 2  , size.height / 5 - 15)
		--	:scale(unit.scale * 1 * dir, unit.scale * 1)
		--cardSprite:setSkin(unit.skin)

		if string.len(figureCfg.resSpine or "") > 0 then
			widget.addAnimationByKey(player:get("role"), figureCfg.resSpine, "figure", "standby_loop", 2)
				:xy(size.width / 2  + dx, size.height / 5 - 15)
				:scale(1.2)
		end
...
function CrossCraftMainScheduleFinalView:onChangePage(_type)
	...
	local promotionColor, finalColor =  ui.NEWCOLORS.OUTLINE.BLUE,  ui.NEWCOLORS.OUTLINE.YELLOW
	if isPromotion then
		promotionColor, finalColor = finalColor, promotionColor
	end
	text.addEffect(self.btnPromotionNote, {outline = {color =  promotionColor}})
	text.addEffect(self.btnFinalNote, {outline = {color =  finalColor}})
