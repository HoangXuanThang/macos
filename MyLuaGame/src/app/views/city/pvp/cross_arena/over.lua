...
-- @desc 跨服竞技场赛后展示界面
local ROLE_INFO = {
	[1] = {pos = cc.p(1055, 530), scale = 0.75},
	[2] = {pos = cc.p(496, 446), scale = 0.73},
	[3] = {pos = cc.p(1636, 446), scale = 0.73},
	[4] = {pos = cc.p(54, 306 ), scale = 0.7},
	[5] = {pos = cc.p(2136, 306), scale = 0.7},
	[6] = {pos = cc.p(1055, -90), scale = 0.7},
	[7] = {pos = cc.p(670, -40), scale = 0.7},
	[8] = {pos = cc.p(1460 , -40), scale = 0.7},
	[9] = {pos = cc.p(286, 32), scale = 0.7},
	[10] = {pos = cc.p(1872, 32), scale = 0.7},
 }
...
function OverView:initRole()
	...
	for i = 1, 3 do
		...
		local panel = self.rolePanel1:clone()
			:addTo(self.centerPanel:get("pos"..i))
			:xy(ROLE_INFO[i].pos)
			:name("role_"..i)
		...
		if role.figure ~= "" then
			local size = panel:get("role"):size()
			local y = (i == 1) and 20 or 50
			local figureCfg = gRoleFigureCsv[role.figure]
			widget.addAnimationByKey(panel:get("role"), figureCfg.resSpine, "figure", "standby_loop", 1)
				:xy(size.width / 2, 50)
	...
	for i = 4, 10 do
		...
		if(i==10)then
			panel:get("atlasLabelRank"):x(panel:get("atlasLabelRank"):x()+25)
		end
