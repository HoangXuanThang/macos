...
SelectEnemyView.RESOURCE_BINDING = {
	...
	["enemyPanel.btnChallenge5.textNote"] = {
		binds = {
			event = "effect",
			data = {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW}},
		},
	},
	["rightDownPanel.btnChange.textNote"] = {
		binds = {
			event = "effect",
			data = {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW}},
		},
	...
	["rightDownPanel.textNote1"] = {
		varname = "textNoteTimes",
		binds = {
			event = "effect",
			data = {outline = {color = ui.NEWCOLORS.OUTLINE.BLACK,size = 3}},
		},
	},
	["rightDownPanel.textTimes"] = {
		varname = "textTimes",
		binds = {
			event = "effect",
			data = {outline = {color = ui.NEWCOLORS.OUTLINE.BLACK,size = 3}},
	...
	["rightDownPanel.textNote2"] = {
		varname = "textNoteChange",
		binds = {
			event = "effect",
			data = {outline = {color = ui.NEWCOLORS.OUTLINE.BLACK,size = 3}},
		},
	},
	["rightDownPanel.textGold"] = {
		varname = "textGold",
		binds = {
			event = "effect",
			data = {outline = {color = ui.NEWCOLORS.OUTLINE.BLACK,size = 3}},
...
function SelectEnemyView:onCreate()
	...
	idlereasy.any({self.vipLevel, self.pwTimes, self.buyPwTimes, self.refreshTimes}, function(_, vipLevel, pwTimes, buyPwTimes, refreshTimes)
		...
		if canChallengeCount < 1 then
			...
			text.addEffect(self.textTimes, {color = ui.COLORS.NORMAL.GREEN1})
		end
...
function SelectEnemyView:initCountDown(  )
	...
	local function setLabel()
		...
		if remainTime.hour < 1 and remainTime.day < 1 then
			...
			text.addEffect(self.textTime, {color = ui.COLORS.NORMAL.GREEN1})
		end
...
function SelectEnemyView:initSelfPanel(role)
	...
	--adapt.oneLineCenterPos(cc.p(251,45), {self.selfPanel:get("imgZlBg.imgZl"),self.selfPanel:get("imgZlBg.textZl")}, cc.p(10,0))
...
function SelectEnemyView:initEnemyPanel(enemys)
	...
	for i = 1, 4 do
		...
		if enemy.rank < self.role:read().rank or (not dataEasy.isUnlock(gUnlockCsv.crossArenaPass)) then
			...
			adapt.oneLineCenterPos(cc.p(100,100), {panel:get("btnChallenge5.imgDiamond"),panel:get("btnChallenge5.textCost")}, cc.p(10,0))
