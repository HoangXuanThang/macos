		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					if(v.pointAwardState == 1)then
						text.addEffect(childs.btnGet:get("txt"), {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW}})
					end
				end,
			},
			handlers = {
...
function CrossArenaPointRewardView:onCreate()
	...
	idlereasy.when(self.pointAward, function(_, pointAward)
		...
		if(canOneKeyReceive)then
			text.addEffect(self.getBtn:get("txt"), {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW}})
		end
	end)
...
function CrossArenaPointRewardView:initModel()
	...
	self.version = gGameModel.cross_arena:read("version") or 0
