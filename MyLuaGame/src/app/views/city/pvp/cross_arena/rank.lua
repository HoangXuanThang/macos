		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					adapt.oneLinePos(childs.down:get("txtNode"), {childs.down:get("txtLv"), childs.down:get("txtServer")}, cc.p(5, 0),"left")
...
function CrossArenaRankView:onCreate(data)
	...
	if itertools.isempty(self.data) then
		...
		for i = 1, 3 do
			...
			text.addEffect(childs.txt, {outline={color = ui.NEWCOLORS.OUTLINE.BLACK, size=3}})
			text.addEffect(childs.name, {outline={color = ui.NEWCOLORS.OUTLINE.BLACK, size=3}})
			text.addEffect(childs.damage, {outline={color = ui.NEWCOLORS.OUTLINE.WHITE1, size=3}})
			adapt.oneLineCenterPos(cc.p(250, 80), {childs.txt, childs.name, childs.vip}, cc.p(15, 0))
