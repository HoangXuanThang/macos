...
function OnlineFightView:onCreate()
	...
	self.switchEffect = widget.addAnimationByKey(self.mainPanel, "battlearena/shishipipei.skel", "switchEffect",
			"kaimen_effect", 0)
		:alignCenter(self.mainPanel:size())
		:scale(2)
	self.rightIconEffect = widget.addAnimationByKey(self.mainPanel, "battlearena/shishipipei.skel", "rightIconEffect",
			"fanpai_1_effect", 1)
		:alignCenter(self.mainPanel:size())
		:scale(2)
	self.startBtnEffect = widget.addAnimationByKey(self.mainPanel, "battlearena/shishipipei.skel", "startBtnEffect",
			"shanzi_loop", 11)
		:alignCenter(self.mainPanel:size())
		:scale(2)
	...
	self.embattleBtnEffect = widget.addAnimationByKey(embattlePanel, "battlearena/shishipipei.skel", "embattleBtnEffect",
			"denglong_loop", 1)
		:xy(90, 135)
		:scale(2)
	self.sceneEffect = widget.addAnimationByKey(self:getResourceNode(), "battlearena/shishipipei.skel", "sceneEffect",
			"kspp_wxzs_effect", 100)
	...
	idlereasy.when(self.longtimeoutModel, function(_, longtimeoutModel)
		self.longtimePanel:get("btn"):texture(longtimeoutModel and "common/icon/radio_selected.png" or
			"common/icon/radio_normal.png")
	end)
			...
			if matchResult.matching and matchResult.matching > 0 then
				...
				local function setLabel(sec)
					...
					if not matchAdapt then
						matchAdapt = true
						adapt.oneLineCenterPos(cc.p(1540, 530),
							{ self.matchingPanel:get("icon"), self.matchingPanel:get("time") }, cc.p(20, 0))
					end
				end
				setLabel(passTime)
	...
	idlereasy.any({ self.round, self.matchState }, function(_, round, matchState)
		...
		itertools.invoke(
			{ self.firstPanel, self.mainPanel, self.matchingPanel, self.displayPanel, self.enemyPanel, self.banTipPanel,
				self
					.banUnlimitedTipPanel, self.rightBtn }, "hide")
		...
		if round == "closed" then
			...
			local endDate = time.getNumTimestamp(gGameModel.cross_online_fight:read("end_date"),
				time.getHourAndMin(awardTime, true))
			...
			local function showState()
				nodetools.invoke(self.mainPanel,
					{ "rankDesk", "rankBtn", "rankRewardBtn", "leftBtnBg", "timePanel", "awardPanel" }, "visible",
					matchState == STATE_TYPE.normal)
				nodetools.invoke(self.mainPanel:get("rightPanel"), { "switchBtn" }, "visible",
					matchState == STATE_TYPE.normal)
				...
				self.mainPanel:get("rightPanel.descPanel"):visible(matchState == STATE_TYPE.normal and
					self.switchModel:read() == "limited")
				self.mainPanel:get("rightPanel.unlimitedPanel"):visible(self.hasBanCard and
					matchState == STATE_TYPE.normal and self.switchModel:read() ~= "limited")
			...
			if lastThemeId ~= themeId then
				...
				if themeId ~= 0 then
					gGameUI:stackUI("city.pvp.online_fight.theme", nil,
						{ full = false, clickClose = true, blackLayer = true })
				end
		...
		if not isFirst then
			if matchState == STATE_TYPE.normal or matchState == STATE_TYPE.matching then
				...
				transition.executeSequence(self.mainPanel:get("startBtn.name"), true)
					...
					:spawnEnd()
					:func(function()
						self.mainPanel:get("startBtn.name"):texture(matchState == STATE_TYPE.normal and
							"city/pvp/online_fight/txt_kspp.png" or "city/pvp/online_fight/txt_qx.png")
					end)
					:delay(60 / 30)
					:spawnBegin()
					:fadeIn(6 / 30)
	...
	idlereasy.when(self.switchModel, function(_, switchModel)
		transition.executeSequence(self.mainPanel:get("rightPanel.name"), true)
			:fadeOut(6 / 30)
			:func(function()
				self.mainPanel:get("rightPanel.name"):texture(switchModel == "limited" and
					"city/pvp/online_fight/txt_gps.png" or "city/pvp/online_fight/txt_wxzs.png")
			end)
			:delay(18 / 30)
			:fadeIn(6 / 30)
			:done()
...
function OnlineFightView:getCloseState()
	...
	local data = gGameModel.cross_online_fight:read("limited_history_top") or
		gGameModel.cross_online_fight:read("unlimited_history_top")
...
function OnlineFightView:setRolePanel()
	...
	if figureCfg.resSpine ~= "" then
		widget.addAnimationByKey(rolePanel, figureCfg.resSpine, "figure", "standby_loop", 4)
			:xy(size.width / 2, 30)
			:scale(1.6)
	end
...
function OnlineFightView:refreshRightPanel()
	...
	idlereasy.any({ self.unlimitedRank, self.unlimitedScore, self.limitedRank, self.limitedScore, self.switchModel },
		function(_, unlimitedRank, unlimitedScore, limitedRank, limitedScore, switchModel)
			local rankPointPanel = self.mainPanel:get("rankPointPanel")
			if switchModel == "limited" then
				rankPointPanel:get("txt1"):text(gLanguageCsv.onlineFightMyRank ..
					(limitedRank == 0 and gLanguageCsv.onlineFightNoMatch or limitedRank))
				rankPointPanel:get("txt2"):text(gLanguageCsv.onlineFightMyPoint .. limitedScore)
			else
				rankPointPanel:get("txt1"):text(gLanguageCsv.onlineFightMyRank ..
					(unlimitedRank == 0 and gLanguageCsv.onlineFightNoMatch or unlimitedRank))
...
function OnlineFightView:onEmbattleClick(show)
	...
	if self.switchModel:read() == "limited" then
		...
		gGameUI:stackUI("city.card.embattle.online_fight", nil, { full = true },
			{ from = "onlineFight", tip = show == true })
	end
end
...
function OnlineFightView:onTargetAwardClick()
	...
	elseif self.weeklyTargetState.flag == 1 then
		...
		gGameUI:showBoxDetail({
			...
			content = string.format(gLanguageCsv.onlineFightWeeklyTargetTip,
				gLanguageCsv["onlineFightWeeklyTarget" .. self.weeklyTargetState.cfg.type],
				self.weeklyTargetState.cfg.count),
...
function OnlineFightView:resetDisplayTop()
	...
	for i = 1, 3 do
		if data[i] then
			...
			if figureCfg.resSpine ~= "" then
				widget.addAnimationByKey(item, figureCfg.resSpine, "figure", "standby_loop", 0)
					:xy(size.width / 2, 50)
...
function OnlineFightView:refreshUnlimitedCards()
	...
	for kk, vv in ipairs(cfg.invalidMarkIDs) do
		...
		for _, val in orderCsvPairs(csv.cards) do
			if val.cardMarkID == vv then
				if val.megaIndex == 0 then
