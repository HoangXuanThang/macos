...
function CrossMineEmbattleView:initShowTab(val)
	local sign = val == 1
	self.btnFight:get("img"):texture(sign and "city/pvp/cross_mine/btn_kfzy1@.png" or
		"city/pvp/cross_mine/btn_kfzy2@.png")
	self.btnGuard:get("img"):texture(sign and "city/pvp/cross_mine/btn_kfzy2@.png" or
		"city/pvp/cross_mine/btn_kfzy1@.png")
...
function CrossMineEmbattleView:initUI()
	...
	adapt.oneLineCenterPos(cc.p(1410 + math.abs(dx) / 4, 425), { self.panelTab[1], self.panelTab[2], self.panelTab[3] },
		cc.p(dot, 0))
end
...
function CrossMineEmbattleView:initHeroSprite(index)
	...
	for k1 = 1, 6 do
		...
		self.heroSprite[index][k1] = {
			sprite = item,
			dbid = cardData and cardData.dbid or nil,
			markId = cfg and
				cfg.cardMarkID or nil
		}
	end
...
function CrossMineEmbattleView:createMovePanelBlock(index, p)
	...
	self.movePanelLast:get("imgBg"):texture(sign and "city/pvp/cross_mine/img_jg_bg.png" or
		"city/pvp/cross_mine/ing_fs_bg.png")
...
-- 卡牌过滤 待继承
function CrossMineEmbattleView:limtFunc(dbid, card_id, skin_id, fighting_point, level, star, advance, created_time,
										nature_choose, inBattle)
