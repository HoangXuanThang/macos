			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					adapt.oneLineCenterPos(cc.p(150, 35),
						{ children.cost:get("textNote"), costText, children.cost:get("imgIcon") }, cc.p(6, 0))
					...
					if canTrubute and vipEnough then
						...
						text.addEffect(children.btnOk:get("textNote"),
					...
					bind.touch(list, children.btnOk, {
						methods = {
...
function CrossMineWishView:onCreate()
	...
	idlereasy.any({ self.serverBuffInfo, self.roleBuffInfo, self.buffFeed },
		function(_, serverBuffInfo, roleBuffInfo, buffFeed)
			self:initDatas()
		end)
...
function CrossMineWishView:initDatas()
	...
	if signLv then
		...
		table.insert(buffDatas,
			{ gLanguageCsv.level, string.format(gLanguageCsv.crossMineWish09, buffInfos.level), string.format(
				gLanguageCsv.crossMineWish09, nextLevel) })
	end
	for index, data in pairs(ROLEBUFF) do
		...
		if signLv then
			...
			table.insert(buffDatas,
				{ data.info, string.format("%s%%", curInfo * 100), string.format("%s%%", nextBuffInfo[key] * 100) })
		end
	end
