...
local function onInitItem(list, node, k, v)
	...
	rich.createByStr(
		string.format(gLanguageCsv.handbookBreakTitle, "#C0xFF57472E#", v.cfg.targetArg, color, natureStr,
			"#C0xFF57472E#"),
		50)
		:anchorPoint(0, 0.5)
		:xy(57, 180)
	...
	elseif v.state == 1 then -- 可领取
		...
		btn:visible(true)
		text.addEffect(btn:get("textNote"), { outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW } })
		widget.addAnimationByKey(node:get("progressBar"), "shengjichangtiao/changtiao.skel", 'changtiao', "effect_loop",
			99)
		...
		text.deleteEffect(btn:get("textNote"), { "outline" })
	end
end
		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					bind.touch(list, node:get("btnBreak"),
						{ methods = { ended = functools.partial(list.clickItem, k, v, node) } })
				end,
				asyncPreload = 4,
			},
			handlers = {
...
function HandbookBreakView:onBreak(list, k, v, item)
	...
	gGameApp:requestServer("/game/role/pokedex_advance", function(tb)
		...
		local effect = widget.addAnimationByKey(item:get("progressBar"), "shengjichangtiao/changtiao.skel", 'changtiao',
			"effect_loop", 99)
		effect:play("effect")
