...
local uiHelper = require("app.views.uiHelper")
...
local BG_PATH = {
	...
	[6] = "city/handbook/tag_caise.png",
	[7] = "city/handbook/tag_colorful.png"
}
		...
		for branch, v in pairs(branchDatas) do
			...
			for i, vv in ipairs(v) do
				...
				bind.touch(list, item:get("cardPanel"),
					{ methods = { ended = functools.partial(list.clickCell, k, vv.order, vv.cfg.cardID, itemType) } })
			end
		end
		table.sort(develops)
		local lastItems = {}
			...
			props = {
				...
				onItem = function(innerList, cell, kk, v)
					...
					bind.touch(list, cell,
						{ methods = { ended = functools.partial(list.clickCell, k, kk, v.cfg.cardID, itemType) } })
				end,
			},
		}
		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					local color = v.hasRole and ui.NEWCOLORS.WHITE or cc.c4b(183, 176, 158, 255)
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					bind.touch(list, node:get("btnNormal"),
						{ methods = { ended = functools.partial(list.clickItem, k, v) } })
				end,
			},
			handlers = {
				clickItem = bindHelper.self("onChangeView"),
...
function HandbookView:onCreate(params)
	...
	-- self.btnComment:visible(false)
	--end)

	local btn = self.btnTupo
	local textLabel = btn:getChildByName("textNote") or btn:getTitleRenderer()
	if textLabel then
		local maxWidth = btn:size().width - 10 -- trừ padding 
		uiHelper.autoFitFont(textLabel, maxWidth, 12)
	end
	...
	local percentKeys = { "hpPercent", "speedPercent", "damagePercent", "defencePercent", "specialDamagePercent",
	...
	idlereasy.when(self.selCardId, function(_, cardId)
		...
		if cardInfo then
			...
			local cardDesc = cardInfo["introduction_"..LOCAL_LANGUAGE] or cardInfo.introduction
			...
			beauty.textScroll({
				list = self.textDescList,
				strs = "#C0xFFFFFF#" .. cardDesc,
				isRich = true,
			})
			local unitID = cardInfo.unitID
			local unitInfo = csv.unit[unitID]
			self.photo:removeAllChildren()

			if unitInfo.showOpen then
				local photoSprite = widget.addAnimation(self.photo, unitInfo.showOpen, "default", 5, true)
				print("===========img=============", unitInfo.showOpen)
				if string.find(unitInfo.showOpen, ".png") then
					photoSprite:scale(0.65)
				else
					photoSprite:scale(0.6)
				end
			end
	...
	if matchLanguage({ "en", "kr" }) then
		adapt.setTextAdaptWithSize(self.textLocation,
			{ size = cc.size(570, 100), vertical = "center", horizontal = "left", margin = -5 })
	end
...
function HandbookView:resetStarData()
	...
	local color = hasRole and ui.NEWCOLORS.WHITE or cc.c4b(183, 176, 158, 255)
...
function HandbookView:onCleanup()
	...
	self._natureDatas = {}
	for k, v in self.natureDatas:ipairs() do
		-- if (k < 6) then
