...
local function itemShow(list, node, k, v)
	...
	childs.textName:text(cfg["title_"..LOCAL_LANGUAGE] or cfg.title)
	childs.textDesc:text(cfg["desc_"..LOCAL_LANGUAGE] or cfg.desc)
	...
	text.addEffect(childs.btnGet:get("title"), {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4}})
	text.addEffect(childs.btnGo:get("title"), {outline = {color = ui.NEWCOLORS.OUTLINE.BLUE, size = 4}})
	...
	if v.progressCurr then
		local color = tonumber(v.progressCurr) >= tonumber(cfg.targetDisplay) and
			ui.NEWCOLORS.GREENLIGHT or ui.NEWCOLORS.RED1
...
function TaskView:onCreate(showTab)
	...
	idlereasy.when(self.btnOneKeyGray, function (obj, state)
		...
		if state then
			text.addEffect(self.oneKeyTxt, {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4}, color = ui.COLORS.NORMAL.WHITE})
		else
	...
	uiEasy.setPrivilegeRichText(game.PRIVILEGE_TYPE.DailyTaskExpRate, self.dailyPanel, gLanguageCsv.exps, cc.p(7, 100))
end
