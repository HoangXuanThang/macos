		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					local childs = panel:multiget("name", "left", "right", "icon", "subName", "iconMask", "btnBox", "lock", "btnUp", "lockPanel","cardbg")
					...
					if not v.isNil then
						...
						childs.cardbg:texture("config/trainer/trainer_"..v.id ..".png")
						childs.subName:hide()
						if v.level then
							...
							if boxCanOpen then
								local effect = widget.addAnimationByKey(panel, "effect/jiedianjiangli.skel", "spine", "effect_loop", 99)
								effect:scale(0.5)
									:xy(panel:size().width - 80, 80)
							end
							uiEasy.addVibrateToNode(list,childs.btnBox,boxCanOpen, node:getName()..k.."vibrate")
							text.addEffect(childs.btnBox:get("txt"), {outline = {color = ui.NEWCOLORS.OUTLINE.BLACK}})
							if not v.unLock then
								...
								cache.setShader(childs.cardbg, false, v.level < v.id and "hsl_gray" or "normal")
		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					elseif matchLanguage({"en","vn","ma","ph","in","th"}) then
						adapt.oneLinePos(childs.desc, childs.desc1, cc.p(-5, 0))
						adapt.oneLinePos(childs.desc1, childs.desc, cc.p(10, 0), "right")
					end
					if v.level then
						...
						text.addEffect(childs.flag, {color = state and ui.NEWCOLORS.GRAY or ui.NEWCOLORS.RED1})
					...
					text.addEffect(childs.btnUp:get("txt"), {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 3}})
					bind.touch(list, childs.btnUp, {methods = {
...
function TrainerView:onCreate()
	...
	local jinjiedownEffect = widget.addAnimation(self.panelup, "xunlianshi1/xunblianshi1.skel", "jinjiedown_loop", 100)
