...
function TrainSkillUpgrade:onCreate(data)
	...
	idlereasy.when(self.items, function (_, val)
		...
		text.addEffect(self.percent, {color = isEnough and ui.NEWCOLORS.GREENLIGHT or ui.NEWCOLORS.REDLIGHT})
		if isEnough then
			text.addEffect(self.txt, {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4}})
		else
