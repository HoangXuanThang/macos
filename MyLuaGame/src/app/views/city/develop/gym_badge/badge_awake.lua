	...
	idlereasy.any({self.items, self.badges}, function(_, items, badges)
		...
		for k, v in ipairs(dataEasy.getItemData(item)) do
			if v.key ~= "rmb" and v.key ~= "gold" then
				table.insert(costData, {key = v.key, id = v.key, num = items[v.key] or 0, targetNum = v.num})
			end
		end
		if self.isShowGuideGift then
			uiEasy.goGuideGift(costData)
			self.isShowGuideGift = false
		end
		self.item = costData
		...
		adapt.oneLineCenterPos(cc.p(163, 190), {self.btnPanel:get("text1"), self.btnPanel:get("text"), self.btnPanel:get("icon")}, cc.p(6, 0))
		...
		if self.canAwake < 6 then
			...
			--self.btnPanel:get("txt"):color(ui.NEWCOLORS.RED1)
			text.addEffect(self.btnPanel:get("txt"), {color = ui.NEWCOLORS.RED1})
		else
			self.btnPanel:get("txt"):text(string.format(gLanguageCsv.badgeNeedToUp, self.preTalentLevel))
			text.addEffect(self.btnPanel:get("txt"), {color = ui.NEWCOLORS.GREENLIGHT})
			--self.btnPanel:get("txt"):color(ui.NEWCOLORS.GREENLIGHT)
		end
		...
		str = string.gsub(str, "#C0x5b545b#", "#C0x8A5B0C#")
		str = string.gsub(str, "#C0x5C9970#", "#C0x01a001#")
		...
		adapt.oneLineCenterPos(cc.p(310, 120), {self.leftPanel:get("name"), self.leftPanel:get("nameLv")}, cc.p(6, 0))
		self.awake = awake
	end)
