...
CardGuardFilterView.RESOURCE_BINDING = {
	...
	["filterBtn.btn.title"] = {
		...
		binds = {
			...
			data = { color = ui.COLORS.NORMAL.WHITE, outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 2 } },
		}
	},
	...
	["filterBtn.attrListPanel.allBtn.title"] = {
		binds = {
			event = "effect",
			data = { color = ui.COLORS.NORMAL.WHITE, outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 3 } },
		}
	},
	["filterBtn.rarityListPanel.allBtn.title"] = {
		binds = {
			event = "effect",
			data = { color = ui.COLORS.NORMAL.WHITE, outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 3 } },
		}
...
function CardGuardFilterView:onCreate(params)
	...
	self.attr1 = idler.new(5)             --第一个属性
	self.attr2 = idler.new(5)             --第二个属性
	...
	local rarityDatas = {}
	for i = 1, 18 do
		table.insert(attrDatas, ui.ATTR_ICON[i])
	end
	for i = 1, 6 do
		table.insert(rarityDatas, ui.RARITY_DATAS[i])
	end
	self.attrDatas = arraytools.map(attrDatas, function(i, v) return { icon = v } end)
	self.rarityDatas = rarityDatas
...
function CardGuardFilterView:initData()
	self.attr1:set(19)            --第一个属性
	self.attr2:set(19)            --第二个属性
