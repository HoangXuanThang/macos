...
local BALLPOSITION = {
	{x = 110, y = 60, scale = 1.4, scaleBattom = 1.4},
	{x = 88, y = 40, scale = 1.3, scaleBattom = 1.3},
	{x = 75, y = 14, scale = 1.2, scaleBattom = 1.2},
	{x = 75, y = 20, scale = 1.2, scaleBattom = 1.2},
	{x = 88, y = 0, scale = 1.3, scaleBattom = 1.3},
	{x = 138, y = 60, scale = 1.4, scaleBattom = 1.4},
}
...
function gymBadgeTalentView:onCreate(params)
	...
	self:initModel()
	self.isShowGuideGift = idler.new(false)
			...
			for k, v in ipairs(dataEasy.getItemData(item)) do
				...
				if v.key ~= "rmb" and v.key ~= "gold" then
					...
					if index == #talentItem then
						table.insert(talentItem, {key = v.key, id = v.key, num = items[v.key] or 0, targetNum = v.num})
					end
				else
	...
	idlereasy.any({self.badges, self.badgeNumb}, function(_, badges, badgeNumb)
		...
		adapt.oneLineCenterPos(cc.p(110, 23), {self.talentBottom1:get("txt"), self.talentBottom1:get("num")}, cc.p(0, 0))
		adapt.oneLineCenterPos(cc.p(90, 2), {self.talentBottom2:get("txt"), self.talentBottom2:get("num")}, cc.p(0, 0))
		adapt.oneLineCenterPos(cc.p(75, -20), {self.talentBottom3:get("txt"), self.talentBottom3:get("num")}, cc.p(0, 0))
		adapt.oneLineCenterPos(cc.p(75, -13), {self.talentBottom4:get("txt"), self.talentBottom4:get("num")}, cc.p(0, 0))
		adapt.oneLineCenterPos(cc.p(90, -35), {self.talentBottom5:get("txt"), self.talentBottom5:get("num")}, cc.p(0, 0))
		adapt.oneLineCenterPos(cc.p(140, 23), {self.talentBottom6:get("txt"), self.talentBottom6:get("num")}, cc.p(0, 0))
		local tmpCardDatas = {}
		...
		adapt.oneLineCenterPos(cc.p(800, 719), {self.badgeName, self.list}, cc.p(10, 0))
	...
	idlereasy.when(self.badgeNumb, function(_, badgeNumb)
		...
		self.isShowGuideGift:set(false)
	end)
	--自己所查看的天賦球
	for i = 1, 6 do
	...
	idlereasy.any({self.items, self.nodeNum, self.myNum, self.gold, self.badgeNumb, self.badges, self.checkBoxState}, function(_, items, nodeNum, myNum, gold, badgeNumb, badges, checkBoxState)
		...
		if myNum == nodeNum then
			...
			if self.costGold > 0 then
				...
				local color = gold < self.costGold and ui.COLORS.NORMAL.RED or ui.COLORS.NORMAL.WHITE
			...
			text.addEffect(self.title, {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4}})
		else
			cache.setShader(self.lightBtn, false, "hsl_gray")
			text.deleteAllEffect(self.title)
		end
		self.guideItmsData = talentItem
	end)

	self.guideItmsData = {}
	idlereasy.when(self.isShowGuideGift, function(_, isShowGuideGift)
		if isShowGuideGift then
			uiEasy.goGuideGift(self.guideItmsData)
		end
	end)
...
function gymBadgeTalentView:onAwake(badgeNumb, event)
	if type(badgeNumb) == "number" then
		...
		if event.name == "began" then
			self.leftTop:getChildByName("badgeSpine"):scale(1.8)
		elseif event.name == "ended" then
			self.leftTop:getChildByName("badgeSpine"):scale(2)
