...
gymBadgeGuardView.RESOURCE_BINDING = {
	...
	["item.btn1.txt"] = {
		binds = {
			event = "effect",
			data = { color = ui.COLORS.NORMAL.WHITE, outline = { color = ui.NEWCOLORS.OUTLINE.BLUE, size = 4 } },
		}
	},
	["item.btn2.txt"] = {
		binds = {
		...
		binds = {
			...
			props = {
				...
				onCell = function(list, node, k, v)
					local childs = node:multiget("bgPanel", "textAttr", "icon1", "icon", "rarity", "btn1", "btn2", "add",
						"txt", "btnPanel")
					list:setScrollBarEnabled(false)
					node:setTouchEnabled(true)
					local skinAction = childs.bgPanel:get("skinAction")
					bind.touch(list, childs.bgPanel,
						{ methods = { ended = functools.partial(list.showDetail, node, list:getIdx(k).k, v) } })
					...
					if v.id ~= 0 then
						...
						adapt.oneLineCenterPos(cc.p(300, 133),
							{ childs.rarity, childs.textAttr, childs.icon, childs.icon1 }, cc.p(15, 0))
						bind.touch(list, childs.btn1,
							{ methods = { ended = functools.partial(list.chanceSetUp, list:getIdx(k).k, v) } })
						bind.touch(list, childs.btn2,
							{ methods = { ended = functools.partial(list.setUp, list:getIdx(k).k, v) } })
					else
						...
						if v.isOpen == true then
							...
							if v.isFee == true then
								widget.addAnimationByKey(childs.bgPanel, "daoguanhuizhang/jiesuo.skel", "suoEffect",
									"suo_loop", 1)
									:xy(190, 180)
									:scale(2)
								childs.btnPanel:show()
								bind.touch(list, childs.btnPanel:get("btn"),
									{ methods = { ended = functools.partial(list.openLock, node, list:getIdx(k).k, v) } })
								local txt = ""
								for k, v in ipairs(v.openCost) do
									if v.key == "rmb" then
										if v.bool then
											txt = txt .. "#C0xFFFFFF#" ..
												v.num .. " #Icommon/icon/icon_diamond.png-56-56# "
										else
											txt = txt .. "#C0xF13B54#" ..
												v.num .. " #Icommon/icon/icon_diamond.png-56-56# "
										end
									elseif v.key == "gold" then
										if v.bool then
											txt = txt .. "#C0xFFFFFF#" .. v.num .. "# Icommon/icon/icon_gold.png-56-56# "
										else
											txt = txt .. "#C0xF13B54#" .. v.num .. " #Icommon/icon/icon_gold.png-56-56# "
										end
									else
										if v.bool then
											txt = txt .. "#C0xFFFFFF#" .. v.num .. " #I" .. v.icon .. "-56-56# "
										else
											txt = txt .. "#C0xF13B54#" .. v.num .. " #I" .. v.icon .. "-56-56# "
										end
									end
								end
								local richText = rich.createWithWidth("#C0xFFFFFF#" .. txt, 40, nil, 1250)
									:addTo(childs.btnPanel, 10)
									:anchorPoint(cc.p(0, 0.5))
									:formatText()
								adapt.oneLineCenterPos(cc.p(722, 124), { childs.btnPanel:get("text1"), richText },
									cc.p(15, 0))
...
function gymBadgeGuardView:onCreate(badge)
	...
	idlereasy.any({ self.rmb, self.gold, self.badgeNumb, self.badges, self.items },
		function(_, rmb, gold, badgeNumb, badges, items)
							...
							if guardValue.openParam <= awake then
								...
								for k, v in ipairs(costItem) do
									...
									elseif v.key == "gold" then
										...
										table.insert(openCost,
											{
												key = v.key,
												bool = items[v.key] and items[v.key] >= v.num,
												num = v.num,
												icon =
													csvItems[v.key].icon
											})
...
function gymBadgeGuardView:onSetUp(list, key, val)
	gGameUI:stackUI("city.develop.gym_badge.guard_choose", nil, nil,
		{ key = val.guardId, badgeNumb = self.badgeNumb:read(), setType = 1 })
end

function gymBadgeGuardView:onShowDetail(list, node, key, val)
	if val.id ~= 0 then
		local view = gGameUI:stackUI("city.develop.gym_badge.guard_detail", nil,
			{ clickClose = true, blackLayer = false, dispatchNodes = self.list },
			{ selectDbId = val.selectDbId, badgeNumb = self.badgeNumb:read() })
		tip.adaptView(view, self,
			{ relativeNode = node:get("bgPanel"), canvasDir = "horizontal", childsName = { "baseNode" } })
	else
		if val.isOpen == true then
			gGameUI:stackUI("city.develop.gym_badge.guard_choose", nil, nil,
				{ key = val.guardId, badgeNumb = self.badgeNumb:read(), setType = 0 })
...
function gymBadgeGuardView:onChooseBadge(list, key, val)
	...
	if val.isOpen then
		...
		if preBadgeType == 1 then
			gGameUI:showTip(string.format(gLanguageCsv.noOpenPleaseToGymView1, csvBadge[preBadgeID].name,
				csvBadge[key].preLevel))
		else
			gGameUI:showTip(string.format(gLanguageCsv.noOpenPleaseToGymView2, csvBadge[preBadgeID].name,
				csvBadge[key].preLevel))
		end
	end
end
...
function gymBadgeGuardView:onOpenLock(list, node, key, val)
	...
	for k, v in ipairs(val.openCost) do
		if v.key == "rmb" then
			if v.bool then
				content = content .. "#C0x8A5B0C#" .. v.num .. " #Icommon/icon/icon_diamond.png-64-64# "
			else
				content = content .. "#C0xF13B54#" .. v.num .. " #Icommon/icon/icon_diamond.png-64-64# "
				uiEasy.showDialog("rmb", nil, { dialog = true })
				return
			end
		elseif v.key == "gold" then
			if v.bool then
				content = content .. "#C0x8A5B0C#" .. v.num .. "# Icommon/icon/icon_gold.png-56-56# "
			else
			...
			if v.bool then
				content = content .. "#C0x8A5B0C#" .. v.num .. " #I" .. v.icon .. "-56-56# "
			else
	...
	content = "#C0x57472E#" .. gLanguageCsv.consumptionOrNot .. " " ..
		content .. "#C0x57472E#" .. gLanguageCsv.openGuardsText
	gGameUI:showDialog({
		content = content,
		cb = function()
