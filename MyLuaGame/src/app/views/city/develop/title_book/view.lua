									...
									if v.id == id then
										...
										if v.data and v.data[1] ~= 0 then
											...
											list:schedule(function()
												...
												text.addEffect(list.rightPanelTip, { color = ui.NEWCOLORS.BROWN1 })
												if countdown <= 0 then
													list.timeLimit:hide()
													return false
												end
											...
											if v.data then
												strTyp = ""
												text.addEffect(list.timeLimit, { color = ui.NEWCOLORS.GREENLIGHT })
												text.addEffect(list.rightPanelTip, { color = ui.NEWCOLORS.BROWN1 })
											else
												text.addEffect(list.timeLimit, { color = ui.NEWCOLORS.GRAY1 })
												text.addEffect(list.rightPanelTip, { color = ui.NEWCOLORS.GRAY1 })
											end
											if titleCfg.days > 0 then
												list.timeLimit:text(string.format(gLanguageCsv.titleHaveTimeLimitDays,
													strTyp, titleCfg.days))
											else
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					if v.isSelected and v.isDropDown then
						...
						text.deleteAllEffect(childs.name)
...
function TitleBookView:onCreate()
	...
	idlereasy.when(self.checkState, function(_, state)
		self.checkStatus:texture(state and "common/icon/radio_normal1.png" or "common/icon/radio_selected1.png")
	...
	idlereasy.any({ self.id, self.titleId }, function(_, currId, titleId)
		...
		if currId == titleId then
			...
			text.addEffect(txt,
				{ outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4 }, color = ui.COLORS.NORMAL.WHITE })
...
function TitleBookView:onNodeClick(list, k, v)
	...
	for i = 1, 3 do
		...
		local titleTxt = (typ == 0) and gLanguageCsv.allSprite or
			string.format(gLanguageCsv.someSprite, gLanguageCsv[game.NATURE_TABLE[typ]])
