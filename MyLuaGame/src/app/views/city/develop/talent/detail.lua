...
function TalentDetailView:onCreate(params)
	...
	if self.isLock then
		...
		idlereasy.when(self.level, function (_, level)
			local color = level >= cfg.roleLevel and "C0x01A001" or "C0xF76B45"
		...
		idlereasy.any({self.talentTree, self.talentPoint, self.gold}, function (_, talentTree, talentPoint, gold)
			...
			self.Upgradelevel:setTextColor(level < cfg.levelUp and ui.COLORS.NORMAL.ALERT_ORANGE or ui.NEWCOLORS.BROWN1)
			...
			self.num1:setTextColor(costGold > gold and ui.COLORS.NORMAL.ALERT_ORANGE or ui.NEWCOLORS.BROWN1)
			self.num2:text(costTalent)
			self.talentPointEnough = costTalent <= talentPoint
			self.num2:setTextColor(costTalent > talentPoint and ui.COLORS.NORMAL.ALERT_ORANGE or ui.NEWCOLORS.BROWN1)
	...
	self.nodeName:setTextColor(ui.NEWCOLORS.BROWN1)
...
function TalentDetailView:getDesc(cfg, level)
	...
	local strs = {}
	local space = matchLanguage({"en","cn"}) and " " or ""
	local attrAddDesc1 = cfg["attrAddDesc1_"..LOCAL_LANGUAGE] or cfg.attrAddDesc1
	table.insert(strs, attrAddDesc1.. space .. self:getAttributeNum(cfg.attrType1, attrNum, level))
	if cfg.attrNum2 ~= "" then
		local attrAddDesc2 = cfg["attrAddDesc2_"..LOCAL_LANGUAGE] or cfg.attrAddDesc2
		table.insert(strs, attrAddDesc2 .. space .. self:getAttributeNum(cfg.attrType2, cfg.attrNum2, level))
	end
	local str = table.concat(strs, "\n")
	self.desc:text(str)
	self.desc:setTextColor(ui.COLORS.NORMAL.FRIEND_GREEN)
	self.descAttr:text(cfg["attrDesc_"..LOCAL_LANGUAGE] or cfg.attrDesc)
	self.descAttr:setTextColor(ui.NEWCOLORS.BROWN1)
end
