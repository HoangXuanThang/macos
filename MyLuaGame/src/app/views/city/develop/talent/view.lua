...
local TITLE_BG = {
	{bg = "city/develop/talent/logo_red.png", glow = cc.c4b(198,94,52,255)},
	{bg = "city/develop/talent/logo_purple.png", glow = cc.c4b(167,157,55,255)},
	{bg = "city/develop/talent/logo_green.png", glow = cc.c4b(111,123,205,255)},
}
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					if v.selected then
						normal:hide()
						panel = selected:show()
						panel:get("txt"):setTextColor(ui.NEWCOLORS.BLACK)
					else
						selected:hide()
						panel = normal:show()
						...
						panel:get("txt"):setTextColor(ui.NEWCOLORS.WHITE)
					end
			...
			props = {
				...
				onCell = function(list, node, k, v)
					...
					elseif v.tab == 4 then
						...
						text.addEffect(childs.title1:get("title"), {outline = {color = t.glow}})
					end
					commonItemShow(list, childs, v)
				end,
...
function TalentView:onCreate()
	...
	self.showTab:addListener(function(val, oldval, idler)
		...
		--改成富文本
		local desc = rich.createWithWidth(string.format("#C0xFFFFFFFF#%s", self.leftDatas:atproxy(val).cfg.des), 40, nil, richWidth, nil, cc.p(0, 0.5))
...
function TalentView:refreshShowData()
	...
	self.cost1:setTextColor(isLock and ui.COLORS.NORMAL.ALERT_ORANGE or ui.COLORS.NORMAL.WHITE)
...
function TalentView:initRootNode(id, level, lineState)
	...
	childs.center:show()
	if #lineState == 2 then
		childs.center:size(160, 43)
		childs.center:x(386)
	else
		childs.center:size(252, 43)
		childs.center:x(486)
	end
