...
ExploreView.RESOURCE_BINDING = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					text.addEffect(childs.name,
						{
							outline = { color = ui.COLORS.QUALITY[v.cfg.quality], size = 4 },
					...
					local props = {
						...
						props = {
							...
							onNode = function(node)
								--node:xy(panel:size().width +220,panel:size().height - 5)
							end
						}
					}
		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					childs.name:text(getLanguageAttr(v.attrType))
					local nameLen = string.utf8len(getLanguageAttr(v.attrType))
					local fontSize = nameLen > 5 and math.max(35, 40 - 5 * (nameLen - 5)) or 40
	...
	["btnSoul"] = {
		...
		binds = {
			event = "touch",
			methods = { ended = bindHelper.self("onSoulClick") }
		},
	},
}
...
function ExploreView:onCreate(drawType)
	...
	self:initModel()
	self.isShowGuideGift = false
	self.isBtnSoulShow = false 

	-- 激活/进阶特效
	self.kejihuoEffect = widget.addAnimationByKey(self.btnEffect, "jinjie/kejinjiekejiesuo.skel", 'kejihuoEffect',
			"kejihuo_loop", 99)
	...
	self.showTab:addListener(function(val, oldval)
		...
		if string.find(proxy.cfg.res, "skel") then
			...
			local spine = widget.addAnimationByKey(self.upLeftPanel, proxy.cfg.res, "explorer", "default", 3, true)
			spine:xy(845, 550):scale(1.8)
		else
			self.icon:show()
		...
		adapt.setTextAdaptWithSize(self.activeTip,
			{ size = cc.size(650, 80), vertical = "bottom", horizontal = "center" })
		local isShow = false
		...
		self.btnSoul:hide()
		-- local showSoul = proxy.cfg.quality >= 5 and not isNotActive
		-- self.btnSoul:visible(showSoul)
		-- self.isBtnSoulShow = showSoul
	end)
				...
				ended = function()
					if v.count > 0 and v.level == 0 then
						gGameApp:requestServer("/game/explorer/component/strength", function(tb)
							gGameUI:stackUI("city.develop.explorer.component_success", nil,
								{ blackLayer = true, clickClose = true }, v)
						end, v.id)
					else
						self.isShowGuideGift = false
...
function ExploreView:componentDetailShow(index, componentData, explorer, advance)
	...
	if componentData.level == 0 then
		...
		if componentData.count == 0 then
			...
			childs.btnPath:show()
			text.addEffect(childs.btnPath:get("txt"), { outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4 } })
		end
	elseif componentData.level == explorer.levelMax then
		...
		adapt.oneLineCenterPos(cc.p(self.upgradePanel:width() / 2, childs.name:y()), { childs.name, childs.level },
			cc.p(20, 0))
		local richtext = rich.createByStr(newStr1, 40, nil, nil, cc.p(0, 0.5))
		richtext:setVerticalAlignment(cc.VERTICAL_TEXT_ALIGNMENT_CENTER)
		richtext:addTo(childs.txtPos)
		...
		if self.isShowGuideGift and (not isCanUp) then
			uiEasy.goGuideGift(t)
			self.isShowGuideGift = false
		end
		...
		text.addEffect(childs.btnPath:get("txt"),
			{ color = isCanUp and ui.COLORS.NORMAL.WHITE or ui.COLORS.NORMAL.WHITE })
			...
			methods = {
				ended = function()
					if isCanUp then
						gGameApp:requestServer("/game/explorer/component/strength", function(tb)
							local data = self.bottomData:atproxy(self.showTab:read())
							self.isShowGuideGift = true
							self:componentDetailShow(index, data.components[index], data.cfg, advance)
							gGameUI:stackUI("city.develop.explorer.component_success", nil,
								{ blackLayer = true, clickClose = true }, data.components[index])
...
function ExploreView:initComponent(pos, data, levelMax, advance)
	...
	if not labelLv then
		...
		text.addEffect(labelLv, { outline = { color = ui.NEWCOLORS.OUTLINE.BLACK } })
	end
	...
	if not txtLvNum then
		...
		text.addEffect(txtLvNum, { outline = { color = ui.NEWCOLORS.OUTLINE.BLACK } })
	end
...
function ExploreView:onSoulClick()
	gGameUI:stackUI("city.develop.explorer.ability.view", nil, nil, self:createHandler("detailSendParams"))
end
