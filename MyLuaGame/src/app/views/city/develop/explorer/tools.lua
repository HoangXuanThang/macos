...
function ExplorerTools.effectShow(mainList, extraList, data)
	...
	for k,v in pairs(data.cfg.effect) do
		...
		local str
		local effectDesc = effect["effectDesc_"..LOCAL_LANGUAGE] or effect.effectDesc
		if effect.skillID then
			...
			local currStr = string.format(effectDesc, eval.doMixedFormula(describe, {skillLevel = skillLevel,math = math}, nil))
			str = "#C0x57472e#" .. currStr
		else
			if effect.attrType1 and effect.attrType2 and effect.attrType3 and effect.attrType4 and effect.attrType5 and effect.attrType6 then
				...
				str = string.format(effectDesc, attrNum)
			else
				local params = {}
				for i = 1, 6 do
				...
				str = string.format(effectDesc, unpack(params))
			end
		end
	...
	for i,v in ipairs(data.cfg.extraEff) do
		...
		if effect.skillID then
			local skillCsv = csv.skill[effect.skillID]
			local describe = skillCsv["describe_"..LOCAL_LANGUAGE] or skillCsv.describe
			local currStr = string.format(str, eval.doMixedFormula(describe, {skillLevel =  skillLevel,math = math}, nil))
			...
			local str = string.format(effectDesc, unpack(params))
