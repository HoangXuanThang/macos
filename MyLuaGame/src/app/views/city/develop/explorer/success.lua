...
function ExplorerSuccessView:onCreate(dbHandler)
	...
	for k,v in pairs(self.data.cfg.effect) do
		...
		if effect.skillID then
			...
			str = "#C0xFFFFFF#" .. currStr
			str = string.gsub(str, "57472e", "FFFFFF")
		else
	...
	beauty.textScroll({
		list = self.topList,
		effect = {color= ui.COLORS.NORMAL.WHITE},
		strs = mainStrs,
		isRich = true,
		margin = 20,
		align = "left",
	...
	if index then
		...
		if effect.skillID then
			local skillCsv =csv.skill[effect.skillID]
			local str = "#C0xFFFFFF#" .. eval.doMixedFormula(skillCsv.describe, {skillLevel = skillLevel,math = math}, nil)
			str = string.gsub(str, "57472e", "FFFFFF")
			table.insert(strs, {str = "#C0xFFFFFF#" .. str})
		else
			local params = {}
			for i = 1, 6 do
				if effect["attrType"..i] and effect["attrNum"..i] then
