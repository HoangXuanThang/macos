...
local function onInitItem(list, node, k, v)
	...
	children.textNote1:text(v.cfg["title_"..LOCAL_LANGUAGE] or v.cfg.title)
	local desc = v.cfg["desc_"..LOCAL_LANGUAGE] or v.cfg.desc
	...
	local descRichText, height = beauty.textScroll({
		...
		effect = {color = ui.NEWCOLORS.BROWN1}
	})
	local y = 15
	if height < 110 then
		y = 15 - (110 - height) / 2
	end
	descRichText:xy(children.textNote2:x(), y)
	...
	local color = ui.NEWCOLORS.GREENLIGHT
	if v.flag == TASKSTATE.DOING then
		color = ui.NEWCOLORS.RED1
	end
	text.addEffect(children.textPro, {color = color})

	--text.addEffect(children.btnGo.textNote, {outline = {color=ui.NEWCOLORS.OUTLINE.BLUE, size=4}})
...
achievementView.RESOURCE_BINDING = {
	...
	["rightAll.btnRank.textNote"] = {
		binds = {
			event = "effect",
			data = {outline = {color = ui.NEWCOLORS.OUTLINE.BLUE, size = 3}}
		},
	},
	["item"] = "item",
	["item.btnGo.textNote"] = {
...
function achievementView:onCreate(idx)
	...
	local itemAdaptPos1 = self.item:multiget("imgIcon", "textCount", "textNote1", "textNote2","Image_17")
	local itemAdaptPos2 = self.item:multiget("list", "btnGet", "btnGo", "textPro", "imgGot","Image_18")
	adapt.centerWithScreen("left", "right", nil, {
		{self.leftPanel, "pos", "left"},
		{self.leftBG, "pos", "left"},
...
function achievementView:addAllAchievementEffect(curType, curLv, allListPageIdx)
	...
	text.addEffect(self.btnTextNote1, {color = color1,outline = {color = ui.NEWCOLORS.OUTLINE.BLUE, size = 3}})
	text.addEffect(self.btnTextNote2, {color = color2,outline = {color = ui.NEWCOLORS.OUTLINE.BLUE, size = 3}})
	text.addEffect(targetNode, {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 3}})
