...
local CHANNELS = {
	news = {name = gLanguageCsv.system, color = cc.c4b(238, 115, 143, 255),outlinecolor = cc.c4b(96, 18, 12, 255)},
	world = {name = gLanguageCsv.world, color = cc.c4b(139, 175, 223, 255),outlinecolor = cc.c4b(14, 42, 79, 255)},
	union = {name = gLanguageCsv.guild, color = cc.c4b(204, 143, 223, 255),outlinecolor = cc.c4b(107, 19, 134, 255)},
	team = {name = gLanguageCsv.formTeam, color = cc.c4b(236, 183, 43, 255),outlinecolor = cc.c4b(89, 56, 5, 255)},
	huodong = {name = gLanguageCsv.activity, color = cc.c4b(255, 94, 66, 255),outlinecolor = cc.c4b(110, 20, 4, 255)},
	private = {name = gLanguageCsv.privateChat, color = cc.c4b(204, 143, 223, 255),outlinecolor = cc.c4b(107, 19, 134, 255)},
	cross = {name = gLanguageCsv.crossChat, color = cc.c4b(204, 143, 223, 255),outlinecolor = cc.c4b(107, 19, 134, 255)},
}
local CHAT_PAGE_IDX = {
	news = 1,
	world = 2,
...
local function getLeftBtnsData()
	return {
		{
			key = "signIn",
			icon = "city/new_main/qiandao.png",
		...
		},
		{
			key = "rank",
			unlockKey = "rank",
			icon = "city/new_main/paihang.png",
		...
		},
		{
			key = "union",
			unlockKey = "union",
			icon = "city/new_main/gonghui.png",
...
require("app.views.city.view_scene")(CityView)
require("app.views.city.view_action")(CityView)
require("app.views.city.view_activity")(CityView)
...
CityView.RESOURCE_BINDING = {
	...
	["leftTopPanel.bg.name"] = "nameTxt",
	["leftTopPanel.bg.level"] = "levelTxt",
	["leftTopPanel.bg.vip"] = "roleVip",
	["rightPanel.leftBtn.txt"] = {
		binds = {
			event = "effect",
			data = {color = ui.COLORS.NORMAL.WHITE, outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 3}},
		},
	},

	["leftTopPanel.bg.vip.vip"] = {
		varname = "vipLabel",
		binds = {
			event = "effect",
	...
	["leftPanel.list"] = {
		...
		binds = {
			...
			props = {
				...
				item = bindHelper.self("item_leftList"),
				margin = 0,
				onItem = function(list, node, k, v)
					...
					local name = label.create(v.name, {
						color = ui.NEWCOLORS.WHITE,
						fontSize = 42,
						fontPath = "font/youmi.ttf",
					})
					text.addEffect(name, {outline = {color=ui.NEWCOLORS.OUTLINE.BLACK, size=4}})
	...
	["leftBottomPanel.panel.btnFriend"] = {
		varname = "btnFriend",
		binds = {
			{
				event = "touch",
				methods = {ended = bindHelper.defer(function(view)
					return view:onItemClick(nil, view.leftBottomBtns[4])
				end)}
			},
			{
				event = "extend",
		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					text.addEffect(childs.labelTime, {outline={color=ui.NEWCOLORS.OUTLINE.BLACK, size=3}})
		...
		binds = {
			{
				...
				clicksafe = true,
				methods = {ended = bindHelper.defer(function(view)
					gGameUI:stackUI("city.activity.guide_gift")
				end)}
			}
		},
	},
...
function CityView:onCreate()
	...
	local grIcon = ccui.ImageView:create("login/icon_groupfb.png")
    grIcon:setTouchEnabled(true)
    grIcon:setAnchorPoint(cc.p(0.5, 0.5))
	grIcon:setScale(0.8)

	local winSize = display.size  -- hoặc cc.Director:getInstance():getWinSize()
	local iconHalfWidth = grIcon:getContentSize().width * grIcon:getScale() / 2
    local posX = display.board_left + iconHalfWidth + 50 -- cách trái 20px
    local posY = winSize.height / 2 - 150 
    grIcon:setPosition(cc.p(posX , posY))
    self:addChild(grIcon, 1)
	grIcon:addClickEventListener(function()
	...
	self.roleVip:visible(false)
	idlereasy.when(self.vipLevel, function(_, vipLevel)
		if vipLevel > 0 then
			self.roleVip:visible(true)
			self.vipLabel:text("V" .. vipLevel)
		else
			self.roleVip:visible(false)
		end
	end)

	idlereasy.any({self.level, self.roleName,self.levelExp}, function (obj, level, name,levelExp)
	...
	self.autoWeeklyCardFrId = idler.new(0)
	self.autoLimitDayGift = idler.new(0)
	...
	self:initAutoWeekCardFR()
	self:initAutoLimitDayGift()
	...
	idlereasy.any({self.level, self.isRefresh}, function(_, level, isRefresh)
		...
		local function isOK(v)
			...
			if v.unionlevel then
				...
				if not unionId or not unionLv or unionLv < v.unionlevel then
					return false
				end
			end
			--判断开服天数
	...
	if device.platform == "windows" then
		...
		for _, card in gGameModel.cards:pairs() do
			...
			if not csv.cards[cardId] then
				gGameUI:showDialog({content = string.format("Dữ liệu chứa thẻ%d chưa mở, kiểm tra xem ngôn ngữ cục bộ có language phù hợp với máy chủ không", cardId)})
				break
			end
		end
	end
...
function CityView:onItemClick(list, v)
	...
	if v.func then
		v.func(function(...)
			local params = {}
			if v.independent == 4 or  v.independent == 5 or v.independent == 6 then
				if v.independent == 4 or v.independent == 6 then
...
function CityView:initTalkPanel()
	self.talkList:setScrollBarEnabled(false)
		...
		if messages and #messages > 0 then
			for i=#messages, 1, -1 do
				if messages[i].channel ~= "private" then
					...
					text.addEffect(childs.textChannel, {color = CHANNELS[message.channel].color,outline = {color =  CHANNELS[message.channel].outlinecolor, size = 4}})
...
-- 首充
function CityView:initAutoWeekCardFR()
	local weeklyCardFr = gGameModel.currday_dispatch:getIdlerOrigin("weeklyCardFR"):read()
	if weeklyCardFr == true then
		return
	end
	idlereasy.when(self.autoWeeklyCardFrId, function(_, id)
		if id > 0 then
			table.insert(self.autoPopBoxInfo, {
				viewName = "city.activity.weekly_card_fr",
				params = {},
			})
			self.autoWeeklyCardFrId:set(-1)
		end
	end)
end

function CityView:initAutoLimitDayGift()
	local popLimitDayGift = userDefault.getCurrDayKey("popLimitDayGift", false)
	if popLimitDayGift == true then
		return
	end
	idlereasy.when(self.autoLimitDayGift, function(_, id)
		if id > 0 then
			table.insert(self.autoPopBoxInfo, {
...
-- 引导礼包
function CityView:initGiftPanel()
	idlereasy.when(self.guideGifts, function(_, guideGifts)
		local function updateGiftTime()
			local giftData
			local endTime = 0
			local nowTime = time.getTime()
			if guideGifts == nil then
				return
			end
			for i, v in pairs(guideGifts) do
				giftData = csv.yunying.guidegift[i]
				if not v.flag or v.flag == 0 then
					if v.expired_date > nowTime then
						if endTime == 0 then
							endTime = v.expired_date
						else
							endTime = math.min(endTime, v.expired_date)
						end
					end
				end
			...
			if endTime <= nowTime then
				...
				self.giftGuide:show()
				local timeLabel = self.giftGuide:get("textNote2")
				timeLabel:show()
				CityView.setCountdown(self, timeLabel, {endTime = endTime, tag = 13, cb = updateGiftTime})

				widget.addAnimationByKey(self.giftGuide, "effect/xiangdao.skel", "getEffect", "effect_loop", 3)
					:xy(-1078, 143)
			end
		end
	...
	adapt.oneLinePos(self.growGuide, self.giftGuide, cc.p(20, 0), "right")
