				...
				onItem = function(childList, item, k, v)
					idlereasy.when(list.showTab, function(_, showTab)
						if not tolua.isnull(item) then
							...
							bind.touch(childList, normal,
								{ methods = { ended = functools.partial(list.clickChildCell, v.tabIndex, v) } })
						end
...
local function initShopItemOne(list, node, k, v)
	...
	if item then
		...
		if matchLanguage({ "en", "kr" }) then
			adapt.setTextAdaptWithSize(childs.name,
				{ size = cc.size(node:width() - 150, 120), vertical = "center", horizontal = "center", margin = -5, maxLine = 2 })
		else
	...
	if not lockShow and shopDetail and shopDetail.fishingLevelRange then
		...
		if fishLevel < shopDetail.fishingLevelRange[1] or fishLevel > shopDetail.fishingLevelRange[2] then
			lockShow = true
			lockShowStr = gLanguageCsv.angling ..
				string.format(gLanguageCsv.reachLevelCanBuy, shopDetail.fishingLevelRange[1])
		end
	end
	...
	elseif shopDetail.exchangeLimit then
		...
		if shopDetail.exchangeLimit == -1 then
			...
			local color = currTime >= 1 and "#C0x5B545B#" or "#C0x5B545B#"
			local str
...
local function initShopItemTwo(list, node, k, v)
	...
	local rect    = cc.rect((spSize.width - ssize.width) / 2 - soff.x, (spSize.height - ssize.height) / 2 - soff.y,
	...
	buys.icon:texture(dataEasy.getIconResByKey(key))
	buys.txt:text(val)
	text.addEffect(buys.txt, { outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4 } })
	local iconSize = buys.icon:getBoundingBox()
	local txtSize = buys.txt:size()
	buys.icon:x(childs.btnBuy:width() / 2 - iconSize.width - txtSize.width / 2 + 20)
	adapt.oneLinePos(buys.icon, buys.txt, cc.p(10, 0), "left")
...
ShopView.RESOURCE_BINDING = {
		...
		binds = {
			{
				...
				props = {
					...
					onItem = function(list, node, k, v)
						...
						panel:scale(1)
						local maxWidth = panel:size().width - 10
						adapt.setTextScaleWithWidth(panel:get("txt"), v.name, maxWidth)			

						selected:setTouchEnabled(false)
						setUnlockIcon(v.isLocked and not v.select, normal, { pos = cc.p(320, 110) })
	...
	--["leftPanel.itemParent.normal.txt"] = {
	--	binds = {
	--		event = "effect",
		...
		binds = {
			{
				...
				props = {
					...
					onItem = function(list, node, k, v)
						...
						if v.select == true then
							bind.touch(list, node:get("selected"),
								{ methods = { ended = functools.partial(list.clickCell, k, v) } })
						else
							bind.touch(list, node:get("normal"),
								{ methods = { ended = functools.partial(list.clickCell, k, v) } })
						end
						adapt.setTextScaleWithWidth(node:get("selected"):get("txt"), v.name,
							node:get("selected"):width() - 20)
						adapt.setTextScaleWithWidth(node:get("normal"):get("txt"), v.name,
							node:get("normal"):width() - 20)
						list:forceDoLayout()
					end,
				},
				handlers = {
			...
			props = {
				...
				onAfterBuild = function(list)
					...
					if #list.data <= 2 * list.columnSize then
						...
						list:setScrollBarColor(cc.c3b(245, 156, 2))
	...
	["rightPanel.bottomPanel.refreshTime"] = {
		varname = "refreshTime",
		binds = {
			event = "effect",
	...
	["rightPanel.bottomPanel.btn.txt"] = {
		binds = {
			event = "effect",
...
function ShopView:initIndexModel(index)
	...
	if isPvpShop(index) then
		local datas = {
			...
			[SHOP_INIT.HELD_SHOP] = {
				shop = self.heldShop,
				shopItems = self.heldShopItems,
				title = "held_item_shop",
			},
		}

		idlereasy.any({ datas[index].shop, datas[index].shopItems, self.shopLimit, self.vipLevel, self.level },
			function(obj, data, items, shopLimit, vipLevel, level)
						...
						if data[csvID] then
							...
							if refData.lastRecoverTime then
								...
								if currTotalTime >= cfg.exchangeLimit then
									...
									refData.timeStr = time.getCutDown(math.abs(math.floor((times + 1) * cfgTime - time1)))
										.str
								end
	...
	elseif index == SHOP_INIT.EQUIP_SHOP then
		gGameApp:requestServer(SHOP_GET_PROTOL[index], function()
			...
			idlereasy.any({ shopItems, self.showTab, shopBuy, self.shopLimit, self.vipLevel, self.level },
				function(obj, items, showTab, buyDict, shopLimit, vip, level)
...
function ShopView:initIdler(index)
	...
	idlereasy.any({ shopItems, self.showTab, shopBuy, self.shopLimit, self.vipLevel, self.level, self.fishLevel },
		function(obj, items, showTab, buyDict, shopLimit, vip, level)
	...
	idlereasy.any({ idlerData.refreshShopTime, self.vipLevel, self.showTab, self.items },
		function(obj, times, vipLevel, showTab, items)
			...
			if showTab == SHOP_INIT.EQUIP_SHOP or showTab == SHOP_INIT.ONLINE_FIGHT_SHOP then
				...
				text.addEffect(self.textTimes,
					{ color = leftTimes == 0 and ui.COLORS.NORMAL.ALERT_ORANGE or ui.COLORS.QUALITY[2], outline = { color = ui.NEWCOLORS.OUTLINE.BLACK, size = 2 } })
				...
				adapt.oneLinePos(self.btnRefresh,
					{ self.txt, self.textTimes, self.txt2, self.bottomCost, self.fixDiamond, self.txt3 },
					{ cc.p(5, 0), cc.p(5, 0), cc.p(5, 0), cc.p(30, 0), cc.p(0, 0), cc.p(0, 0) }, "right")
				adapt.oneLinePos(self.refreshTime, { self.txt4, self.icon1, self.numTxt, self.btn },
					{ cc.p(30, 0), cc.p(0, 0), cc.p(0, 0), cc.p(20, 0) }, "left")
			end
		end):anonyOnly(self)
end
...
function ShopView:onCreate(pageIdx)
	...
	adapt.centerWithScreen("left", "right", nil, {
		...
		{ self.list, "width", function(width)
			...
			adapt.oneLinePos(self.btnRefresh,
				{ self.txt, self.textTimes, self.txt2, self.bottomCost, self.fixDiamond, self.txt3, self.btn, self
					.numTxt, self.icon1, self.txt4 },
				{ cc.p(15, 0), cc.p(15, 0), cc.p(25, 0), cc.p(50, 0), cc.p(0, 0), cc.p(0, 0), cc.p(20, 0), cc.p(0, 0), cc
					.p(0, 0), cc.p(0, 0) }, "right")
	...
	self.crossCraftShopItems = idlertable.new(getShopItems(csv.cross.craft.shop, SHOP_INIT.CROSS_CRAFT_SHOP,
		self.specialTimeCrossCraft))
	self.crossArenaShopItems = idlertable.new(getShopItems(csv.cross.arena.shop, SHOP_INIT.CROSS_ARENA_SHOP,
		self.specialTimeCrossArena))
	self.onlineFightShopItems = idlertable.new(getShopItems(csv.cross.online_fight.shop, SHOP_INIT.ONLINE_FIGHT_SHOP,
		self.specialTimeOnlineFight))
	self.crossMineShopItems = idlertable.new(getShopItems(csv.cross.mine.shop, SHOP_INIT.CROSS_MINE_SHOP,
		self.specialTimeCrossMine))
	self.huntingShopItems = idlertable.new(getShopItems(csv.cross.hunting.shop, SHOP_INIT.HUNTING_SHOP,
		self.specialTimeHunting))

	-- self.heldShopItems = idlertable.new(getShopItems(csv.held_item.shop, SHOP_INIT.HELD_SHOP,
	-- 	self.heldShopTime))
	--src che a
	...
	local leftDatas = {
		...
		--[17] = { tabIndex = 17, unlockKey = "heldShop", name = gLanguageCsv.heldShop, subName = "held", show = true, topui = "held" },
	}
	--保存解锁状态
	...
	self.showTab:addListener(function(val, oldval, idler)
		...
		if self.leftDatas:atproxy(val).show then
			...
			self.topView = gGameUI.topuiManager:createView(self.leftDatas:atproxy(val).topui, self,
	...
	performWithDelay(self, function()
		self:enableSchedule():schedule(function()
			...
			refreshShopItem(self.specialTimeCrossCraft, self.crossCraftShopItems, csv.cross.craft.shop,
				SHOP_INIT.CROSS_CRAFT_SHOP)
			refreshShopItem(self.specialTimeCrossArena, self.crossArenaShopItems, csv.cross.arena.shop,
				SHOP_INIT.CROSS_ARENA_SHOP)
			refreshShopItem(self.specialTimeCrossMine, self.crossMineShopItems, csv.cross.mine.shop,
				SHOP_INIT.CROSS_MINE_SHOP)
			refreshShopItem(self.specialTimeHunting, self.huntingShopItems, csv.cross.hunting.shop,
				SHOP_INIT.HUNTING_SHOP)
			-- refreshShopItem(self.heldShopTime, self.heldShopItems, csv.held_item.shop,
			-- 	SHOP_INIT.HELD_SHOP)
...
function ShopView:initModel()
	...
	self.heldShop = gGameModel.role:getIdler("held_item_shop")
	...
	self.idlerDatas = {
		...
		[3] = {
			...
			allNum = gVipCsv[curVipLevel].fragShopRefreshLimit +
				(MonthCardView.getPrivilegeAddition("fragShopRefreshLimit") or 0),
		},
		[5] = {
...
function ShopView:onItemClick(list, t, v)
	...
	-- 精选商店折扣计算(其它商店目前暂无折扣)
	local discount = (v.type == SHOP_INIT.FIX_SHOP) and
		(1 - (MonthCardView.getPrivilegeAddition("fixShopDiscount") or 0)) or 1
	gGameUI:stackUI("common.buy_info", nil, nil, shopDetail.costMap, { id = data.itemID, num = self.num },
		{ maxNum = maxNum, discount = discount, contentType = contentType }, self:createHandler("showBuyInfo"))
end
