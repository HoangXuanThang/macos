...
local function getActivityInfoData(self)
	...
	return {
		...
		-- 新首充
		[YY_TYPE.weeklyCardFR] = {
			viewName = "city.activity.weekly_card_fr",
			sortWeight = -9,
			redHint = {
				class = "red_hint",
				props = {
					specialTag = "activityWeeklyCardFR",
				}
			}
		},
		--开服嘉年华
		...
		--周卡 独立图标和活动界面都显示f
		[YY_TYPE.weeklyCard] = {
			viewName = "city.activity.weekly_view",
			styles = { full = true },
			func = function(cb, params)
				gGameApp:requestServer("/game/yy/active/get", function(tb)
					cb("main")
		...
		-- 永久月卡
		[YY_TYPE.monthlyCard] = {
			viewName = "city.activity.supreme_card",
			redHint = {
				class = "red_hint",
				props = {
					specialTag = "supremeCard",
				}
			}
		},
		-- 尊享限定
		...
		-- 限定神兽
		[YY_TYPE.limitUpgradeSprite] = {
			viewName = "city.activity.star_upgrade.view",
			styles = { full = false },
			func = defaultFunc,
			redHint = {
				class = "red_hint",
				props = {
					specialTag = "limitUpgradeSprite",
				},
			},
		},
		-- 本命神兽
		[YY_TYPE.customizeUpgradeSprite] = {
			viewName = "city.activity.star_upgrade.view",
			styles = { full = false },
			func = defaultFunc,
			redHint = {
				class = "red_hint",
				props = {
					specialTag = "customizeUpgradeSprite",
				},
			},
		},
		-- 圣诞祈愿
		[YY_TYPE.christmasWish] = {
			viewName = "city.activity.christmas.view",
			styles = { full = false },
			func = defaultFunc,
			redHint = {
				class = "red_hint",
				props = {
					specialTag = {
						"activityDrawBoxAward",
						"activityChristmasWishGift",
						"activityChristmasWish",
					},
					listenData = {
		...
		[YY_TYPE.themeSkin] = {
			viewName = "city.activity.theme_skin.view",
			styles = { full = false },
			func = defaultFunc,
			redHint = {
				class = "red_hint",
				props = {
					specialTag = {
						"activityThemeSkinSignIn",
						"activityThemeSkinGift",
						"activityThemeSkinGive",
						"activityThemeSkinExchange",
					},
				},
			},
		},
		[YY_TYPE.zhutiYingxiong] = {
			viewName = "city.activity.zhuti_yingxiong.view",
			func = defaultFunc,
			tag = true,
			redHint = {
				class = "red_hint",
				props = {
					specialTag = {
						"activityDrawBoxAward",
						"activityZhutiYingxiongShop",
						"playPassportZhutiYingxiong",
						"activityZhutiYingxiongSignIn"
					},
					listenData = {
						drawBoxType = game.DRAW_BOX_TYPE.ZHUTI_YINGXIONG
					}
				},
			},
		},
		[YY_TYPE.jiuWei] = {
			viewName = "city.activity.jiu_wei.view",
			func = defaultFunc,
			tag = true,
			redHint = {
				class = "red_hint",
				props = {
					specialTag = {
						"activityJiuWeiSignIn",
						"activityJiuWeiGift",
						"activityJiuWeiExchange",
						"playPassportJiuWei",
					},
				},
			},
		},
		[YY_TYPE.monopolyWalk] = {
			viewName = "city.activity.monopoly.view",
			func = function(cb, params)
				gGameApp:requestServer("/game/yy/monopoly/main",function (tb)
					cb()
				end, params[1])
			end,
			tag = true,
		...
		-- 登天之阶
		[YY_TYPE.throneDraw] = {
			viewName = "city.activity.throne_draw.view",
			func = defaultFunc,
			redHint = {
				class = "red_hint",
				props = {
		...
		-- 通灵契约
		[YY_TYPE.drawLimitExplor] = {
			viewName = "city.activity.draw_active.explor",
			func = defaultFunc,
			redHint = {
				class = "red_hint",
				props = {
					specialTag = "activityDrawLimitExplor",
				},
			},
		},
		-- 超凡宝库
		[YY_TYPE.drawLimitHelditem] = {
			viewName = "city.activity.draw_active.helditem",
			func = defaultFunc,
			redHint = {
				class = "red_hint",
				props = {
					specialTag = "activityDrawLimitHelditem",
				},
			},
		},
		[YY_TYPE.limitDayGift] = {
			-- 限时礼包
			viewName = "city.activity.limit_day_gift",
			func = defaultFunc,
			timeLabelFunc = function(yyId, huodong)
				local cfg = csv.yunying.yyhuodong[yyId]
				local stamps2 = huodong.stamps2 or {}
				local endTime = stamps2[yyId] + cfg.paramMap.countdown * 60
			...
			redHint = {
				...
				props = {
					...
					listenData = {
						sign = game.DAILY_REDHINT.limitDayGift,
					},
				},
			},
		},
...
function CityView:initActivityData()
	...
	self.limitGiftDatas = idlereasy.new({}) -- 限时礼包

	-- 活动列表刷新标识
	self.updateActivity = idler.new(0)

	-- 弹出限时礼包
	self.limitGiftDatas:addListener(function(curval, oldval)
		if not curval.valinfo then
			return
		end

		for i, v in pairs(curval.valinfo) do
			if oldval.valinfo and (not oldval.valinfo[i]) then
				if not gGameUI:goBackInStackUI(curval.btns.viewName) then
					local _, viewName = gGameUI:getTopStackUI()
					if viewName ~= "city.view" then
						-- 不在主界面 等返回到主界面再弹出
						table.insert(self.autoPopBoxInfo, {
							viewName = curval.btns.viewName,
							params = curval.btns.params,
							styles = curval.btns.styles
						})
					else
						self:stackUI(curval.btns.viewName, nil, curval.btns.styles, unpack(curval.btns.params))
					end
					return
				end
			end
	...
	self.level:addListener(function(curval, oldval)
		if curval == oldval then
			return
		end
		local btns = self.limitGiftDatas:read().btns
		if btns and (not gGameUI:goBackInStackUI(btns.viewName)) then
			self:stackUI(btns.viewName, nil, btns.styles, unpack(btns.params))
		end
	end, true)

	idlereasy.any({ self.yyOpen, self.yyhuodongs, self.reunion, self.updateActivity },
		function(_, yyOpen, yyhuodongs, reunion, updateActivity)
			self:dropSate()
			self:showMXIcon()
				...
				if not v.showTheme and not v.cityTheme and (not v.independentStyle or ActivityView.isDataExist(v.independentStyle)) then
					...
					if v.redHint then
						...
						if type(data.specialTag) == "string" then
							expendRedHintSpecialTag = arraytools.merge_inplace(expendRedHintSpecialTag,
								{ { data.specialTag } })
						else
							expendRedHintSpecialTag = arraytools.merge_inplace(expendRedHintSpecialTag,
								{ data.specialTag or {} })
			...
			local onlyShowOne = {
				...
				[YY_TYPE.weeklyCardFR] = false,
				[YY_TYPE.limitBuyGift] = false,
				[YY_TYPE.weeklyCard] = false,
				[YY_TYPE.directBuyGift] = {
					noraml = false,
					double11 = false,
				},
			...
			for _, id in ipairs(yyOpen) do
				...
				local function isOK()
					if cfg.type == YY_TYPE.directBuyGift then
						if cfg.clientParam.double11 then
							if onlyShowOne[cfg.type]["double11"] == true then
								return false
							end
						else
							if cfg.clientParam.theme == "zhuangbei" or cfg.clientParam.theme == "shengxing" or cfg.clientParam.theme == "shenqi" or cfg.clientParam.theme == "zhibao" or cfg.clientParam.theme == "zhoumo" then

							elseif cfg.clientParam.theme == "kfzx" then

							elseif cfg.clientParam.theme == "dch" then --盘龙神迹
								return false
							elseif cfg.independent ~= 1 then
								return false
							else
								if onlyShowOne[cfg.type]["normal"] == true then
									return false
								end
							end
						end
					...
					local clientParam = cfg.clientParam
					local ency = clientParam.tag
					if ency and ency ~= APP_CURRENCY then
						return false
					end
				...
				if isOK() then
					...
					if cfg.type == YY_TYPE.directBuyGift then
						if cfg.clientParam.double11 then
							onlyShowOne[cfg.type]["double11"] = true
						else
							if cfg.clientParam.theme == "zhuangbei" or cfg.clientParam.theme == "shengxing" or cfg.clientParam.theme == "shenqi" or cfg.clientParam.theme == "zhibao" or cfg.clientParam.theme == "zhoumo" then
							...
							elseif cfg.clientParam.theme == "kfzx" then
								...
								onlyShowOne[cfg.type]["normal"] = true
							end
						end
					end
					...
					if cfg.type == YY_TYPE.luckyCat then
						...
						elseif cfg.paramMap.type == "gold" then
							t.viewName = "city.activity.lucky_cat_gold"
							-- 两种招财猫显示不同红点
							t.redHint = {
								class = "red_hint",
								props = {
					...
					if cfg.type == YY_TYPE.directBuyGift then
						if cfg.clientParam.double11 then
							...
							if cfg.clientParam.theme == "zhuangbei" or cfg.clientParam.theme == "shengxing" or cfg.clientParam.theme == "shenqi" or cfg.clientParam.theme == "zhibao" then
								t.viewName = "city.activity.session_gift"
								t.styles = nil
								t.endTime = self.yy_endtime[id]
								t.tag = SCHEDULE_TAG.session_gift
								t.redHint = {
									class = "red_hint",
									props = {
										specialTag = "activitySessionGift",
									},
								}
							elseif cfg.clientParam.theme == "zhoumo" then
								t.viewName = "city.activity.session_gift_zhoumo"
								t.endTime = self.yy_endtime[id]
								t.tag = SCHEDULE_TAG.session_gift
								t.styles = nil
								t.redHint = {
							...
							elseif cfg.clientParam.theme == "kfzx" then
								t.viewName = "city.activity.miix_gift"
								t.endTime = self.yy_endtime[id]
								t.tag = SCHEDULE_TAG.miix_gift
								t.styles = nil
								t.redHint = {
									class = "red_hint",
									props = {
										specialTag = "activityMiixBuyGift",
									},
								}
							else
					...
					if t.tag and cfg.clientParam.showCountTime then
						...
						if cfg.type == YY_TYPE.limitDayGift then
							local stamps2 = huodong.stamps2 or {}
							t.endTime = stamps2[id] + cfg.paramMap.countdown * 60

							if self.autoLimitDayGift:read() == 0 then
								self.autoLimitDayGift:set(id)
							end
						end
					end
				...
				if datas.cityTheme and cityThemeDatas[datas.cityTheme] then
					...
					for _, v in ipairs(cityThemeDatas[datas.cityTheme]) do
						if v.redHint then
							local data = v.redHint.props
							if type(data.specialTag) == "string" then
								cityThemeRedHintSpecialTag = arraytools.merge_inplace(cityThemeRedHintSpecialTag,
									{ { data.specialTag } })
							else
								cityThemeRedHintSpecialTag = arraytools.merge_inplace(cityThemeRedHintSpecialTag,
									{ data.specialTag or {} })
			...
			self.limitGiftDatas:set(limitGiftDatas)
...
function CityView:isActivityShow(id, huodong, activityInfo)
	local cfg = csv.yunying.yyhuodong[id]
	-- 只需要独立显示图标（1、不收纳 2、收纳 4、外部收纳+活动 5、外部收纳+福利 6、周卡）
	if cfg.independent ~= 1 and cfg.independent ~= 2 and cfg.independent ~= 4 and cfg.independent ~= 5 and cfg.independent ~= 6 then
		return false
	end
	-- 首充已领取的不显示
	if cfg.type == YY_TYPE.firstRecharge and (huodong.flag == 2 or huodong.stamps) then
		if huodong.flag == 2 then
			return false
		end
		if huodong.stamps then
			local giftID = cfg.clientParam.giftID
			local buyTimes = huodong.stamps[giftID] or 0
			if buyTimes > 0 then
				return false
			end
		end
	...
	-- 新首充
	if cfg.type == YY_TYPE.weeklyCardFR then
		if huodong.buy then
			local dayDatas = dataEasy.getFirstRechargeData(cfg.huodongID)
			local allGet = true
			for k, data in pairs(dayDatas) do
				-- 买了没领取
				...
				-- 没领取完
				if huodong.stamps and huodong.stamps[data.id] ~= 0 then
					allGet = false
					break
				end
			end
			...
			if not allGet then
				if self.autoWeeklyCardFrId:read() == 0 then
					self.autoWeeklyCardFrId:set(id)
				end
			end
	...
	if cfg.type == YY_TYPE.monthlyCard then
		if cfg.independent ~= 3 then
			return not huodong.enddate
		end
		return false
	end
...
function CityView:resetShowList(yyhuodongs, showDatas)
	...
	for i, v in ipairs(showDatas) do
		...
		text.addEffect(labelTime, { outline = { color = ui.NEWCOLORS.OUTLINE.BLACK, size = 3 } })
...
function CityView:initOnlineGift()
	...
	local function setItemShow(flag)
		...
		if flag and not self.topShowList:getChildByName(name) then
			...
			bind.touch(self.topShowList, item, {
				methods = {
					ended = function()
						self:onOnlineGift()
					end
	...
	-- 领取特效
	idlereasy.any({ self.onlineGift, self.level, dataEasy.getListenUnlock(gUnlockCsv.onlineGift) },
		function(obj, val, level, isUnlock)
			...
			OnlineGiftView.setCountdown(self, self.onlineGiftPanel:get("labelTime"),
				{
					data = self.onlineGift:read(),
					tag = SCHEDULE_TAG.onlineGift,
					cb = function()
	...
	idlereasy.when(self.onlineGiftState, function(_, onlineGiftState)
		...
		if onlineGiftState == 1 then
			...
			text.addEffect(self.onlineGiftPanel:get("labelTime"),
				{ outline = { color = ui.NEWCOLORS.OUTLINE.BLACK, size = 3 } })
			local img = self.onlineGiftPanel:get("xiaohupa")
			if img then
				img:removeSelf()
			end
		elseif onlineGiftState == 2 then
			text.addEffect(self.onlineGiftPanel:get("labelTime"), { color = ui.COLORS.GREEN })
			text.addEffect(self.onlineGiftPanel:get("labelTime"),
				{ outline = { color = ui.NEWCOLORS.OUTLINE.BLACK, size = 3 } })
			self.onlineGiftPanel:get("labelTime"):text(gLanguageCsv.onlineGiftGet)
			local onlineGiftSpine = widget.addAnimationByKey(self.onlineGiftPanel, "xiaohupa/xiaohupa.skel", 'xiaohupa',
					"effect_loop", 99)
...
function CityView:onOnlineGift()
	if self.onlineGiftState:read() == 1 then
		local data = {
			view = self,
			uiTime = self.onlineGiftPanel:get("labelTime"),
			tag = SCHEDULE_TAG.onlineGift,
			cb = function()
