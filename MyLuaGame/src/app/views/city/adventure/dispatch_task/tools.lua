...
local function setConditionText(txtNode, str, state, typ)
	local color = ui.NEWCOLORS.BROWN2
	if typ ~= 2 then
		color = state and ui.NEWCOLORS.GREEN1 or ui.NEWCOLORS.RED1
	end
...
local function setCondition2(txtNode, cardNums, csvTask, status)
	...
	if status ~= 2 then
		...
		txtNode:text("")
		local str = string.format("#C0x8a5b0c#"..gLanguageCsv.troopsOnTheBattlefield, "#C0xF41F0E#"..csvTask.cardNums.."#C0x8a5b0c#")
...
local function setExtraCondition1(txtNode, condition1Num, csvTask, status)
	...
	local colorNum = "#C0xF41F0E#"
	local colorNote = "#C0x8a5b0c#"
	if status ~= 2 then
		local color = condition1Num >= num and "#C0x04b508#" or "#C0xF41F0E#"
		colorNum = color
		colorNote = color
	end
