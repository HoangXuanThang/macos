...
local function setCostTxt(panel, myCoin, needCoin, isFree, curRefreshTimes)
	...
	if isFree then
		coinColor = ui.NEWCOLORS.GREEN2
	elseif myCoin < needCoin then
		coinColor = ui.NEWCOLORS.REDLIGHT
	end
		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					text.addEffect(childs.btnReward:get("textNote"), {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4}, color = ui.COLORS.NORMAL.WHITE})
...
function DispatchTaskView:onCreate(datas)
	...
	idlereasy.any({self.dispatchTasks, self.vipLevel},function(_, dispatchTasks, vipLevel)
		...
		local color = accessibleNum == 0 and ui.COLORS.NORMAL.WHITE or ui.NEWCOLORS.GREEN2
