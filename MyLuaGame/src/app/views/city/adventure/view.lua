...
local function judgeItemUnlock(cfg)
	...
	-- 战力要求
	if cfg.fightInfo then
		local lock = false -- 跨服天数不满足，则界面锁住状态
		local fightPoints = 0
		if cfg.fightInfo.funcType == "less" then
			local roleFightPoints = gGameModel.role:getIdler("top6_fighting_point"):read()
			fightPoints = getCsv(cfg.fightInfo.sevCsv)
			lock = roleFightPoints < fightPoints
		end
		if lock then
			return "fightPointsError", string.format(gLanguageCsv.unlockFight, fightPoints)
		end
	end
...
local function setItem(list, node, v, redHintPos, lockImg, lockPos)
	node:get("baseNode.imgBG"):texture(v.cfg.lockbg1 .. ".png")
	node:get("baseNode.black.imgBlack"):texture(v.cfg.lockbg1 .. "1.png")
		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					setItem(list, node, v, cc.p(700,1100), "", cc.p(325, 558))
	...
	["pvpList"] = {
		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					node:setName(v.cfg.unlockFeature)
					setItem(list, node, v, cc.p(530,945), "", cc.p(354, 552))
				end,
				onAfterBuild = function(list)
					list:setItemAlignCenter()
				end
