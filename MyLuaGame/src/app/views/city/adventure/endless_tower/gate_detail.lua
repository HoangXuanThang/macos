...
--设置限制条件
local function setLimitType(sceneCsv, textNote,textcolor)
	...
	elseif limitData.typ == 2 then
		local richTextNote = createRichTxt(textcolor.. limitData.txt1, x, y, textNote)
		...
		for i,v in csvPairs(sceneCsv.limitArg) do
			...
			local str = textcolor.. getLanguageAttr(v[1])..limitData.parameter[v[2]].." "..color.. dataEasy.getAttrValueString(v[1], v[3])
		...
		for i,v in csvPairs(sceneCsv.limitArg) do
			...
			local str = textcolor.. string.format(limitData.txt2, "", v[2])
...
function EndlessTowerGateDetail:onCreate(idx, gateId, handler)
	...
	setLimitType(sceneCsv, self.conditionNote,"#C0x57472E#")
...
function EndlessTowerGateDetail:showTip(view)
	...
	local sceneCsv = csv.endless_tower_scene[self.gateId]
	setLimitType(sceneCsv, view.textLimit,"#C0xFFFFFF#")
