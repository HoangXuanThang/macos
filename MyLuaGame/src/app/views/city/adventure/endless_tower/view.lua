...
EndlessTowerView.RESOURCE_BINDING = {
	...
	["leftDown.btnRank.textNote"] = {
		binds = {
			event = "effect",
			data = {outline = {color = ui.NEWCOLORS.OUTLINE.BLACK}},
		}
	},
	["leftDown.btnRule.textNote"] = {
		binds = {
			event = "effect",
			data = {outline = {color = ui.NEWCOLORS.OUTLINE.BLACK}},
		}
...
function EndlessTowerView:onCreate()
	...
	idlereasy.when(self.selGateIdx, function(_, selGateIdx)
		...
		if selGateIdx % 5 == 0 then
			...
			self.bossEffect:xy(cc.p(225, 240))
			self.bossEffect:visible(true)
			self.bossEffect:z(10)
		end
...
function EndlessTowerView:addItem(data, i, state)
	...
	if i % 5 == 0 then
		...
		item:size(450, 320)
		item:get("baseNode.textGate"):xy(225, 73)
		item:get("baseNode.imgRedPoint"):xy(225, 6)
