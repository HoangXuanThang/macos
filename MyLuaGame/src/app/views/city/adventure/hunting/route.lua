...
local SPINE_CARDS = {
	charizard = { cardId = 333, x = 150, y = 0, scale = 0.9 },
	bulbasaur = { cardId = 3353, x = 430, y = -20, scale = 0.9 },
	squirtle = { cardId = 2242, x = 660, y = 0, scale = 0.9 },
}
...
local GATE_SPINE_TYPE = {
	[1] = { res = "city/adventure/hunting/icon_zd1_red.png", scale = 1.5 },
	[2] = { res = "city/adventure/hunting/icon_zd2_red.png", scale = 1.5 },
	[3] = { res = "city/adventure/hunting/icon_boss.png", scale = 1.5 }
}
local actionTime = 3

local EVENT_SPINE = {
	[1] = { res = "qimukakaxi/qimukakaxi.json", spineName = "gate", action = "standby_loop", x = 3000, y = -550, scale = 1.1 },
	[2] = { res = "effect/jiedianjiangli.skel", spineName = "box", action = "effect_loop", x = 3000, y = -550, scale = 1.1 },
	[3] = { res = "qimukakaxi/qimukakaxi.json", spineName = "supply", action = "standby_loop", x = 3000, y = -550, scale = 1.1 },
	[4] = { res = "qimukakaxi/qimukakaxi.json", spineName = "mul", action = "standby_loop", x = 3000, y = -550, scale = 1.1 },
}
...
function HuntingRouteView:initUnderground(route)
	...
	self.bg:get("norbg"):visible(route == 1)
	self.bg:get("exbg"):visible(route ~= 1)
	for i = 1, 6 do
		for j = 1, 5 do
	...
	idlereasy.any({ self.clientRouteInfoFlag }, function()
		...
		if len > 0 then
			...
			performWithDelay(self, function()
				self.isOpen = false
				self:onBuffBtnClick()
				gGameUI:stackUI("city.adventure.hunting.select_buff", nil, { blackLayer = true },
					{ route = self.route, node = self.curNode, cb = self:createHandler("playBuffSpine") })
			end, 1 / 60)
		end
	end)
end
...
function HuntingRouteView:initSpine(type) -- 1 待机 2 run 3 win
	local effect = widget.addAnimationByKey(self.runSpine, "loading/loading_dog.skel", "effect", "effect_loop", 10)
		:xy(0, 10)
		:scale(1.3)
	self.spinePanel:removeAllChildren()
	if type then
		for id, v in pairs(SPINE_CARDS) do
			local unitID = csv.cards[v.cardId].unitID
			local unitData = csv.unit[unitID]
			local cardSprite1 = widget.addAnimationByKey(self.spinePanel, unitData.unitRes, unitData.unitRes,
					ACTION_TYPE[type], 1000)
				:scale(v.scale)
				:xy(v.x, v.y)
		end
	end
...
function HuntingRouteView:initProgress()
	...
	for id, val in pairs(self.routeData) do
		if val.count == itertools.size(self.routeData) then
			local bar = self.prograssPanel:get("proPanel3"):clone():show()
				...
				:xy(self.prograssPanel:get("proPanel2"):x() + 252 * (val.count - 2) + 170,
					self.prograssPanel:get("proPanel2"):y())
			...
			local bar = self.prograssPanel:get("proPanel2"):clone():show()
				:addTo(self.prograssPanel, 5, "bar" .. id)
				:xy(self.prograssPanel:get("proPanel2"):x() + 253 * (val.count - 1) - 2,
					self.prograssPanel:get("proPanel2"):y())
	...
	for id = self.minId, self.minId + itertools.size(self.routeData) - 1 do
		...
		if self.routeData[id].count == itertools.size(self.routeData) then -- 最后一关特殊处理
			...
			local spineX = self.routeData[id].count > 3 and self.routeData[id].count < itertools.size(self.routeData) - 2 and
				0 or 250
			local panelX = self.routeData[id].count > 3 and self.routeData[id].count < itertools.size(self.routeData) - 2 and
				250 or 0
...
function HuntingRouteView:refreshProgress(curProIndex)
	...
	elseif curProIndex >= 2 then
		...
		self.bar[self.prograssData[curProIndex].prograssIndex]:get("pro"):setPercent(self.prograssData[curProIndex]
			.percent)
	end
end
...
function HuntingRouteView:initEventSpine(flag)
	...
	--if(self.curProIndex == nil)then
	self:checkCurIndex()
	--end

	local function eventCb()
		self.bg:removeChildByName("eventPanel")
		if flag == 1 or flag == 3 then
			self.bg:removeChildByName("gatePanel")
		end
		if flag ~= 3 then
			self.bg:setRotation(0)
		end
		local x = EVENT_SPINE[self.eventID].x
		local y = EVENT_SPINE[self.eventID].y

		local xadd = flag == 3 and 1000 or 0

		xadd = xadd + (self.curProIndex - 1) * 500

		local rotation = flag == 2 and 0 or 0
		rotation = flag == 3 and 0 or rotation
		-- 挑战关卡
		if self.eventID == EVENT_TYPE.gate then
			...
			performWithDelay(self, function()
				gGameApp:requestServer("/game/hunting/battle/info", function(tb)
					...
					rotation = flag == 3 and 0 or rotation
					-- 创建一个层容器用来点击
					local panel = self.bg:get("gatePanel")
					if flag == 2 and panel then
						panel:xy(x + xadd, y)
					else
						local gatePanel = ccui.Layout:create()
							:size(400, 500)
							:addTo(self.bg, 10, "gatePanel")
							:anchorPoint(0, 0)
							:xy(x + xadd - 200, y)
						gatePanel:setTouchEnabled(true)
						gatePanel:setRotation(rotation)
						--精灵
						local nodeSpine = widget.addAnimationByKey(self.bg:get("gatePanel"), csv.unit[unitId].unitRes,
								EVENT_SPINE[self.eventID].spineName, EVENT_SPINE[self.eventID].action, 10)
							:xy(200, 0)
							:anchorPoint(0, 0)
						...
						local iconBg = cc.Sprite:create("city/adventure/hunting/logo_sldd.png")
							...
							:xy(0, 280 + csv.unit[unitId].everyPos.headPos.y)
							:scale(0.7)
						local animate = cc.Sequence:create(
							cc.MoveTo:create(0.5, cc.p(0, 470)),
							cc.MoveTo:create(0.5, cc.p(0, 470 + 20)))
						local action = cc.RepeatForever:create(animate)
						iconBg:runAction(action)
						--:scale(1 / EVENT_SPINE[self.eventID].scale)
						local respurse = GATE_SPINE_TYPE
							[csv.cross.hunting.gate[self.routeData[self.curNode].gateIDs[1]].type]
						...
						if flag == 3 then
							...
							local sequence = cc.Sequence:create(
								cc.MoveTo:create(actionTime - 1, cc.p(panel:x() - 1000, panel:y()))
							)
			...
			local nodeSpine = widget.addAnimationByKey(self.bg:get("eventPanel"), EVENT_SPINE[self.eventID].res,
					EVENT_SPINE[self.eventID].spineName, EVENT_SPINE[self.eventID].action, 10)
				:xy(200, 0)
				:anchorPoint(0, 0)
				:scale(EVENT_SPINE[self.eventID].scale)
			...
			if self.eventID == EVENT_TYPE.mul then
				--local label = cc.Label:createWithTTF("!", "font/youmi.ttf", 100)
				--	:align(cc.p(1, 0), 40, 30)
				--	:addTo(nodeSpine:get("iconBg"), 10, "icon")
				--	:xy(100, 50)
				--	text.addEffect(label, {color = cc.c4b(241, 59, 84, 255)})
				local iconPic = cc.Sprite:create("city/adventure/hunting/shijian.png")
					:anchorPoint(0.5, 0.5)
					:addTo(nodeSpine:get("iconBg"), 10, "icon")
					:xy(80, 110)
					:scale(1.5)
			elseif self.eventID == EVENT_TYPE.HP then
				local iconPic = cc.Sprite:create("city/adventure/hunting/img_box_jy.png")
					...
					:xy(80, 110)
				if flag == 3 then
					local panel = self.bg:get("eventPanel")
					local sequence = cc.Sequence:create(
						cc.MoveTo:create(actionTime - 1, cc.p(panel:x() - 1000, panel:y()))
					)
	...
	--可达鸭逃跑
	local panel = self.bg:get("eventPanel") and self.bg:get("eventPanel"):get("mul")
	if self.routeData[self.curNode].type == EVENT_TYPE.mul and panel then
		self.delayTime = 1.5
		self.bg:get("eventPanel"):setTouchEnabled(false)
		panel:play("run_loop")
		panel:scaleX(EVENT_SPINE[self.eventID].scale)
		local sequence = cc.Sequence:create(
			cc.MoveTo:create(self.delayTime, cc.p(panel:x() + 800, panel:y())),
	...
	if flag == 2 then
		self.bg:x(-(self.curProIndex - 1) * 500 - 1000)
	end
end
...
function HuntingRouteView:initPanel()
	if self.curNode > self.minId + itertools.size(self.routeData) - 1 then
		...
		local normalContent = string.format(gLanguageCsv.huntingRouteDetail, self.routeData[self.curNode].count,
			itertools.size(self.routeData))
...
function HuntingRouteView:updatePerformance()
	...
	performWithDelay(self, function()
		self:initSpine(1)
		if self.bg:get("eventPanel") then
			bind.click(self, self.bg:get("eventPanel"), {
				method = function()
					self:onNodeClick()
				end
			})
	...
	self.bg:x(-(self.curProIndex - 1) * 500)
	local bgmove = cc.Sequence:create(
		cc.MoveTo:create(actionTime, cc.p(self.bg:x() - 1000, self.bg:y()))
	)
	self.bg:runAction(bgmove)
end
...
function HuntingRouteView:onNodeClick()
	...
	if self.eventID == EVENT_TYPE.gate then
		if self.curNode < self.minId + itertools.size(self.routeData) then
			local enemyID = self.boardId > 0 and self.boardId or self.routeData[self.curNode].gateIDs[1]
			gGameUI:stackUI("city.adventure.hunting.gate_detail", nil, nil, self.gateDatas, enemyID, self.route, node,
				self:createHandler("onClientRouteRefresh"))
		end
	elseif self.eventID == EVENT_TYPE.box then
		gGameUI:stackUI("city.adventure.hunting.open_box", nil, { clickClose = true, blackLayer = true }, self.route,
			node, self:createHandler("onClientRouteRefresh"))
	elseif self.eventID == EVENT_TYPE.HP then
		local group = csv.cross.hunting.route[node].supplyGroup or 1
		gGameUI:stackUI("city.adventure.hunting.supply", nil, { blackLayer = true },
			{ route = self.route, node = node, group = group }, self:createHandler("onClientRouteRefresh"))
	elseif self.eventID == EVENT_TYPE.mul then
		gGameUI:stackUI("city.adventure.hunting.select_event", nil, { blackLayer = true }, node,
			self:createHandler("onClientRouteRefresh"))
	end
end
...
function HuntingRouteView:onItemClick(self, list, node, v)
	...
	local view = tip.create(name, nil,
		{ relativeNode = node, canvasDir = canvasDir, childsName = childsName, dir = "top", offy = 150 }, v)
