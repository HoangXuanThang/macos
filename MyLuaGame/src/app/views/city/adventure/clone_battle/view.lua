...
CloneBattleCityView.RESOURCE_BINDING = {
	...
	["btnRule.text"] = {
		binds = {
			event = "effect",
			data = { outline = { color = ui.NEWCOLORS.OUTLINE.BLACK }, glow = { color = ui.COLORS.OUTLINE.DEFAULT } },
		}
	},
	["btnShowList"] = {
		varname = "btnShowList",
...
function CloneBattleCityView:onCreate(data, baseView)
	...
	if self.data.beKicked and self.kickNum < gCommonConfigCsv.cloneDailyBeKickedMax and self.kickNum > beKickedNum then
		gGameUI:showDialog({
			title = "",
			content = string.format("#C0x5b545b#" .. gLanguageCsv.cloneBattleRecord8,
				gCommonConfigCsv.cloneDailyBeKickedMax - self.kickNum),
			btnType = 1,
			isRich = true
		})
	end
	...
	for idx, tb in pairs(data.nature) do
		...
		for i, id in pairs(tb[2]) do
			local cardId = id
			print(csv.clone.monster[id])
			if (csv.clone.monster[id] ~= nil) then
				cardId = csv.clone.monster[id].cardID
			end
...
function CloneBattleCityView:initNatureItem()
	...
	for i, pos in pairs(ITEM_POS) do
		...
		if natureTb and csv.clone.nature[natureTb.natureId] then
			local natureId = natureTb.natureId
			local size = item:size()
			local res = csv.clone.nature[natureTb.natureId].spine
			local scaleNum = 1.3 --i <= 2 and 1.74 or 2
			...
			local textItem = item:get("text")
			textItem:text(gLanguageCsv[game.NATURE_TABLE[natureId]] .. gLanguageCsv.huanxiang)
