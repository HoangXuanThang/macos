		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					children.text:text(gLanguageCsv[game.NATURE_TABLE[natureId]] .. gLanguageCsv.huanxiang)
					text.addEffect(children.text,
						{ outline = { color = ui.COLORS.OUTLINE.WHITE, size = 4 }, color = ui.COLORS.ATTR[natureId] })
...
function CloneBattleSpriteList:onCreate(data, posX, posY)
	...
	self.list:height(targetH):xy(25, 50)
