...
function HistoryView:onCreate(parms)
	...
	for date, data in pairs(dateRecord) do
		...
		local richText = rich.createByStr("#Pfont/youmi.ttf##C0xFFFFFF#" .. date, 40)
			:addTo(item)
			:anchorPoint(0,0.5)
			:xy(10,32)
			:height(45)
		...
		for i, v in ipairs(data) do
			local item = self.item:clone()
			local t = time.getDate(v.time)
			local time = "#C0xFFFFFF##Pfont/youmi.ttf#" ..string.format("%02d:%02d", t.hour, t.min)
			local str = string.formatex("#C0xFFFFFF#"..TIP[v.type], {name = "#C0x5FC355#".. v.name .."#C0xFFFFFF#"})
