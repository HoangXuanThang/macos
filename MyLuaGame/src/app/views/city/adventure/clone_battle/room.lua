...
CloneBattleRoomView.RESOURCE_BINDING = {
	...
	["leftPanel.spriteArea.item.baseNode.maskOutBox.text"] = {
		binds = {
			{
				event = "effect",
	...
	["rightPanel.centerPanel.mainItem.btnWorldInvited.text"] = {
		binds = {
			event = "effect",
			data = { outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW } },
		}
	},
	["rightPanel.centerPanel.mainItem.btnUnionInvited.text"] = {
		binds = {
			event = "effect",
			data = { outline = { color = ui.NEWCOLORS.OUTLINE.BLUE } },
		}
	},
}
...
local function setBtnTime(self, btnOrStr, fmt, time)
	...
	local btn = getBtn()
	local olColor = ui.NEWCOLORS.OUTLINE.YELLOW;
	text.deleteAllEffect(btn:get("text"))
	if (btnOrStr == "btnUnionInvited") then
		olColor = ui.NEWCOLORS.OUTLINE.BLUE;
	end
	...
	self:enableSchedule():schedule(function(dt)
		...
		if t > time then -- 计时结束
			...
			text.addEffect(btn:get("text"), { outline = { color = olColor } })
...
function CloneBattleRoomView:initModel()
	...
	idlereasy.any({ self.beasIdler.places, self.beasIdler.voteRound, self.beasIdler.fast },
				...
				if dt then
					...
					if isLeader then
						...
						bind.touch(self, self.radioJoin,
							{ methods = { ended = functools.partial(self.onFastJoin, self) } })
						bind.touch(self, self.btnWorldInvited,
							{ methods = { ended = functools.partial(self.onWorldInvite, self, "btnWorldInvited") } })
						bind.touch(self, self.btnUnionInvited,
							{ methods = { ended = functools.partial(self.onUnionInvite, self, "btnUnionInvited") } })
					...
					bind.touch(self, item,
						{ methods = { ended = functools.partial(self.onPosItemClick, self, item, data) } })
				end
			end
...
function CloneBattleRoomView:refreshTimeLabel()
	...
	self.btnRobot:visible(not isRoomFull)
...
function CloneBattleRoomView:initSelfInfo(dt)
	...
	if (canBattle) then
		text.addEffect(self.btnChallenge:get("text"), { outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW } })
	else
		text.deleteAllEffect(self.btnChallenge:get("text"))
	end
...
function CloneBattleRoomView:onPosItemClick(item, data)
	...
	self:sendServerRequest("/game/clone/friend/online/list", nil, function(tb)
		...
		gGameUI:stackUI("city.adventure.clone_battle.invite", nil, nil, { roles = roles, size = size },
...
function CloneBattleRoomView:onSpriteClick(node, data, isMine, i)
	...
	gGameUI:stackUI("city.chat.personal_info", nil, nil, pos, { role = data },
		{ isKickNum = isKickNum, isLeader = self.isLeader })
end
...
function CloneBattleRoomView:onRecord()
	...
	gGameUI:stackUI("city.adventure.clone_battle.history", nil, nil,
		{ historyTab = historyTab, refreshNumber = self.refreshNumber })
end
