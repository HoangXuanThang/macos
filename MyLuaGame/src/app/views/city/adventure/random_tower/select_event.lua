		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					childs.textDesc:text(v["name_"..LOCAL_LANGUAGE] or v.name)
...
function RandomTowerSelectEventView:onCreate(boardID, cb, eventId)
	...
	self.textTitle:text(cfg["desc_"..LOCAL_LANGUAGE] or cfg.desc)
	if matchLanguage({"kr", "en","vn","th","in","ma", "ph"}) then
		self.textTitle:x(display.sizeInView.width/2)
    	adapt.setTextAdaptWithSize(self.textTitle, {size = cc.size(1900, 150)})
	end
	local eventDatas = {}
	for i=1,3 do
		local name = cfg["choice"..i.."_"..LOCAL_LANGUAGE]
		if cfg["choice"..i.."_"..LOCAL_LANGUAGE] ~= "" then
