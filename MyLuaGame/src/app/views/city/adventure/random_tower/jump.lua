		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					local desc = buffCsv["desc_"..LOCAL_LANGUAGE] or buffCsv.desc
					...
					beauty.textScroll({
						effect = {color= ui.NEWCOLORS.BLACK,outline={color= ui.NEWCOLORS.OUTLINE.WHITE1}},
						list = node:get("desc"),
						strs = desc,
						align = "center",
						fontSize = 34,
		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					local desc = eventCsv["desc_"..LOCAL_LANGUAGE] or eventCsv.desc
...
function RandomTowerJumpView:refreshProgressPanel(step)
	...
	nodetools.invoke(panel, textTab, "setTextColor", ui.NEWCOLORS.ORANGE)
end
