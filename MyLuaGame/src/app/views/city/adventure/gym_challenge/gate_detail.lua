...
local function setSpecialEff(sceneCsv, textNote)
	...
	for i, v in csvPairs(sceneCsv.specialEff) do
		...
		if v[3] == 0 then
			...
			str = str .. gLanguageCsv.reduce .. "#C0x57472e#"
		end
		str = str .. dataEasy.getAttrValueString(attrType, v[4])
		local icon = v[3] == 0 and "#Icommon/icon/logo_arrow_green.png-30-45#" or
			"#Icommon/icon/logo_arrow_red.png-30-45#"
		str = str .. icon
		createRichTxt(str, x, y, textNote)
	end
	if sceneCsv.specialEffDesc ~= "" then
		local rich = rich.createWithWidth("#C0x57472e#" .. sceneCsv.specialEffDesc or "", 40, nil, 960)
			:anchorPoint(0, 1)
			:xy(0, y + 25 - 50)
			:addTo(textNote)
