...
function GymCrossGate:initUI()
	...
	local imgOwner = self.panel1:get("imgOwner")
	imgOwner:texture("city/adventure/gym_challenge/map/" .. csv.gym.gym[self.id].namePng):scale(1.2):y(imgOwner:y() - 110)
	--馆主信息
	self:initMember()
end
...
function GymCrossGate:initMember()
	...
	idlereasy.any({ self.crossAlGatesDatas, self.inCd, self.round }, function(_, _crossGatesDatas, inCd, round)
		...
		for i = 1, 8 do
			...
			if i == 1 then
				text.addEffect(panel:get("textTime"), { outline = { color = ui.NEWCOLORS.OUTLINE.BLACK, size = 2 } })
			end

			if data then
				infoPanel:show()
				emptyPanel:hide()
				local childs = infoPanel:multiget("textFight", "textName", "textLv", "textSever", "txt", "figurePanel",
					"figurePanel")
				text.addEffect(childs.textFight, { outline = { color = ui.NEWCOLORS.OUTLINE.BLACK, size = 2 } })
				text.addEffect(childs.textLv, { outline = { color = ui.NEWCOLORS.OUTLINE.BLACK, size = 2 } })
				text.addEffect(childs.textName, { outline = { color = ui.NEWCOLORS.OUTLINE.BLACK, size = 2 } })
				text.addEffect(childs.textSever, { outline = { color = ui.NEWCOLORS.OUTLINE.BLACK, size = 2 } })
				if i ~= 1 then
					text.addEffect(childs.txt, { outline = { color = ui.NEWCOLORS.OUTLINE.BLACK, size = 2 } })
				end
				...
				if i == 1 then
					...
					adapt.oneLineCenterPos(cc.p(150, childs.textSever:y()),
						{ childs.txt, childs.textFight, childs.textSever }, { cc.p(10, -5), cc.p(0, 0) })
				else
					adapt.oneLineCenterPos(cc.p(150, childs.textLv:y()), { childs.textLv, childs.textName }, cc.p(5, 6))
					adapt.oneLineCenterPos(cc.p(150, childs.textSever:y()),
						{ childs.txt, childs.textFight, childs.textSever }, { cc.p(10, 0), cc.p(0, 0) })
				end
				if data.figure ~= "" then
					local size = infoPanel:size()
					local figureCfg = gRoleFigureCsv[data.figure]
					widget.addAnimationByKey(childs.figurePanel, figureCfg.resSpine, "figure", "standby_loop", 1)
						:xy(size.width / 2, 0)
						:scale(i == 1 and 0.8 or 0.6)
					bind.touch(self, childs.figurePanel, {
						methods = {
							ended = function()
								gGameApp:requestServer("/game/gym/role/info", function(tb)
									gGameUI:createView("city.adventure.gym_challenge.master_info", self):init(tb.view,
										self.id, true, true, i)
				...
				if gGameModel.role:read("gym_record_db_id") == data.record_id then
					...
					if inCd then
						...
						text.addEffect(btnText, { outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4 } })
					end
					btnChallenge:show()
				end
			else
...
function GymCrossGate:onBtnChallenge(k, data)
	...
	local fightCb = function(view, battleCards)
		...
		battleEntrance.battleRequest("/game/cross/gym/battle/start", cardDatas, self.id, k, data.game_key, data
			.record_id)
...
function GymCrossGate:onBtnOccupy(k)
	...
	local saveCb = function(view, clientbattleCards, battleCards, bCloseView)
		...
		gGameApp:requestServerCustom("/game/cross/gym/battle/occupy")
			:onErrClose(function(tb)
				if tb.err == "gymNotEmpty" then
					gGameApp:requestServer("/game/gym/main")
					if view then
						view.haveSaved = true
						view:onClose(false)
					end
...
function GymCrossGate:getRuleContext(view)
	...
	for k, v in orderCsvPairs(csv.gym.gym) do
		table.insert(context, c.clone(view.awardItem, function(item)
			...
			childs.text:text("")
			rich.createByStr("#C0xFFFFFF#" .. v.crossLeaderAwardDesc, 40)
				:anchorPoint(1, 0.5)
				:xy(280, 100)
				:addTo(childs.text, 6)
		...
		table.insert(context, c.clone(view.awardItem, function(item)
			local childs = item:multiget("text", "list")
			childs.text:text("")
			rich.createByStr("#C0xFFFFFF#" .. v.crossSubAwardDesc, 40)
				:anchorPoint(1, 0.5)
				:xy(280, 100)
				:addTo(childs.text, 6)
