...
function GymBuffDetail:onCreate(id, unlocked, preLv, needPerLv)
	...
	idlereasy.when(self.gymDatas, function(_, gymDatas)
		...
		local desc = cfg["desc_".. LOCAL_LANGUAGE] or cfg.desc
		if cfg.effectType == 1 then
			...
			local function safeFormat(str, ...)
				-- escape % lỗi (như % m, % máu,...)
				str = str:gsub("%%([^dsf])", "%%%%%1")
				return string.format(str, ...)
			end
			
			local ok, result = pcall(safeFormat, desc, table.unpack(effectData))
			if ok then
				self.textDesc:text(result)
			else
				print("FORMAT ERROR:", desc)
				self.textDesc:text(desc)
			end
		else
			self.textDesc:text(getSkillStr(desc, lv or 0))
		end
		...
		elseif lv >= cfg.levelUp then
			...
			text.addEffect(self.btnAct:get("textNote"), {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4}})
			self.lockPanel:hide()
		end
		self.lv = lv
	end)
...
function GymBuffDetail:onDetailClick(target)
	...
	if cfg.effectType == 1 then
		...
		local ok, result = pcall(safeFormat, cfg.desc, table.unpack(effectData))
if ok then
    content = content .. result
else
    content = content .. cfg.desc
end
