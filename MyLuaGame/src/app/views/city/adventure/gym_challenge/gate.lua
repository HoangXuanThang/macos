				...
				onItem = function(list, node, k, v)
					...
					elseif v.state == STATE.UNLOCK then
						...
						if v.canPass then
							...
							text.addEffect(btnChallenge2:get("textNote"), {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 3}})
							text.addEffect(btnPass:get("textNote"), {outline = {color = ui.NEWCOLORS.OUTLINE.BLUE, size = 3}})
						else
							local btnChallenge = node:get("btnChallenge"):show()
							uiEasy.setBtnShader(btnChallenge, btnChallenge:get("textNote"), 1)
							text.addEffect(btnChallenge:get("textNote"), {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 3}})
							node:get("btnChallenge2"):hide()
							node:get("btnPass"):hide()
						end
					else
...
function GymGate:initModel()
	...
	idlereasy.any({self.gymDatas, self.gatesDatas, self.inCd}, function(_, gymDatas, gatesDatas, inCd)
		...
		for k, v in orderCsvPairs(cfg.hardDegreeID) do
			...
			text.deleteAllEffect(self.btnOwerChallenge:get("textNote"))
			if gymDatas.gym_fuben[self.id] > maxDegreeID then
				if inCd then
					...
					text.addEffect(self.btnOwerChallenge:get("textNote"), {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4}})
				end
...
function GymGate:initOwner()
	...
	adapt.oneLineCenterPos(cc.p(443,self.panelLeft:get("textLv"):y()), {self.panelLeft:get("textLv"),self.panelLeft:get("textName")},cc.p(15,0))
