...
function GymChallengeView:onClose(t)
	if not self.notCross:read() then
		...
		local cloudSpine = widget.addAnimationByKey(self:getResourceNode(), "gym/kuafu_yun.skel", "cloudSpine",
				"kuafu_effect", 6)
...
function GymChallengeView:initGyms()
	...
	for id, cfg in orderCsvPairs(csv.gym.gym) do
		local gym = self.gymItem:clone()
			...
			:xy(cfg.pos)
		local spine = widget.addAnimationByKey(gym, "city/adventure/gym_challenge/house/house" .. id .. ".png",
				"spine" .. id, cfg.spine, 0)
			:xy(cfg.posSkew)
			:scale(2)
			...
			methods = {
				ended = function()
					if self.notCross:read() == false then
						local owners = self.round:read() == "start" and self.gymCrossOwners:read()[id] or
							self.lastCrossGymOwners:read()[id]
						if owners and owners[1] then
							...
							gGameApp:requestServer("/game/gym/role/info", function(tb)
								...
								local unlocked = self.gymsInfo:atproxy(id).unlocked and
									self.gymDatas:read().gym_fuben[id] > maxDegreeID
								gGameUI:createView("city.adventure.gym_challenge.master_info", self):init(tb.view, id,
									true, unlocked, 1)
							end, recordId, owners[1].game_key)
						end
					else
						local gymOwners = self.round:read() == "start" and self.gymOwners:read() or
							self.lastGymOwners:read()
...
function GymChallengeView:initModel()
	...
	idlereasy.any({ self.gymsInfo, self.notCross }, function(_, gymsInfo, notCross)
		for id, info in ipairs(gymsInfo) do
			...
			self.btnCross:get("imgLock"):setVisible(notCross and gGameModel.gym:read("crossKey") == "" and
				itertools.size(lastCrossGymRoles) == 0 or time.getTime() < openTime)
		end
	end)

	idlereasy.any({ self.gymOwners, self.gymCrossOwners, self.notCross, self.round },
	...
	self.notCross:addListener(function(val, oldval)
		...
		if self.first == true then
			...
			local x, y = scrollViewSize.width / 2 - centerPosX * self.mapScale,
				scrollViewSize.height / 2 - centerPosY * self.mapScale
			x = math.min(0, x)
			y = math.min(0, y)
			...
			self.scrollView:getInnerContainer():runAction(cc.Sequence:create(cc.Spawn:create(ationScale, ationMove),
...
function GymChallengeView:onGymClick(id)
	if not self.notCross:read() then
		...
		if self.gymsInfo:atproxy(id).completed == true then
			gGameUI:stackUI("city.adventure.gym_challenge.cross_gate", nil, { full = true }, id,
				self.gymsInfo:atproxy(id).completed)
		else
	...
	if self.notCross:read() then
		...
		for k, v in orderCsvPairs(csv.gym.gym) do
			table.insert(context, c.clone(view.awardItem, function(item)
				...
				childs.text:text("")
				rich.createByStr("#C0xFFFFFF#" .. v.leaderAwardDesc, 40)
					:anchorPoint(1, 0.5)
					:xy(250, 100)
					:addTo(childs.text, 6)
