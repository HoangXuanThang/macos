...
function VipDistinguishedView:onCreate(cb)
	self.roleVip = gGameModel.role:read("vip_level") or 0
	...
	if matchLanguage({ "en" }) then
		adapt.setTextAdaptWithSize(self.text1,
			{ size = cc.size(960, 200), vertical = "center", horizontal = "center", margin = -5, maxLine = 2 })
		self.text2:setPositionY(885)
	end

	uiEasy.createItemsToList(self, self.list, gVipCsv[self.roleVip].monthGift,
		{
			margin = 40,
