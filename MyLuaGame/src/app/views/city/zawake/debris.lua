			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					if v.isSelected then
						text.addEffect(txt,
							{ color = ui.COLORS.NORMAL.WHITE, outline = { color = ui.COLORS.NORMAL.BLACK, size = 2 } })
					else
						text.addEffect(txt,
							{ color = ui.COLORS.NORMAL.RED, outline = { color = ui.COLORS.NORMAL.BLACK, size = 2 } })
					end
				end,
			},
			handlers = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					text.addEffect(childs.price,
						{
							color = dataEasy.getNumByKey(v.key) >= v.num and ui.COLORS.NORMAL.DEFAULT or
								ui.COLORS.NORMAL.ALERT_ORANGE
						})
					childs.icon:texture(ui.COMMON_ICON[v.key])
					adapt.oneLineCenterPos(cc.p(size.width / 2, size.height / 2),
						{ childs.txt1, childs.price, childs.icon }, cc.p(10, 0))
				end,
...
function ZawakeDebrisView:onCreate(params)
	...
	self.fragExchangeRate = exchangeCsv.type == DEBRIS_TYPE.currency and exchangeCsv.needFrags[1][3] or
		exchangeCsv.needSpecialFrags[1][2]
