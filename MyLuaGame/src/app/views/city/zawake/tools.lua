...
function zawakeTools.getCardByZawakeID(zawakeID)
	...
	if bagCardsHash[zawakeID] then
		...
		return {
			cardId = cardId,
			cfg = gCardsZawake[zawakeID][cardId],
			zawakeID = zawakeID,
			dbId = bagCardsHash
				[zawakeID].dbId
		}
...
function zawakeTools.getAllCards()
	...
	for zawakeID, zawakeDates in pairs(gCardsZawake) do
		if zawakeTools.isOpenByStage(zawakeID) then
			if bagCardsHash[zawakeID] then
				allCards[zawakeID] = {
					cfg = gCardsZawake[zawakeID][bagCardsHash[zawakeID].cardId],
					maxFightPoint =
						bagCardsHash[zawakeID].fightPoint
				}
			else
				local cardId
...
function zawakeTools.getAttrAddTypeStr(attrAddType, natureType)
	if attrAddType == ATTR_TYPE.designated then
		return string.format("[ %s ]",
			string.format(gLanguageCsv.zawakeFirstAttr, gLanguageCsv[game.NATURE_TABLE[natureType]]))
	end
	...
	if valSame() then
		...
		for k, num in pairs(allAttr) do
			if isGray then
				...
				str = str ..
					string.format("#C0x5B545B#%s#C0x60C456#+%s ", getLanguageAttr(k), dataEasy.getAttrValueString(k, num))
			end
		end
	end
	return str
...
function zawakeTools.getActiveCondition(zawakeID, stageID, cfg)
	...
	for key, val in csvMapPairs(cfg.activeReq) do
		...
		local str = "#C0xFFFFFF#·" .. zawakeTools.getLabelByLimit(key, val, state)
...
function zawakeTools.isUnlockByStage(stageCfg, zawakeID)
	...
	for cardMarkID, limitDatas in pairs(unlockDatas) do
		for key, data in pairs(limitDatas.data) do
			local str = "#C0x5B545B#" ..
				limitDatas.firstText .. zawakeTools.getLabelByLimit(data.key, data.val, data.state)
			table.insert(labelDatas, str)
