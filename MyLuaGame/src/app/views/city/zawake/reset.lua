...
function ZawakeResetView:onCreate(params)
	Dialog.onCreate(self)
	self.cb = params.cb
	self.zawakeID = params.zawakeID
	local zawakeData = gGameModel.role:read("zawake")[self.zawakeID]
	local costDatas = zawakeTools.getResetCostItems(self.zawakeID, zawakeData)
	...
	adapt.oneLineCenterPos(cc.p(size.width / 2, size.height / 2), { self.txt1, self.cost, self.icon },
		{ cc.p(0, 0), cc.p(10, 0) })
end
