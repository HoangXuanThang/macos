		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					local childs = node:multiget("bg1", "name", "fullAwake", "ruleBtn", "attrPanel", "skillPanel",
						"emptyPanel", "icon", "icon_name")
					childs.name:text(gLanguageCsv['symbolRome' .. v.cfg.level])
					...
					if isZawake then
						childs.icon_name:texture("city/ming_ge/tab_" .. v.stageID .. "_2.png")
					else
						childs.icon_name:texture("city/ming_ge/tab_" .. v.stageID .. "_3.png")
					end

					itertools.invoke(
						{ childs.bg1, childs.fullAwake, childs.attrPanel, childs.skillPanel, childs.emptyPanel, childs
							.ruleBtn }, "hide")
					childs.icon:texture("city/ming_ge/icon_" .. v.stageID .. ".png")
					if v.isOpen == zawakeTools.STAGE_OPEN_STATE.open then
						...
						if not itertools.isempty(v.labelDatas) then
							...
							bind.touch(list, childs.ruleBtn,
								{ methods = { ended = functools.partial(list.clickUnlock, k, v, pos) } })
						end
					else
					...
					if v.isOpen ~= zawakeTools.STAGE_OPEN_STATE.close then
						...
						if v.skillID > 0 then
							...
							local skillChilds = childs.skillPanel:multiget("title", "list", "skill", "name",
								"iconUp")
							...
							local skillName = csv.skill[v.skillID]["skillName_"..LOCAL_LANGUAGE] or csv.skill[v.skillID].skillName
							...
							local describe = skillCsv["describe_"..LOCAL_LANGUAGE] or skillCsv.describe
							local zawakeEffectDesc = skillCsv["zawakeEffectDesc_"..LOCAL_LANGUAGE] or skillCsv.zawakeEffectDesc
							local desc = isStarEffrct and skillCsv.describe or skillCsv.zawakeEffectDesc
							local starStr = uiEasy.getStarSkillDesc({
								skillId = v.skillCfg.id,
								cardId = v.skillCfg
									.cardId,
								star = 12,
								isZawake = true
							...
							if skillCsv.zawakeSimpleDesc and skillCsv.zawakeSimpleDesc ~= "" then
								...
								local simpleTitle2 = skillCsv.zawakeSimpleType == 1 and gLanguageCsv.zawakeSimpleType1 or
									gLanguageCsv.zawakeSimpleType2
								local simpleColor = skillCsv.zawakeSimpleType == 1 and "#C0x5c9970#" or "#C0x5B545B#"
								str = string.format("#C0xE69900#[%s%s%s]:\n#F20# \n#F40#\t%s%s", simpleTitle1,
									isStarEffrct and gLanguageCsv.star or "", simpleTitle2, simpleColor,
									skillCsv["zawakeSimpleDesc_"..LOCAL_LANGUAGE] or skillCsv.zawakeSimpleDesc)
							end
							beauty.textScroll({
							...
							for k, v in csvMapPairs(v.cfg.extraAttrs) do
								table.insert(t, (getLanguageAttr(k) or "") .. "+" .. dataEasy.getAttrValueString(k, v))
							end
...
function ZawakePreviewView:onUnlockInfoClick(list, k, v, pos)
	...
	local align = "left"
	gGameUI:stackUI("city.zawake.unlock_tips", nil, nil,
		{
			title = gLanguageCsv.zawakeStageLevelAwake,
			labelDatas = v.labelDatas,
			align = align,
			pos = pos,
			stageID = v
				.stageID
