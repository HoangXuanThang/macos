			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					if v.effect then
						...
						local listHeight = list.innerList:height() * verticalNum +
							childs.innerList:getItemsMargin() * (verticalNum - 1)
...
function GateSweepView:onCreate(params)
	...
	if params.targetId and params.targetNum then
		...
		if hasNum >= params.targetNum then
			...
			adapt.setTextAdaptWithSize(sweepInfo,
				{ size = cc.size(300, 100), vertical = "center", horizontal = "left", margin = -4 })
	...
	adapt.oneLinePos(self.sweepInfo:get("textNote1"), { self.textNoteNum1, self.sweepInfo:get("textNoteGe1") },
		{ cc.p(10, 0) }, "left")
	adapt.oneLinePos(self.sweepInfo:get("textNote2"), { self.textNoteNum2, self.sweepInfo:get("textNoteGe2") },
		{ cc.p(10, 0) }, "left")
...
function GateSweepView:cloneTitleItem(index, exp, isExtra, isTotal, dailyDatas)
	...
	if isExtra == true then
		...
		if self.from == "dailyAssistant" then
			...
			adapt.oneLinePos(titleItem:get("textTitle"), { titleItem:get("textExpNote"), titleItem:get("textExpNum") },
				cc.p(20, 0), "left")
		end
	end
