...
local function showSpine(unitID, item)
	local unitCfg = csv.unit[unitID]
	if unitCfg then
		local unitRes = string.gsub(unitCfg.unitRes, ".skel", "_ui.skel")
		widget.addAnimation(item, unitRes, "standby_loop", 5)
			:xy(item:getContentSize().width / 2, 0)
			:scale(unitCfg.scaleU * 0.1)
...
-- 对高级关卡按钮的详细设置
local function setItemLevelName(item, icon, sceneName, bossNodeRes, unitID)
	...
	if bossNodeRes then
		item:get("imgBG"):loadTexture(bossNodeRes):visible(false)
	end

	if icon then
		item:get("imgIcon"):texture(icon):visible(true):opacity(0)
		showSpine(unitID, item:get("imgIcon"))
	end


	-- 为精灵图标设置遮罩
	-- local sp = cc.Sprite:create(icon):setContentSize(cc.size(80, 80))
	-- local mask = ccui.Scale9Sprite:create("city/gate/kunnanzhezhao.png"):scale(3)
...
local function setNightmareLevelItem(item, info, node, isPlayAction, allPointTime, isBossNode)
	...
	if isPlayAction then
		...
		performWithDelay(item, function()
			setItemLevelName(item, info.icon, info.sceneName, nil, info.unitID)
...
local function setSpeChapterLevelItem(item, info, node, isPlayAction, allPointTime, isBossNode)
	if isPlayAction then
		...
		performWithDelay(item, function()
			setItemLevelName(item, info.icon, info.sceneName, isBossNode and "city/gate/boss.png" or nil, info.unitID)
		end, delayTime + allPointTime)
...
local function setNormalLevelItem(item, info, node, isPlayAction, allPointTime, isBossNode)
	if isPlayAction then
		...
		if info.icon then
			...
			item:get("spePanel"):get("imgIcon"):texture(info.icon):opacity(0)

			item:size(cc.size(170, 315))
			item:anchorPoint(0.5, 0.3)
			performWithDelay(node, function()
				item:get("spePanel"):visible(true)
				showSpine(info.unitID, item:get("spePanel"):get("imgIcon"))
			end, delayTime + allPointTime)
		end
		performWithDelay(item, function()
		...
		item:get("spePanel"):get("imgTopBG"):texture(bgPath):visible(false)
...
function GateView:onCreate(gateId)
	self.limitCapture:setVisible(false)
	-- 1、屏幕大小适配 参数设置
	...
	idlereasy.any({ self.mapOpen, self.mapType, self.level, self.gateOpen },
		function(_, mapOpen, mapType, level, gateOpen)
			self.chapterOpen = {}
	...
	--世界等级显示
	idlereasy.any({ self.mapType, self.worldLevelInfoListen, self.level },
		function(_, mapType, worldLevelUnlock, selfLevel)
	...
	idlereasy.when(self.pageIdler, function(_, privilegeIndex)
		...
		local chapterString = string.format(gLanguageCsv.levelChapterId, privilegeIndex) ..
			" " .. csv.world_map[chapterId].name
		if matchLanguage({ "en" }) then
			chapterString = string.format(gLanguageCsv.levelChapterId, privilegeIndex) ..
				" " .. csv.world_map[chapterId].name
		end
	...
	-- 左右滑动
	uiEasy.addTouchOneByOne(self.mask, {
		ended = function(pos, dx, dy)
...
function GateView:onInitPage(pageview, list, node, k, v)
	...
	node:get("imgBG"):texture(v.baseMap):xy(1332 + self.off, 752)
	for idx, gateId in ipairs(v.seq) do
		...
		if not item then
			...
			local function playBoxAni(pos, boxName, state)
				...
				bind.touch(list, box,
					{ methods = { ended = functools.partial(list.clickBox, gateId, info.chestAward, idx) } })
			...
			if info.awardPos and starNum > 0 then
				boxIdx = boxIdx + 1
				local awardBox = playBoxAni(cc.p(info.awardPos.x + self.off, info.awardPos.y), "box" .. boxIdx,
					starInfo.chest)
			...
			-- 				bind.touch(list, captureItem,
			-- 					{ methods = { ended = functools.partial(list.captureClick, cfgID, cfg, captureItem) } })
			-- 			end
...
function GateView:checkPreGateIsPass(curChapterCsvId, isTips)
	...
	if preChapterId then
		...
		if not itertools.include(self.chapterOpen[preMapCsv.chapterType], preChapterId) then
			...
			if isTips then
				gGameUI:showTip(string.format(fmt[preMapCsv.chapterType],
					preChapterId - CHAPTER_NUM[preMapCsv.chapterType]))
			end
			return false
		end
	else
...
function GateView:onAfterBuild(list)
	...
	local chapterString = string.format(gLanguageCsv.levelChapterId, self.curPageIdx) ..
		" " .. csv.world_map[chapterId].name
	if matchLanguage({ "en" }) then
		chapterString = string.format(gLanguageCsv.levelChapterId, self.curPageIdx) .. " " ..
			csv.world_map[chapterId].name
	end
	gGameUI.topuiManager:updateTitle(chapterString)
end
...
function GateView:cloudAnimation(mapType)
	...
	self['cloud' .. 2]:visible(false)
	self['cloud' .. 6]:visible(false)
