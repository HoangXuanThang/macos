				...
				onItem = function(list, node, k, v)
					...
					for key, val in ipairs(v.chapterTable) do
						...
						if val.surplusTimes == 0 then
							...
							item:get("btn"):texture("common/btn/btn_nomal_4white.png")
							text.addEffect(item:get("btn.txt"), {outline = {color = ui.NEWCOLORS.OUTLINE.BLUE, size = 3}})
						else
							text.addEffect(item:get("btn.txt"), {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 3}})
						end
						if val.star ~= 3 then
			...
			props = {
				...
				onCell = function(list, node, k, v)
					...
					if v.surplusTimes == 0 then
						...
						childs.btn:texture("common/btn/btn_nomal_4white.png")
						text.addEffect(childs.btn:get("txt"), {outline = {color = ui.NEWCOLORS.OUTLINE.BLUE, size = 3}})
					else
						text.addEffect(childs.btn:get("txt"), {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 3}})
					end
					if v.star ~= 3 then
...
function GateQuickView:onSweepBtn(list, key, val)
	...
	if surplusTimes == 0 then
		...
		local strs = {
			...
			"#C0x8A5B0C#"..string.format(gLanguageCsv.resetNumberEliteLevels2,buyHerogateTimes[val.id] or 0,buyTimeMax)
		}
