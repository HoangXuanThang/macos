			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					if v.isSelected then
						text.addEffect(textNote, {color = ui.COLORS.NORMAL.WHITE, outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 3}})
					else
						--text.deleteEffect(textNote, {"outline"})
						text.addEffect(textNote, {color = ui.COLORS.NORMAL.WHITE,outline = {color = ui.NEWCOLORS.OUTLINE.BLUE, size = 3}})
					end
				end,
			},
			handlers = {
...
function MailView:onCreate()
	...
	idlereasy.any({self.mailboxIdler, self.readMailboxIdler}, function (_, mailbox, readMailbox)
		...
		text.addEffect(self.textNum1, {color = num1 - num2 >= -5 and ui.COLORS.NORMAL.ALERT_ORANGE or ui.NEWCOLORS.BROWN1})
	...
	idlereasy.when(self.btnGetShow, function (obj, state)
		...
		--cache.setShader(self.getTextNote, false, state and "normal" or "hsl_gray")
		text.addEffect(self.getTextNote, {outline = {color = state and ui.NEWCOLORS.OUTLINE.YELLOW or ui.NEWCOLORS.OUTLINE.GRAY, size = 4}})
