...
local function onInitItem(list, node, k, v)
	...
	text.addEffect(node:get("btnBuy.txt"), {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 3}})
		...
		binds = {
			...
			props = {
				...
				onBeforeBuild = function(list)
					...
					list:setScrollBarPositionFromCorner(cc.p(listX + listSize.width - x-20, (listSize.height - size.height) / 2 + 5))
