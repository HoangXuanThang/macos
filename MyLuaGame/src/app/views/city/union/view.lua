		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					richText:xy(100, richText:height()/2)
...
function UnionView:onCreate(_, crossUnionData)
	...
	local effectName = (self.dailyGiftTimes:read() >= 1 or isLock or dataEasy.notUseUnionBuild()) and "standby_loop" or "kelingqu_loop"
	widget.addAnimationByKey(self.dailygift, "union/gonghuimeiri.skel", "effect", effectName, 1):xy(170, 90)
	...
	idlereasy.when(self.unionLv, function(_, unionLv)
		...
		for i,v in pairs(BUILDINGS) do
			...
			if lockInfo then
				-- mask:visible(isLock)
				cache.setShader(item:get("imgIcon"), false, isLock and "gray" or "normal")
				-- mask:visible(false)
