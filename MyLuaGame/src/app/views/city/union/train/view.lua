...
local function setPanelChange(list, panelChange, v)
	if v then
		...
		local color = v.level >= v.roleLv and ui.COLORS.NORMAL.FRIEND_GREEN or ui.NEWCOLORS.BROWN1
...
local function setTextColor(childs, state)
	local color = not state and ui.NEWCOLORS.BROWN1 or ui.NEWCOLORS.BROWN2
...
UnionTrainView.RESOURCE_BINDING = {
	...
	["myItem"] = "myItem",
	["mySubList"] = "mySubList",
	["myItem.panelUnlock.btnUnlock.title"] = {
		binds = {
			event = "effect",
		...
		binds = {
			...
			props = {
				...
				onCell = function(list, node, k, v)
					...
					text.addEffect(btnChange:get("title"), {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4}, color = ui.COLORS.NORMAL.WHITE})
					local btnPutIn = childs.panelPutIn:get("btnPutIn")
					text.addEffect(btnPutIn:get("title"), {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4}, color = ui.COLORS.NORMAL.WHITE})
		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					if(canClick)then
						text.addEffect(btn:get("title"), {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4}})
					else
						text.deleteAllEffect(btn:get("title"))
					end
...
function UnionTrainView:onCreate()
	...
	idlereasy.when(self.unionTrainingSpeedup,function(_, unionTrainingSpeedup)
		...
		if(unionTrainingSpeedup)then
			text.addEffect(self.btnSpeedUp:get("title"), {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4}})
		else
			text.deleteAllEffect(self.btnSpeedUp:get("title"))
		end
