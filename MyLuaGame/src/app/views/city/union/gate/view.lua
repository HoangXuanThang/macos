...
function UnionGateView:onCreate()
	...
	idlereasy.when(self.unionFbTimes, function(_, unionFbTimes)
		...
		if(canBattle)then
			text.addEffect(self.btnFight:get("title"), {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4}})
			text.addEffect(self.btnThreeFight:get("title"), {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4}})
		else
			text.deleteAllEffect(self.btnFight:get("title"))
			text.deleteAllEffect(self.btnThreeFight:get("title"))
		end
	end)
...
function UnionGateView:setRightPanel(gateData)
	...
	self.textAttrNote:text(gLanguageCsv.unionGateAttrNote)
