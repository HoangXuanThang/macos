...
local function setCostTxt(childs, myCoin, needCoin)
	...
	local coinColor = (myCoin >= needCoin) and ui.NEWCOLORS.BROWN1 or ui.NEWCOLORS.REDLIGHT
		...
		binds = {
			...
			props = {
				...
				onCell = function(list, node, k, v)
					...
					text.addEffect(node:get("level"), {outline={color=ui.NEWCOLORS.OUTLINE.BLACK}})
...
function UnionSkillView:setRightPanel(skinId, myCoin)
	...
	text.addEffect(childs.level, {outline={color=ui.NEWCOLORS.OUTLINE.BLACK}})
	--公会经验表
	...
	if(not notTouch)then
		text.addEffect(childs.btnPractice:get("title"), {outline = {color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4}})
	else
		text.deleteAllEffect(childs.btnPractice:get("title"))
	end
