...
local function initChipPanel(list, panel, suitItem, k, v)
	...
	if v.txt or v.txt1 then
		...
		txt:text(v.txt or "")
		txt1:text(v.txt1 or "")
		adapt.oneLineCenterPos(cc.p(childs.txtPanel:width() / 2, childs.txtPanel:height() / 2), { txt, txt1 },
			cc.p(20, 0))
		if v.leftTimes and v.leftTimes == 0 then
			text.addEffect(txt1, { color = cc.c3b(251, 96, 35) })
		end
...
local function initNormalPanel(list, panel, title, k, v)
	...
	if v.rich then
		...
		rich:setContentSize(cc.size(850, 250))
	...
	if v.selected then
		...
		if txt:width() > childs.btn:width() then
			...
			adapt.oneLineCenterPos(cc.p(childs.selectPanel:width() / 2, childs.selectPanel:height() / 2 + 7),
				{ sPanel, txt }, cc.p(5, 0))
		end
...
DailyAssistantView.RESOURCE_BINDING = {
	...
	["rightPanel.btnOneClick.txtNode"] = {
		binds = {
			event = "effect",
			data = { outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4 } }
		},
	},
	["item.normalPanel.btn.txtNode"] = {
		binds = {
			event = "effect",
			data = { outline = { color = ui.NEWCOLORS.OUTLINE.YELLOW, size = 4 } }
		},
	...
	["list"] = {
		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					local childs = node:multiget("title", "activeImg", "unionGatePanel", "normalPanel",
						"unionRedpacketPanel", "chipPanel")
					...
					bind.touch(list, childs.normalPanel:get("btn"),
						{ methods = { ended = functools.partial(list.clickCell, k, v) } })
					bind.touch(list, childs.unionGatePanel:get("btnReward"),
						{ methods = { ended = functools.partial(list.unionGateCell, k, v) } })
...
function DailyAssistantView:onCreate()
	...
	for k, val in pairs(gDailyAssistantCsv) do
		...
		if cfg.inUnlock == 0 or dataEasy.isShow(k) then
			self.allDatas[cfg.type][k] = { csvId = val.csvId, sortID = cfg.sortID, name = cfg.name, type = cfg.type, feature =
			cfg.features }
		end
	end
	...
	-- 冒险之路
	idlereasy.any({ self.curChallengeId, self.resetCount, self.maxGateId },
		function(_, curChallengeId, resetCount, maxGateId)
			local selGateIdx
	...
	-- 公会副本奖励,公会副本次数
	idlereasy.any({ self.unionFubenPassed, self.unionFbTimes, self.unionFbAward },
		function(_, unionFubenPassed, unionFbTimes, unionFbAward)
	...
	-- 钓鱼
	idlereasy.any({ self.fishingSelectScene, self.selectBait, self.selectRod, self.items },
		function(_, fishingSelectScene, selectBait, selectRod, items)
...
local UPDATE_SHOW_FUN = {
	...
	gainGold = function(view, val, featuresLeftTimes, activeTxt)
		-- 聚宝
		local str, leftTimes = dailyAssistantTools.getGainGoldTimes(view.lianjinTimes:read(),
			view.lianjinFreeTimes:read())
		val.rich = str
	...
	chip = function(view, val)
		-- 秘籍抽取 显示次数，若等级不够则显示unlock对应开放等级
	...
	activityGate = function(view, val, featuresLeftTimes, activeTxt)
		local leftTimes, allTimes, DoubleActivityTxt, isDoubleActivity, haveReunion = dailyAssistantTools
		.getActivityGateInfo()
	...
	endlessTower = function(view, val, featuresLeftTimes, activeTxt)
		...
		if not dataEasy.isUnlock("fishing") then
			...
			if val.active then
				local str = string.format("%s: %s", gDailyAssistantCsv[val.feature].cfg.name,
					gLanguageCsv.FishingCompetition)
				table.insert(activeTxt, str)
			end
			featuresLeftTimes[val.feature] = leftTimes
		end
...
function DailyAssistantView:updateShowDatas(showType)
	...
	self.btnOnekeyState:set({ showType = showType, leftFinished = leftFinished, featuresLeftTimes = featuresLeftTimes, activeTxt =
	activeTxt })
			...
			for k, val in pairs(gDailyAssistantCsv) do
				if val.csvId == csvID then
					...
					elseif k == "unionFragDonate" then
						name = dailyAssistantTools.getCardFragmentsName(self.dailyAssistant:read()
						.union_frag_donate_card_id)
					end
					if name then
...
function DailyAssistantView:showSweepView(featuresLeftTimes, view, award)
	...
	if award then
		for k, val in pairs(award) do
			...
			if val.bossKilled then
				local content = "#Pfont/youmi.ttf##C0xFF57472E#" .. gLanguageCsv.crossMineBossHasKilled
	...
	if not itertools.isempty(beizhuFeatures) then
		...
		if itertools.isempty(sweepData) then
			...
			table.insert(notes, "#C0xFF57472E#" .. content)
		end
	end
...
function DailyAssistantView:onBtnThreeFight()
	...
	local notBattled = (gateData.surplusHp == 0)
	...
	if battleNumRemain > 0 and (not notBattled) then
		postThree()
	else
		self:sendRequestServerOnekey()
	end
...
function DailyAssistantView:getRuleContext(view)
	...
	local context = {
		c.clone(view.title, function(item)
			item:get("text"):text(string.format(gLanguageCsv.dailyAssistantActiveTxt,
				TAB_TYPE_NAME[self.tabSelected:read()]))
		end),
	}
