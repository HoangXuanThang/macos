...
function GuideManager:canTriggerGuide(cfg, params)
	...
	local extendCondition = eval.doFormula(cfg.extendCondition, { finish = functools.partial(self.checkFinished, self) },
		cfg.extendCondition)
...
function GuideManager:showDialog(cfg, cb, params)
	...
	if not gGameUI.guideLayer:get("dialog") then
		...
		text.addEffect(self.guideView.skipBtn:get("text"),
	...
	local talkContent = cfg['talkContent_'..LOCAL_LANGUAGE] or cfg.talkContent
	-- 对话
	local str = "#C0xFFFFFF##F50#" .. talkContent
...
function GuideManager:showClickGuide(cfg, cb, params)
	...
	maskPanel:onClick(function(event)
		...
		if not params.isBattle then
			...
			local isRightTop = (event.x >= display.sizeInView.width - range.width) and
				(event.y >= display.sizeInView.height - range.height)
	...
	if talkContent ~= "" then
		...
		if cfg.roleName and cfg.roleName ~= "" then
	...
	local function targetNodeCb(targetNode)
		if targetNode then
			if not cfg.anyPosCancel and not targetNode:isTouchEnabled() then
				printWarn("guide targetNode:isTouchEnabled(目标控件定位可能存在问题) id(%s): <%s>", tostring(self.curGuideId),
					tostring(table.concat(cfg.tagName, ";")))
			end
			...
			local colorLayer = cc.LayerColor:create(cc.c4b(0, 0, 0, 150), display.sizeInView.width,
				display.sizeInView.height)
			...
			local clickLayer = cc.LayerColor:create(cc.c4b(255, 0, 255, 150), display.sizeInView.width,
					display.sizeInView.height)
		...
		view:schedule(function(dt)
			...
			if (targetNode or delta > 1) then
				if not targetNode then
					printWarn("guide id(%s) 检查控件配置当前状态定位不到: <%s>", tostring(self.curGuideId),
						tostring(table.concat(cfg.tagName, ";")))
				end
