...
local function postDisableWordCheck(msg, msgType, cb)
	print(" Bỏ qua check từ khoá, URL nil")
	return cb({ ret = true }, nil)
	-- if DISABLE_WORD_CHECK_URL == nil then
	-- 	print(" Bỏ qua check từ khoá, URL nil")
	-- 	return cb({ ret = true }, nil)
	-- end
	-- print(" Bắt đầu check từ khoá với URL:", DISABLE_WORD_CHECK_URL)
	...
	-- local reqUrl = string.format("%s?account=%s&server=%s&role=%s&type=%s&level=%d&vip=%d", DISABLE_WORD_CHECK_URL,
	-- 	accountID, gameKey, roleID, msgType, level, vip)
	...
	-- 				gGameUI:onBan()
	-- 			end
	-- 			print("Kết thúc check: Từ khoá không hợp lệ")
	-- 			return cb(nil, { err = "msgInvalid" })
	-- 		end
	-- 	else
	-- 		print(" Lỗi hoặc timeout khi check, cho phép pass (fallback)")
	-- 		return cb({ ret = true }, nil)
	-- 	end
	-- 	print("Kết thúc check: Hợp lệ")
	-- 	return cb({ ret = true }, nil)
	-- end, 5)
end
...
function GameProtocol:login(url, cb, userName, pass, isregister)
	-- userName = 'tc_2854956' --me
	return self.net:doLogin(userName, pass, isregister, cb)
end

GameProtocol["/game/get-user-info"] = function(self, url, cb, data)
	-- Gửi packet lên server
	return self.net:sendPacket(url, data, cb)
end

GameProtocol["/game/update-device-info"] = function(self, url, cb, data)
	-- Gửi packet lên server
	return self.net:sendPacket(url, data, cb)
end
...
-- 好友挑战
GameProtocol["/game/society/friend/fight"] = function(self, url, cb, roleID, recordID, serverKey)
	local data = { roleID = roleID, recordID = recordID, serverKey = serverKey }
	return self.net:sendPacket(url, data, cb)
end
...
-- 已经是通用接口
GameProtocol["/game/role_info"] = function(self, url, cb, roleID, serverKey)
	local data = { roleID = roleID, serverKey = serverKey }
	return self.net:sendPacket(url, data, cb)
end

-- 已经是通用接口
GameProtocol["/game/card_info"] = function(self, url, cb, cardID, serverKey)
	local data = { cardID = cardID, serverKey = serverKey }
	return self.net:sendPacket(url, data, cb)
end
...
-- 卡牌动画和立绘切换
GameProtocol["/game/card/spine/switch"] = function(self, url, cb, cardID)
	local data = { cardID = cardID }
	return self.net:sendPacket(url, data, cb)
end
...
GameProtocol["/game/explorer/ability/strength"] = function(self, url, cb, seqID, position)
	local data = { seqID = seqID, position = position }
	return self.net:sendPacket(url, data, cb)
end
...
-- 试炼塔 试炼战斗结束
GameProtocol["/game/random_tower/end"] = function(self, url, cb, battleID, result, star, cardStates, enemyStates,
												  battleRound)
	local data = {
		battleID = battleID,
		result = result,
...
--跨服资源战战斗开始
GameProtocol["/game/cross/mine/battle/start"] = function(self, url, cb, flag, myRank, enemyRank, enemyRoleID,
														 enemyRecordID)
	local versionPlist = cc.FileUtils:getInstance():getValueMapFromFile('res/version.plist')
	local data = {
		flag = flag,
		...
		enemyRecordID =
			enemyRecordID,
		patch = tonumber(versionPlist.patch)
	}
	return self.net:sendPacket(url, data, cb)
end
...
-- 勇者挑战 结束挑战
GameProtocol["/game/yy/brave_challenge/battle/end"] = function(self, url, cb, battleID, floorID, result, cardStates,
															   monsterStates, battleRound, damage, actions)
	local data = {
		battleID = battleID,
		floorID = floorID,
		result = result,
...
-- 普通勇者挑战 结束挑战
GameProtocol["/game/brave_challenge/battle/end"] = function(self, url, cb, battleID, floorID, result, cardStates,
															monsterStates, battleRound, damage, actions)
	local data = {
		battleID = battleID,
		floorID = floorID,
...
-- 夏日挑战 结束挑战
GameProtocol["/game/yy/summer_challenge/battle/end"] = function(self, url, cb, battleID, gateID, result, actions, skills,
																choices)
	local data = {
		battleID = battleID,
		gateID = gateID,
...
-- fere house cloth unlock
GameProtocol["/game/lover/cloth/unlock"] = function(self, url, cb, fereId, page, index)
	local data = { loverID = fereId, clothID = page, location = index }
	return self.net:sendPacket(url, data, cb)
end

-- fere house video unlock
GameProtocol["/game/lover/video/unlock"] = function(self, url, cb, fereId, page, index)
	local data = { loverID = fereId, videoID = page, location = index }
	return self.net:sendPacket(url, data, cb)
end

-- fere house enhance
GameProtocol["/game/lover/bless/card"] = function(self, url, cb, fereId, cardID)
	local data = { loverID = fereId, cardID = cardID }
	return self.net:sendPacket(url, data, cb)
end

-- fere house award
GameProtocol["/game/lover/award"] = function(self, url, cb, fereId)
	local data = { loverID = fereId }
	return self.net:sendPacket(url, data, cb)
end

-- 神兽升星锁定
GameProtocol["/game/fatecard/lock"] = function(self, url, cb, yyID, cardID)
	local data = { yyID = yyID, cardID = cardID }
	return self.net:sendPacket(url, data, cb)
end

-- 神兽升星免费领取
GameProtocol["/game/fatecard/star/freeaward/get"] = function(self, url, cb, yyID, csvID)
	local data = { yyID = yyID, csvID = csvID }
	return self.net:sendPacket(url, data, cb)
end

-- 举报
GameProtocol["/game/role/report"] = function(self, url, cb, kind, serverKey, roleId, level, vip, name, msg, msgID)
	local data = {
		kind = kind,
		server = serverKey,
		roleId = roleId,
...
-- 代币购买
GameProtocol["/game/buy/daibi"] = function(self, url, cb, yyID, csvID, rechargeID, amount, propName)
	local data = { yyID = yyID, csvID = csvID, rechargeID = rechargeID, amount = amount, prop_name = propName }
	return self.net:sendPacket(url, data, cb)
end

-- 橙色携带商店购买
GameProtocol["/game/helditem/shop/buy"] = function(self, url, cb, csvID, count)
	if count and count < 0 then
		return showCheatError(cb)
	end
	local data = { csvID = csvID, count = count }
...
-- 指引礼包
GameProtocol["/game/role/guide/gift/active"] = function(self, url, cb, itemID)
	local data = { itemId = itemID }
	return self.net:sendPacket(url, data, cb)
end

--宝可梦世界锦标赛 主界面
GameProtocol["/game/cross/supremacy/main"] = function(self, url, cb)
	local data = {}
	return self.net:sendPacket(url, data, cb)
end

--宝可梦世界锦标赛 战斗开始
GameProtocol["/game/cross/supremacy/battle/start"] = function(self, url, cb, enemyRoleID, enemyRecordID, flag)
	local data = { enemyRoleID = enemyRoleID, enemyRecordID = enemyRecordID, flag = flag }
	return self.net:sendPacket(url, data, cb)
end

--宝可梦世界锦标赛 战斗结束
GameProtocol["/game/cross/supremacy/battle/end"] = function(self, url, cb, result, isTopBattle)
	local data = { result = result, isTopBattle = isTopBattle }
	return self.net:sendPacket(url, data, cb)
end

--宝可梦世界锦标赛 获取对方信息
GameProtocol["/game/cross/supremacy/enemy/get"] = function(self, url, cb, recordID, gameKey)
	local data = { recordID = recordID, gameKey = gameKey }
	return self.net:sendPacket(url, data, cb)
end

--宝可梦世界锦标赛 布阵
GameProtocol["/game/cross/supremacy/deploy"] = function(self, url, cb, cards, defenceCards, eliteCards)
	local data = { cards = cards, defenceCards = defenceCards, eliteCards = eliteCards }
	return self.net:sendPacket(url, data, cb)
end

--宝可梦世界锦标赛 排名
GameProtocol["/game/cross/supremacy/rank"] = function(self, url, cb, offset, size, season)
	local data = { offset = offset, size = size, season = season }
	return self.net:sendPacket(url, data, cb)
end

--宝可梦世界锦标赛 战报
GameProtocol["/game/cross/supremacy/playrecord/get"] = function(self, url, cb, recordID, gameKey)
	local data = { recordID = recordID, gameKey = gameKey }
	return self.net:sendPacket(url, data, cb)
end

--宝可梦世界锦标赛 领取产出奖励
GameProtocol["/game/cross/supremacy/res/get"] = function(self, url, cb)
	local data = {}
	return self.net:sendPacket(url, data, cb)
end

GameProtocol["/cross/supremacy/times/buy"] = function(self, url, cb, flag)
	local data = { flag = flag }
	return self.net:sendPacket(url, data, cb)
end

--宝可梦世界锦标赛 商店
GameProtocol["/game/cross/supremacy/shop"] = function(self, url, cb, csvID, count)
	if count and count < 0 then
		return showCheatError(cb)
	end
	local data = { csvID = csvID, count = count }
...
--宝可梦世界锦标赛 匹配
GameProtocol["/game/cross/supremacy/match"] = function(self, url, cb, isRematch)
	local data = { isRematch = isRematch }
	return self.net:sendPacket(url, data, cb)
end

--宝可梦世界锦标赛 放弃匹配
GameProtocol["/game/cross/supremacy/match/cancel"] = function(self, url, cb)
	local data = {}
	return self.net:sendPacket(url, data, cb)
end
...
GameProtocol["/game/card/star/swap"] = function(self, url, cb, fieldID, cardID, targetCardID, action)
	return self.net:sendPacket(url, {
		fieldID = fieldID,
		cardID = cardID,
...
-- 主题英雄抽奖 无双盛典
GameProtocol["/game/lottery/yy/draw"] = function(self, url, cb, yyID, drawType)
	local data = { yyID = yyID, drawType = drawType }
	return self.net:sendPacket(url, data, cb)
end

-- 主题英雄抽奖 宝箱领取
GameProtocol["/game/lottery/yy/draw/box/get"] = function(self, url, cb, yyID, csvID)
	local data = { yyID = yyID, csvID = csvID }
	return self.net:sendPacket(url, data, cb)
end

-- 主题皮肤抽奖 盘龙神迹
GameProtocol["/game/lottery/skin/draw"] = function(self, url, cb, yyID, drawType)
	local data = { yyID = yyID, drawType = drawType }
	return self.net:sendPacket(url, data, cb)
end

--走格子主界面
GameProtocol["/game/yy/monopoly/main"] = function(self, url, cb, yyID)
	local data = { yyID = yyID }
	return self.net:sendPacket(url, data, cb)
end

--走格子使用道具
GameProtocol["/game/yy/monopoly/itemuse"] = function(self, url, cb, yyID, itemID, count)
	local data = { yyID = yyID, itemID = itemID, count = count }
	return self.net:sendPacket(url, data, cb)
end

--登阶
GameProtocol["/game/yy/throne/itemuse"] = function(self, url, cb, yyID, itemID, count)
	local data = { yyID = yyID, itemID = itemID, count = count }
	return self.net:sendPacket(url, data, cb)
end

GameProtocol["/game/yy/throne/ranking"] = function(self, url, cb, yyID)
	local data = { yyID = yyID }
	return self.net:sendPacket(url, data, cb)
end
