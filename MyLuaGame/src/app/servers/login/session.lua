...
function LoginSession:init(host, port, userName, pass, isregister, cb)
...
local function getGuarderMD5()
	...
	--print("MD5 img:", guarder.get_file_md5("res/img/helper/70ba7e14140b0097.jpg"))
	local files = {
	...
	for _, path in ipairs(files) do
		...
		local m, s = guarder.get_file_md5(path, md5str)
		--print(string.format("[DEBUG] FILE: %s MD5: %s SIZE: %d", path, m, s))
		md5str, filesize = m, s
		sum = sum + filesize
	end
	...
	-- win下会自动关闭客户端, 线上不让登陆
	local GUARDER_MD5 = "eaf92613d6c94c98713fa59895717d68"
	if device.platform == "windows" or device.platform == "android" then
		--print("[LOG] Checking GUARDER_MD5...")
		if md5str ~= GUARDER_MD5 then
			-- printError('GUARDER_MD5 err in login %s %s %s', GUARDER_MD5, md5str, sum)
			-- print("[LOG] MD5 mismatch:")
			-- print("[LOG] EXPECT:", GUARDER_MD5)
			-- print("[LOG] FOUND :", md5str)
			-- print("[LOG] SUM   :", sum)
			-- printError("MD5 mismatch, closing...")
...
function LoginSession:_onSendLogin()
	...
	self.net:sendPacket("/login/check", {
		...
		channel = dev.ONLINE_VERSION_LANGUAGE or sdk.getChannel(self.userName),
