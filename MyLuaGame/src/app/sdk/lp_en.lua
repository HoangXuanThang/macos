	...
	sdk.callPlatformFunc("login", "", function(info)
		...
		elseif info == "cancel" then
			...
			if APP_CHANNEL =="lp_en" then
				...
				cc.UserDefault:getInstance():setStringForKey("accessToken", infoTable.token)
				cc.UserDefault:getInstance():setStringForKey("username", infoTable.display_name)
				cc.UserDefault:getInstance():flush()
				--print("save username = ", cc.UserDefault:getInstance():getStringForKey("username", ""))
...
-- function lp_en.fastLogin(cb)
--     print("Lua lp_en.fastLogin")

--     sdk.callPlatformFunc("fastLogin", "game", function(info)
-- 		print("logout ret = ", info)
-- 		-- 有回调就是成功
-- 		cb(0, "ok")
-- 	end)
			...
			if datas ~=nil then
				if datas["code"] =="0" then
				 ...
				 if datas["code"] =="-1" then
					gGameUI:showTip("Insufficient balance in your account. Please top up to continue.")
					cb(-1, "error")
				 end

				if datas["code"] =="-2" then
					gGameUI:showTip("Your information hasn't been updated. Please restart the game and log in again.")
					cb(-1, "error")
				end
			else
				gGameUI:showTip("Unknown error. Please contact the administrator.")
				cb(-1, "error")
			end
		else
			gGameUI:showTip("Unknown error. Please contact the administrator.")
...
function lp_en.purchase(username,serverId,roleId,skuId,cb)
	print("Lua lp.purchase")
	local roleInfo = gGameModel.role
    local accountId = stringz.bintohex(gGameModel.role:read("account_id"))
    local userName = gGameModel.account:read("name")
	local tmp = {
	    username = userName,
	    serverId = tostring(roleInfo:read("area")),
	    roleId = stringz.bintohex(roleInfo:read("id")),
	    skuId   =  skuId
	}
	sdk.callPlatformFunc("purchase", json.encode(tmp), function(info)
		if info ~=nil then
			local datas = json.decode(info)
			if datas ~=nil then
				if datas["code"] =="0" then
...
function lp_en.openUrl(url, cb)

	sdk.callPlatformFunc("openUrl", url, function(info)
		print("lp_en call openURL=====",url)
	end)
end
