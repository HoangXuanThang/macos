...
-- 	local params = string.format("accountId=%s&orderStatus=1&orderId=%s&amount=%d&game_extra=%s",
...
function none.pay(orderId, extInfo, amount, rechargeId, productDesc, notify_url, sign, cb)
	...
	local accountId = stringz.bintohex(gGameModel.role:read("account_id"))
	local params = string.format("accountId=%s&orderStatus=1&orderId=%s&amount=%d&game_extra=%s",
		accountId, orderId, amount, extInfo)

	gGameApp.net:sendHttpRequest("POST", PAYMENT_URL.. "/payment",
		params,
		cc.XMLHTTPREQUEST_RESPONSE_STRING,
		function(xhr)
			if xhr.status == 200 then
