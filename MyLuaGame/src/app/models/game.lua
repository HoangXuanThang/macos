...
local PLAYRECORDS = {
	...
	cross_supremacy_playrecords = "cross_supremacy_battle",    -- 跨服PVP（世界锦标赛）战斗
...
function GameModel:ctor()
	...
	self.skinCardBalance = idler.new(0)
	...
	if t.model then
		...
		if t.model.role then
			...
			self.currday_dispatch:init({
				...
				luckyCat = false,
				miixGiftClick = false,
				themeSkinGiftClick = false,
				themeSkinGiveClick = false,
				themeSkinExchangeClick = false,
				goldLuckyCat = false,
				randomTower = false,
				passport = false,
		...
		if t.model.cross_online_fight_banpick then
			local banpick = require('app.models.cross_online_fight_banpick').new(self):init(t.model
				.cross_online_fight_banpick)
...
function GameModel:playRecordBattle(play_record_id, cross_key, interface, exChangeType, roleId)
	...
	if data.limited_card_deck and data.banpick_input_steps then
		gGameUI:stackUI("city.pvp.online_fight.ban_embattle", nil, { full = true },
			{ recordData = data, startFighting = functools.partial(self.onPlayRecord, self, battle) })
		return
	end
