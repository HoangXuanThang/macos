	...
	for k, v in csvPairs(csvTasks) do
		if v.huodongID == huodongID then
			if YYHuoDongTasksWatchTargetMap[v.taskType] then
				...
				progress[k] = getTargetProgress(v.taskType, val, v.taskParam, v.taskSpecialParam, self
				._taskProgressCache)
			else
...
local AchieveDefs = {
	...
	DrawChipRMB = 95,      --钻石抽秘籍次数
	DrawChipItem = 96,     --道具抽秘籍次数
}
...
local AchievementMap = {
	...
	[AchieveDefs.ExplorerActiveCount] = function(self)
		...
		cache.ExplorerActiveCount = cache.ExplorerActiveCount or
		itertools.count(self:read("explorers"), function(_, t) return t.advance >= 1 end)
	...
	[AchieveDefs.ExplorerLevel] = function(self)
		local cache = self.explorersCache
		cache.ExplorerLevel = cache.ExplorerLevel or
		(itertools.max(self:read("explorers"), function(_, t) return t.advance end) or 0)
