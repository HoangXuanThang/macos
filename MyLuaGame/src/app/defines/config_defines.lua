...
-- if APP_CURRENCY ~= "cny" then
-- 	csv.recharges = csv["recharges_" .. APP_CURRENCY]
-- 	csv.language = csv["language_" .. APP_CURRENCY]
-- end
		...
		if matchLanguage(v.languages) then
			if v.canDevelop then
				...
				if t[v.cardMarkID][v.develop][v.branch] then
					printError("cards id(%d), develop(%d), branch(%d) 与 id(%d) 重复，检查配置", k, v.develop, v.branch,
						t[v.cardMarkID][v.develop][v.branch].id)
					return
				end
...
local ignoreDuplicateKey = arraytools.hash({ "iconNumNormal", "rarity0", "monthCardPrivilege11", "shopTab1", "shopTab2",
	"rarityFrag0", "rarityFrag1", "rarityFrag2", "rarityFrag3", "rarityFrag4" })

globals.gLanguageCsv = (function(t)
	for k, v in orderCsvPairs(csv.language) do
			local text = v["text_" .. LOCAL_LANGUAGE] or v.text
		if t[v.key] then
				error(string.format("gLanguageCsv key duplicate! 【%d】: %s(%s)", k, v.key, text))
		end
			if text ~= "" and textDuplicateTest[text] then
				local duplicate = textDuplicateTest[text]
			-- 忽略服务器的文本重复
			...
			t[v.key] = text
			textDuplicateTest[text] = { id = k, key = v.key }
	end
	return t
end)({})
...
-- 神兵特性
_genGlobalDynamicIndex("gExplorerAbilityCsv", function(t)
	for i, v in orderCsvPairs(csv.explorer.ability) do
		if not t[v.seqID] then
			t[v.seqID] = {}
		end
		t[v.seqID][v.position] = v
	end
end)
...
globals.gShopType = {
	...
	--[17] = csv.held_item.shop,
	[18] = csv.cross.supremacy.shop,
}
-- 判别可以从商店中获取的道具
...
local destServerKey = {}
local curSvrTag = ""
if sdk and sdk.getServerKind then
	curSvrTag = sdk.getServerKind()
	if curSvrTag == "DLDX1" then
		curSvrTag = "DLDX"
	end
end
printInfo("curSvrTag %s", curSvrTag)
for k, v in orderCsvPairs(csv.server.merge) do
	if curSvrTag == v.kind then
		if destServerKey[v.destServer] then
			error(string.format("csv.server.merge: (%s) can't exist in (%d) and (%d) at the same time", v.destServer,
				destServerKey[v.destServer], k))
		end
		destServerKey[v.destServer] = k
		for _, key in ipairs(v.servers) do
			if serverKeys[key] then
				error(string.format("csv.server.merge: (%s) can't exist in (%d) and (%d) at the same time", key,
					serverKeys[key], k))
			end
			serverKeys[key] = k
		end
...
-- 伴侣好感经验道具
_genGlobalDynamicIndex("gFereExpItemCsv", function(t)
	for k, v in orderCsvPairs(csv.items) do
		if v.type == game.ITEM_TYPE_ENUM_TABLE.fereFeelExpType then
			tinsert(t, v)
		end
	end
end)
