...
local function getColor(str)
	...
	if #str > 8 then
		return cc.c4b(math.floor(num / 65536 % 256), math.floor(num / 256 % 256), num % 256,
...
local function _generateRichTexts(array, fontSize, deltaSize, adjustWidth)
	...
	for k, t in ipairs(array) do
		...
		elseif t.type == "spine" then -- 特效
			...
			if t.action == "default" then
				spine:play(t.action, t.loop)
			else
				spine:play(t.action)
			end
						...
						table.insert(T2,
							{
								type = "flags",
								flags = "outlineColor",
								val = cc.c4b(cfg.color[1], cfg.color[2],
									cfg.color[3], 255)
							})
