...
function tableview:onItem_(node, k, v)
	if node.listviewAction_ == true then
		...
		for _, baseList in pairs(node:getChildren()) do
...
function tableview:buildExtend()
	...
	self.paddings = self.paddingData
...
function tableview:building()
	...
	self.preloadCenterPos = self.preloadCenterIndex or
		(self.keyHash_[self.preloadCenter_] and getRowCol(self.keyHash_[self.preloadCenter_], self.columnSize))
