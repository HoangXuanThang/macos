...
local ListViewItemActionTag = 2010231612
...
function listview:buildExtend()
	...
	if self.padding then
		...
		if self:getDirection() == ccui.ListViewDirection.horizontal then
			local panel1 = ccui.Layout:create():size(self.padding + (self.paddings and self.paddings.left or 0),
				height)
			self:pushBackCustomItem(panel1)
			local panel2 = ccui.Layout:create():size(self.padding + (self.paddings and self.paddings.right or 0),
				height)
			self:pushBackCustomItem(panel2)
		else
			local panel1 = ccui.Layout:create():size(width,
				self.padding + (self.paddings and self.paddings.up or 0))
			self:pushBackCustomItem(panel1)
			local panel2 = ccui.Layout:create():size(width,
				self.padding + (self.paddings and self.paddings.down or 0))
			self:pushBackCustomItem(panel2)
		end
	end
	...
	if self.itemAction and self.itemAction.isAction then
		if not self.asyncPreload or cnt <= self.asyncPreload then
			...
			for _, baseNode in pairs(item:getChildren()) do
				...
				if itemAction.call then
					item:stopAllActionsByTag(ListViewItemActionTag)
					local itemAct = transition.executeSequence(item)
						:delay(actionTime + duration)
						:func(function()
							itemAction.call(item, pos)
						end)
						:done()

					itemAct:setTag(ListViewItemActionTag)
				end
			end
		else
