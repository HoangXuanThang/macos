...
local FROM_TABLE_FUNC = {
	[game.EMBATTLE_FROM_TABLE.default] = {
		...
		getSendRequest = function(self)
			-- 空阵容返回直接关闭界面
			if itertools.size(self.clientBattleCards) == 0 then
				return
			end
	...
	[game.EMBATTLE_FROM_TABLE.gymChallenge] = {
		initModelFunc = function(self)
			...
			if gymbattleCards == nil or itertools.size(gymbattleCards) == 0 then
				--没有缓存则取主场景布阵
	...
	idlereasy.when(self.clientBattleCards,function (_, battle)
		...
		for i = 1, self.panelNum do
			...
			if battle[i] then
				...
				local unitCsv = self:getUnitCsv(card_id, skin_id)

				local dbdata = {dbid = dbid, card_id = card_id, skin_id = skin_id}
				if not (spriteTb.dbdata and itertools.equal(spriteTb.dbdata, dbdata)) or i == self.lastDraggingIndex then
...
function CardEmbattleView:createMovePanel(data, i)
	if self.movePanel then
		self.movePanel:removeSelf()
	end
	local unitCsv = self:getUnitCsv(data.card_id, data.skin_id)
...
function CardEmbattleView:onTestBtn()
	...
	local label1 = label.create("unitID:", {fontSize = 45, color = ui.COLORS.NORMAL.DEFAULT})
		:anchorPoint(0, 0.5)
		:xy(50, 500)
		:addTo(panel)
	...
	local label2 = label.create("name:", {fontSize = 45, color = ui.COLORS.NORMAL.DEFAULT})
		:anchorPoint(0, 0.5)
		:xy(50, 400)
		:addTo(panel)
	...
	local label1 = label.create("scale:", {fontSize = 45, color = ui.COLORS.NORMAL.DEFAULT})
		:anchorPoint(0, 0.5)
		:xy(50, 300)
		:addTo(panel)
	...
	bind.touch(self, btn, {methods = {ended = function()
		...
		for i = 1, 10000 do
			...
			if cfg then
				...
				self.input3:setText(self:getScale(cfg))
				break
			end
		end
	end}})
...
function CardEmbattleView:getUnitCsv(cardId, skinId)
	if self.input1 then
		local unitId = tonumber(self.input1:getText())
	...
	return dataEasy.getUnitCsv(cardId, skinId)
end

function CardEmbattleView:getScale(unitCsv)
	if self.input3 then
