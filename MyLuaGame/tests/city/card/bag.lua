			...
			props = {
				...
				onCell = function(list, cell, k, v)
					...
					bind.touch(list, cell, {
						methods = {
							ended = functools.partial(list.itemClick, cell, t, v, k)
						}
					})
				end,
...
function CardBagView:onCreate(bagType)
	...
	idlereasy.any({self.cards, self.battleCards}, function(_, cards, battleCards)
		...
		for i,v in ipairs(cards) do
			...
			if card then
				local cardData = card:multigetIdler("card_id","skin_id", "name", "fighting_point", "level", "star", "advance", "created_time", "locked", "equips", "effort_values")
				idlereasy.any(cardData, function(_, card_id, skin_id, name, fighting_point, level, star, advance, created_time, locked, equips, effort_values)
					...
					star = star,
						...
						lock = locked,
						equips = equips,
						effortValue = effort_values,
						atkType = cardCsv.atkType,
					}
					if #datas==datasCount then
...
--排序item点击 and
function CardBagView:onItemClick(list,item, t, v, k)
	local unitId = self:getUnitId()
	if unitId then
		local data = clone(v)
		data.id = csv.unit[unitId].cardID
		data.icon = csv.unit[unitId].cardShow
		data.scale = tonumber(self.input3:getText()) or 1
		local x = tonumber(self.input4:getText()) or 0
		local y = tonumber(self.input5:getText()) or 0
		data.posOffset = cc.p(x, y)
		list:onItem(item, k, data)
		return
	end
	if v.isSprite then
...
function CardBagView:onStackHide(...)
	self.cardList:pauseFor()
	self.fragList:pauseFor()
	return ViewBase.onStackHide(self, ...)
end

-- resume aysncload when myself show
function CardBagView:onStackShow()
	self.cardList:resumeFor()
	self.fragList:resumeFor()
	return ViewBase.onStackShow(self)
end

function CardBagView:onTestBtn()
	local panel = ccui.Layout:create()
		:addTo(self, 10)
		:size(600, 600)
		:xy(display.sizeInView.width - display.uiOrigin.x - 600, 600)
	self.testPanel = panel
	panel:setBackGroundColorType(1)
	panel:setBackGroundColor(cc.c3b(200, 200, 200))
	panel:setBackGroundColorOpacity(200)

	label.create("unitID:", {fontSize = 45, color = ui.COLORS.NORMAL.DEFAULT})
		:anchorPoint(0, 0.5)
		:xy(50, 500)
		:addTo(panel)
	self.input1 = ccui.EditBox:create(cc.size(300, 60), "common/box/box_topui.png")
		:anchorPoint(0, 0.5)
		:xy(200, 500)
		:addTo(panel)
	self.input1:setFontSize(45)
	self.input1:setFontColor(ui.COLORS.NORMAL.DEFAULT)
	self.input1:registerScriptEditBoxHandler(function(name)
		if name == "return" then
			local unitId = tonumber(self.input1:getText())
			if not csv.unit[unitId] then
				self.input2:text("")
				self.input3:setText("")
				gGameUI:showTip("unitId:(%s)不存在", unitId)
			else
				self.input2:text(csv.unit[unitId].name)
				self.input3:setText(self:getScale(csv.unit[unitId]))
			end
		end
	end)

	label.create("name:", {fontSize = 45, color = ui.COLORS.NORMAL.DEFAULT})
		:anchorPoint(0, 0.5)
		:xy(50, 400)
		:addTo(panel)
	self.input2 = label.create("", {fontSize = 45, color = ui.COLORS.NORMAL.DEFAULT})
		:anchorPoint(0, 0.5)
		:xy(200, 400)
		:addTo(panel)

	label.create("scale:", {fontSize = 45, color = ui.COLORS.NORMAL.DEFAULT})
		:anchorPoint(0, 0.5)
		:xy(50, 300)
		:addTo(panel)
	self.input3 = ccui.EditBox:create(cc.size(300, 60), "common/box/box_topui.png")
		:anchorPoint(0, 0.5)
		:xy(200, 300)
		:addTo(panel)
	self.input3:setFontSize(45)
	self.input3:setFontColor(ui.COLORS.NORMAL.DEFAULT)


	label.create("x:", {fontSize = 45, color = ui.COLORS.NORMAL.DEFAULT})
		:anchorPoint(0, 0.5)
		:xy(50, 200)
		:addTo(panel)
	self.input4 = ccui.EditBox:create(cc.size(100, 60), "common/box/box_topui.png")
		:anchorPoint(0, 0.5)
		:xy(100, 200)
		:addTo(panel)
	self.input4:setFontSize(45)
	self.input4:setFontColor(ui.COLORS.NORMAL.DEFAULT)

	label.create("y:", {fontSize = 45, color = ui.COLORS.NORMAL.DEFAULT})
		:anchorPoint(0, 0.5)
		:xy(350, 200)
		:addTo(panel)
	self.input5 = ccui.EditBox:create(cc.size(100, 60), "common/box/box_topui.png")
		:anchorPoint(0, 0.5)
		:xy(400, 200)
		:addTo(panel)
	self.input5:setFontSize(45)
	self.input5:setFontColor(ui.COLORS.NORMAL.DEFAULT)
	...
	bind.touch(self, btn, {methods = {ended = function()
		...
		for i = 1, 10000 do
			local cfg = csv.unit[unitId + i]
			if cfg then
				self.input1:setText(unitId + i)
				self.input2:text(cfg.name)
				self.input3:setText(cfg.cardShowScale)
				local pos = cfg.cardShowPosC
				self.input4:setText(pos.x or 0)
				self.input5:setText(pos.y or 0)
				break
			end
		end
	end}})
...
function CardBagView:getUnitId()
	if self.input1 then
		local unitId = tonumber(self.input1:getText())
		if csv.unit[unitId] then
			return unitId
		end
	end
end

function CardBagView:getScale(unitCsv)
	if self.input3 then
		local scale = tonumber(self.input3:getText())
		if scale then
			return scale
		end
	end
	return unitCsv.scale
end
