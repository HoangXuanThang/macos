				...
				bind.extend(list, item:get("cardPanel"), {
					...
					props = {
						...
						onNode = function(node)
							node:xy(14, 3)
							local cardID = vv.cfg.cardID
							node:removeChildByName("_cardID_")
							local txt = label.create(cardID, {
								color = ui.COLORS.NORMAL.DEFAULT,
								fontSize = 40,
								fontPath = "font/youmi1.ttf",
								effect = {outline = {color=ui.COLORS.NORMAL.WHITE, size = 3}},
							})
							txt:addTo(node, 111, "_cardID_")
...
HandbookView.RESOURCE_BINDING = {
	...
	["center.textDesc"] = {
		binds = {
			event = "text",
			idler = bindHelper.self("textDesc"),
		},
	},
...
function HandbookView:onCreate(params)
	...
	if matchLanguage({"en", "kr"}) then
    	self.textLocation:ignoreContentAdaptWithSize(false)
        self.textLocation:setContentSize(cc.size(570,100))
        self.textLocation:setTextVerticalAlignment(cc.VERTICAL_TEXT_ALIGNMENT_CENTER)
       	self.textLocation:setTextHorizontalAlignment(cc.TEXT_ALIGNMENT_LEFT)
       	self.textLocation:getVirtualRenderer():setLineSpacing(-5)
    end
...
function HandbookView:onShowAllAttrPanel()
	gGameUI:stackUI("city.handbook.add_attr", nil, {blackLayer = false})
end
