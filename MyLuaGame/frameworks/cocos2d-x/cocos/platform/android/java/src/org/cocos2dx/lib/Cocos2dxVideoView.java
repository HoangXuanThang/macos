...
public class Cocos2dxVideoView extends SurfaceView implements MediaPlayerControl {
    ...
    private void openVideo() {
        ...
        try {
            ...
            if (mIsAssetRouse) {
                ...
                try {
					// 如果没有在apk包里会抛异常 by huangwei 2020/11/6
                    afd = mCocos2dxActivity.getAssets().openFd(mVideoFilePath);
                } catch (IOException ex) {
                    Log.w(TAG, "Unable to open content in apk:" + mVideoFilePath, ex);
                }
    ...
    private boolean mIsComplete = true; //插入一个变量，用来记录是不是完成播放
    ...
    SurfaceHolder.Callback mSHCallback = new SurfaceHolder.Callback()
    {
        public void surfaceChanged(SurfaceHolder holder, int format,
                                    int w, int h)
        {
            boolean isValidState =  (mTargetState == STATE_PLAYING) || !mIsComplete; //2.加入isComplete判断
        ...
        public void surfaceDestroyed(SurfaceHolder holder)
        {
            if(mCurrentState == STATE_PLAYING) { 
                mIsComplete = mMediaPlayer.getCurrentPosition() == mMediaPlayer.getDuration(); //保存一下当前进度和是否播放完成
