...
class CCPluginCompile(cocos.CCPlugin):
    ...
    def _add_custom_options(self, parser):
        ...
        group = parser.add_argument_group("jenkins likely")
        group.add_argument("--j_fake", dest="j_fake", action="store_true", help="jenkins fake, 跳过更新和login版本验证")
        group.add_argument("--j_texturepack", dest="j_texturepack", action="store_true", help="进行texturepack")
    ...
    def _check_custom_options(self, args):
        ...
        if self._project._is_lua_project():
            ...
            # encrypt by src_pack by huangwei 2019/11/8
            self._lua_encrypt = False

        self.j_fake = args.j_fake
        self.j_texturepack = args.j_texturepack
    ...
    def compile_lua_scripts(self, src_dir, dst_dir, build_64):
        ...
        # no diff with 32/64, modify by huangwei 2019.11.8
        build_64 = False
    ...
    def build_android(self):
        ...
        # only support gradle, old method no edited and support modify by huangwei 2018.8.22
        if not gradle_support_ndk:
            raise cocos.CCPluginError('only support Gradle !!!')
    ...
    def backup_dir(self, dir_path, child_dirs=None, keep_dirs=None):
        JENKINS_RUN = str(os.environ.get('JENKINS_RUN', '')) == 'True'
        cocos.Logging.info("JENKINS_RUN: %s" % str(JENKINS_RUN))
        if child_dirs:
            backup_dir = os.path.join(dir_path, "iospack")
            if os.path.exists(backup_dir) and not JENKINS_RUN:
        ...
        if child_dirs:
            for child in child_dirs:
                ...
                backup_child_dir = os.path.join(backup_dir, child)
                if os.path.exists(child_dir):
                    if keep_dirs and child in keep_dirs:
                        if not os.path.exists(backup_child_dir):
                            shutil.copytree(child_dir, backup_child_dir)
                    else:
                        if os.path.exists(backup_child_dir):
                            shutil.rmtree(backup_child_dir)
                        shutil.copytree(child_dir, backup_child_dir)
        else:
            shutil.copytree(dir_path, backup_dir)
        return backup_dir

    def reset_backup_dir(self, dir_path, had_childs=False):
        if had_childs:
            backup_dir = os.path.join(dir_path, "iospack")
            # keep it
        else:
    ...
    def prepare_like_jenkins_ios(self):
        JENKINS_RUN = str(os.environ.get('JENKINS_RUN', '')) == 'True'
        if JENKINS_RUN:
            return

        # TODO: temp shenhe setting
        shenhe = True

        os.environ['JENKINS_RUN'] = "True" # it will ignore rm in xcode run scripts
        os.environ['TJ_CACHE_PATH'] = "%s/Documents/" % os.environ.get('HOME')

        ios_project_dir = os.path.abspath(self._platforms.project_path())
        asset_dir = os.path.join(ios_project_dir, "assets")
        game_dir = os.path.abspath(os.path.join(ios_project_dir, "../../.."))
        pack_dir = os.path.join(game_dir, "iospack")
        nopack_dir = os.path.join(game_dir, "iosnopack")
        trunk_dir = os.path.abspath(os.path.join(ios_project_dir, "../../../../../.."))
        if trunk_dir.find('trunk') < 0:
            trunk_dir = os.path.abspath(os.path.join(ios_project_dir, "../../../../.."))
        tjtool_dir = os.path.join(trunk_dir, "tools/tjpack")
        cocos.Logging.info("asset_dir: %s" % asset_dir)
        cocos.Logging.info("pack_dir: %s" % pack_dir)
        cocos.Logging.info("trunk_dir: %s" % trunk_dir)
        cocos.Logging.info("tjtool_dir: %s" % tjtool_dir)

        #if trunk_dir.find('trunk') < 0:
            #raise Exception("not found trunk")

        self._run_cmd("chmod +x xxtea_dll", cwd=os.path.join(tjtool_dir, "bin/mac/"))
        self._run_cmd("chmod +x pngquant", cwd=os.path.join(tjtool_dir, "bin/mac/"))
        svn_version = self._output_for(r'svn info|grep "Revision:"|grep -E -o "[0-9]+"', cwd=trunk_dir)
        svn_version = svn_version.strip()
        git_version = self._output_for(r"""git log -n 1|grep -Eo "commit\s+([0-9a-z]+)"|awk '{print $2}'""", cwd=game_dir)
        git_version = git_version.strip()
        cocos.Logging.info("svn_version: %s" % svn_version)
        cocos.Logging.info("git_version: %s" % git_version)
        self._run_cmd("bash ../../application%s/build.sh" % ('_shenhe' if shenhe else ''), cwd=game_dir)

        if os.path.exists(pack_dir):
            shutil.rmtree(pack_dir)
        os.makedirs(pack_dir)
        if os.path.exists(nopack_dir):
            shutil.rmtree(nopack_dir)
        os.makedirs(nopack_dir)
        os.makedirs(os.path.join(nopack_dir, 'res'))

        shutil.copytree(os.path.join(game_dir, 'res'), os.path.join(pack_dir, 'res'))

        # for manully
        # res + src + cocos + updater
        # shutil.copytree(os.path.join(game_dir, 'iosres'), pack_dir)

        # xcode also had scripts
        # but if res with repo-style, you need copy first, then res_pack second
        self._run_cmd("cp -rf %s/* %s" % (asset_dir, pack_dir))
        self._run_cmd('sed -i "" "s/2.1.0.0/2.1.0.%s/g" res/version.plist' % svn_version, cwd=pack_dir)
        self._run_cmd('sed -i "" "s/GIT_VERSION/%s/g" res/version.plist' % git_version, cwd=pack_dir)

        # no such plist in repo
        self._run_cmd('mv res/*.plist %s/res' % nopack_dir, cwd=pack_dir)

        # res + src + cocos + updater in repo
        self.compile_lua_in_iospack()

        cocos_cmd_path = os.path.join(os.path.dirname(os.path.abspath(sys.argv[0])), "cocos")
        # compile_cmd = "\"%s\" rescompile --res-path \"%s\" --inplace" % (cocos_cmd_path, os.path.join(pack_dir, 'res'))
        compile_cmd = "\"%s\" rescompile --res-path \"%s\" --inplace" % (cocos_cmd_path, pack_dir)
        self._run_cmd(compile_cmd)

        if self.j_fake:
            self._run_cmd("echo > fake", cwd=pack_dir)
        self._run_cmd("type res/version.plist", cwd=nopack_dir)
        self._run_cmd("type res/channel.plist", cwd=nopack_dir)

        # encode plist and put back in repo
        plists = ['version.plist', 'channel.plist', 'language.plist', 'micro.plist']
        plists_nopack = [os.path.join(nopack_dir, 'res', x) for x in plists]
        self._run_cmd("python pack_tool.py %s" % ' '.join(plists_nopack), cwd=tjtool_dir)

        if not os.path.exists(os.path.join(pack_dir, 'res')):
            os.makedirs(os.path.join(pack_dir, 'res'))
        for x in plists:
            self._run_cmd("mv res/%s.encode %s/res/%s" % (x, pack_dir, x), cwd=nopack_dir)

        if self.j_texturepack:
            tool_p = os.path.join(trunk_dir, 'tools/texturePack2pvrccz')
            p = os.path.join(trunk_dir, 'tools/texturePack2pvrccz/TexturePack/updatePack')
            if os.path.exists(p):
                shutil.rmtree(p)
            os.makedirs(p)
            shutil.copytree(os.path.join(game_dir, 'res/spine'), os.path.join(p, 'spine'))
            self._run_cmd("python csv2py.py", cwd=tool_p)
            self._run_cmd("python texture_pack.py n", cwd=tool_p)
            shutil.rmtree(os.path.join(pack_dir, 'res/spine'))
            shutil.move(os.path.join(tool_p, "TexturePack/output/spine"), os.path.join(pack_dir, 'res'))

    def compile_lua_in_iospack(self):
        if self._project._is_lua_project():
            # copy it to iospack by huangwei 2019/11/8
            dirs = ["cocos", "src", "updater"]
            pack_src_dir = self.backup_dir(self._project.get_project_dir(), dirs + ["res"], set(["res"]))
            ...
            # build bytecode
            for d in dirs:
                p = os.path.join(pack_src_dir, d)
                self.compile_lua_scripts(p, p, False)
    ...
    def build_ios(self):
        ...
        try:
            ...
            if need_reset_dir:
                if self._project._is_lua_project():
                    self.reset_backup_dir(self._project.get_project_dir(), True)
                else:
                    script_src_dir = os.path.join(self._project.get_project_dir(), "src")
    ...
    def _get_export_options_plist_path(self):
        ...
        ios_project_dir = os.path.join(self._platforms.project_path(), 'ios')
    ...
    def build_mac(self):
        ...
        if self._project._is_script_project():
            ...
            if self._project._is_lua_project():
                # copy it to iospack by huangwei 2019/11/8
                dirs = ["cocos", "src", "updater"]
                pack_src_dir = self.backup_dir(self._project.get_project_dir(), dirs)
