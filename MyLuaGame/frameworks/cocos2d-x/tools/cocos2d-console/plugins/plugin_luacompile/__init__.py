...
class CCPluginLuaCompile(cocos.CCPlugin):
    ...
    def run(self, argv, dependencies):
        ...
        cocos.Logging.warning("lua dest path: " + self._dst_dir)
        ...
        with cocos.pushd(tjpack_path):
            for src in self._src_dir_arr:
                cmd_str = "python src_pack.py y -s %s --onebyone --inplace" % (src)
                dirname = src
                if dirname[-1] in ('/', '\\'):
                    dirname = dirname[:-1]
                ...
                # shutil.copytree("./src_pack/%s" % dirname, self._dst_dir)
