    ...
    def __init__(self, opts):
        ...
        if opts['skip']:
            ...
            for skip in list_of_skips:
                ...
                if class_name not in self.skip_classes:
...
def main():
    ...
    # global skip.list
    skiplist = os.path.join(os.path.dirname(args[0]), "skip.list")
    if os.path.exists(skiplist):
        print "skiplist in", skiplist
        with open(skiplist, 'rb') as fp:
            skiplist = ''.join([x.strip() for x in fp.readlines()])
            if len(skiplist.strip()) == 0:
                skiplist = ""
            print skiplist
