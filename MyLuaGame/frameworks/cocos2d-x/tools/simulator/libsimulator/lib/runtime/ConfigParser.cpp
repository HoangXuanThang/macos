...
void ConfigParser::readConfig(const string &filepath)
{
	...
	cocos2d::allocator::TJFixedMem content = FileUtils::getInstance()->tjGetDataFromFile(fullPathFile);
    ...
    if (_docRootjson.Parse<0>(content.bytes, content.size).HasParseError()) {
