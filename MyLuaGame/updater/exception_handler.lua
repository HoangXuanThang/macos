...
local function post2TJCrashCollector(msg, trace)
	local reqUrl = string.format("%s/exception?app=%s&patch=%d&min_patch=%d&lang=%s&channel=%s&tag=%s&account=&server=&role=&", REPORT_CONF_URL, APP_VERSION, PATCH_VERSION, PATCH_MIN_VERSION, LOCAL_LANGUAGE, APP_CHANNEL, APP_TAG)
	local reqBlob = json.encode({msg = msg, traceback = trace})
	local NetManager = require("net.manager")
	return NetManager.sendHttpRequest(gGameApp.net, "POST", reqUrl, reqBlob, cc.XMLHTTPREQUEST_RESPONSE_BLOB, function(xhr)
