...
local function getFlags(v)
	...
	if flags then
		...
		-- 2^7:glow 2^6:shader 2^5:outline 2^4:url 2^3:删除线 2^2:下划线 2^1:粗体(使用 youmi.ttf) 2^0:斜体
...
local function _generateRichTexts(array, fontSize, deltaSize, adjustWidth)
	...
	for k, t in ipairs(array) do
		...
		elseif t.type == "text" then
			...
			table.insert(elems,
				{ element, { color, opacity, text, ttf, fontSize, flags.val or 0, url, outlineColor, outlineSize, shadowColor, shaderOffset, shadowBlurRadius, glowColor } })
			flags = {}
			ttf = ui.FONT_PATH
			...
			elseif ch == 'T' then -- 头衔
				...
				if num and csv.title[num] then
					...
					elseif showType == "txt" then
						...
						table.insert(T2,
							{ type = "flags", flags = "outlineColor", val = cc.c4b(cfg.color[1], cfg.color[2],
								cfg.color[3], 255) })
						t2 = { type = "text", text = cfg.title }
					elseif showType == "spine" then
						t2 = { type = "spine", path = cfg.res, action = "effect_loop", scale = hOrScale or 1, width = cfg
						.spineSize[1], height = cfg.spineSize[2] }
					end
				end
			elseif ch == 'I' then -- 图片
...
local function _binarySearchSplit(richTextTest, params, lineWidth)
	local tag = 1 --先写死
	local color, opacity, s, ttf, fontSize, val, url, outlineColor, outlineSize, shadowColor, shaderOffset, shadowBlurRadius, glowColor =
	unpack(params)
	local l, r = 1, #s + 1
	while l < r do
		local mid = math.floor((l + r) / 2)
		local left = s:sub(1, mid)
		local elem = ccui.RichElementText:create(tag, color, opacity, left, ttf, fontSize, val, url, outlineColor,
			outlineSize, shadowColor, shaderOffset, shadowBlurRadius, glowColor)
...
local function _getRichTextsWordLineFeed(strOrArray, size, deltaSize, lineWidth, onlyElems)
	...
	for _, t in ipairs(elems) do
		...
		richTextTest:formatText()
		local size = richTextTest:getContentSize()
		-- printInfo("width=%s | %s | %s", size.width, tostring(size.width > lineWidth), tostring(params and params[3]))
		while size.width > lineWidth do
			...
			if tolua.type(elem) == "ccui.RichElementText" then
				...
				table.insert(lineElems,
					{ ccui.RichElementText:create(tag, color, opacity, left, ttf, fontSize, val, url, outlineColor,
						outlineSize, shadowColor, shaderOffset, shadowBlurRadius, glowColor),
				...
				elem = ccui.RichElementText:create(tag, color, opacity, right, ttf, fontSize, val, url, outlineColor,
					outlineSize, shadowColor, shaderOffset, shadowBlurRadius, glowColor)
				params[3] = right
			end
			-- fill line
