...
--      示例:strs = {{str = "xxx",fontSize= XXX,effect = XXX},}; 可简化 strs = {"xxx", "xxx"}
--          如果只有一条的话 strs = {str = "xxx",fontSize= XXX,effect = XXX}; 可简化 strs = "xxx"
...
function beauty.textScroll(params)
	...
	local function generateText(list, size)
		...
		local function createLabel(strParams)
			...
			if params.isRich then
				...
				-- 超过最大行宽 就该用createWithWidth 主要为了区别一行和两行的显示
				-- 一行居中 两行不用
				if str == "" or richWidth > textWidth then
					richText = rich.createWithWidth(str, strParams.fontSize or fontSize, nil, textWidth)
				end
				list:addChild(richText)

				if leftPadding ~= 0 then
					local containerPosX = richText:getPositionX()
					richText:setPositionX(containerPosX + leftPadding)
				end
				itemHeight = itemHeight + richText:getContentSize().height
			else
...
function beauty.singleTextAutoScroll(params)
	...
	local function generateText(list)
		if params.isRich then
			local richText = rich.createByStr(params.strs, fontSize)
	...
	local function setLabelSetting(label, params)
		...
		if type(params.waitTime) == "table" then
			waitTimeSt = params.waitTime[1] or 1
			waitTimeEnd = params.waitTime[1] or 1
		else
			waitTimeSt = params.waitTime or 1
			waitTimeEnd = params.waitTime or 1
		end
...
-- @param params {width, replaceStr, align}
-- align: "center", 默认居中对齐, "left", "right"
