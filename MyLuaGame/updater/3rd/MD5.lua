...
local memutils = require "3rd.memutils"
...
globals.MD5_CTX = ffi.typeof("MD5_CTX");
...
local function printmd5ctx(ctx)
	for i=0,3 do
...
local function MD5Update(ctx, buf, len)
	...
	if (t > 0) then
		...
		t = 64 - t;
		if (len < t) then
			memutils.memcpy(p, buf, len);
			return;
		end

		memutils.memcpy(p, buf, t);
	...
	while (len >= 64) do
		memutils.memcpy(ctx.input, buf, 64);
		byteReverse(ctx.input, 16);
		MD5Transform(ctx.buf, ffi.cast("uint32_t *", ctx.input));
		buf = buf + 64;
		len = len - 64;
	...
	memutils.memcpy(ctx.input, buf, len);
end
...
local function MD5Final(digest, ctx)
	...
	if (count < 8) then
		memutils.memset(p, 0, count);
		byteReverse(ctx.input, 16);
		MD5Transform(ctx.buf, ffi.cast("uint32_t *", ctx.input));
		memutils.memset(ctx.input, 0, 56);
	else
		memutils.memset(p, 0, count - 8);
	end
	...
	memutils.memcpy(digest, ctx.buf, 16);
	memutils.memset(ffi.cast("char *", ctx), 0, ffi.sizeof(ctx));
end


globals.md5 = function (luastr)
